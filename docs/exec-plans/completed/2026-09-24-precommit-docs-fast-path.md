...
| W3 guard detected, did not prevent | CLEAN_ENV also drops NODE_TEST_CONTEXT (the nested hook run); an assert at MODULE LOAD on the filtered copy (SUPERSEDED at r2: it proved nothing about what a child receives - see the r2 fix record); comment and the incident record corrected | load-time assert | sanitizer removed, run under a decoy WORKTREE-shaped hook env (GIT_DIR = decoy worktree gitdir, GIT_INDEX_FILE, GIT_CONFIG_PARAMETERS, no GIT_WORK_TREE): `pass 0 fail 1` at load, decoy unchanged (1 commit, core.bare false, worktree index a); fixed file under the same env `pass 10 fail 0`, decoy unchanged |
...
## Gate r2 - adversary (@a328d2f0)

Delta only (`git diff 94e4f023 a328d2f0`: the test file and this plan). Measured in a `git archive a328d2f0` sandbox under /tmp and decoy repos (a plain repo plus a linked worktree); nothing ran in this repo or in a real hook here. Baseline: the new test file `# tests 10 # pass 10 # fail 0`, and 0 ft-precommit-fast dirs left in /tmp before and after every run, guard-firing mutants included.

- W1 fixed as prescribed. Drop `--no-renames`: `# pass 9 # fail 1` (the code-INTO-docs rename test).
- W2 fixed as prescribed. The end-to-end test goes red (`# pass 9 # fail 1`) for each hook mutant: `|| true` on the fast-path run, `exit 0` before it, `set +e` in the block, the `:` no-op prefix, `set -e` dropped globally, and the file list dropped from `--test`. Dropping NODE_TEST_CONTEXT from the strip list: also red (9/1), so that change is bound too.
- W4 fixed as prescribed. Drop the quoted-docs alternative: `# pass 9 # fail 1`; drop the `docs/` alternative: `# pass 7 # fail 3`.
- S2 fixed: cleanup is registered before any sandbox work (0 leaked dirs, measured). S1 disclosed.
- W3 NOT fixed (fixed differently; the deviation does not hold). The new load-time assert checks the CLEAN_ENV OBJECT, which the line above has just filtered, so it catches only a broken filter (CLEAN_ENV = process.env: 0 pass / 1 fail at load, decoy unchanged: reproduced). It does not check that a child RECEIVES CLEAN_ENV, which is how the incident happened. My r1 mutant (the sandbox git helper without `env: CLEAN_ENV`) under the worktree-shaped hook env (GIT_DIR = the decoy worktree's gitdir, GIT_INDEX_FILE, GIT_CONFIG_PARAMETERS, no GIT_WORK_TREE): `# pass 3 # fail 7`, and the decoy's shared core.bare went false -> true (config md5 5b603c2c -> 9daff807) before the post-init check fired. The same mutant run INSIDE a real pre-commit of the decoy worktree: commit refused, core.bare flipped to true. So one dropped `env:` in a future edit of this file still corrupts the repo when the hook runs it. The fix record's claim "an assert at MODULE LOAD that no GIT_* key reaches a child" overstates what the assert checks. My own r1 prescription was insufficient: neither option I gave covers a call site that drops `env:`. Corrected prescription, verified in the sandbox: at load, scrub the process env in place, `for (const k of Object.keys(process.env)) if (k.startsWith('GIT_')) delete process.env[k];` (GIT_* only; also deleting NODE_TEST_CONTEXT in place broke the runner's reporting, 1 pass). With the scrub, the fixed file is `# pass 10 # fail 0` and my mutant stacked on it is still `# pass 10 # fail 0`, decoy byte-identical (core.bare false, config md5 unchanged, index a.txt). Keep CLEAN_ENV (it still drops NODE_TEST_CONTEXT for the nested hook run). Fix the comment and the fix-record wording to match.
- S3 SUGGESTION (pre-existing, NOT introduced by this diff; non-blocking): nothing binds the FULL path of the hook. An `exit 0` inserted right after the fast-path `fi` (so a code commit skips eslint, the ratchet and the unit suite) stays `# pass 10 # fail 0` here, and ci-pipeline-locks + comment-debt-census are unchanged (the same 2 git-dependent census tests fail identically without the mutant, because the archive sandbox is not a git repo). The same edit is possible on main today. Cheap binding: in the end-to-end test, stage a code file and assert "→ eslint" appears and the commit is refused (the sandbox has no package.json).
- Verified in a decoy: the unmutated a328d2f0 test, run inside a real pre-commit of a decoy worktree with a path staged: `# pass 10 # fail 0`, commit landed with only a.txt and staged.md, core.bare false.

Gate: CHANGES r2 @a328d2f0 — adversary

## r2 fix record (gate r2 @a328d2f0, adversary CHANGES; round 3 by Dean's call)

| Finding | Change | Binding | Mutant (under a decoy WORKTREE-shaped hook env: GIT_DIR = the decoy worktree gitdir, GIT_INDEX_FILE, GIT_CONFIG_PARAMETERS, no GIT_WORK_TREE) |
|---|---|---|---|
| W3 the load assert checked a filtered COPY; a call site dropping `env:` still corrupted the repo | the test deletes every GIT_* key from its OWN process.env at load, before any child exists, so every child inherits a clean env whatever its call site passes; the copy assert is gone; CLEAN_ENV now only drops NODE_TEST_CONTEXT (not scrubbed from the process: its own runner reports through it); comment corrected | the scrub itself | the seat's mutant (the sandbox git helper drops `env:`): `pass 10 fail 0`, decoy unchanged (1 commit, core.bare false); scrub removed: `pass 3 fail 7`, decoy core.bare flipped to true (the decoy took the damage - the scrub is load-bearing) |
| S3 (pre-existing) the full-check path was unbound | the end-to-end test also stages a code file: the hook must print the eslint step, never the docs-only line, refuse the commit (the sandbox has no package.json) and leave HEAD | "the real hook on a docs-only commit" (code arm) | `exit 0` right after the fast-path block: `pass 9 fail 1` |
