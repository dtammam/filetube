...
| site | selector | decl (linter-exact) | bucket | after | expected delta | notes |
| ---- | -------- | ------------------- | ------ | ----- | -------------- | ----- |
| public/css/style.css:609 | .logo span.tube | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand wordmark |
| public/css/style.css:751 | .btn-primary | `color: white` | SEMANTIC-RESIDUE | - | - | white-on-brand |
...
| public/css/style.css:1159 | .progress-bar-container | `background-color: rgba(255, 255, 255, 0.3)` | NO-TOKEN | - | - | translucent WHITE watch-progress track |
| public/css/style.css:1538 | .empty-state, .error-state | `padding: 40px var(--space-10)` | B1-PARTIAL-DONE | - | - | OPENER DONE 2026-07-31 - adopted members in place; this residual is deliberate and stays in the burn-down |
| public/css/style.css:1791 | .art-play-glyph::before | `filter: drop-shadow(0 2px 6px rgba(0, 0, 0, 0.6))` | NO-TOKEN | - | - | drop-shadow() glyph art |
| public/css/style.css:1825 | .audio-player-visual | `background: radial-gradient(circle, #2c3e50 0%, #0f171e 100%)` | NO-TOKEN | - | - | decorative gradient art |
| public/css/style.css:1826 | .audio-player-visual | `color: white` | SEMANTIC-RESIDUE | - | - | text on gradient art |
| public/css/style.css:1838 | .audio-artwork | `border: 2px solid rgba(255,255,255,0.2)` | NO-TOKEN | - | - | artwork border |
| public/css/style.css:1864 | .audio-vinyl::after | `border: 3px solid white` | NO-TOKEN | - | - | vinyl spindle art |
| public/css/style.css:1887 | .audio-subtitle | `color: #aaa` | NO-TOKEN | - | - | #aaa on art surface; --on-overlay-muted is #ccc = visible delta, no zero-delta adoption exists |
| public/css/style.css:1928 | .transcode-spinner | `border: 4px solid rgba(255, 255, 255, 0.25)` | NO-TOKEN | - | - | spinner ring art |
| public/css/style.css:2035 | .skip-ripple | `background: rgba(255, 255, 255, 0.14)` | NO-TOKEN | - | - | white ripple wash |
...
| public/css/style.css:2141 | .watch-autoplay-thumb | `background-color: #fff` | SEMANTIC-RESIDUE | - | - | toggle knob, form control |
| public/css/style.css:2287 | .uploader-avatar | `color: white` | SEMANTIC-RESIDUE | - | - | glyph over generated avatar color |
| public/css/style.css:2330 | .uploader-subs | `padding: 1px var(--space-3)` | B2-PARTIAL-DONE | - | - | 3a DONE 2026-07-31 - 7px->6px applied; the 1px residual is deliberate and stays in the burn-down |
| public/css/style.css:5126 | #player-wrapper:not(.audio-expanded) | `padding-bottom: 40px` | LAYOUT-CONSTANT | - | - | 40px control-strip reserve |
| public/css/style.css:5165 | #player-slot #player-wrapper | `padding-bottom: 80px` | LAYOUT-CONSTANT | - | - | 80px two-row strip reserve |
| public/css/style.css:5168 | #player-wrapper (mobile) | `padding-bottom: 44px` | LAYOUT-CONSTANT | - | - | 44px strip mirror (not a control height context) |
| public/css/style.css:5194 | #player-dock #player-wrapper | `padding-bottom: 26px` | LAYOUT-CONSTANT | - | - | 26px mini-bar reserve |
...
| public/css/style.css:5509 | .cc-overlay-text | `background: rgba(0, 0, 0, 0.72)` | CC-PROTECTED | - | - | live value is rgba(0,0,0,0.72), NOT the 0.85 the amendment comment documents - comment-accuracy finding; protection unaffected |
...
| public/css/style.css:6067 | .sub-row-pin-active | `color: #e0a800` | NO-TOKEN | - | - | deliberate era-invariant gold pin accent (documented) |
| public/css/style.css:6114 | .pinned-avatar-generated | `color: #fff` | SEMANTIC-RESIDUE | - | - |  |
| ~~public/css/style.css:6219~~ | ~~.hard-delete-modal-confirm-btn~~ | ~~`border-radius: calc(var(--radius) + 1px)`~~ | ~~RADIUS-DERIVED~~ | ~~-~~ | ~~-~~ | ~~~~ |
| public/css/style.css:6235 | .hard-delete-modal-confirm-btn | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand |
...
| public/css/style.css:6578 | .dl-status-chip-retry-btn | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand |
| public/css/style.css:6761 | .pinned-unpin-btn | `border-radius: 2px` | G-RADIUS-DONE-RAW | - | - | was 3px; 3g DONE 2026-07-31 - consolidated per-site per ruling B; the result is DELIBERATELY a raw 2/4px literal (no small-radius token exists - that design is Tier 4 R7), so the site stays in the burn-down as R7 population |
| public/css/style.css:6769 | .pinned-unpin-btn.armed | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand armed state |
| public/css/style.css:6858 | .books-shelf-chip.active | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand; mid-line |
...
| public/css/style.css:6936 | #reader-pane.pdf-scroll canvas | `box-shadow: 0 1px 6px rgba(0,0,0,0.35)` | SHADOW-OTHER | - | - | paper-page ambient shadow on PDF content, not UI elevation - no action |
...
| public/css/style.css:6950 | .reader-progress-track | `border-radius: 3px` | RADIUS-GEOMETRY | - | - | half of 6px track height - never a token |
| public/css/style.css:7049 | .reader-np-cover | `border-radius: 4px` | G-RADIUS-DONE-RAW | - | - | was 3px; 3g DONE 2026-07-31 - consolidated per-site per ruling B; the result is DELIBERATELY a raw 2/4px literal (no small-radius token exists - that design is Tier 4 R7), so the site stays in the burn-down as R7 population |
| public/css/style.css:7272 | .reloc-preview-badge | `padding: 1px var(--space-3)` | B1-PARTIAL-DONE | - | - | OPENER DONE 2026-07-31 - adopted the member in place; the 1px hairline residual is deliberate and stays in the burn-down |
| public/css/style.css:7273 | .reloc-preview-badge | `border-radius: 2px` | G-RADIUS-DONE-RAW | - | - | was 3px; 3g DONE 2026-07-31 - consolidated per-site per ruling B; the result is DELIBERATELY a raw 2/4px literal (no small-radius token exists - that design is Tier 4 R7), so the site stays in the burn-down as R7 population |
| public/css/style.css:7336 | .login-wordmark .tube | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand |
| public/css/style.css:7404 | .login-submit | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand |
| public/css/style.css:7452 | .login-era-switch button[aria-pressed="true"] | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand |
| public/css/style.css:7717 | .music-eq i | `background: #fff` | SEMANTIC-RESIDUE | - | - | white EQ glyph-art bar; --on-overlay is a text token - semantic stretch, lean residue |
| public/css/style.css:7718 | .music-eq i | `border-radius: 1px` | RADIUS-GEOMETRY | - | - | 1px cap on 3px EQ bar |
| public/css/style.css:8026 | .attr-picker-avatar | `color: #fff` | SEMANTIC-RESIDUE | - | - | glyph over generated avatar color |
| public/css/style.css:8106 | .notif-bell-badge | `border-radius: 8px` | RADIUS-GEOMETRY | - | - | half of 16px badge height |
| public/css/style.css:8108 | .notif-bell-badge | `color: #fff` | SEMANTIC-RESIDUE | - | - | white-on-brand |
| public/css/style.css:8211 | .notif-row-avatar-generated | `color: #fff` | SEMANTIC-RESIDUE | - | - |  |
