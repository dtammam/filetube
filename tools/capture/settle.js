...
async function settlePageImages(page, timeoutMs = 8000) {
  ...
  for (let pass = 0; pass < 3; pass++) {
    const result = await page.evaluate(async (budget) => {
      ...
      return { total: now.length, pending: now.filter((i) => !i.complete || i.naturalWidth === 0).length, outcome };
    }, timeoutMs);
...
const VOLATILE_MASK_CSS = '.sub-list-header-status,.notif-row-time{visibility:hidden!important}';
