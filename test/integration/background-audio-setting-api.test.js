...
// v1.27.0 (EXPERIMENTAL) -- POST/GET /api/settings must round-trip the new
// `backgroundAudioForVideo` preference (boolean, OFF by default) exactly
// like `autoplayNext` (test/integration/autoplay-next-api.test.js): it must
// not 400 as an unknown key (KNOWN_KEYS allowlist) and must persist/read
// back correctly. Isolated DATA_DIR before requiring the app, own process
// per file (node --test) -- mirrors that same file's setup.
...
test('GET /api/settings never omits backgroundAudioForVideo, and it defaults to false (OFF) on a fresh db', async () => {
  ...
  assert.equal(json.backgroundAudioForVideo, false);
});
...
test('POST /api/settings accepts a backgroundAudioForVideo key without 400ing (KNOWN_KEYS regression)', async () => {
  ...
  const res = await fetch(`${base}/api/settings`, {
    ...
    body: JSON.stringify({ backgroundAudioForVideo: true }),
  });
  assert.equal(res.status, 200);
  const json = await res.json();
  assert.equal(json.backgroundAudioForVideo, true);
});
...
test('POST /api/settings can turn backgroundAudioForVideo back OFF (explicit false)', async () => {
  ...
  const res = await fetch(`${base}/api/settings`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ backgroundAudioForVideo: false }),
  });
  assert.equal(res.status, 200);
  assert.equal((await res.json()).backgroundAudioForVideo, false);
});
...
test('POST /api/settings rejects a non-boolean backgroundAudioForVideo with 400 and mutates nothing', async () => {
  ...
  const res = await fetch(`${base}/api/settings`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ backgroundAudioForVideo: 'yes' }),
  });
...
test('POST /api/settings with an invalid backgroundAudioForVideo does not partially persist alongside other valid keys in the same request', async () => {
  ...
  const res = await fetch(`${base}/api/settings`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ pruneMissing: false, backgroundAudioForVideo: null }),
  });
  assert.equal(res.status, 400);
