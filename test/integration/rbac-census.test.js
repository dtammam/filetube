...
const { app, updateDatabase, userStore, __mintTestSession, viewCountStore, musicDb, booksDb, podcastsDb, ytdlpDb } = require('../../server');
...
// +8 = the perf-diagnostics suite (Settings > Experimental), registered
// unconditionally so it flips live with the toggle: GET /diag, GET /api/diag/
// ping|blob|payload, POST + GET /api/diag/runs, GET + DELETE /api/diag/runs/:id.
// ALL requireAdmin (member-403 first, then the feature 404s while off) and NON
// content-serving (synthetic probes + diagnostic run data, no library/user
// content) - so no per-user visibility check: the GETs are ADMIN in
// route-read-classification, the POST/DELETE are 'admin' + na() in
// route-write-classification (which binds the member-403).
const EXPECTED_ROUTE_COUNT = 250; // Chapter Snap (2026-09-24): +4 = GET /api/videos/:id/chapter-snap (GATED in route-read-classification) + POST .../chapter-snap/scan + POST .../chapter-snap + POST .../chapter-snap/revert (library-write + enforced in route-write-classification); every one requireModifyLibrary FIRST then the single-item 404 for a restricted item, the writes re-checking visibility inside the write tick - proven in chapter-snap.test.js (403 without modify, 404 restricted on all four, no ffmpeg run started for a restricted item). // v1.265: +2 = GET + POST /api/prefs (cross-device preference sync; per-CALLER only - no user param exists; GET is OWN_STATE in route-read-classification, POST is 'personal' in route-write-classification + VISIBILITY; the 21-key server allowlist, byte/batch caps, LWW, and second-session scoping proven in prefs-api.test.js; the store axes incl. the backup carrier in prefs-store.test.js). // v1.238: +3 = POST + GET + DELETE /api/me/sticker (per-user custom player sticker; self-service, disk-only, mirrors the v1.82 avatar routes; GET self-only NO_CONTENT in route-read-classification, POST/DELETE 'personal' in route-write-classification + VISIBILITY; upload/serve/delete/cascade proven in sticker-upload.test.js). // v1.211: +1 GET /api/music/channels (the "Channels in Music" manager list; GATED, visibility-scoped - only channels with visible audio, count is the visible count; proven in music-library-projection.test.js). // Wave G: +2 = GET + POST /api/folders/music-flag (the per-folder "show in Music" mark). GET is GATED (scoped to the user's VISIBLE audio); POST is library-write + enforced visibility (requireModifyLibrary AND a visible-audio existence probe, 404 otherwise) - classified in route-read/write-classification.test.js, behavioral member-403 + visibility proof in music-library-projection.test.js. // v1.205 Wave B: +1 GET /api/search - the UNIFIED cross-content search. CONTENT-AGGREGATING (returns titles of every media type), so it is the highest-leak-risk read surface; each provider routes through its EXISTING per-kind *VisibleTo gate (never a divergent second gate), proven by the /api/search leak assertions in the LIST SWEEP above + search-api.test.js. // transcript export: + GET /api/transcript/:id (GATED, mirrors /api/subtitles/:id) // v1.198.1: +1 GET /tvthumb/:id (per-episode art for the Up-next rail; GATED, 404-on-restricted proven in rbac-tv-enforcement.test.js). v1.198: -2 (the v1.196 admin set-poster POST/DELETE routes REMOVED per Dean - posters are folder-image/generated only). v1.197 TV wrap wave: +2 = POST /api/tv/episode/:id/prepare-audio (the bg-audio sidecar pre-warm; personal write + enforced visibility) + GET /tvaudio/:id (GATED; both 404-on-restricted proven in rbac-tv-enforcement.test.js). v1.196 TV player integration: +7 = GET /api/tv/episode/:id (detail/status, GATED) + POST /api/tv/progress + POST /api/tv/played + DELETE /api/tv/played (personal writes) + GET /api/tv/continue (GATED, visibility-filtered) + (the poster POST/DELETE pair, since REMOVED in v1.198). All proven in rbac-tv-enforcement.test.js. v1.195 TV Shows: +9 total = +4 config/scan (Phase 3a) + 4 browse/serve (Phase 3c: GET /api/tv, GET /api/tv/:showId, GET /tvposter/:showId, GET /tvepisode/:id - all GATED, visibility-filtered, proven in rbac-tv-enforcement.test.js) + 1 GET /tv shell (Phase 4, classified NO_CONTENT in route-read-classification.test.js - a static HTML shell, no library/user content). The progress/played/liked routes land in a later wave. v1.171: +4 critter pool management (POST /api/critters/upload + DELETE /api/critters/item + DELETE /api/critters/all - all admin on BOTH axes in route-write-classification.test.js; GET /api/critters/archive - NO_CONTENT in route-read-classification.test.js + requireAdmin in-route; decorative assets under public/critters/ only, no library/user content). v1.166: +1 (GET /api/critters - decorative critter-folder asset listing, NO library/user content; behind the session gate like every /api route, classified NO_CONTENT in route-read-classification.test.js; serves filenames from public/critters/ only). v1.159: +1 (GET /api/library-items - visibility-scoped A/V list for the Stats table, GATED in route-read-classification.test.js + proven in library-items.test.js). v1.158: +2 (POST /api/trash/purge-all - library-write + enforced/visibility-filtered in route-write-classification.test.js + proven in trash-purge-all.test.js; GET /api/storage-summary - GATED in route-read-classification.test.js + proven visibility-scoped in storage-summary.test.js). v1.146: +3 downloader-engine (GET /api/ytdlp/engine + POST /api/ytdlp/engine + POST /api/ytdlp/engine/update; the POSTs classified admin on BOTH axes in route-write-classification.test.js). v1.126 history: +1 folders/display-name.
...
test('LIST SWEEP: a member blocked from all libraries sees NO seeded content in any list', async () => {
  ...
  // v1.205 Wave B: the UNIFIED /api/search aggregates every provider, so it is
  // the highest-leak-risk surface - a blocked member must see NONE of the
  // seeded titles of ANY type through it, while admin does (discrimination, not
  // blanket-empty). Each provider routes through its EXISTING *VisibleTo gate.
  const searchIds = (q) => json(`/api/search?q=${encodeURIComponent(q)}&limit=50`).then((r) => ids(r.items));
  assert.ok(!(await searchIds('V')).includes('vid'), '/api/search video leak');
  assert.ok(!(await searchIds('T')).includes('trk'), '/api/search music leak');
  assert.ok(!(await searchIds('B')).includes('bk'), '/api/search book leak');
  assert.ok(!(await searchIds('Ep')).includes(epId), '/api/search podcast-episode leak');
  assert.ok(!(await searchIds('Show')).includes(subId), '/api/search podcast-show leak');
  const adminSearchIds = (q) => asAdmin(`/api/search?q=${encodeURIComponent(q)}&limit=50`).then((r) => r.json()).then((r) => ids(r.items));
  assert.ok((await adminSearchIds('V')).includes('vid'), 'admin /api/search finds the video (discrimination)');
  assert.ok((await adminSearchIds('Show')).includes(subId), 'admin /api/search finds the podcast show (discrimination)');
