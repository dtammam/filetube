...
const EXPECTED_ROUTE_COUNT = 206; // v1.159: +1 (GET /api/library-items - visibility-scoped A/V list for the Stats table, GATED in route-read-classification.test.js + proven in library-items.test.js). v1.158: +2 (POST /api/trash/purge-all - library-write + enforced/visibility-filtered in route-write-classification.test.js + proven in trash-purge-all.test.js; GET /api/storage-summary - GATED in route-read-classification.test.js + proven visibility-scoped in storage-summary.test.js). v1.146: +3 downloader-engine (GET /api/ytdlp/engine + POST /api/ytdlp/engine + POST /api/ytdlp/engine/update; the POSTs classified admin on BOTH axes in route-write-classification.test.js). v1.126 history: +1 folders/display-name.
