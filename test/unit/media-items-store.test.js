...
const {
  SQLITE_FILENAME, SqliteAdapter, SCHEMA_VERSION, DOC_OBJECT_KEYS, readPersistedDatabase, importParsedJson,
...
test('migration v32: the doc rows move VERBATIM (json text untouched, rowid order kept); an unaddressable (empty) key is skipped with a log line; doc rows deleted; stamp 32; no doc namespace is left', () => {
  ...
  assert.strictEqual(require('../../lib/db/sqlite').DOC_KV_NAMESPACES, undefined, 'Wave 7: the doc lists are gone');
...
test('the diff through the adapter: changed rows only, vanished ids deleted, absent key = rows kept, empty map = wiped, undefined dropped, a NUL id refused, the base advances only after COMMIT', () => {
  ...
  assert.throws(() => adapter.save({ metadata: { a: it('a'), [`x${NUL}y`]: it('x') } }), /contains U\+0000.*truncated on Node 24\.14/s, 'a NUL id is refused at the write (the message states the measured premise - #225)');
...
test('the seams: import extracts viewCount to its handle and routes items through insertItem (refused without it when items exist); exclusiveReplace wipes + insertItem + rebuilds the base; the test read surfaces the index only when rows exist', () => {
  ...
  assert.throws(() => importParsedJson({ metadata: { v1: { id: 'v1' } } }, {}), /no insertItem handle/);
  assert.doesNotThrow(() => importParsedJson({ metadata: {} }, {}), 'an empty index needs no handle');
...
test('source lock: server.js never names media_items; lib/media/items.js is the table\'s only runtime writer; the adapter routes the doc object\'s metadata through the store', () => {
  ...
  assert.ok(/this\.items\.planDiff\(db\.metadata\)/.test(sqlite) && /this\.items\.applyPlan\(itemsPlan\)/.test(sqlite) && /this\.items\.advancePlan\(itemsPlan\)/.test(sqlite), 'save() runs the store\'s diff inside its transaction and advances the base after the commit');
  assert.ok(/const items = this\.items\.getAll\(\);/.test(sqlite), 'load() assembles the object from the table');
  assert.ok(!/doc_kv|doc_single/.test(sqlite.slice(sqlite.indexOf('class SqliteAdapter'))), 'Wave 7: nothing from the adapter class down names a doc table (the migration blocks above it are the only readers left)');
});
