...
test('source lock: the route surface never names the ytdlp tables or the dead doc spellings in CODE; the reads take the store; the scan consumes the bridge on a holder and queues its diff into the commit; the module writes ONLY through ytdlpDb.mutate', () => {
  ...
  // them there are four new crossings of the SAME store. Wave 7b (slice S7)
  // took it from 15 to 17: the backup bundle's `ytdlpDb.read()` (asserted
  // above) moved to lib/admin/backup.js, so server.js's one new deps-bundle
  // entry and the one destructure that receives it there are two more.
