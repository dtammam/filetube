...
test('a tap on the video or the bar reveals + re-arms (additive/passive; skip-gesture untouched)', () => {
  ...
  // v1.134: the video listener grew the tap-consume STAMP (tap-to-pause needs
  // to know when its own touch-down woke a hidden bar) - the reveal is still
  // here, still immersive-gated, still passive; the stamp mechanics are bound
  // in player-video-tap-pause.test.js.
  assert.match(loop[1], /mediaPlayer\.addEventListener\(evt, function \(\) \{\n[\s\S]{0,400}?revealControlsAndReArm\(\);\n\s*\}, \{ passive: true \}\);/, 'video reveal');
