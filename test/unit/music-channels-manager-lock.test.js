...
test('the manager renders only for a library-write user (admin or canModifyLibrary)', () => {
  const m = SRC.match(/musicChannelsGroup && musicChannelsList[\s\S]{0,2200}?musicChannelsGroup\.hidden = false/);
...
test('the manager reads the channel list and writes an explicit on/off per toggle', () => {
  ...
  assert.match(SRC, /fetch\('\/api\/folders\/music-flag',[\s\S]{0,160}?method: 'POST'[\s\S]{0,160}?folderName: ch\.folderName, music: cb\.checked \? 'on' : 'off'/,
    'writes an explicit on/off for the tapped channel');
