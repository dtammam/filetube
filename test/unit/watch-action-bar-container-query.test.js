...
test('style.css: the compact (959px) container query exists; the glyph line-box rule stands; the phone block hides only More\'s word (v1.203)', () => {
  ...
  assert.ok(!/max-width: 639px/.test(css), 'v1.203: the glyph-only threshold is gone - primary words show at every width');
  assert.match(css, /\.watch-action-btns \.btn > i,\s*\.watch-action-btns \.btn > svg\s*\{\s*min-height: calc\(var\(--lh-relaxed\) \* 1em\);\s*\}/, 'the glyph keeps the label\'s line box so a glyph-only More stays 32px');
