...
const scanOrchestrator = require('./lib/scan/orchestrator'); // Wave 7b S9: the require sits at its call site (scan-helpers-extraction.test.js asserts every lib/scan module is required by server.js)
const {
  parseFfprobeStreams,
  ...
  resolveOnDiskPath,
  runScanDirectories,
} = scanOrchestrator.createScanOrchestrator({
  ALL_EXTENSIONS,
  AUDIO_EXTENSIONS,
  READ_ONLY_MEDIA,
  THUMBNAIL_DIR,
  TRASH_DIR_NAME,
  __getPersistedStateEpoch, // LIVE reader: server.js let bumped by replacePersistedState (a snapshot would freeze the epoch)
  buildFfprobeArgs,
  ...
  loadDatabase,
  maybeYieldScan,
  needsTranscode, // stays in server.js (its TRANSCODE_EXTENSIONS list is text-locked there by tv-scan.test.js)
  ...
  probeCodecsOnly,
  progressStore,
  pushDelivery, // the download-complete push bridge
  queueAudioExtract: (...args) => queueAudioExtract(...args), // S8 (parallel): lazy so it survives S8 turning this hoisted fn into a factory const below
  reconcileTranscode: (...args) => reconcileTranscode(...args), // S8 (parallel): lazy so it survives S8 turning this hoisted fn into a factory const below
  restoreMissingPreviewClip,
  restoreMissingStoryboard,
  restoreMissingThumbnail,
  runFfmpegQuiet,
  scanState, // the LIVE scan-state object (const, never reassigned; /api/scan-status reads the same object)
  settingsStore,
  storyboardPath,
  storyboardSeekTimes,
  subtitles,
  tombstoneStore,
  transcodedPath,
  trashOrphanFile: (...args) => trashOrphanFile(...args), // S5: a const from createTrashOps below this call site - lazy defers the read past TDZ
  trashStore,
  updateDatabase,
  userStore,
  viewCountStore,
