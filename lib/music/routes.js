...
function registerLibraryRoutes(app, deps) {
  ...
  app.get('/api/music', (req, res) => {
    ...
    if (req.query.filter === 'recent-listening') {
      ...
      // signal. Same list, same visibility gate (the filter runs on the gated `list`).
      const withFinished = req.query.include === 'finished';
      list = list.filter((t) => {
        const p = progressMap[t.id];
        if (!p) return false;
        if (Number(p.position) > 0) return true;
        return withFinished && typeof p.updatedAt === 'string' && p.updatedAt !== '';
      });
