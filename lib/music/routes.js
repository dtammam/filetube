...
function registerLibraryRoutes(app, deps) {
  ...
  app.get('/api/music', (req, res) => {
    ...
    if (req.query.filter === 'recent-listening') {
      ...
      // Pocket menus (2026-09-24, gate r1 Q6): `include=finished` (opt-in; every existing
      // caller sends nothing and gets exactly the rows above) also keeps a track PLAYED TO
      // ITS END. The player's 'ended' arm writes position 0 to the same progress row (the
      // C2 reset), so the row survives with a fresh updatedAt - that row is the finished-play
      // signal. Same list, same visibility gate (the filter runs on the gated `list`). (Every
      // progress row carries an updatedAt - both writers stamp one - so a row's presence is the
      // signal; gate r2 dropped a dead updatedAt check.)
      const withFinished = req.query.include === 'finished';
      list = list.filter((t) => {
        const p = progressMap[t.id];
        if (!p) return false;
        return Number(p.position) > 0 || withFinished;
      });
