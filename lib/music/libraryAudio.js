...
//
// Framing (from the LIVE data - see docs/exec-plans/completed/2026-08-29-
// music-library-projection.md): yt-dlp embeds only { title, artist, date, genre, ... } - NO
...
// v1.242 (Dean: "anything that's audio only can be listened to in the audio player";
// "everything flipped instantly"): the projection is now UNCONDITIONAL for audio - EVERY
// type 'audio' item is in Music, for everyone, unless its channel is EXPLICITLY marked
// 'off' (that opt-out survives). No per-user opt-in, no genre/majority heuristic. The
// genre-majority helpers above stay ONLY for the Settings channel-toggle's default display;
// the projection predicate is now this. RBAC (mediaVisibleTo) is applied by the caller and
// is UNCHANGED - a restricted user still cannot see hidden audio.
function isEligibleAudioUniversal(item, channelMarks) {
  if (!item || item.type !== 'audio') return false;
  const marks = channelMarks && typeof channelMarks === 'object' ? channelMarks : {};
  const fn = item.folderName;
  return !(typeof fn === 'string' && marks[fn] === 'off'); // eligible unless explicitly opted out
}

// v1.242: a CHANNEL is in Music (universal projection) unless it carries an explicit 'off'
// mark. The "Channels in Music" manager is now an OPT-OUT list, so this is the honest
// `effective` state it displays. (channelEffectiveOn above - the retired majority heuristic -
// stays only for its own unit tests; the manager + flag endpoints use THIS.)
function channelEffectiveOnUniversal(folderName, channelMarks) {
  const marks = channelMarks && typeof channelMarks === 'object' ? channelMarks : {};
  return !(typeof folderName === 'string' && marks[folderName] === 'off');
}
...
function epochToIso(n) {
  // new Date(n).toISOString() THROWS ("Invalid time value") for an epoch outside
  // the ±8.64e15 ms Date range, so a garbage numeric addedAt must not reach it.
  if (!Number.isFinite(n) || n < -8.64e15 || n > 8.64e15) return '';
  return new Date(n).toISOString();
}
function addedAtToIso(v) {
  if (typeof v === 'number') return epochToIso(v);
  if (typeof v === 'string' && v) {
    if (/^\d+$/.test(v)) return epochToIso(Number(v));
...
// M3 chapter likes (v1.317): the INVERSE of chapterTrackId, and the ONE place the
// `::c<idx>` shape is decoded for storage purposes. Returns `{ baseId, index }` for
// a well-formed chapter id, null for anything else (a plain media id, a bare
// `::c`, a non-integer index, an empty base). The caller still has to prove the
// base item exists AND that `chapterTrackId(baseId, index)` is in the item's REAL
// expansion (expandAudioToTracks skips invalid chapters but keeps the original
// index, so "index < chapters.length" is NOT the validity rule).
function parseChapterTrackId(id) {
  if (typeof id !== 'string') return null;
  const m = /^(.+)::c(\d+)$/.exec(id);
  if (!m) return null;
  return { baseId: m[1], index: Number(m[2]) };
}
// The media id a like row belongs to: the base id of a chapter like, else the id itself.
function chapterLikeBaseId(id) {
  const parsed = parseChapterTrackId(id);
  return parsed ? parsed.baseId : id;
}
...
module.exports = {
  ...
  channelEffectiveOn,
  isEligibleAudio,
  isEligibleAudioUniversal,
  channelEffectiveOnUniversal,
  projectAudioItem,
  expandAudioToTracks,
  chapterTrackId,
  parseChapterTrackId,
