...
function sortTracks(tracks, sortKey, rng) {
  ...
  switch (sortKey) {
    ...
    // Friction pass (Dean): sort by the content's RELEASE date - the natural
    // order for yt-dlp downloads (the release/upload date we capture). A track
    // with no releaseDate (most native rips) sorts by addedAt within the 0 bucket.
    case 'release-newest':
      return list.sort((a, b) => ((Number(b.releaseDate) || 0) - (Number(a.releaseDate) || 0))
        || String(b.addedAt).localeCompare(String(a.addedAt)));
    case 'release-oldest':
      return list.sort((a, b) => {
        const av = Number(a.releaseDate) || 0;
        const bv = Number(b.releaseDate) || 0;
        return (av - bv) || String(a.addedAt).localeCompare(String(b.addedAt));
      });
    case 'album-order':
