...
function groupArtists(tracks, sortKey) {
  ...
  for (const t of tracks) {
    ...
    if (!rep) {
      g.albums.set(key, { album: typeof t.album === 'string' ? t.album : '', artId: t.id, hasArt: !!t.hasEmbeddedArt });
    } else if (!rep.hasArt && t.hasEmbeddedArt) {
      // Upgrade the album's representative to a track that carries embedded art.
      rep.artId = t.id;
      rep.hasArt = true;
    }
  }
