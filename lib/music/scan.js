 ...
 * Symlinked directories are NOT followed (loop hygiene); an unreadable
 * directory is skipped with a warn (NEVER a throw) AND its path is recorded in
 * the optional `erroredDirs` array. Recording matters (gate ADV-WARNING-1): a
 * transient per-directory EACCES / failed automount is NOT an emptied library,
 * so the prune must protect tracks UNDER an errored dir even when the root as a
 * whole still has survivors (the whole-root mount-loss guard alone does not
 * cover a subtree blip).
 */
function walkMusicRoot(root, erroredDirs) {
  const found = [];
  const stack = [root];
  while (stack.length > 0) {
    ...
    for (const dirent of dirents) {
      ...
      if (dirent.isDirectory()) {
        // v1.65: the media trash dir is invisible to every walker (a music
...
async function collectTracks(folders, previousTracks, deps) {
  ...
  for (const root of Array.isArray(folders) ? folders : []) {
    ...
    for (const filePath of walkMusicRoot(root, erroredDirs)) {
      processed += 1;
  ...
  return { tracks, survivingIds, missingRoots, erroredDirs };
}
