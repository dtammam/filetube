...
// Wave 5 of the relational-migration arc: the namespace's TABLES -
// music_folders (the root list), music_tracks (one row per track, verbatim),
// music_settings (one row per key), music_channels (the per-channel-folder
// "show in Music" marks - a doc_single MAP before, one row per folder now).
// server.js creates ONE store (`musicDb`); reads take `musicDb.read()`,
// mutators run `musicDb.mutate((holder) => { const ns = ensureMusic(holder); ... })`.
const { defineFeatureStore } = require('../db/featureStore');
const FEATURE = defineFeatureStore({
  name: 'music',
  parts: {
    folders: { kind: 'list', column: 'path' },
    tracks: { kind: 'map', keyColumn: 'track_id' },
    settings: { kind: 'kv' },
    channels: { kind: 'map', keyColumn: 'folder_name', docSingleMap: true },
  },
});

module.exports = {
  FEATURE,
  createMusicStore: (adapter, opts) => FEATURE.createStore(adapter, opts),
  ensureMusic,
  readMusic,
