...
//      width. ONE decoder context resident, so RSS is ~a single-frame grab
//      (v1.92-equivalent) no matter the file size.
//   2. ASSEMBLE (buildStoryboardAssembleArgs): one ffmpeg reads the grabbed
//      frames as an image2 numbered sequence and tiles them into the cols x
//      rows sprite - it only ever decodes the small tile JPEGs, never the
//      source.
//
...
// while capping resident memory at one decoder. Output geometry is unchanged
// (same cols x rows, same SB_TILE_W tiles), so sprites already on disk stay
// valid and never need regenerating.

// One single-input ffmpeg that grabs the frame at `seekTime` and writes it,
// scaled to `tileW` wide (even height for the encoder), to `outFramePath`.
...
// (`inputPattern` = e.g. `.../f%03d.jpg`, starting at 0) and tiles them into a
