...
function buildTranscriptLines(cues) {
  ...
  let prevOut = []; // out[] index of each of prev.lines (so a rolled line can extend its endMs)
  for (const cue of Array.isArray(cues) ? cues : []) {
    ...
    // A rolled line stays ON SCREEN through this cue: extend its endMs so the
    // prose paragraph rule measures the pause from when the line really left
    // the screen (gate: a line first shown in a short cue and then rolled
    // through a long one used to open a false paragraph).
    const curOut = [];
    for (let i = 0; i < overlap; i++) {
      const idx = prevOut[prev.lines.length - overlap + i];
      if (idx !== undefined) out[idx].endMs = Math.max(out[idx].endMs, cue.endMs);
      curOut.push(idx);
    }
    for (let i = overlap; i < cue.lines.length; i++) {
      curOut.push(out.length);
      out.push({ startMs: cue.startMs, endMs: cue.endMs, text: cue.lines[i] });
    }
    prev = cue;
    prevOut = curOut;
  }
...
// v1.201 (Dean): in prose mode a new paragraph starts only at a real pause -
// this much silence between one line's cue END and the next line's cue
// START. Rolling captions abut (0ms); a breath between sentences is well
// under this; a topic change / cut usually is not.
const PARAGRAPH_GAP_MS = 2000;

/**
 * Render transcript lines to text. With `timestamps` each line is its own
 * row, `[m:ss] text` (v1.200 behaviour, byte-identical). Without, the
 * lines are joined into PROSE paragraphs (Dean: "a flat text block
 * without all of those new lines") - spaces between lines, a blank line
 * only where the captions paused for PARAGRAPH_GAP_MS or more. Pure.
 * @param {Array<{startMs:number, endMs?:number, text:string}>} lines
...
function renderTranscriptBody(lines, opts) {
  ...
  if (opts && opts.timestamps) {
    return list.map((l) => `[${formatTranscriptTime(l.startMs)}] ${l.text}`).join('\n');
  }
  const paragraphs = [];
  let current = [];
  let prevEnd = null; // null until the first line - so no break can precede it
  for (const l of list) {
    if (!l || typeof l.text !== 'string') continue;
    const start = Number(l.startMs);
    if (prevEnd !== null && Number.isFinite(start) && start - prevEnd >= PARAGRAPH_GAP_MS) {
      paragraphs.push(current.join(' '));
      current = [];
    }
    current.push(l.text);
    // buildTranscriptLines always sets endMs; a caller that omits it gets
    // the start (the pause is then measured start-to-start).
    prevEnd = Number.isFinite(Number(l.endMs)) ? Number(l.endMs) : (Number.isFinite(start) ? start : prevEnd);
  }
