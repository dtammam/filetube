...
function buildTranscriptLines(cues) {
  ...
  for (const cue of Array.isArray(cues) ? cues : []) {
    ...
    for (let i = overlap; i < cue.lines.length; i++) out.push({ startMs: cue.startMs, endMs: cue.endMs, text: cue.lines[i] });
    prev = cue;
  }
  return out;
}
...
function renderTranscriptBody(lines, opts) {
  ...
  for (const l of list) {
    ...
    if (current.length > 0 && Number.isFinite(prevEnd) && Number.isFinite(start) && start - prevEnd >= PARAGRAPH_GAP_MS) {
      paragraphs.push(current.join(' '));
      current = [];
    }
    current.push(l.text);
    // A line with no endMs (older callers) keeps the previous end.
    if (Number.isFinite(Number(l.endMs))) prevEnd = Number(l.endMs);
  }
