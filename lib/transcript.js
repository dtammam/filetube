...
// tags and drops a line that the immediately-preceding, time-CONTIGUOUS cue
// already carried. Hand-authored `.srt`/`.vtt` captions (one cue per line,
// gaps between cues) pass through that rule untouched -- a genuinely repeated
// line ("Yeah." twice) survives whenever the two cues are not contiguous.
//
...
function buildTranscriptLines(cues) {
  ...
  for (const cue of Array.isArray(cues) ? cues : []) {
    ...
    for (const text of cue.lines) {
      if (contiguous && prev.lines.includes(text)) continue; // the rolled-forward repeat
      out.push({ startMs: cue.startMs, text });
    }
    prev = cue;
  }
  return out;
