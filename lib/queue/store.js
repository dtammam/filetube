...
// kind (v1.71): 'media' | 'podcast'; v1.72 adds 'track' (music joins the
// ONE queue - cap 3). A podcast entry's mediaId holds a podcast EPISODE
// id and a track entry's a music TRACK id - md5 hex exactly like media
// ids, so the kind is CARRIED on the entry, never inferred (the v1.69
// id-space lesson). The reducers are otherwise kind-agnostic: entries
// pass through filter/slice untouched, so only reduceAdd (which MINTS
// entries) names the roster.
...
function reduceAdd(state, mediaId, position, kind) {
  ...
  const entry = { uid: crypto.randomUUID(), mediaId, kind: (kind === 'podcast' || kind === 'track') ? kind : 'media' };
