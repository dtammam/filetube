...
// an argv array). `-map 0` = keep EVERY input stream (gate WARNING: bare
// `-c copy` defaults to one stream per type, silently DROPPING a yt-dlp mp4's
// embedded-thumbnail attached_pic or a multi-audio track -- and the overwrite is
// irreversible; -map 0 makes "no quality loss" literally true). `-c copy` = no
// re-encode; `-movflags +faststart` = moov first.
function buildFaststartRemuxArgs(inPath, outPath) {
  return ['-hide_banner', '-loglevel', 'error', '-y', '-i', inPath, '-map', '0', '-c', 'copy', '-movflags', '+faststart', outPath];
}
