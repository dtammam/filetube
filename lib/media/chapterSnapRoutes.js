...
// Chapter Snap (v1.319, Dean 2026-09-24): the four routes behind the chapter
...
// that changed since (another tab, the text editor, a reheat, a revert) is
// refused with 409 inside the write tick, so a stale editor can never
// overwrite newer chapters. A save may change start times ONLY: the chapter
...
function registerChapterSnapRoutes(app, deps) {
  ...
  function gate(req, res) {
    ...
    if (!item || !mediaVisibleTo(req, item)) { res.status(404).json({ error: 'Media file not found' }); return null; }
    return item;
  }
  function leadIn() {
  ...
  function editorState(item) {
    ...
    return {
      ...
      type: item.type,
      duration,
      version: snap.chaptersVersion(item, resolved),
  ...
  function freshForWrite(db, req, version, outcome) {
    ...
    const resolved = resolveItemChapters(item);
    if (typeof version !== 'string' || version !== snap.chaptersVersion(item, resolved)) {
      outcome.status = 409;
  ...
  app.post('/api/videos/:id/chapter-snap', async (req, res) => {
    ...
    const body = req.body || {};
    if (!Array.isArray(body.starts) || body.starts.length > snap.MAX_SNAP_CHAPTERS) {
    ...
    try {
      await updateDatabase((db) => {
        ...
        outcome.status = 200;
        outcome.body = { success: true, chapters: after.chapters, chaptersSource: after.chaptersSource, chaptersEdited: true, version: snap.chaptersVersion(fresh.item, after) };
        return true;
      });
    } catch (err) {
    ...
    try {
      await updateDatabase((db) => {
        ...
        if (plan.count !== from && body.allowCountChange !== true) {
          outcome.status = 409;
          outcome.body = { error: `The source now has ${plan.count} chapters, not ${from}. Reverting changes the chapter count, so liked chapters can move to a different song.`, countChange: { from, to: plan.count } };
          return false;
        }
        ...
        const after = resolveItemChapters(fresh.item);
        outcome.status = 200;
        outcome.body = { success: true, chapters: after.chapters, chaptersSource: after.chaptersSource, chaptersEdited: false, version: snap.chaptersVersion(fresh.item, after) };
        return true;
      });
    } catch (err) {
