...
function registerBrowseRoutes(app, deps) {
  const {
    ...
    chaptersSnapEdited, // v1.319 Chapter Snap: lib/media/chapterSnap isSnapEdited (the "Edited" badge flag)
    chaptersVersionOf, // v1.319 gate r1: the chapter editors' optimistic-concurrency token for an item
  ...
  app.get('/api/videos/:id', (req, res) => {
    ...
    res.json({
      ...
      // v1.319 Chapter Snap: the chapter times were corrected in the snap
      ...
      // v1.319 gate r1 (adversary S8): the token the TEXT editor sends back with its
...
function registerLibraryRoutes(app, deps) {
  const {
    ...
    moveItemToFolder, // lib/media/move.js's collision-safe file mover (slice S5)
    musicDb,
    chapterSnap, // v1.319: lib/media/chapterSnap - the version token + the snap provenance carry
    parseManualChapterText, // v1.319 gate r1: the editor grammar (optional .mmm fraction, no silent dedup)
    path,
    progressStore,
    refuseIfReadOnlyMedia,
    requireAdmin,
  ...
  // v1.319 - server.js parseManualChapterText, beside parseChapterLines) and the parsed result is stored as `chaptersManual` --
  ...
  app.post('/api/videos/:id/chapters', async (req, res) => {
    ...
    // v1.319 (Chapter Snap gate r1, adversary W1): the editor grammar round-trips a
    ...
    // v1.319 (gate r1, adversary S8): the editor may carry the `version` it was seeded
    ...
    if (notFound) return res.status(404).json({ error: 'Media file not found' });
    if (stale) return res.status(409).json({ error: 'These chapters changed since the editor opened. Close it and open it again to see the current list.', stale: true });
