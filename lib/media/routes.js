...
// Chapter Snap gate r2 (security-brief S-5, 2026-09-24): the media index is a PLAIN
// object, so `db.metadata['__proto__']` is Object.prototype (and 'constructor' /
// 'toString' are inherited functions) - a write route that looked an id up that way
// could write onto EVERY object in the process (a `__proto__` chapters save polluted
// Object.prototype, and the next scan's Phase-2 merge persisted it onto items). Every
// WRITE route in this module resolves the request id through this OWN-property lookup.
function ownMediaItem(db, id) {
  return db && db.metadata && typeof id === 'string' && Object.prototype.hasOwnProperty.call(db.metadata, id) ? db.metadata[id] : null;
}
...
function registerBrowseRoutes(app, deps) {
  const {
    ...
    chaptersSnapEdited, // Chapter Snap (2026-09-24): lib/media/chapterSnap isSnapEdited (the "Edited" badge flag)
    chaptersVersionOf, // chapter snap (2026-09-24) gate r1: the chapter editors' optimistic-concurrency token for an item
    ...
    sourceShare, // v1.337: lib/media/source-share resolver - a non-YouTube download's page URL, read from its file's tags
  ...
  app.get('/api/search', (req, res) => {
    ...
    const deps = {
      ...
      // v1.338 (plan first-class-any-site D5): a non-YouTube download's saved page link (re-checked).
      sourceShareUrl: (item) => (sourceShare ? sourceShare.saved(item, typeof item.youtubeId === 'string' ? buildWatchUrl(item.youtubeId) : null) : undefined),
  ...
  app.get('/api/videos', (req, res) => {
    ...
    const items = page.map(item => {
      ...
      // v1.338 (plan first-class-any-site D5/D6): a download from another site shares its saved page
      // link from the card corner too (never as `watchUrl`, which is YouTube's).
  ...
  app.get('/api/channels', (req, res) => {
    ...
    const siteBadgeByFolder = new Map(); // v1.338: folderName -> a site badge, used only when no item has a real photo
    for (const id of Object.keys(db.metadata || {})) {
      const item = db.metadata[id];
      ...
      if (!g.avatarUrl) {
        ...
        // v1.338 gate (adversary r1 W2): a REAL photo first, from any item of the folder; a site
        // badge (D7) only when none has one - a download from another site sharing a YouTube
        // channel's folder must not replace that channel's photo.
        const resolvedAvatar = ytdlp.resolveItemChannelAvatarUrl(ytView, item, { siteBadge: false });
        if (resolvedAvatar) g.avatarUrl = resolvedAvatar;
        else if (!siteBadgeByFolder.has(item.folderName)) {
          const badge = ytdlp.resolveItemChannelAvatarUrl(ytView, item);
          if (badge) siteBadgeByFolder.set(item.folderName, badge);
        }
      }
    ...
    for (const g of groups.values()) {
      if (!g.avatarUrl && siteBadgeByFolder.has(g.folder)) g.avatarUrl = siteBadgeByFolder.get(g.folder);
  ...
  app.get('/api/videos/:id', (req, res) => {
    ...
    const body = {
      ...
      // Chapter Snap (2026-09-24): the chapter times were corrected in the snap
      // editor (provenance rides inside chaptersManual) - drives the "Edited"
      // badge on the watch page's chapters list.
      chaptersEdited: chaptersSnapEdited(item),
      // chapter snap (2026-09-24) gate r1 (adversary S8): the token the TEXT editor sends back with its
      // save, so a list edited elsewhere since the page loaded is refused, not overwritten.
      chaptersVersion: chaptersVersionOf(item),
      progress: progress.timestamp,
    ...
    // we captured"): a yt-dlp download from another site has no YouTube `watchUrl`, so its watch page
    // gets `sourceShareUrl` - the page URL yt-dlp wrote into the file's own tags (cached per file,
    // time-limited). A SEPARATE field on purpose: `watchUrl` also feeds the player's chapter share and
    // the skins' timed share (`?t=` starts), which stay YouTube-only; v1.338 D6 made the cards, search
    // and the skins' plain share use `watchUrl || sourceShareUrl`. Every other item keeps this
    // synchronous path and a byte-identical response.
    if (!sourceShare || !sourceShare.wants(item, watchUrl)) {
      res.json(body);
      return;
    }
    // v1.338 (plan first-class-any-site D5): the SAVED page link answers at once; the file probe below
    // stays the fallback (an Opus file keeps its tags per stream; a comment that was the description).
    const savedShareUrl = sourceShare.saved(item, watchUrl);
    if (savedShareUrl) {
      res.json({ ...body, sourceShareUrl: savedShareUrl });
      return;
    }
    Promise.resolve()
      .then(() => sourceShare.resolve(item))
      .catch(() => null) // the resolver never rejects; belt: no link, never no answer
      .then((sourceShareUrl) => {
        res.json(sourceShareUrl ? { ...body, sourceShareUrl } : body);
      })
      .catch(() => { // res.json itself threw (a body that will not serialize): the sync path's 500
...
function registerHandoffAndDeleteRoutes(app, deps) {
  ...
  app.delete('/api/videos/:id', async (req, res) => {
    ...
    const db = getCachedDatabase();
    const item = ownMediaItem(db, req.params.id); // chapter snap gate r2 (S-5): own property only
    if (!item) {
      return res.status(404).json({ error: 'Media file not found' });
    }
...
function registerLibraryRoutes(app, deps) {
  const {
    ...
    booksDb,
    buildStoreZip,
    chapterLikeBaseId, // M3 chapter likes: a `<id>::c<n>` like counts by its BASE item's visibility
    chapterLikeTrack, // #235: ...and only while that file still has the chapter (the Liked listing's rule)
    ...
    moveItemToFolder, // lib/media/move.js's collision-safe file mover (slice S5)
    musicDb,
    chapterSnap, // chapter snap (2026-09-24): lib/media/chapterSnap - the version token + the snap provenance carry
    parseManualChapterText, // chapter snap (2026-09-24) gate r1: the editor grammar (optional .mmm fraction, no silent dedup)
    path,
    progressStore,
    refuseIfReadOnlyMedia,
    requireAdmin,
  ...
  app.get('/api/stats', (req, res) => {
    ...
    // Tracker #235 (music follow-ups): a member's like counts exactly when their Liked listing
    // (GET /api/liked) would list it - a plain like by its visible item; a `<id>::c<n>` chapter
    // like by its visible base file AND a chapter that file still has (chapterLikeTrack, the ONE
    // rule both readers share). Before, a re-chapter that dropped the index left this count at 1
    // while the listing showed 0. (The ADMIN inventory below reads a different namespace -
    // likedStore, the frozen pre-auth likes - and is untouched here; see tracker #243.)
    const likeCounts = (visMap, id) => {
      const base = chapterLikeBaseId(id);
      if (!has(visMap, base)) return false;
      return base === id || !!chapterLikeTrack(visMap[base], id);
    };
    ...
    if (isAdmin) {
      ...
      inventoryInput = {
        ...
        liked: userStore.getLiked(uid).filter((id) => likeCounts(visibleMetadata, id)), // M3 + #235: the Liked listing's own membership rule
  ...
  // same grammar description parsing uses, plus an optional .mmm fraction since
  // chapter snap (2026-09-24) - server.js parseManualChapterText, beside parseChapterLines) and the parsed result is stored as `chaptersManual` --
  ...
  app.post('/api/videos/:id/chapters', async (req, res) => {
    ...
    // chapter snap (2026-09-24, gate r1, adversary W1): the editor grammar round-trips a
    // sub-second start ("1:01.75 Title") and REFUSES a list that would put two
    // chapters on one start - a silent dedup drops a chapter and re-points every
    // later `<id>::c<n>` like.
    const manual = clearing ? { chapters: [], error: null } : parseManualChapterText(body.text);
    if (manual.error) return res.status(400).json({ error: manual.error });
    const parsed = manual.chapters;
    ...
    // chapter snap (2026-09-24, gate r1, adversary S8): the editor may carry the `version` it was seeded
    // with (GET /api/videos/:id `chaptersVersion`); a record that changed since (a snap
    // save in another tab, a reheat) is refused inside the write tick, never overwritten.
    ...
    try {
      await updateDatabase(db => {
        ...
        if (typeof body.version === 'string' && body.version !== chapterSnap.chaptersVersion(item, resolveItemChapters)) {
          stale = true;
          return false;
        }
        // chapter snap gate r2 (adversary S3, data-loss hardening): a snap edit's
        // sub-second times are only ever replaced by a client that proves what it saw -
        // a save WITHOUT the version (a tab still running pre-upgrade code) is refused.
        // A plain typed list keeps the old optional contract.
        if (typeof body.version !== 'string' && chapterSnap.isSnapEdited(item)) {
          stale = true;
          return false;
        }
        if (clearing) {
          ...
          // A title-only edit of a snap edit (same count, same starts) keeps its
          // provenance, so "Edited" and Revert survive a rename; any time or count
          // change makes it a plain typed list.
          item.chaptersManual = chapterSnap.carrySnapProvenance(item, parsed);
        }
        resolved = resolveItemChapters(item);
        version = chapterSnap.chaptersVersion(item, resolveItemChapters);
        edited = chapterSnap.isSnapEdited(item);
        return true;
      });
    } catch (err) {
    ...
    if (notFound) return res.status(404).json({ error: 'Media file not found' });
    if (stale) return res.status(409).json({ error: 'These chapters changed since the editor opened (or this page is out of date). Reload the page and open the editor again.', stale: true });
    res.json({ success: true, ...resolved, chaptersEdited: edited, version });
  });
  ...
  app.post('/api/videos/:id/attribute-channel', async (req, res) => {
    ...
    try {
      await updateDatabase(db => {
        result = applyManualAttribution(ownMediaItem(db, req.params.id) || undefined, identity, clearing); // chapter snap gate r2 (S-5)
    ...
    if (result === 'attributed') {
      const db = getCachedDatabase();
      const item = ownMediaItem(db, req.params.id);
