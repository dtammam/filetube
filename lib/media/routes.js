...
// server.js - the progress and Liked routers, the trash routes,
// /api/storage-summary and the byte-stream routes (/thumbnail, /storyboard,
// /preview, /video, /audio) are registered between them and stay behind - so
...
//                                after GET /audio/:id, which stays)
//
// The last three were ONE contiguous run of routes in server.js and could have
...
//     reads it as `ffmpegAvailable()`; server.js passes `() => ffmpegAvailable`.
...
// prepare-audio's "see GET /audio/:id's own comment" all mean server.js's,
// which is where those still live. Named here rather than edited in place -
// editing them would break the byte-identity the slice's verification rests on.
...
// trash routes and after `moveItemToFolder` and its collaborators, which stay
// in server.js until slice R3 moves them.
...
function registerPrepareAudioRoute(app, deps) {
  const {
    audioPath,
    ffmpegAvailable, // LIVE reader: a server.js `let` the async ffmpeg probe flips after boot
    fs,
  ...
  app.post('/api/videos/:id/prepare-audio', (req, res) => {
    ...
    if (!ffmpegAvailable()) { // S10a live seam: a destructured boolean would freeze at `false`
