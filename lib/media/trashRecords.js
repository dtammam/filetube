...
function createTrashRecordStore(adapter) {
  ...
  return {
    ...
    expiredBefore(cutoffMs) {
      if (!expiredStmt) expiredStmt = adapter.sql.prepare(`SELECT media_id, json FROM ${def.table} WHERE trashed_at IS NOT NULL AND trashed_at < ? ORDER BY trashed_at ASC, media_id ASC`);
      const out = {};
