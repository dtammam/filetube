...
function registerLikedRoutes(app, deps) {
  const {
    ...
    effectiveProgress,
    getCachedDatabase,
    itemChapterTracks, // M3 chapter likes: the ONE music expansion of a media item (server.js)
  ...
  app.post('/api/liked/:id', (req, res) => {
    ...
    let likeId = item.id;
    if (chapter) {
      const track = item.type === 'audio' ? itemChapterTracks(item).find((t) => t.id === req.params.id) : null;
  ...
  // expansion (a re-chaptered file drops a stale index at read time; the
  ...
  function shapedLikedChapterItems(db, req, likedIds, watchedSet) {
    ...
    for (const likeId of likedIds) {
      ...
      const track = itemChapterTracks(item).find((t) => t.id === likeId);
      if (!track) continue;
