...
//
// Wave 2 of the relational-migration arc (docs/exec-plans/completed/
...
//     of nesting BEGIN;
//   - ids: non-empty strings, no U+0000 (read back truncated on Node <= 24.14 - #225);
...
// The one id rule, shared by the read-side tolerance, the write-side assert,
// and the adapter's migrations (a legacy doc row whose key fails it was never
// addressable by any route and is skipped, not moved).
...
function assertMediaId(label, id) {
  ...
  if (id.includes('\u0000')) {
    throw new Error(`${label}: media id contains U+0000 - node:sqlite reads a NUL-bearing TEXT back truncated on Node 24.14 and older (verbatim from 24.20 - tech-debt #225), so a persisted NUL-bearing key would read back as a colliding prefix and silently corrupt the key. Refusing.`);
  }
}
...
// `keyColumn` (Wave 4): the PRIMARY KEY column's name - `media_id` for the
// media stores; a store keyed by a root path or a folder name names its
// column honestly instead of carrying a lying one.
function defineJsonRowStore({ label, table, columns = [], keyColumn = 'media_id' }) {
  const colNames = columns.map((c) => c.name);
  const allCols = [keyColumn, ...colNames, 'json'];
  const placeholders = allCols.map(() => '?').join(', ');
  const updates = [...colNames, 'json'].map((c) => `${c} = excluded.${c}`).join(', ');
  const UPSERT_SQL = `INSERT INTO ${table}(${allCols.join(', ')}) VALUES(${placeholders}) ON CONFLICT(${keyColumn}) DO UPDATE SET ${updates}`;
  ...
  function statementsFor(adapter) {
    ...
    const s = {
      get: sql.prepare(`SELECT json FROM ${table} WHERE ${keyColumn} = ?`),
      has: sql.prepare(`SELECT 1 AS x FROM ${table} WHERE ${keyColumn} = ?`),
      all: sql.prepare(`SELECT ${keyColumn} AS id, json FROM ${table} ORDER BY ${keyColumn}`),
      upsert: sql.prepare(UPSERT_SQL),
      del: sql.prepare(`DELETE FROM ${table} WHERE ${keyColumn} = ?`),
      // OR REPLACE: a destination row (a same-path re-add) is replaced by
      // the moving row - the other carriers' rule (lib/auth/store.js).
      rekey: sql.prepare(`UPDATE OR REPLACE ${table} SET ${keyColumn} = ? WHERE ${keyColumn} = ?`),
  ...
  function createStore(adapter) {
    ...
    return {
      ...
      // READS are tolerant: an id that could never have been persisted (not a
      // string, empty, NUL-bearing) simply reads as ABSENT. Routes hand these
      // stores request-derived ids (`/api/trash/%00`, a hostile bundle row),
      // and a throwing read turned a 404 into a hung async handler / a 500 -
      // the Wave 3 gate's CRITICAL. The WRITE-side assert stays: that is where
      // a NUL would silently corrupt a key.
      get(id) {
      ...
      // Wave 4: write a whole { [id]: record } map's entries WITHOUT wiping
      // the others (the update half of replaceAll) - validated whole first,
      // one transaction.
      setMany(map) {
        if (map == null || typeof map !== 'object' || Array.isArray(map)) throw new Error(`${label}: setMany expects an object map`);
        const entries = [];
        ...
        tx(() => { for (const [id, record] of entries) st().upsert.run(...rowParams(id, record)); });
      },
  ...
  return { createStore, UPSERT_SQL, rowParams, table, keyColumn, assertMediaId: (id) => assertMediaId(label, id) };
}

module.exports = { defineJsonRowStore, isPersistableId };
