...
// Chapter Snap (v1.319): the silence SCAN - one bounded ffmpeg `silencedetect`
...
const SILENCE_PARAMS_KEY = `n${SILENCE_NOISE_DB}d${SILENCE_MIN_SEC}v1`;
...
const NUM = '(-?\\d+(?:\\.\\d+)?(?:[eE][-+]?\\d+)?)';
const START_RE = new RegExp(`silence_start:\\s*${NUM}`);
const END_RE = new RegExp(`silence_end:\\s*${NUM}`);

// One stderr line -> an event or null. Only `[silencedetect @ ...]` lines
// count; any other line naming the words (a file title in the banner, say) is
// ignored.
function parseSilenceLine(line) {
  if (typeof line !== 'string' || line.indexOf('[silencedetect') === -1) return null;
  const e = END_RE.exec(line);
  if (e) return { kind: 'end', t: Number(e[1]) };
  const s = START_RE.exec(line);
  if (s) return { kind: 'start', t: Number(s[1]) };
  return null;
}
...
function createSilenceCache(dir) {
  ...
  function read(id) {
    ...
    try {
      const rec = JSON.parse(fs.readFileSync(fileFor(id), 'utf8'));
...
function createSilenceService(opts) {
  ...
  function start(item) {
    ...
    const job = chain.then(async () => {
      try {
        const silences = await run(filePath, { durationSec });
