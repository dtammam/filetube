...
//   - ids are the store's rule: non-empty, NUL-free (node:sqlite truncates
//     TEXT at NUL) - the WRITE refuses, reads are tolerant; a `__proto__` id
//     is inert own data (getAll defines own properties).
//
...
function migrateFromDoc(sql, log = console.error) {
  const rows = sql.prepare("SELECT key, json FROM doc_kv WHERE namespace = 'metadata' ORDER BY rowid").all();
  if (rows.length === 0) return 0;
  const upsert = sql.prepare(def.UPSERT_SQL);
  let moved = 0;
  for (const row of rows) {
    if (!isPersistableId(row.key)) { log(`[db] migration: skipping a metadata key no route could have written (${JSON.stringify(row.key)})`); continue; }
