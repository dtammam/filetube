...
// Chapter Snap (Dean 2026-09-24): the PURE core of the chapter-time
...
// chaptersManual carries the provenance with it. The TEXT editor's save (a full
// manual replace) keeps it only for a TITLE-ONLY edit - the same chapter count
// and every start within half a millisecond of the stored one
// (carrySnapProvenance) - so "Edited" and Revert survive a rename; any time or
// count change makes it a plain typed list with no provenance. REVERT restores
// TIMES and keeps the stored titles while the count is unchanged (typed titles are
// user data; Chapter Snap is times-only); only a count-changing revert (an
// explicit yes) takes the source's titles with its list (planRevert).
//
...
function planRevert(item, resolveItemChapters) {
  ...
  // Chapter snap gate r2 (Architect ruling on adversary W1): with the count UNCHANGED a
  // revert restores the source TIMES and KEEPS the stored titles - a rename typed in
  // the text editor is user data, and this editor is times-only. When the titles
  // already match the source, the manual list is simply dropped (the item resolves to
  // its source again). A count change takes the source list whole, titles included
  // (the route requires allowCountChange, and the confirm says so).
  if (r.chapters.length === cm.length) {
    const titlesMatch = r.chapters.every((c, i) => (typeof c.title === 'string' ? c.title : '') === (typeof cm[i].title === 'string' ? cm[i].title : ''));
    if (!titlesMatch) {
      const restore = r.chapters.map((c, i) => ({ startTime: Number(c.startTime), title: typeof cm[i].title === 'string' ? cm[i].title : '' }));
      return { restore, chapters: restore, chaptersSource: r.chaptersSource, count: restore.length, damaged: false, keepsTitles: true };
    }
  }
  return { restore: null, chapters: r.chapters, chaptersSource: r.chaptersSource, count: r.chapters.length, damaged: false, keepsTitles: false };
}
