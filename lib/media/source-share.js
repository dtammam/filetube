...
// new stored). Plan: docs/exec-plans/completed/2026-09-26-share-any-download.md.
//
// A yt-dlp download from a site other than YouTube (an item carrying `sourceExtractor` and no YouTube
// link) has no `watchUrl`, so the watch page showed no Share. The library does not store its page
// URL, but every download runs with `--embed-metadata`, and yt-dlp writes the page URL (`webpage_url`)
...
// This module turns that into the watch page's `sourceShareUrl`: validated, cached per file, one probe
