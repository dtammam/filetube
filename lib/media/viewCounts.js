...
//
// Wave 1 of the relational-migration arc (docs/exec-plans/active/
...
//   - every RUNTIME write to the table goes through this module. The two bulk
//     seams that also touch it live in lib/db/sqlite.js and are the arc's
//     tracked residual (tech-debt #224): the boot-time db.json import
//     (rule 2, retired in Wave 7) and exclusiveReplace (the restore /
//     test-reset wipe-and-replace primitive, which must wipe every table the
//     bundle repopulates). Both use UPSERT_SQL exported below - one SQL text.
...
// CORRUPT: a NUL (node:sqlite truncates TEXT at U+0000 - the repo's
...
function assertMediaId(id) {
  ...
  if (id.includes('\u0000')) {
    throw new Error('viewCounts: media id contains U+0000 - node:sqlite truncates TEXT at NUL, so persisting it would silently corrupt the key. Refusing.');
  }
}

function assertCount(n) {
  if (!Number.isInteger(n) || n < 0) {
    throw new Error(`viewCounts: count must be a non-negative integer (got ${String(n)})`);
  }
}

// A persisted value that is usable as a count: finite, non-negative. Used by
// the migration backfill and the import seams, where a legacy value may be a
// float or junk - anything else is dropped (the v1.42 import made the same
// call: "vid3's viewCount:0 is dropped; missing reads as 0").
...
function statementsFor(adapter) {
  ...
  const s = {
    ...
    // an existing row steps by exactly one. RETURNING hands back the new
    // value without a second read (the view route answers with it).
    increment: sql.prepare(`INSERT INTO ${TABLE}(media_id, count) VALUES(?, ?) ON CONFLICT(media_id) DO UPDATE SET count = count + 1 RETURNING count`),
...
function createViewCountStore(adapter) {
  ...
  return {
    ...
    // default 0 - never a reason to reprocess anything).
    get(id) {
      assertMediaId(id);
