...
//
// From-Liked orders by `addedAt` (item recency), NOT liked-recency: the store's
// media/music liked accessors return ids only, no liked_at, so ordering the
// mixed-kind Liked row by item recency is the honest choice that needs no field
// we cannot populate uniformly across all three kinds.
//
...
// HIDDEN when there is no UNSEEN item (spec row-def #2): a "New from your
// subscriptions" row that shows only already-watched items is a mislabel this
// repo's honesty norm guards against (QA gate WARNING). Zero subscriptions is
// the empty-pool sub-case of the same guard.
function selectNewFromSubs(records, cap) {
  const pool = arr(records).filter((r) => r && r.isSub);
  if (!pool.some((r) => !r.watched)) return []; // nothing unseen -> no row
...
// 5. From your Liked - explicitly-liked items across all kinds, newest item
// first (see the header note: no liked_at is available uniformly). An explicit
// per-user signal (no floor: even one favorite is a real, wanted row).
// Replaces a "Popular" row the store has no view count for.
...
function assembleHomeRows(input) {
  ...
  // See-all -> the subscription-SCOPED classic browse (?subs=1), newest-released
  // first - NOT the /subscriptions management page (v1.79.1, Dean: that page was
  // "practically useless" as a See-all target).
  push({ id: 'new-from-subs', title: 'New from your subscriptions', kind: 'row', seeAllHref: '/?subs=1', itemIds: subsIds });
  ...
  // See-all -> the classic grid via ?browse=1, which FORCES classic even for a
  // logged-in feed user (a bare '/' would just re-show the feed - v1.79.1, Dean:
  // "Recently added See-all does nothing"). All videos, the user's default sort.
  push({ id: 'recently-added', title: 'Recently added', kind: 'row', seeAllHref: '/?browse=1', itemIds: recentIds });
  ...
  // See-all -> the Liked playlist VIEW (?liked=1), not /playlists (which 404s -
  // there is no such route; the Liked grid lives at ?liked=1). v1.79.1, Dean.
  push({ id: 'from-liked', title: 'From your Liked', kind: 'row', seeAllHref: '/?liked=1', itemIds: selectFromLiked(records, cap) });
  push({ id: 'watch-again', title: 'Watch again', kind: 'row', seeAllHref: null, itemIds: selectWatchAgain(records, cap) });

  return { rows };
}

// ---- v1.84 Modern Mode: the flat-grid chip filter (PURE) -------------------
//
// Modern Mode replaces the rows with ONE flat grid spanning the media library
// (video + audio) plus downloaded podcasts; music keeps its own place (it is not
// a chip). The route gathers candidates (reusing the SAME RBAC + hidden-folder
// guards as the row path), then this pure predicate decides membership per chip.
// A grid candidate is { kind:'media'|'podcast', type:'video'|'audio', inProgress,
// watched }.
const MODERN_GRID_FILTERS = ['all', 'videos', 'audio', 'podcasts', 'continue', 'unwatched'];

// Unknown/absent -> 'all' (the client resolver defends too; the server bounds it
// so a hand-typed ?filter= can never 500 or leak an unintended set).
function resolveGridFilter(raw) {
  return MODERN_GRID_FILTERS.includes(raw) ? raw : 'all';
}

// v1.86.0 (Dean): the modern grid gains a real sort. These are exactly the keys
// videoQuery.sortItems understands (and the classic /api/videos sort menu
// offers), so the modern grid stays behaviourally identical to the classic grid.
// The sort is applied to the FULL candidate set BEFORE the 60-item cap, so
// "oldest"/"largest"/"feeling lucky" span the whole library, not just the newest
// snapshot. Unknown/absent -> 'newest' (server-bounded so a hand-typed ?sort=
// can never reach videoQuery with an unexpected key).
const MODERN_GRID_SORTS = ['newest', 'oldest', 'release-date', 'title-asc', 'title-desc', 'size-desc', 'size-asc', 'random'];

function resolveGridSort(raw) {
  return MODERN_GRID_SORTS.includes(raw) ? raw : 'newest';
}

// Pure membership. Semantics (disclosed in the exec plan):
//   all       -> every gathered candidate (media + podcasts)
//   videos    -> media, type 'video'
//   audio     -> media, type 'audio' (yt-dlp MP3s)
//   podcasts  -> podcast episodes
//   continue  -> anything with a saved, unfinished position (media OR podcast)
//   unwatched -> media neither latched NOR finished-by-threshold (podcasts carry
//                NO watched latch -> out). `finished` (deriveWatchState: latch OR
//                pct>=90) is excluded too, so a 95%-watched-but-unlatched legacy
//                position is NOT called "unwatched" (QA gate suggestion).
function matchesGridFilter(rec, filter) {
  const f = resolveGridFilter(filter);
  const isMedia = !!rec && rec.kind === 'media';
  switch (f) {
    case 'videos': return isMedia && rec.type === 'video';
    case 'audio': return isMedia && rec.type === 'audio';
    case 'podcasts': return !!rec && rec.kind === 'podcast';
    case 'continue': return !!rec && !!rec.inProgress;
    case 'unwatched': return isMedia && !rec.watched && !rec.finished;
    case 'all':
    default: return true;
  }
}
