...
// Wave 5 of the relational-migration arc: the namespace's TABLES. `folders`
// (tv_folders, the operator's root list), `episodes` (tv_episodes, one row per
// episode id, the record verbatim) and `settings` (tv_settings, one row per
// key). server.js creates ONE store (`tvDb`) with the doc commit's
// inSaveTransaction hook: reads take `tvDb.read()` (the readTv shape),
// mutators run `tvDb.mutate((holder) => { const ns = ensureTv(holder); ... })`
// and the diff rides the commit. The CREATE TABLEs stay literal in
// lib/db/sqlite.js's v27 block (the diagrams census reads them there).
...
module.exports = {
  ...
  FEATURE,
  createTvStore: (adapter, opts) => FEATURE.createStore(adapter, opts),
};
