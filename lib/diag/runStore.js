...
//
// DATA_DIR/.diag is created lazily by ensureDir() - only once an admin actually
// uses the feature (save() persisting a run, OR list() reading the run browser)
// with the perfDiagnosticsEnabled setting (or FT_DIAG) on. A normal install with
// the feature off never calls save()/list(), so the directory is never created.
...
function createRunStore({ fs, path, diagDir }) {
  ...
  // Strip everything outside the safe slug charset, so a caller-supplied id can
  // never escape diagDir (no '.' or '/' survives). Returns '' for an id that is
  // empty after stripping - callers treat that as "no such run" rather than an
  // error, so a malformed :id is a clean 404, not a 500.
  ...
  function get(id) {
    if (!safeSlug(id)) return null; // malformed id -> no such run (404, not a 500)
    const p = runPath(id);
  ...
  function remove(id) {
    if (!safeSlug(id)) return false; // malformed id -> nothing to remove (404, not a 500)
    const p = runPath(id);
    if (fs.existsSync(p)) {
      fs.unlinkSync(p);
      return true;
