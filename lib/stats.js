...
// a metadata-shaped id->item map. Every function here is a plain synchronous
// transform -- object/array in, plain data out -- with NO filesystem access,
// no database I/O, and no `require('../server')`. `GET /api/stats`
// (server.js) is a thin HTTP wrapper: one cached read, then
// `computeLibraryStats(...)` exactly once per request. This is a deliberate
// LIVE compute, not a cached aggregate -- see the exec-plan's Design section
// ("C4 cached stats aggregate ... rejected"): at home-server scale
// (thousands of items, not millions) an O(n) pass per request is trivial and
// always fresh; a cached aggregate would need its own invalidation story for
// zero real benefit.
//
// "Most-watched" reads `item.viewCount` off the items it is HANDED. The
// persisted counter was extracted out of the metadata item in v1.42 (where
// the scan could clobber it) and since Wave 1 of the relational-migration arc
// lives in the `media_view_counts` table (lib/media/viewCounts.js); the stats route
// overlays effective counts onto COPIES of the items
// (`withEffectiveViewCounts`, server.js) before calling in, so this module
// stays a pure item-shape consumer with no knowledge of the extraction. The
// counter increments ONCE per watch-page open via the dedicated
// `POST /api/videos/:id/view` route -- see that route's own comment for why
// it is deliberately NOT folded into `POST /api/progress` or the
// `/video/:id` Range-serve.
...
// generally first-class experiences"): a download from another site (a
// `sourceExtractor`, no YouTube `channelUrl`) groups by site + uploader - its
// captured `channelName`, else the folder it landed in (the universal lane
// folds by uploader, lib/ytdlp/args.js). `null` for anything else.
function universalChannelGroup(item) {
  if (!item || hasText(item.channelUrl) || !hasText(item.sourceExtractor)) return null;
  const uploader = hasText(item.channelName) ? item.channelName.trim() : (hasText(item.folderName) ? item.folderName.trim() : '');
  if (uploader === '') return null;
  const sourceExtractor = item.sourceExtractor.trim();
  return { key: `${sourceExtractor}:${uploader}`, sourceExtractor, channelName: uploader };
}

// Grouped by `channelUrl` -- only ever set on yt-dlp-managed YouTube items
// (see server.js's Phase-2 channel-identity bridge). v1.338 D8b: a download
// from another site has no `channelUrl`; it joins as its own row per site +
// uploader (`{ sourceExtractor, channelName, ... }`, grouped in a SEPARATE
// pass so its key can never merge into a YouTube row). Plain library files
// have neither and are correctly excluded (they don't belong to any
// channel), not folded into a misleading catch-all bucket.
function computeBreakdownByChannel(items) {
  const youtube = groupBy(items, (item) => item && item.channelUrl).map((g) => ({
    channelUrl: g.key, count: g.count, totalDurationSeconds: g.totalDurationSeconds, totalSizeBytes: g.totalSizeBytes,
  }));
  const labels = new Map();
  const universal = groupBy(items, (item) => {
    const group = universalChannelGroup(item);
    if (!group) return undefined;
    if (!labels.has(group.key)) labels.set(group.key, group);
    return group.key;
  }).map((g) => ({
    sourceExtractor: labels.get(g.key).sourceExtractor, channelName: labels.get(g.key).channelName,
    count: g.count, totalDurationSeconds: g.totalDurationSeconds, totalSizeBytes: g.totalSizeBytes,
  }));
  if (universal.length === 0) return youtube;
  // groupBy's own order (count desc, then key asc) across both kinds.
  const keyOf = (row) => row.channelUrl || `${row.sourceExtractor}:${row.channelName}`;
  return [...youtube, ...universal].sort((a, b) => (b.count - a.count) || keyOf(a).localeCompare(keyOf(b)));
}
...
// ---- v1.44.3 (Dean): library inventory -------------------------------------
//
// "More stats -- the things the backups show": a plain COUNT of each persisted
// namespace (the same namespaces the backup bundle carries), so the Stats page
// answers "what is actually in my database". Pure: the caller passes the
// already-read namespace objects (no DB/fs access here, per this module's
// header contract). `users` is passed as an integer count (SQLite row count);
// everything else is a namespace object (id-keyed map) or array whose SIZE is
// the count.
function inventorySize(ns) {
  if (Array.isArray(ns)) return ns.length;
  if (ns && typeof ns === 'object') return Object.keys(ns).length;
  return 0;
}

function computeInventory(parts) {
  const p = parts || {};
  const books = (p.books && typeof p.books === 'object') ? p.books : {};
  const music = (p.music && typeof p.music === 'object') ? p.music : {};
  return {
    videos: inventorySize(p.metadata),            // db.metadata -- the library index
    watchProgress: inventorySize(p.progress),     // saved video/audio positions
    viewCounts: inventorySize(p.viewCounts),      // per-item watch counters
    liked: inventorySize(p.liked),                // the frozen pre-auth likes (an array; media_liked since Wave 4)
    deleteTombstones: inventorySize(p.deleteTombstones),
    scanFolders: inventorySize(p.folders),
    books: {
      items: inventorySize(books.items),
      progress: inventorySize(books.progress),    // reading positions
      narrationAudio: inventorySize(books.audio), // books with TTS synthesis state
    },
    music: {
      tracks: inventorySize(music.tracks),
      folders: inventorySize(music.folders),
    },
    users: Number.isFinite(p.users) ? p.users : inventorySize(p.users),
  };
}
...
//     applied here. v1.338 D8c: a download from another site joins by its
//     (site, id) pair instead - see universalSourceKey below.
//
...
// regex. Omitting it just drops the filename-bracket lens (the persisted
// `youtubeId` and, v1.338 D8c, the site-id key still group).
...
// v1.338 D8c (Dean: "non-YouTube things supported by YT DLP should have
// generally first-class experiences"): a download from another site has no
// YouTube id; its identity is the (site, id) pair the scan persisted
// (`sourceExtractor` + `sourceId`, the same pair the delete archives). The key
// always carries a ':' - a YouTube id (VIDEO_ID_SHAPE, or the bracket
// extractor's 11 chars) never does - so a site id can never group with a
// YouTube id, even one spelled the same. Asked only when no YouTube id
// resolved (a proxy-host YouTube download keeps its YouTube key). `null`
// when either half is missing.
function universalSourceKey(item) {
  if (!item || !hasText(item.sourceExtractor) || !hasText(item.sourceId)) return null;
  return `${item.sourceExtractor.trim()}:${item.sourceId}`;
}
...
module.exports = {
  ...
  // v1.44.3 (Dean): the library-inventory namespace counts.
  computeInventory,
