...
function computeDuplicateReport(metadata, opts) {
  ...
  for (const [vid, group] of byVideoId) {
    ...
    const distinctNames = new Set(group.map((item) => path.basename(item.filePath)));
  ...
  const push = (section, groups) => {
    for (const group of groups) {
      for (const item of group.items) {
        rows.push([section, group.key, item.filePath, item.size, group.items.length, group.wastedBytes]);
      }
    }
  };
