...
function shiftVttCues(text, offsetSeconds) {
  ...
  // Gate fix (adversarial): cue-BLOCK context tracking. A real WebVTT parser
  // only reads a timing line at a block boundary -- once a cue is open,
  // every line up to the blank terminator is PAYLOAD, even one that happens
  // to look exactly like a timing line (captions quoting timestamps are a
  // real genre). Without this, such a payload line was re-parsed as a new
  // cue and shifted/dropped, corrupting the enclosing cue.
  let inCueBlock = false;
  for (const line of lines) {
    if (line.trim() === '') inCueBlock = false;
    const m = !inCueBlock ? VTT_CUE_TIMING_LINE.exec(line) : null;
    if (m) {
      ...
      if (shiftedEnd <= 0) {
        ...
        droppingCue = true;
        // Gate fix (adversarial delta): a DROPPED cue opens a block too --
        // without this, a timestamp-shaped PAYLOAD line inside the dropped
        // cue whose own time would independently evaluate to "keep" was
        // promoted into a phantom kept cue, leaking the dropped cue's
        // remaining payload under fabricated timing. Symmetric with the
        // keep branch below.
        inCueBlock = true;
        continue;
      }
 ...
 *      unspecified ordering. v1.47.4: yt-dlp's `-orig` marker is DEMOTED
 *      below real language tags within that order (see
 *      `isOriginalLangMarker`) -- it still wins when it is the only sidecar,
 *      it just no longer beats e.g. `en-US` on alphabetical accident.
 *   3. a bare `<base>.srt`.
 *
 ...
 */
// v1.47.4 (Dean, found while investigating his "unknown language" report):
// yt-dlp's `-orig` SUFFIX is not a language tag at all -- it is YouTube's
// marker for the auto-caption track in the video's ORIGINAL spoken language,
// minted at extractor/youtube/_video.py:4294 as `f'{trans_code}-orig'`. A real
// download of an English video lands three English sidecars:
//
//   ...en.vtt        the plain auto-caption
//   ...en-US.vtt     the manually-authored track
//   ...en-orig.vtt   the "(Original)" auto-caption variant
//
// `langVttRe` happily accepts `en-orig` as `<2-3 letters>-<subtag>`, and the
// tie-break below used to be plain alphabetical by filename -- where "en-orig"
// sorts BEFORE "en-US". So on any video offering no bare `en` track, FileTube
// served YouTube's raw original-language variant in preference to the clean,
// human-authored regional one.
//
// Deliberately a DEMOTION, not an exclusion: an `-orig` sidecar is still a
// perfectly good caption file, and when it is the only one present it must
// still be served. It just must never beat a real language tag.
function isOriginalLangMarker(lang) {
  return typeof lang === 'string' && lang.toLowerCase().endsWith('-orig');
}

/**
 * The ONE deterministic winner rule for lang-tagged sidecars, shared by both
 * the cached and no-cache paths below.
 *
 * Extracted in v1.47.4 because it was previously duplicated verbatim in those
 * two paths -- and this repo's recurring bug class is exactly that (v1.41.4:
 * "the bug is the seat that forgot to CALL the shared helper"). Two copies of a
 * preference rule is two places for the next fix to miss.
 *
 * Order: an exact `en` wins outright; otherwise the first match by FILENAME in
 * a stable order (never `readdirSync`'s unspecified order), skipping `-orig`
 * markers unless nothing else is available.
 *
 * v1.47.4 gate SUGGESTION (adversarial seat): the tie-break used
 * `localeCompare`, and the comment claimed an order that "never depends on the
 * environment". That was false -- `localeCompare` varies with the ICU build and
 * the default locale. Demonstrated divergence on the very filenames this
 * function exists to order:
 *
 *   'v.en-US.vtt'.localeCompare('v.en-orig.vtt')  ->  1   (en-orig first)
 *   'v.en-US.vtt' < 'v.en-orig.vtt'               -> true (en-US first)
 *
 * A CODE-UNIT comparison is deterministic everywhere, which is the property
 * that was actually wanted: the same directory must resolve the same sidecar on
 * every machine, or two FileTube instances over the same library would serve
 * different caption tracks.
 */
function pickPreferredLangMatch(langMatches) {
  const exactEn = langMatches.find((m) => m.lang === 'en');
  if (exactEn) return exactEn;
  const sorted = langMatches.slice().sort((a, b) => {
    if (a.name === b.name) return 0;
    return a.name < b.name ? -1 : 1;
  });
  return sorted.find((m) => !isOriginalLangMarker(m.lang)) || sorted[0];
}
...
function findSubtitleSidecar(filePath, fsImpl, dirCache) {
  ...
  if (dirCache instanceof Map) {
    ...
    if (langMatches.length > 0) {
      return { path: path.join(dir, pickPreferredLangMatch(langMatches).name), format: 'vtt' };
    }
  ...
  if (langMatches.length > 0) {
    // Deterministic preference -- see `pickPreferredLangMatch`. The rule lives
    // in exactly one place so the cached path above can never drift from this
    // one (they were duplicated verbatim before v1.47.4).
    return { path: path.join(dir, pickPreferredLangMatch(langMatches).name), format: 'vtt' };
  }
...
// v1.41.1 (Dean): force every cue to BOTTOM-CENTER. A WebVTT cue's horizontal
// position + text justification come from per-cue SETTINGS on the timing line
// (`position`/`align`/`line`/`size`) -- CSS `::cue` can restyle the text but
// cannot move the cue box. Captions from SRT (converted -> no settings, browser
// default) OR from yt-dlp `.vtt` sidecars (often author-positioned) can render
// off-center / left-justified. This strips each cue's existing position/align/
// line/size settings and forces `position:50% align:center` (line left at its
// default => bottom), so ALL served captions sit bottom-center consistently --
// native inline, native fullscreen, desktop, and mobile-native all honor VTT
// cue settings. Cue-BLOCK aware (same discipline as shiftVttCues): only the
// timing line at a block boundary is rewritten, never a payload line that
// merely looks like one (captions quoting a "00:01 --> ..." are a real genre).
const CUE_CENTER_SETTINGS = 'position:50% align:center';
function centerVttCues(text) {
  const raw = typeof text === 'string' ? text : '';
  const lines = raw.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
  const out = [];
  let inCueBlock = false;
  for (const line of lines) {
    ...
    if (m) {
      inCueBlock = true;
      // Keep the (possibly already-shifted) times; replace ALL settings.
      out.push(`${m[1]} --> ${m[2]} ${CUE_CENTER_SETTINGS}`);
      continue;
    }
    out.push(line);
  }
...
module.exports = {
  ...
  parseVttTimeMs,
  formatVttTimeMs,
  // v1.47.4: the pure winner rule + its `-orig` predicate, exported so tests
  // exercise the shared logic directly rather than only through two filesystem
  // paths that must agree.
  pickPreferredLangMatch,
  isOriginalLangMarker,
  // Transcript export (lib/transcript.js): the ONE cue-timing regex, shared so
  // the transcript parser can never disagree with shiftVttCues/centerVttCues
  // about what a timing line is.
  VTT_CUE_TIMING_LINE,
};
