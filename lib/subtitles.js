 ...
 *
 * Order: an exact `en` wins outright; otherwise the alphabetically-first match
 * by FILENAME (stable across environments, never `readdirSync`'s unspecified
 * order), skipping `-orig` markers unless nothing else is available.
 */
function pickPreferredLangMatch(langMatches) {
  const exactEn = langMatches.find((m) => m.lang === 'en');
  if (exactEn) return exactEn;
  const sorted = langMatches.slice().sort((a, b) => a.name.localeCompare(b.name));
