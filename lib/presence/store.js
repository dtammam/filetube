...
//   {deviceId, deviceLabel, mediaId, kind, surface, position, duration, state,
//    lastSeenAt, orderTok}   (surface: 'watch'|'listen', v1.304)
...
// tight; the device id is a client-minted crypto.randomUUID (36 chars) or,
// on a non-secure-context fallback, a 32-char hex / short timestamp id
// (see mintDeviceId in common.js) - DEVICE_ID_MAX 64 sits well above all of
// them. (Do NOT resize this to 32 "because ids are md5 hex" - they are not;
// a real randomUUID is 36 and would be rejected. QA gate SUGGESTION 1.)
const LABEL_MAX = 32;
...
function createPresenceStore(options) {
  ...
  function record(userId, event) {
    ...
    // v1.304 handoff modality: which SURFACE the playback is on - the video
    // player ('watch') or the audio/listening player ('listen', the iPod/music
    // and podcasts skins). Whitelisted with a 'watch' default so an old client
    // that never sends it (and the tv/plain-video case) behaves byte-identically
    // to pre-v1.304 - only an explicit 'listen' re-routes the handoff. This is
    // the field the resolver needs but cannot derive itself: a LISTENED video
    // still has a video extension, so the client is the source of truth here.
    const surface = ev.surface === 'listen' ? 'listen' : 'watch';
    const now = clock();
  ...
  function readOther(userId, requestingDeviceId) {
    ...
    return {
      ...
      kind: best.kind,
      surface: best.surface === 'listen' ? 'listen' : 'watch', // belt-and-braces: never leak undefined for a pre-field entry
