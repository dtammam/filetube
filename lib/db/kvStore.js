...
function assertKey(label, key) {
  ...
  if (key.includes('\u0000')) {
    throw new Error(`${label}: key contains U+0000 - node:sqlite reads a NUL-bearing TEXT back truncated on Node 24.14 and older (verbatim from 24.20 - tech-debt #225), so a persisted NUL-bearing key would read back as a colliding prefix and silently corrupt the key. Refusing.`);
  }
}
