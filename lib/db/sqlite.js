...
// instance has any user row to lose (v1.42-v1.295's openAdapter had a
// schema-empty probe that confirmed this in practice; it left with the import
// path in Wave 7, after every instance had long passed v2).
//
...
function migrateSchema(sql) {
  ...
  // A real database below v33 is EXPECTED to still have them (the v1 block
  // created them, nothing before v33 drops them); this re-creates them for a
  // file stamped back below v33 after the drop - the migration tests'
  // rewinds - so every block below runs against the schema it was written
  // for. Same DDL as the v1 block. The assumption is not silently absorbed
  // (gate): a STAMPED file that lacks a table - tampering, a bad manual copy -
  // is logged loudly, because a v1.42-v1.295 build would have refused to open
  // it and nothing that table held can be recovered by re-creating it empty.
  if (current < 33) {
    const missing = ['doc_kv', 'doc_single'].filter((t) => !sql.prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ?").get(t));
    if (current >= 1 && missing.length > 0) {
      console.error(`[db] migration: this v${current} database was MISSING its document table(s) ${missing.join(', ')} - re-created empty so the v21-v32 drains can run. A v1.42-v1.295 build would have refused to open this file; whatever those tables held is not recoverable by this step.`);
    }
    sql.exec(LEGACY_DOC_TABLES_DDL);
  }
  if (current < 1) {
    sql.exec(`
      CREATE TABLE IF NOT EXISTS doc_kv (
  ...
  if (current < 33) {
    ...
    // A NUL-bearing name is reported as its BYTES (hex) - node:sqlite hands a
    // truncated string back on Node <= 24.14 (#225), which would print two
    // strays under one name and send the operator's DELETE after the wrong
    // row. The list is capped so a hostile file cannot flood the boot log.
    const label = (kind, r) => `${kind} ${r.nul ? `<bytes ${r.hex}>` : `'${r.name}'`} (${r.n} row(s))`;
    const strays = [
      ...sql.prepare("SELECT namespace AS name, hex(CAST(namespace AS BLOB)) AS hex, instr(CAST(namespace AS BLOB), x'00') > 0 AS nul, COUNT(*) AS n FROM doc_kv GROUP BY namespace ORDER BY namespace").all()
        .map((r) => label('doc_kv namespace', r)),
      ...sql.prepare("SELECT name, hex(CAST(name AS BLOB)) AS hex, instr(CAST(name AS BLOB), x'00') > 0 AS nul, COUNT(*) AS n FROM doc_single GROUP BY name ORDER BY name").all()
        .map((r) => label('doc_single name', r)),
    ];
    if (strays.length > 0) {
      const CAP = 20;
      const shown = strays.slice(0, CAP).join('; ') + (strays.length > CAP ? `; ... and ${strays.length - CAP} more` : '');
      throw new Error(
        `FileTube database migration v33 refuses to drop the document tables while they still hold rows: ${shown}. `
        + 'No FileTube build reads these rows any more (every namespace moved to its own table in v21-v32). Export or delete them with '
        + 'the sqlite3 CLI (tables doc_kv and doc_single in filetube.db), then start FileTube again. The document tables and their '
        + 'rows are unchanged; the database is still at v32 and still opens on v1.295.x.'
      );
    }
    sql.exec('BEGIN');
    try {
...
class SqliteAdapter {
  ...
  constructor(dbPath, { log = console.error } = {}) {
    ...
    // Wave 7 (gate): a brand-new file opens at user_version 0 - the one
    // signal an operator gets that boot did NOT find an existing library here
    // (the pre-v1.42 import is gone; docs/CONFIGURATION.md has the way).
    let stampBefore = 0;
    ...
    // catches and continues would otherwise hold a lock on its temp db. (The
    // stamp read sits INSIDE the guarded block too: on a file that is not a
    // database it is the first statement to throw.)
    try {
      stampBefore = this.sql.prepare('PRAGMA user_version').get().user_version;
    ...
    if (stampBefore === 0) {
      log(`[db] created a fresh, EMPTY database (schema v${SCHEMA_VERSION}) at ${dbPath} - no existing library was found in this DATA_DIR. If one was expected here, stop and check the path (docs/CONFIGURATION.md, "The database").`);
    }
...
// `insertViewCount` handle. (Until Wave 7 a `source` option told the two
// callers apart; it is gone with the second caller.)
function importParsedJson(parsed, handles) {
  const summary = {};
...
module.exports = {
  ...
  // (test/helpers/legacy-doc-tables.js) - no production caller of this EXPORT
  // (migrateSchema's below-v33 guard uses the constant directly).
