...
const DOC_KV_NAMESPACES = [
  ...
  'books.audio',
  'music.tracks',
  // v1.69: podcast episode records, one row per episode - a 484-episode
  // backfill writes per-episode rows, never rewriting one giant blob.
  'podcasts.episodes',
...
const SINGLETON_NAMES = [
  ...
  'music.folders',
  'music.settings',
  // Wave G: per-channel-folder "show in Music" marks { [folderName]: 'on'|'off' }
  // - the same singleton shape + folderName keying as folderDisplayNames. An
  // absent key means "genre-seeded default" (item.tags.genre === 'Music').
  // Feature-owned by the music namespace so it rides the backup bundle via the
  // existing 'music' container (no BACKUP_NAMESPACE_KEYS edit).
  'music.channels',
  // v1.69: podcast subscription records carry NO feed URLs (those live in
  // the 0600 secrets file outside the db - lib/podcasts/secrets.js).
...
const CONTAINER_KEYS = ['books', 'music', 'podcasts', 'ytdlp']; // (tv: relational since Wave 5)
...
const RELATIONAL_IMPORT_KEYS = new Set(['viewCounts', 'progress', 'deleteTombstones', 'trash', 'settings', 'folders', 'folderSettings', 'folderDisplayNames', 'liked', 'tv']);
