...
const RELATIONAL_IMPORT_KEYS = new Set(['viewCounts', 'progress', 'deleteTombstones', 'trash']);
const { TABLE: VIEW_COUNTS_TABLE, UPSERT_SQL: VIEW_COUNT_UPSERT_SQL, usableCount } = require('../media/viewCounts');
// Wave 2: the two id-keyed RECORD namespaces (verbatim JSON rows). Each
// module exports its table, its upsert text and the row-params builder, so
// the typed columns (a tombstone's deleted_at) can never drift from the json.
