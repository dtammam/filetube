...
const RELATIONAL_IMPORT_KEYS = new Set(['viewCounts']);
    ...
    try {
      ...
      populateFn({
        ...
        insertViewCount: (id, count) => {
          ...
          if (!Number.isInteger(count) || count < 0) throw new Error(`exclusiveReplace: view count for '${id}' must be a non-negative integer`);
...
// caller's `insertViewCount` handle (never doc_kv); `source` is kept for the
// callers' own logging and is no longer a branch here.
function importParsedJson(parsed, handles, { source = 'db.json' } = {}) {
  ...
  // A bundle (or a hand-edited db.json) carries `viewCounts` first-class:
  // { id: count }. Same per-key-map shape check as the doc namespaces, same
  // "usable value or dropped" rule as the v1.42 extraction below.
  if (parsed.viewCounts !== undefined && parsed.viewCounts !== null) {
    const vc = parsed.viewCounts;
    if (typeof vc !== 'object' || Array.isArray(vc)) {
      throw new Error("import: namespace 'viewCounts' is not an object — refusing");
    }
    let n = 0;
    for (const id of Object.keys(vc)) {
      const value = usableCount(vc[id]);
    ...
    if (ns === 'metadata') {
      ...
      for (const id of Object.keys(obj)) {
        ...
        if (item != null && typeof item === 'object' && Object.prototype.hasOwnProperty.call(item, 'viewCount')) {
          ...
          if (usable !== null) {
            routeViewCount(id, usable); // Wave 1: straight into media_view_counts
            extracted++;
          }
