...
//
// doc_kv: mutated per-key in practice → one row per key. (Wave 5: only
//   `metadata` is left here; the feature containers - `books.audio` and its
//   siblings - are feature-store tables now, see FEATURE_DEFS.)
...
class SqliteAdapter {
  ...
  rebuildSnapshotFromDisk() {
    if (this.items) this.items.rebuildSnapshot(); // Wave 6 (absent only during the constructor's first call, before the store exists)
    this.snapshot.clear();
