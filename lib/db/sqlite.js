...
const DOC_KV_NAMESPACES = [
  'metadata', // the last doc_kv namespace - Wave 6 moves it to media_items
...
function migrateSchema(sql) {
  ...
  if (current < 31) {
    ...
    try {
      ...
      // `id = md5(channelUrl)` since v1.20, but a hand-edited db.json could
      // not; mint the id addSubscription would have, rather than drop the
      // channel. (A record with neither id nor channelUrl is unaddressable by
      // every route and is dropped with the migration's own log line.)
      ...
      if (subsRow) {
        ...
        if (Array.isArray(list)) {
          ...
          for (const rec of list) {
            if (rec && typeof rec === 'object' && !Array.isArray(rec) && (typeof rec.id !== 'string' || rec.id === '') && typeof rec.channelUrl === 'string' && rec.channelUrl !== '') {
              rec.id = crypto.createHash('md5').update(rec.channelUrl).digest('hex');
              minted += 1;
            }
          }
          if (minted > 0) {
...
class SqliteAdapter {
  ...
  save(db, { alsoInTransaction = null } = {}) {
    ...
    if (writes.length === 0 && deletes.length === 0 && !alsoInTransaction) {
