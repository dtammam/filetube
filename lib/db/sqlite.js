...
const DOC_KV_NAMESPACES = [
  'metadata',
  // v1.65: trashed-item records keyed by trashId = md5(trashPath). One row
  // per trashed file; joins the backup bundle via BACKUP_NAMESPACE_KEYS.
  'trash',
  'books.items',
  'books.progress',
  'books.audio',
...
const RELATIONAL_IMPORT_KEYS = new Set(['viewCounts', 'progress', 'deleteTombstones']);
...
function readPersistedDatabase(dataDir) {
  ...
  try {
    ...
    for (const [key, table] of [['progress', progressDef.TABLE], ['deleteTombstones', tombstoneDef.TABLE]]) {
