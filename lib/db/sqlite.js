...
function sleepSync(ms) {
  try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms); } catch (_) { /* fall through: retry immediately */ }
}
...
function switchJournalMode(sql, pragma, budgetMs) {
  ...
  for (;;) {
    try {
      ...
      if (!err || err.errcode !== 5 || Date.now() >= deadline) throw err; // not SQLITE_BUSY, or out of budget
      sleepSync(25); // SQLITE_BUSY: wait, don't burn the CPU
    }
  }
}
...
//
// The MEASURED mechanism (adversarial W3 corrected my first write-up): the
// failures need a concurrent WRITER on a shared db file - parallel readers alone
// never contended. `PRAGMA journal_mode = WAL` is simply the FIRST statement that
// takes any file lock (new DatabaseSync and `PRAGMA foreign_keys` touch nothing),
// which is why every stack trace pointed there; on a fresh shared dir the seat
// measured the deaths landing in migrateSchema's CREATE TABLE instead. With a
// writer present and timeout 0: 21 journal_mode + 327 write failures over 420
// iterations; with this timeout: 0.
//
