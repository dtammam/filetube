...
// Strict import-time classifier: which db.json top-level keys map where.
// Unknown keys in the SOURCE file abort the import (never silently dropped —
// a db.json written by a NEWER FileTube than this binary must not lose data
...
function importParsedJson(parsed, handles) {
  ...
  for (const ns of DOC_KV_NAMESPACES) {
    if (ns === 'viewCounts') continue; // synthesized below, never read from db.json
