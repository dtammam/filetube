...
// - Collection namespaces persisted as one row PER KEY (doc_kv), so a one-item
//   mutation writes one row, not the whole 175 KB. Small whole-array/object
//   namespaces persisted as one row each (doc_single). Wave 6 of the
//   relational-migration arc: BOTH doc tables are empty - the media index
//   (`metadata`, the only key of the doc object, DOC_OBJECT_KEYS) is the
//   media_items table behind lib/media/items.js, still diffed row by row.
...
// the db.json boot import, the bundle restore), rather than dropping the
// channel or refusing the whole import. A record with neither id nor
...
// Snapshot-map key separator: U+0000, written as an ESCAPE SEQUENCE — never
// a raw byte (the v1.37.5 lesson: raw control bytes in source render
// invisibly and break text tooling; ugrep binary-sniffed this very file
// while it carried them). NUL is the separator because a SPACE cannot be
// one: doc_kv keys legitimately contain spaces (universal downloadMeta
// keys are '<extractor> <id>'), and a space-separated snapshot key made
// the delete-sweep truncate such keys — the delete-resurrect class.
const SNAPSHOT_SEP = '\u0000';
...
//
// doc_kv: mutated per-key in practice → one row per key. (Wave 6: EMPTY -
//   `metadata` is media_items now; the feature containers - `books.audio` and
//   its siblings - are feature-store tables since Wave 5, see FEATURE_DEFS.)
//   - `viewCounts` WAS here from v1.42 (the import extracted the one
...
//     Wave 1 of the relational-migration arc (schema v21, 2026-09-13) moved it
//     to the `media_view_counts` TABLE owned by lib/media/viewCounts.js; the
//     v21 migration backfills the rows and deletes the doc_kv namespace, and
//     its absence from this list makes assertNoUnknownKeys REFUSE any stray
//     `db.viewCounts` write (the lock is the net). The import classifier
//     still ACCEPTS a `viewCounts` key from a db.json/bundle and routes it to
//     the table (RELATIONAL_IMPORT_KEYS below).
//   - `progress` and `deleteTombstones` LEFT this list in Wave 2 (schema v22):
//     `media_progress` (the frozen pre-auth positions the first admin adopts
//     once, lib/media/progress.js) and `media_delete_tombstones`
//     (lib/media/deleteTombstones.js). Same v21 pattern: the migration
//     backfills the rows and deletes the doc rows in one transaction; the
//     save-lock refuses a stray write; the import classifier routes the
//     keys to the tables.
//   - `trash` LEFT this list in Wave 3 (schema v23): `media_trash`
//     (lib/media/trashRecords.js) - the trashed-item records keyed by
//     trashId = md5(trashPath), minted/retired inside the doc commit's
//     transaction. Same migration + lock + import-routing pattern.
// Wave 6: `metadata` - the LAST doc_kv namespace - moved to the `media_items`
// table (lib/media/items.js). The list is EMPTY; the doc object the routes
// read and the mutators write is `{ metadata }`, assembled from that table by
// load() and diffed back into it by save() (DOC_OBJECT_KEYS below). Wave 7
// drops the two doc tables.
const DOC_KV_NAMESPACES = [
  // (metadata: relational since Wave 6 - media_items / lib/media/items.js)
  // (ytdlp.downloadMeta / ytdlp.channelAvatars: relational since Wave 5 -
  // ytdlp_download_meta / ytdlp_channel_avatars, lib/ytdlp/store.js)
  // (books.items / books.progress / books.audio: relational since Wave 5 - lib/books/store.js)
  // (music.tracks: relational since Wave 5 - music_tracks / lib/music/store.js)
  // (podcasts.episodes: relational since Wave 5 - podcasts_episodes / lib/podcasts/store.js)
  // v1.195: TV Shows episode records, one row per episode (id = md5(filePath)).
  // Feature-owned by lib/tv/store.js (the persist-gate discipline); UI = "Shows".
  // (tv.episodes: relational since Wave 5 - tv_episodes / lib/tv/store.js)
];

// doc_single: small arrays/objects mutated as wholes today → one row each.
const SINGLETON_NAMES = [
  // (folders / folderSettings / folderDisplayNames / settings: relational since
  // Wave 4 - library_folders, library_folder_settings,
  // channel_folder_display_names, app_settings; the lib/config/ stores.
  // liked: relational since Wave 4 too - media_liked / lib/media/liked.js. No
  // top-level doc_single name is left; the rest are container sub-keys.)
  // (books.folders / books.settings / books.pins: relational since Wave 5)
  // (music.folders / music.settings / music.channels: relational since Wave 5)
  // (podcasts.subscriptions / podcasts.settings: relational since Wave 5 -
  // podcasts_subscriptions / podcasts_settings. The records still carry NO
  // feed URLs - those live in the 0600 secrets file, lib/podcasts/secrets.js.)
  // v1.195: TV Shows configured roots + feature settings (lib/tv/store.js).
  // (tv.folders / tv.settings: relational since Wave 5 - tv_folders / tv_settings)
  // (ytdlp.subscriptions / ytdlp.pins / ytdlp.allowMembersOnly: relational
  // since Wave 5 - ytdlp_subscriptions / ytdlp_pins / ytdlp_settings. No
  // doc_single name is left at all; Wave 7 drops the table.)
];

// The container keys whose sub-keys the lists above enumerate. `save()`
// walks these to catch an unknown `db.books.X`/`db.music.X`/`db.ytdlp.X` the
// same way it catches an unknown top-level key.
const CONTAINER_KEYS = []; // (tv, music, books, podcasts, ytdlp: relational since Wave 5 - every container is a feature store now)
...
// Wave 6: `metadata` is a table now but stays a key of that object - the
// save-lock (assertNoUnknownKeys) keeps refusing every other stranger.
const DOC_OBJECT_KEYS = ['metadata'];
const KNOWN_TOP_LEVEL = new Set(
  [...DOC_OBJECT_KEYS, ...DOC_KV_NAMESPACES, ...SINGLETON_NAMES]
    .map((p) => p.split('.')[0])
);
...
// Wave 1 (relational-migration arc): keys a db.json / backup bundle may carry
// that are NOT doc-model namespaces any more - the import classifier accepts
// them and routes each to its relational table via a dedicated handle
// (`insertViewCount`), while assertNoUnknownKeys (the SAVE lock) keeps
// refusing them on the mega-object. The store module owns the table's SQL;
// the two bulk seams in this file (boot import, exclusiveReplace) reuse its
// upsert text so there is one statement, not a drifting copy.
...
// node:sqlite binds TEXT parameters with C-string semantics: a value
// containing U+0000 is SILENTLY TRUNCATED at the NUL (verified empirically
// 2026-07-17: a row key of 'evil' + U+0000 + 'key' persists as 'evil'). A
// truncated key is silent identity corruption — the wrong row gets written,
// diffed, and deleted — so NUL-bearing row keys are REFUSED loudly at every
// write boundary (save, import, restore). No legitimate key carries NUL
// (metadata ids are md5 hex; downloadMeta keys are '<extractor> <id>'); only
// a hostile or corrupt source could, and honest refusal beats silent
// truncation. JSON VALUES are safe without a guard: JSON.stringify escapes
// all control characters, so the json column never carries a raw NUL.
function assertRowKeySafe(ns, key) {
  if (typeof key === 'string' && key.includes(SNAPSHOT_SEP)) {
    throw new Error(`SqliteAdapter: a row key in '${ns}' contains U+0000 — node:sqlite truncates TEXT at NUL, so persisting it would silently corrupt the key. Refusing (fix or remove the offending key).`);
  }
}
...
// (JSON.parse happily materializes one; reachable via a hand-edited db.json
// or a crafted restore bundle) would make `obj[key] = value` a PROTOTYPE
...
function migrateSchema(sql) {
  ...
  if (current < 31) {
    ...
    try {
      ...
      // db.json could not; mint the id addSubscription would have (the SAME
...
class SqliteAdapter {
  ...
  constructor(dbPath, { log = console.error } = {}) {
    ...
    // per-row diff base is rebuilt beside the doc snapshot below.
  ...
  rebuildSnapshotFromDisk() {
    this.items.rebuildSnapshot(); // Wave 6: the index's diff base, rebuilt with the doc snapshot (the store is created before the first call)
    this.snapshot.clear();
    for (const row of this.stmts.allKv.all()) {
      this.snapshot.set(`kv${SNAPSHOT_SEP}${row.namespace}${SNAPSHOT_SEP}${row.key}`, row.json);
    }
    for (const row of this.stmts.allSingles.all()) {
      this.snapshot.set(`single${SNAPSHOT_SEP}${row.name}`, row.json);
    }
  }
  ...
  load() {
    const db = {};
    // Wave 6: the media index from its table, in rowid order, own properties
    // only; absent when empty (the empty-is-absent normalization the doc
    // namespace had - server.js's loadDatabase backfills `{}`).
    ...
    for (const row of this.stmts.allKv.all()) {
      ...
      defineRowProperty(obj, row.key, JSON.parse(row.json));
    }
    return db;
  }
  ...
  // transaction, after the doc rows and before COMMIT, so a relational write
  // that must be atomic with the doc commit (a delete's tombstone mint, a
  ...
  // transaction opens even when the doc rows are unchanged.
  save(db, { alsoInTransaction = null } = {}) {
    ...
    for (const name of SINGLETON_NAMES) {
      ...
      const snapKey = `single${SNAPSHOT_SEP}${name}`;
      seen.add(snapKey);
    ...
    for (const ns of DOC_KV_NAMESPACES) {
      ...
      for (const key of Object.keys(obj)) {
        assertRowKeySafe(ns, key);
        const snapKey = `kv${SNAPSHOT_SEP}${ns}${SNAPSHOT_SEP}${key}`;
        seen.add(snapKey);
    ...
    for (const snapKey of this.snapshot.keys()) {
      ...
      const parts = snapKey.split(SNAPSHOT_SEP);
      if (parts[0] === 'single') {
        ...
        // Re-join everything after the namespace: a HOSTILE row key could
        // itself contain U+0000 (JSON permits \u0000 in object keys), and
        // truncating at parts[2] would delete the wrong row — the same
        // delete-resurrect class the space→NUL separator change fixed for
        // ordinary spaced keys ('<extractor> <id>').
        deletes.push(['kv', parts[1], parts.slice(2).join(SNAPSHOT_SEP)]);
      }
    }

    // Wave 6: the media index's own diff (lib/media/items.js) - an absent
    // `metadata` key plans nothing (rows kept), a present map is compared
    // row by row; applied inside THIS transaction, its base advanced after
    // the commit exactly like the doc snapshot below.
    const itemsPlan = this.items.planDiff(getPath(db, 'metadata'));
    const itemChanges = itemsPlan.writes.length + itemsPlan.deletes.length;

    if (writes.length === 0 && deletes.length === 0 && itemChanges === 0 && !alsoInTransaction) {
    ...
    try {
      ...
      this.items.applyPlan(itemsPlan); // Wave 6
    ...
    this.items.advancePlan(itemsPlan); // Wave 6: the diff base advances with the doc snapshot, after COMMIT
    ...
    for (const [kind, name, key, json] of writes) {
      this.snapshot.set(kind === 'single' ? `single${SNAPSHOT_SEP}${name}` : `kv${SNAPSHOT_SEP}${name}${SNAPSHOT_SEP}${key}`, json);
    }
    for (const [kind, name, key] of deletes) {
      this.snapshot.delete(kind === 'single' ? `single${SNAPSHOT_SEP}${name}` : `kv${SNAPSHOT_SEP}${name}${SNAPSHOT_SEP}${key}`);
    }
    return { rowsWritten: writes.length + itemsPlan.writes.length, rowsDeleted: deletes.length + itemsPlan.deletes.length };
  }
...
//   - the boot-time db.json import (source: 'db.json' — a legacy source
//     carries `viewCount` EMBEDDED on metadata items; the extraction below
//     lifts it out)
//   - the instance-bundle restore (source: 'bundle' — the bundle carries
//     `viewCounts` as a first-class { id: count } key and it must round-trip)
// Wave 1: both shapes land in the `media_view_counts` TABLE through the
// caller's `insertViewCount` handle (never doc_kv). `source` is accepted and
// IGNORED (signature stability for the two callers); it is no longer a branch.
function importParsedJson(parsed, handles, { source = 'db.json' } = {}) {
  ...
  // A bundle (or a hand-edited db.json) may carry `viewCounts` first-class:
  ...
  for (const def of FEATURE_DEFS) {
    ...
    for (const part of Object.keys(obj)) {
      if (!def.parts[part]) throw new Error(`import: db.json contains unknown key '${def.name}.${part}' — refusing a lossy import (was this file written by a newer FileTube?)`);
    }
    if (def.name === 'ytdlp') {
  ...
  // handle (a db.json / bundle carries it as `metadata`, the doc-object key).
  ...
  for (const ns of DOC_KV_NAMESPACES) {
    ...
    if (obj === undefined || obj === null) continue;
    if (typeof obj !== 'object' || Array.isArray(obj)) {
      throw new Error(`import: namespace '${ns}' is not an object — refusing`);
    }
    for (const key of Object.keys(obj)) {
      assertRowKeySafe(ns, key);
      insertKv(ns, key, obj[key]);
    }
...
function importDbJson(jsonPath, dbPath, { log = console.error } = {}) {
  ...
  try {
    ...
    // Wave 1: view counts go to their table (the store module's own upsert
    // text - this import is one of the two documented non-store writers).
    const insertViewCountStmt = tmpSql.prepare(VIEW_COUNT_UPSERT_SQL);
    // Wave 2: the record tables, through their stores' own upsert text.
    const insertProgressStmt = tmpSql.prepare(progressDef.UPSERT_SQL);
    const insertTombstoneStmt = tmpSql.prepare(tombstoneDef.UPSERT_SQL);
    const insertTrashStmt = tmpSql.prepare(trashDef.UPSERT_SQL); // Wave 3
    const insertSettingStmt = tmpSql.prepare(settingsDef.UPSERT_SQL); // Wave 4
    const insertFolderStmt = tmpSql.prepare(foldersDef.UPSERT_SQL); // Wave 4
    const insertFolderSettingStmt = tmpSql.prepare(folderSettingsDef.UPSERT_SQL); // Wave 4
    const insertFolderDisplayNameStmt = tmpSql.prepare(folderDisplayNamesDef.UPSERT_SQL); // Wave 4
    const insertLikedStmt = tmpSql.prepare(likedDef.UPSERT_SQL); // Wave 4
    const insertItemStmt = tmpSql.prepare(itemsDef.UPSERT_SQL); // Wave 6: the media index
    tmpSql.exec('BEGIN IMMEDIATE');
    try {
      summary = importParsedJson(parsed, {
        insertItem: (id, record) => { itemsDef.assertMediaId(id); insertItemStmt.run(...itemsDef.rowParams(id, record)); }, // Wave 6
        ...
        insertViewCount: (id, count) => insertViewCountStmt.run(id, count),
        insertProgress: (id, record) => insertProgressStmt.run(...progressDef.rowParams(id, record)),
        insertTombstone: (id, record) => insertTombstoneStmt.run(...tombstoneDef.rowParams(id, record)),
        insertTrash: (id, record) => insertTrashStmt.run(...trashDef.rowParams(id, record)),
        insertSetting: (key, value) => insertSettingStmt.run(key, JSON.stringify(value)), // Wave 4
        replaceFolders: (list) => foldersDef.normalizeList(list).forEach((p, i) => insertFolderStmt.run(p, i)), // Wave 4
        insertFolderSetting: (key, record) => insertFolderSettingStmt.run(...folderSettingsDef.rowParams(key, record)), // Wave 4
        insertFolderDisplayName: (key, record) => insertFolderDisplayNameStmt.run(...folderDisplayNamesDef.rowParams(key, record)), // Wave 4
        replaceLiked: (list) => likedDef.normalizeList(list).forEach((id, i) => insertLikedStmt.run(id, i)), // Wave 4
        replaceFeature: (name, ns) => FEATURE_BY_NAME.get(name).createStore({ sql: tmpSql, inTransaction: true }).replaceAll(ns), // Wave 5
      });
      tmpSql.exec('COMMIT');
    } catch (err) {
...
function isSchemaEmpty(sql) {
  ...
  const items = sql.prepare(`SELECT COUNT(*) AS c FROM ${itemsDef.TABLE}`).get().c; // Wave 6: the index lives here now
  const users = sql.prepare('SELECT COUNT(*) AS c FROM users').get().c;
  return kv === 0 && single === 0 && items === 0 && users === 0;
}
...
//
// SCHEDULED FOR REMOVAL - Wave 7 of the relational-migration arc
// (docs/exec-plans/active/2026-09-13-sqlite-relational-migration.md): rule 2
// (the one-time db.json import, importDbJson above) is the arc's rollback net
// and stays until every legacy namespace has left doc_kv/doc_single. Under
// rule 1 db.json's CONTENT is never read - only its existence and, in the
// stranded-fingerprint arm below, its byte size are checked - which
// test/unit/dbjson-never-read.test.js binds with an fs spy.
...
function readPersistedDatabase(dataDir) {
  ...
  try {
    ...
    for (const row of sql.prepare('SELECT namespace, key, json FROM doc_kv').all()) {
      ...
      defineRowProperty(obj, row.key, JSON.parse(row.json)); // same __proto__ defense as load()
    }
...
module.exports = {
  ...
  DOC_KV_NAMESPACES,
  SINGLETON_NAMES,
  CONTAINER_KEYS, // Wave 5: empty - every container is a feature store (the lock test binds it)
  DOC_OBJECT_KEYS, // Wave 6: the doc object's keys - just `metadata`, a table behind lib/media/items.js
