...
//
// Two contract rules (gate pass B, adversarial seat):
//   - a PARTIAL snapshot (`read(only)` / `holder(only)`) is tagged with the parts
//     it read, and `syncFrom` skips every other part - a subset holder that
//     reaches a commit can never be mistaken for "the user emptied the rest"
//     (a module's ensureX(holder) fills defaults for the missing keys, which is
//     exactly what made that indistinguishable). A mutator that DELETES a key
//     from a FULL holder does wipe that part - every ensureX re-fills, and no
//     writer deletes a key.
//   - ONE `mutate()` per store per updateDatabase tick: each mutate reads a
//     fresh holder from the TABLES and queues its diff, so a second mutate in
//     the same tick would start from rows the first has not written yet and
//     its apply would revert the first. Every production writer is one mutate
//     per tick (the fanout writers nest exactly one); a second write in the
//     same tick belongs inside the same mutator.
...
// The parts a partial snapshot carries (non-enumerable, so the snapshot still
// deep-equals a plain object and JSON.stringify sees nothing extra).
const PARTIAL = Symbol('featureStore.partial');
...
function defineFeatureStore({ name, prefix = name, parts }) {
  ...
  function createStore(adapter, { inSaveTransaction = null } = {}) {
    ...
    function read(only) {
      ...
      if (Array.isArray(only)) Object.defineProperty(out, PARTIAL, { value: new Set(only), enumerable: false });
      return out;
    }
    ...
    function syncFrom(ns) {
      ...
      const partial = ns[PARTIAL] instanceof Set ? ns[PARTIAL] : null;
      return tx(() => {
        const stats = { rowsWritten: 0, rowsDeleted: 0 };
        for (const [part, d] of publicParts()) {
          if (partial && !partial.has(part)) continue; // a part this snapshot never read is not "emptied"
          const next = ns[part];
