...
function defineFeatureStore({ name, prefix = name, parts }) {
  ...
  for (const [part, spec] of Object.entries(parts)) {
    ...
    else if (spec.kind === 'kv') defs[part] = { kind: 'kv', table, def: defineKvStore({ label, table }) };
  ...
  function createStore(adapter, { inSaveTransaction = null } = {}) {
    ...
    function read() {
      const out = {};
      for (const part of Object.keys(defs)) out[part] = readPart(part);
      return out;
    }
      ...
      return tx(() => {
        ...
        for (const [part, d] of Object.entries(defs)) {
          ...
          if (d.kind === 'list' || d.kind === 'records') {
            ...
            const current = sub[d.via].getKey(part);
    ...
    function replaceAll(ns) {
      ...
      for (const key of Object.keys(obj)) {
        if (!defs[key]) throw new Error(`featureStore ${name}: unknown part '${key}' - refusing a lossy write`);
      }
      ...
      tx(() => {
        for (const [part, d] of Object.entries(defs)) {
          ...
          sub[part].replaceAll(staged[part]);
        }
        for (const [part, d] of Object.entries(defs)) {
          if (d.kind !== 'value') continue;
          const v = staged[part];
    ...
    return {
      ...
      isEmpty() {
        return Object.entries(defs).every(([part, d]) => d.kind === 'value' || sub[part].size() === 0);
      },
      __tx: tx,
    };
  }
  ...
  function migrateFromDoc(sql, log = console.error) {
    ...
    for (const [part, d] of Object.entries(defs)) {
      ...
      if (d.kind === 'map' && !d.docSingleMap) {
        const obj = {};
        for (const row of sql.prepare('SELECT key, json FROM doc_kv WHERE namespace = ?').all(docName)) {
  ...
  return { name, prefix, createStore, tables, docKvNamespaces, docSingleNames, migrateFromDoc, readPersisted, parts: defs };
}
