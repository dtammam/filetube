...
// assumptions twice):
//   Piper (rhasspy/piper, src/cpp/main.cpp): text on STDIN (one utterance per
//     line), `--model <onnx>`, `--config <json>` (defaults to <model>.json),
//     `--output_file <wav>`, `--length_scale <n>` (SPEAKING RATE knob — there
//     is NO --rate/--speed flag; >1 = slower, <1 = faster), `--sentence_silence
//     <sec>`, `--quiet`.
