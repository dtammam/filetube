...
// docs/exec-plans/completed/2026-07-12-v1.37.0-books.md §2 for the full assessment).
//
...
function extractEntry(buf, entry) {
  ...
  // v1.37.0 gate fix (adversarial W3): a STORED entry's OUTPUT is its
  // compressedSize bytes -- a forged small uncompressedSize passed the
  // check above while the slice below returned the full stored payload
  // (there is no zlib maxOutputLength backstop on method 0). Cap the
  // stored size independently so a lying header cannot bypass the
  // hygiene invariant on either method.
  if (entry.method === METHOD_STORED && entry.compressedSize > MAX_INFLATED_BYTES) {
    throw codedError('EZIPENTRYTOOBIG', `stored entry exceeds the ${MAX_INFLATED_BYTES}-byte cap: ${entry.name}`);
  }
