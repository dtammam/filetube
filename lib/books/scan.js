 ...
 * throw, and each such dir is recorded into the optional `erroredDirs` array.
 * v1.124 R2: recording matters (mirrors lib/music/scan.js walkMusicRoot) - a
 * transient per-directory EACCES / failed automount is NOT an emptied library,
 * so the prune must protect books UNDER an errored dir even when the root as a
 * whole still has survivors (the whole-root mount-loss guard alone misses a
 * subtree blip, and a book prune destroys the per-user reading position too).
 */
function walkBookRoot(root, erroredDirs) {
  const found = [];
  const stack = [root];
  while (stack.length > 0) {
    ...
    try {
      ...
      if (Array.isArray(erroredDirs)) erroredDirs.push(dir);
      continue;
    }
    for (const dirent of dirents) {
      const full = path.join(dir, dirent.name);
      if (dirent.isDirectory()) {
        // v1.65: the media trash dir is invisible to every walker (a books
        // root sharing a path with a media root must not index trashed files).
 ...
 *   survivingIds: Set<string>, missingRoots: string[], erroredDirs: string[] }}
 */
...
async function collectBooks(folders, previousItems, getMediaId) {
  ...
  for (const root of Array.isArray(folders) ? folders : []) {
    ...
    for (const filePath of walkBookRoot(root, erroredDirs)) {
      processed += 1;
  ...
  return { items, covers, survivingIds, missingRoots, erroredDirs };
}
