 ...
 * on, (b) its file did not survive this walk, (c) its root folder is
 * NOT in `missingRoots` (an unmounted/vanished root prunes NOTHING under
 * it -- absence of a mount is not deletion of a library), AND (d) its file
 * does NOT sit under a directory that errored THIS walk.
 * @param {Object<string, object>} items db.books.items
 * @param {Set<string>|string[]} survivingIds ids the current walk found
 * @param {{missingRoots?: (Set<string>|string[]), pruneMissing?: boolean, erroredDirs?: (Set<string>|string[])}} opts
 * @returns {string[]} ids safe to prune
 */
function selectPrunableBookIds(items, survivingIds, { missingRoots, pruneMissing, erroredDirs } = {}) {
  ...
  for (const id of Object.keys(items || {})) {
    ...
    // v1.124 R2 (mirrors lib/music/store.js selectPrunableTrackIds): a book
    // whose file sits UNDER a directory that errored this pass (EACCES / failed
    // automount) is NOT prunable -- a transient unreadable subtree must never
    // delete a book (or its per-user reading position) whose file is still on
    // disk. The whole-root guard misses this when the root still has survivors.
...
// Wave 5: the deps carry `booksDb` (the feature store); a mutator runs
// through `deps.booksDb.mutate` so its diff rides the doc commit.
function listShelfPins(deps) {
  const pins = deps.booksDb.read().pins;
...
function addShelfPin(deps, { dir, label } = {}) {
  ...
  let record;
  return deps.updateDatabase(() => deps.booksDb.mutate((db) => {
...
// Wave 5 of the relational-migration arc: the namespace's TABLES - books_folders
// (roots), books_items (one row per book), books_progress (the FROZEN pre-auth
// reading positions the first admin adopts once), books_pins (the frozen
// pre-auth shelf pins, an ordered record list), books_settings, books_audio
// (per-book TTS synthesis status). server.js creates ONE store (`booksDb`).
const { defineFeatureStore } = require('../db/featureStore');
const FEATURE = defineFeatureStore({
  name: 'books',
  parts: {
    folders: { kind: 'list', column: 'path' },
    items: { kind: 'map', keyColumn: 'book_id' },
    progress: { kind: 'map', keyColumn: 'book_id' },
    pins: { kind: 'records' },
    settings: { kind: 'kv' },
    audio: { kind: 'map', keyColumn: 'book_id' },
  },
});

module.exports = {
  FEATURE,
  createBooksStore: (adapter, opts) => FEATURE.createStore(adapter, opts),
  ensureBooks,
  readBooks,
