...
// v1.338 (Dean: "non-YouTube things supported by YT DLP should have generally first-class
// experiences"; plan 2026-09-26-first-class-any-site D4): the SCHEMA-ONLY backfill of a universal
// download's saved page link, from the `comment` tag its original probe ALREADY persisted (yt-dlp's
// `--embed-metadata` writes `webpage_url` into `purl` and `comment`; MP4 / M4A keep only `comment`) -
// no fresh probe (the thumbnail-backfill lesson), the `youtubeId` precedent above. Returns undefined
// when the item is not a non-YouTube download (leave the key ABSENT - it may become one later this
// scan), else the sanitized link or null ("attempted, nothing usable": the watch page's on-demand
// probe and a reheat can still find one in `purl` / stream tags). Pure.
function deriveScanSourceUrl(item) {
  if (!item || typeof item !== 'object') return undefined;
  const source = typeof item.sourceExtractor === 'string' ? item.sourceExtractor : '';
  if (source === '' || source.toLowerCase() === 'youtube') return undefined;
  const comment = item.tags && typeof item.tags.comment === 'string' ? item.tags.comment : null;
  return sanitizeSourceShareUrl(comment);
}
