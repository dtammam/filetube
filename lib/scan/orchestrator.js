...
const { extractYtdlpVideoId, deriveScanYoutubeId, deriveReleaseDate, deriveScanSourceUrl } = require('./identity');
  ...
  async function runScanDirectories() {
    ...
    for (const { fp: filePath, fileInfo: info } of orderScannedByRecency(scannedFiles, db.metadata)) {
      ...
      if (reusable) {
        ...
        // v1.338 (plan first-class-any-site D4): SCHEMA-ONLY `sourceUrl` backfill for a download from
        // another site that predates the saved link - the persisted `comment` tag only, no probe; set
        // once (a string, or null for "attempted"). BOTH reuse arms carry it (LESSONS-rules 9).
        if (!Object.prototype.hasOwnProperty.call(existing, 'sourceUrl')) {
          const backfilledSourceUrl = deriveScanSourceUrl(existing);
          if (backfilledSourceUrl !== undefined) {
            existing.sourceUrl = backfilledSourceUrl;
            dbChanged = true;
          }
        }
        ...
        // v1.338: the same schema-only `sourceUrl` backfill as the plain reuse fast-path above.
        if (!Object.prototype.hasOwnProperty.call(existing, 'sourceUrl')) {
          const backfilledSourceUrl = deriveScanSourceUrl(existing);
          if (backfilledSourceUrl !== undefined) existing.sourceUrl = backfilledSourceUrl;
        }
        ...
        if (existing) {
          ...
          // v1.338 (plan first-class-any-site D3): the saved page link carries forward with the source
          // identity it belongs to - the persist-gate checkpoint. Only a real link is carried (a null
          // "attempted" marker is re-derived from the changed file's fresh tags by the bridge block).
          if (typeof existing.sourceUrl === 'string' && existing.sourceUrl !== '') {
            newMetadata[id].sourceUrl = existing.sourceUrl;
          }
        }
        dbChanged = true;
      }
      ...
      for (const item of Object.values(newMetadata)) {
        ...
        // unset), so the next reheat re-persists it with no scan running.
        {
          ...
          if (freshItem) {
            ...
            // v1.338 (plan first-class-any-site D3): the saved page link, the same gap-fill - the
            // persist-gate checkpoint: today only the scan writes `metadata[].sourceUrl` (the capture
            // writes the downloadMeta row; the reheat does not write it), so this guards a future
            // writer, and the scan's own absent / null never erases a live link.
            if ((typeof item.sourceUrl !== 'string' || item.sourceUrl === '')
                && typeof freshItem.sourceUrl === 'string' && freshItem.sourceUrl !== '') {
              item.sourceUrl = freshItem.sourceUrl;
            }
        ...
        if (freshlyScannedIds.has(item.id) && matchRootFolder(item.filePath, ytdlpDownloadRoots)) {
          ...
          if (mediaRef && mediaRef.source) {
            ...
            if (consumedU) {
              ...
              // v1.338 (plan first-class-any-site D1): the page the download came from, captured at
              // download time (re-checked by consumeUniversalDownloadMeta).
              if (typeof consumedU.sourceUrl === 'string' && consumedU.sourceUrl !== '') {
                item.sourceUrl = consumedU.sourceUrl;
              }
            ...
            // v1.338 (plan first-class-any-site D4): a download from another site with no captured
            // page link (a capture from before v1.338, or none bridged) takes it from its own persisted
            // `comment` tag - no probe; written only when it changes (no db churn on a null re-derive).
            if (typeof item.sourceUrl !== 'string' || item.sourceUrl === '') {
              const fromTags = deriveScanSourceUrl(item);
              if (fromTags !== undefined && item.sourceUrl !== fromTags) {
                item.sourceUrl = fromTags;
                dbChanged = true;
              }
            }
