...
function statementsFor(adapter) {
  ...
  const s = {
    ...
    upsertQueueEntry: sql.prepare('INSERT INTO user_queue (user_id, entry_uid, media_id, entry_order) VALUES (?, ?, ?, ?) ON CONFLICT(user_id, entry_uid) DO UPDATE SET media_id = excluded.media_id, entry_order = excluded.entry_order'),
    getQueueEntries: sql.prepare('SELECT entry_uid, media_id FROM user_queue WHERE user_id = ? ORDER BY entry_order, entry_uid'),
    ...
    // rekeyMediaState below, same commit as the table's birth.
    delQueueByMedia: sql.prepare('DELETE FROM user_queue WHERE media_id = ?'),
    rekeyQueueMedia: sql.prepare('UPDATE user_queue SET media_id = ? WHERE media_id = ?'),
    ...
    listNotificationsForUser: sql.prepare(`
      ...
      LIMIT ?
    `),
    countNotificationsSince: sql.prepare('SELECT COUNT(*) AS c FROM notifications WHERE created_at > ?'),
...
module.exports = function createUserStore(adapter) {
  ...
  return {
    ...
    // loop restart or rewatch-from-0 must not un-watch); the ONLY deletes
    // are the user's own history removals (removeHistory/clearHistory,
    // v1.64) and the media-lifecycle prune above.
    ...
    listNotifications(userId) {
      ...
      const items = s.listNotificationsForUser.all(userId, state.clearedAt, NOTIFICATION_CAP)
        .map((r) => ({
          id: r.id,
          mediaId: r.media_id,
    ...
    countUnseenNotifications(userId) {
      ...
      return st().countNotificationsSince.get(since).c;
    },
    ...
    getQueue(userId) {
      const entries = st().getQueueEntries.all(userId).map((r) => ({ uid: r.entry_uid, mediaId: r.media_id }));
    ...
    setQueue(userId, entries, pointerUid, updatedAt) {
      ...
      try {
        ...
        list.forEach((e, i) => {
          if (e && e.uid && e.mediaId) s.upsertQueueEntry.run(userId, e.uid, e.mediaId, i);
        });
        ...
        if (qb) {
          ...
          qEntries.forEach((e, i) => {
            if (e && typeof e.uid === 'string' && e.uid && typeof e.mediaId === 'string' && e.mediaId) {
              s.upsertQueueEntry.run(u.id, e.uid, e.mediaId, i);
            }
          });
