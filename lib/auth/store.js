  ...
  return {
    ...
    rekeyMediaState(oldId, newId) {
      ...
      try {
        ...
        // v1.51 notification feed: reads key by notification id (stable
        // across a media re-key), so only the feed row itself moves.
    ...
    listNotifications(userId) {
      ...
      const items = s.listNotificationsForUser.all(userId, state.clearedAt, NOTIFICATION_CAP)
        .map((r) => ({ id: r.id, mediaId: r.media_id, createdAt: r.created_at, unread: r.unread === 1 }));
    ...
    replaceAllNotificationsRaw(rows) {
      ...
      for (const r of rows.filter(isValidNotificationEntry)) {
