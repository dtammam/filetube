  ...
  return {
    ...
    replaceAllUsersRaw(users, nowMs = Date.now()) {
      ...
      for (const u of users) {
        ...
        // repopulate the server on the next mirrored write).
        for (const pref of Array.isArray(u.prefs) ? u.prefs : []) {
          if (pref && typeof pref.key === 'string' && typeof pref.value === 'string' && Number.isFinite(Number(pref.updatedAt))) {
            s.upsertPrefLWW.run(u.id, pref.key, pref.value, Number(pref.updatedAt));
          }
        }
