'use strict';
const { SYNCED_PREF_KEYS: PREF_KEY_LIST, PREF_VALUE_MAX_BYTES, PREF_CLOCK_SLACK_MS } = require('../prefs-allowlist');
const PREF_KEYS = new Set(PREF_KEY_LIST);
...
// owns storage.
//
// Design source: docs/exec-plans/completed/2026-07-14-v1.42-multiuser-tranche.md §v1.43 +
// its "design-delta review round" (the atomicity, no-id-reuse, and
// warm-connection contracts below are the reviewed spec).

// v1.85: per-user search-history retention. Read shows 20 (SEARCH_HISTORY_CAP in
// server.js); the table keeps a small buffer beyond that and prunes the rest on
// every insert, so distinct terms can never accumulate unbounded.
const SEARCH_HISTORY_RETENTION = 50;
...
function statementsFor(adapter) {
  ...
  const s = {
    ...
    list: sql.prepare('SELECT id, username, display_name, role, can_manage_subscriptions, can_modify_library, disabled, created_at FROM users ORDER BY id'),
    ...
    insertPlain: sql.prepare(`
      INSERT INTO users (username, display_name, password_hash, role, can_manage_subscriptions, can_modify_library, settings_json, token_version, disabled, created_at)
      VALUES (?, ?, ?, ?, ?, ?, '{}', 0, 0, ?)
    `),
    ...
    setCanModifyLibrary: sql.prepare('UPDATE users SET can_modify_library = ? WHERE id = ?'),
    ...
    // M3 chapter likes (v1.317): a like may be keyed `<mediaId>::c<n>` (one chapter
    // of a chaptered audio file - lib/music/libraryAudio.js chapterTrackId). The
    // by-media delete and the re-key match the base id EXACTLY or the `::c`
    // PREFIX, so a file delete/purge/prune sheds its chapter likes and a move/
    // trash/restore carries them under the new base id (else they ORPHAN - the
    // data-loss class this wave's gate was briefed to destroy). Written with
    // substr, not LIKE: ids are opaque, never LIKE on an id (`_`/`%` would be
    // wildcards). The prefix arm ASSUMES a base id never contains `::c` - true
    // today (media ids are md5 hex from getMediaId, trash ids md5 of the trash
    // path); a future id source that mints `::c` inside a base id would let
    // removeMediaState('x') sweep item `x::c...`'s chapter likes too (gate r1
    // security I1 / qa S3). `?1` is the SAME bound value everywhere it appears
    // (SQLite numbered params).
    delLikedByMedia: sql.prepare("DELETE FROM user_liked WHERE media_id = ?1 OR (substr(media_id, 1, length(?1) + 3) = ?1 || '::c' AND length(media_id) > length(?1) + 3)"),
    ...
    // Re-key: `?1` = newId, `?2` = oldId. The exact row becomes newId; a chapter row
    // keeps its `::c<n>` suffix under the new base (substr from length(old)+1).
    rekeyLiked: sql.prepare("UPDATE OR REPLACE user_liked SET media_id = ?1 || substr(media_id, length(?2) + 1) WHERE media_id = ?2 OR (substr(media_id, 1, length(?2) + 3) = ?2 || '::c' AND length(media_id) > length(?2) + 3)"),
    ...
    // v1.97 "Hide from feed" - per-user modern-feed prune (schema v17). SAME
    // id-keyed-carrier shape as user_liked: point add/remove, by-media delete
    // (cascade), OR-REPLACE re-key (move). SEPARATE from the admin visibility
    // flag - this is the member's own feed pruning, never a permission.
    addFeedHidden: sql.prepare('INSERT INTO user_feed_hidden (user_id, media_id, hidden_at) VALUES (?, ?, ?) ON CONFLICT(user_id, media_id) DO NOTHING'),
    removeFeedHidden: sql.prepare('DELETE FROM user_feed_hidden WHERE user_id = ? AND media_id = ?'),
    getFeedHidden: sql.prepare('SELECT media_id FROM user_feed_hidden WHERE user_id = ? ORDER BY hidden_at DESC, media_id'),
    delFeedHiddenByMedia: sql.prepare('DELETE FROM user_feed_hidden WHERE media_id = ?'),
    rekeyFeedHidden: sql.prepare('UPDATE OR REPLACE user_feed_hidden SET media_id = ? WHERE media_id = ?'),
    // v1.85 #1: per-user search history (term-keyed; a re-search updates recency
    // via ON CONFLICT rather than duplicating).
    upsertSearch: sql.prepare('INSERT INTO user_search_history (user_id, term, searched_at) VALUES (?, ?, ?) ON CONFLICT(user_id, term) DO UPDATE SET searched_at = excluded.searched_at'),
    getSearch: sql.prepare('SELECT term FROM user_search_history WHERE user_id = ? ORDER BY searched_at DESC, term LIMIT ?'),
    delSearch: sql.prepare('DELETE FROM user_search_history WHERE user_id = ? AND term = ?'),
    clearSearchByUser: sql.prepare('DELETE FROM user_search_history WHERE user_id = ?'),
    // v1.85 (adversarial gate): search terms are high-cardinality, so the table
    // is RETENTION-capped per user on every insert (not just LIMITed on read) -
    // keep the most-recent SEARCH_HISTORY_RETENTION, prune the rest.
    pruneSearch: sql.prepare('DELETE FROM user_search_history WHERE user_id = ? AND term NOT IN (SELECT term FROM user_search_history WHERE user_id = ? ORDER BY searched_at DESC, term LIMIT ?)'),
    ...
    // v1.80 RBAC: per-user library restrictions (blocklist units).
    getRestrictions: sql.prepare('SELECT kind, value FROM user_restrictions WHERE user_id = ? ORDER BY kind, value'),
    addRestriction: sql.prepare('INSERT INTO user_restrictions (user_id, kind, value) VALUES (?, ?, ?) ON CONFLICT(user_id, kind, value) DO NOTHING'),
    clearRestrictionsByUser: sql.prepare('DELETE FROM user_restrictions WHERE user_id = ?'),
    ...
    // ---- v1.195 TV Shows: prune carriers (episode_id = md5(filePath)). Born with
    // the user_tv_* tables (the v1.42 lesson - a new id-keyed namespace wires its
    // delete carrier the day it exists). Read/write accessors land with the routes.
    delTvProgressByEpisode: sql.prepare('DELETE FROM user_tv_progress WHERE episode_id = ?'),
    delTvPlayedByEpisode: sql.prepare('DELETE FROM user_tv_played WHERE episode_id = ?'),
    delTvLikedByEpisode: sql.prepare('DELETE FROM user_tv_liked WHERE episode_id = ?'),
    // v1.196 TV player integration: the read/write accessors that land with the
    // progress/Continue routes. EPISODE-id keyed (md5(filePath)); no re-key by
    // construction (like podcasts) - a moved episode's stale row is shed by the
    // scan prune's removeTvEpisodeState carrier above. Mirrors the podcast set.
    upsertTvProgress: sql.prepare(`
      INSERT INTO user_tv_progress (user_id, episode_id, position_seconds, duration_seconds, updated_at)
      VALUES (?, ?, ?, ?, ?)
      ON CONFLICT(user_id, episode_id) DO UPDATE SET position_seconds = excluded.position_seconds, duration_seconds = excluded.duration_seconds, updated_at = excluded.updated_at
    `),
    getTvProgress: sql.prepare('SELECT episode_id, position_seconds, duration_seconds, updated_at FROM user_tv_progress WHERE user_id = ?'),
    getOneTvProgress: sql.prepare('SELECT position_seconds, duration_seconds, updated_at FROM user_tv_progress WHERE user_id = ? AND episode_id = ?'),
    setTvPlayed: sql.prepare('INSERT INTO user_tv_played (user_id, episode_id, played_at) VALUES (?, ?, ?) ON CONFLICT(user_id, episode_id) DO UPDATE SET played_at = excluded.played_at'),
    delTvPlayed: sql.prepare('DELETE FROM user_tv_played WHERE user_id = ? AND episode_id = ?'),
    getTvPlayedForUser: sql.prepare('SELECT episode_id, played_at FROM user_tv_played WHERE user_id = ?'),
    addTvLiked: sql.prepare('INSERT INTO user_tv_liked (user_id, episode_id, liked_at) VALUES (?, ?, ?) ON CONFLICT(user_id, episode_id) DO NOTHING'),
    removeTvLiked: sql.prepare('DELETE FROM user_tv_liked WHERE user_id = ? AND episode_id = ?'),
    getTvLiked: sql.prepare('SELECT episode_id, liked_at FROM user_tv_liked WHERE user_id = ? ORDER BY liked_at DESC, episode_id'),
    ...
    // ---- v1.265 cross-device preference sync (LWW at the STATEMENT: the WHERE
    // guard makes a stale write a no-op row-change, which setPrefsLWW reads
    // back as "skipped" - the race window is closed inside sqlite itself) ----
    upsertPrefLWW: sql.prepare(`
      INSERT INTO user_prefs (user_id, key, value, updated_at)
      VALUES (?, ?, ?, ?)
      ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at
      WHERE excluded.updated_at > user_prefs.updated_at
    `),
    getPrefsForUser: sql.prepare('SELECT key, value, updated_at FROM user_prefs WHERE user_id = ? ORDER BY key'),
    ...
    insertNotification: sql.prepare('INSERT INTO notifications (media_id, created_at, kind) VALUES (?, ?, ?)'),
    // v1.73: podcast feed rows retire with the EPISODE purge, kind-scoped.
    notificationIdsByEpisode: sql.prepare("SELECT id FROM notifications WHERE media_id = ? AND kind = 'podcast'"),
    ...
    listNotificationsForUser: sql.prepare(`
      SELECT n.id, n.media_id, n.created_at, n.kind,
    ...
    `),
    // v1.146: the member-facing badge count - engine rows are ADMIN-only
    // and must not tick a member's badge (the panel filters them out, so a
    // count including them would be a phantom badge the member can never
    // clear by reading).
    countNotificationsSinceNoEngine: sql.prepare(`
      SELECT COUNT(*) AS c FROM notifications n
      WHERE n.created_at > ?
        AND n.kind != 'engine'
        AND NOT EXISTS (
    ...
    allNotifications: sql.prepare('SELECT media_id, created_at, kind FROM notifications ORDER BY created_at, id'),
    ...
    notificationsAfterId: sql.prepare('SELECT id, media_id, created_at, kind FROM notifications WHERE id > ? ORDER BY id LIMIT ?'),
...
function rowToUser(row) {
  ...
  return {
    ...
    canModifyLibrary: row.can_modify_library === 1,
...
function isValidNotificationEntry(e) {
  return Boolean(e) && typeof e.mediaId === 'string' && e.mediaId !== '' &&
    ...
    // v1.73: kind is optional (absent = media, the pre-v13 posture) but a
    // PRESENT kind must be a recognized one - garbage is dropped, never coerced.
    // v1.146: 'engine' joins (downloader-engine events; ADMIN-only at every
    // read surface - list branch AND badge count, see countUnseenNotifications).
    (e.kind === undefined || e.kind === 'media' || e.kind === 'podcast' || e.kind === 'engine');
}

// v1.73: the one normalizer for a feed entry's kind at every insert site.
function notificationKindOf(e) {
  if (e && e.kind === 'podcast') return 'podcast';
  if (e && e.kind === 'engine') return 'engine'; // v1.146
  return 'media';
}
  ...
  return {
    ...
    listUsers() {
      return st().list.all().map((r) => ({
        ...
        canModifyLibrary: r.can_modify_library === 1,
    ...
    createUser({ username, displayName, passwordHash, role = 'member', canManageSubscriptions = false, canModifyLibrary = false }, nowIso) {
      const u = normalizeUsername(username);
      ...
      const res = st().insertPlain.run(u, displayName || u, passwordHash, role, canManageSubscriptions ? 1 : 0, canModifyLibrary ? 1 : 0, nowIso);
    ...
    setCanModifyLibrary(id, v) {
      st().setCanModifyLibrary.run(v ? 1 : 0, id);
    },
    ...
    removeMediaState(mediaIds) {
      ...
      try {
        for (const id of ids) {
          ...
          s.delFeedHiddenByMedia.run(id); // v1.97: a purged video sheds every user's feed-hidden row
    ...
    rekeyMediaState(oldId, newId) {
      ...
      try {
        ...
        s.rekeyFeedHidden.run(newId, oldId); // v1.97: a moved video carries its feed-hidden flag to the new id
    ...
    // v1.85 #1: search-history accessors. A term is stored trimmed as-typed
    // (exact dedup via the PK); recency (searched_at) drives the DESC list + cap.
    addSearchTerm(userId, term, searchedAt) {
      const s = st();
      s.upsertSearch.run(userId, term, searchedAt);
      s.pruneSearch.run(userId, userId, SEARCH_HISTORY_RETENTION); // bound growth
    },
    getSearchHistory(userId, limit) {
      const n = Number.isInteger(limit) && limit > 0 ? limit : 20;
      return st().getSearch.all(userId, n).map((r) => r.term);
    },
    removeSearchTerm(userId, term) {
      st().delSearch.run(userId, term);
    },
    clearSearchHistory(userId) {
      st().clearSearchByUser.run(userId);
    },
    removeLiked(userId, mediaId) {
      st().removeLiked.run(userId, mediaId);
    },
    // v1.97 "Hide from feed" accessors - the member's own modern-feed prune.
    // Membership only (id list); newest-first for the You-tab restore section.
    getFeedHidden(userId) {
      return st().getFeedHidden.all(userId).map((r) => r.media_id);
    },
    addFeedHidden(userId, mediaId, hiddenAt) {
      st().addFeedHidden.run(userId, mediaId, hiddenAt); // ON CONFLICT DO NOTHING -> idempotent
    },
    removeFeedHidden(userId, mediaId) {
      st().removeFeedHidden.run(userId, mediaId); // idempotent -> the "restore"/undo verb
    },
    // ---- v1.80 RBAC: per-user restrictions ----------------------------------
    // Returns the user's raw restriction rows [{kind, value}] - fed straight
    // into visibility.buildRestrictionIndex. An admin never has rows (the admin
    // UI does not offer restricting an admin), so admin -> [] -> sees all.
    getRestrictions(userId) {
      return st().getRestrictions.all(userId).map((r) => ({ kind: r.kind, value: r.value }));
    },
    // Replace a user's ENTIRE restriction set atomically (the admin UI PUTs the
    // desired set). Rows are pre-validated by the route; this trusts shape.
    setRestrictions(userId, rows) {
      const s = st();
      adapter.begin();
      try {
        s.clearRestrictionsByUser.run(userId);
        for (const r of Array.isArray(rows) ? rows : []) {
          if (r && typeof r.kind === 'string' && typeof r.value === 'string' && r.value !== '') {
            s.addRestriction.run(userId, r.kind, r.value);
          }
        }
        adapter.commit();
      } catch (err) {
    ...
    recordNotifications(entries) {
      ...
      try {
        for (const e of list) {
          ...
          s.insertNotification.run(e.mediaId, Math.floor(e.createdAt), notificationKindOf(e));
          inserted++;
        }
    ...
    listNotifications(userId) {
      ...
      const items = s.listNotificationsForUser.all(userId, userId, state.clearedAt, NOTIFICATION_CAP)
        .map((r) => ({
          ...
          // v1.73: carried, never inferred. v1.146: 'engine' passes through
          // (the server's list route enforces its admin-only visibility).
          kind: r.kind === 'podcast' || r.kind === 'engine' ? r.kind : 'media',
    ...
    // "unseen" -- clear implies seen). v1.146: engine rows count ONLY when
    // the caller says this user may see them (admins) - the default is the
    // SAFE direction, so a call site that forgets the flag under-counts for
    // an admin rather than phantom-badging a member.
    countUnseenNotifications(userId, opts = {}) {
      const state = this.getNotificationState(userId);
      const since = Math.max(state.lastSeenAt, state.clearedAt);
      const s = st();
      return opts.includeEngine === true
        ? s.countNotificationsSince.get(since, userId).c
        : s.countNotificationsSinceNoEngine.get(since, userId).c;
    },
    ...
    seedNotifications(entries, nowMs) {
      ...
      try {
        ...
        for (const e of list) {
          ...
          s.delNotificationByMedia.run(e.mediaId);
          ids.push(Number(s.insertNotification.run(e.mediaId, Math.floor(e.createdAt), notificationKindOf(e)).lastInsertRowid));
        }
    ...
    exportNotificationsForBackup() {
      // v1.146 (gate round 1 QA W3): 'engine' survives the round trip too -
      // the pre-fix mapping downgraded engine rows to 'media' on export,
      // which broke the admin-only invariant after a restore (a member's
      // badge would transiently count the row).
      return st().allNotifications.all().map((r) => ({ mediaId: r.media_id, createdAt: r.created_at, kind: r.kind === 'podcast' || r.kind === 'engine' ? r.kind : 'media' }));
    },
    ...
    replaceAllNotificationsRaw(rows) {
      ...
      for (const r of kept) {
        s.insertNotification.run(r.mediaId, Math.floor(r.createdAt), notificationKindOf(r));
      }
    ...
    listNotificationsAfter(cursorId, limit) {
      return st().notificationsAfterId.all(cursorId, limit).map((r) => ({ id: r.id, mediaId: r.media_id, createdAt: r.created_at, kind: r.kind === 'podcast' ? 'podcast' : 'media' }));
    },
    ...
    removePodcastEpisodeState(episodeIds) {
      ...
      try {
        for (const id of ids) {
          ...
          // v1.73: the episode's FEED rows retire here too, kind-scoped -
          // a media notification sharing the md5 id is untouchable (its
          // lifecycle is removeMediaState's delNotificationByMedia, which
          // under UNIQUE(media_id) can only ever see the row that WON the
          // documented replace semantics).
          for (const row of s.notificationIdsByEpisode.all(id)) {
            s.delReadsByNotificationId.run(row.id);
            s.delDismissalsByNotificationId.run(row.id);
            s.delNotificationById.run(row.id);
          }
        }
        adapter.commit();
    ...
    // ---- v1.196 TV Shows: per-user resume + played latch + likes ----
    // Episode-id keyed (md5(filePath)); mirrors the podcast accessors exactly.
    getTvProgress(userId) {
      // Null-prototype accumulator (the __proto__ row-key lesson).
      const out = Object.create(null);
      for (const r of st().getTvProgress.all(userId)) {
        out[r.episode_id] = { position: r.position_seconds, duration: r.duration_seconds, updatedAt: r.updated_at };
      }
      return out;
    },
    getOneTvProgress(userId, episodeId) {
      const r = st().getOneTvProgress.get(userId, episodeId);
      return r ? { position: r.position_seconds, duration: r.duration_seconds, updatedAt: r.updated_at } : null;
    },
    setTvProgress(userId, episodeId, { position, duration, updatedAt }) {
      st().upsertTvProgress.run(userId, episodeId, num(position), num(duration), typeof updatedAt === 'string' ? updatedAt : null);
    },
    getTvPlayed(userId) {
      const out = Object.create(null);
      for (const r of st().getTvPlayedForUser.all(userId)) {
        out[r.episode_id] = r.played_at;
      }
      return out;
    },
    setTvPlayed(userId, episodeId, playedAt) {
      st().setTvPlayed.run(userId, episodeId, typeof playedAt === 'string' ? playedAt : null);
    },
    clearTvPlayed(userId, episodeId) {
      st().delTvPlayed.run(userId, episodeId);
    },
    getTvLiked(userId) {
      return st().getTvLiked.all(userId).map((r) => ({ episodeId: r.episode_id, likedAt: r.liked_at }));
    },
    addTvLiked(userId, episodeId, likedAt) {
      st().addTvLiked.run(userId, episodeId, typeof likedAt === 'string' ? likedAt : null);
    },
    removeTvLiked(userId, episodeId) {
      st().removeTvLiked.run(userId, episodeId);
    },

    // v1.195 TV Shows: shed a pruned episode's per-user progress/played/liked in
    // ONE transaction (mirrors removeMusicState/removePodcastEpisodeState).
    removeTvEpisodeState(episodeIds) {
      const ids = Array.isArray(episodeIds) ? episodeIds : [episodeIds];
      if (ids.length === 0) return;
      const s = st();
      adapter.begin();
      try {
        for (const id of ids) {
          s.delTvProgressByEpisode.run(id);
          s.delTvPlayedByEpisode.run(id);
          s.delTvLikedByEpisode.run(id);
        }
        adapter.commit();
      } catch (err) {
        adapter.rollback();
    ...
    // v1.265: the SYNCED preference layer. Values are localStorage strings
    // verbatim; updatedAt is ms. Returns a plain {key: {value, updatedAt}} map.
    getPrefs(userId) {
      const out = {};
      for (const r of st().getPrefsForUser.all(userId)) out[r.key] = { value: r.value, updatedAt: r.updated_at };
      return out;
    },
    // LWW batch: entries [{key, value, updatedAt}] (caller validates shape +
    // allowlist). Returns {applied: [...], skipped: [...]} - a skip is a write
    // whose updatedAt lost to the stored row (the statement's WHERE guard).
    setPrefsLWW(userId, entries) {
      const applied = []; const skipped = [];
      for (const e of entries) {
        const info = st().upsertPrefLWW.run(userId, e.key, e.value, e.updatedAt);
        (info.changes > 0 ? applied : skipped).push(e.key);
      }
    ...
    exportUsersForBackup() {
      ...
      for (const r of sql.prepare('SELECT * FROM users ORDER BY id').all()) {
        ...
        users.push({
          ...
          // v1.97 "Hide from feed" (THIRTEENTH-strike id-keyed carrier: a new
          // per-user namespace MUST ride the SAME backup bundle as the table
          // that carries it, or restore silently erases it -- the class this
          // repo has paid for 5+ times). Media ids are stable md5(path) keys.
          feedHidden: sql.prepare('SELECT media_id, hidden_at FROM user_feed_hidden WHERE user_id = ? ORDER BY hidden_at, media_id').all(r.id)
            .map((row) => ({ mediaId: row.media_id, hiddenAt: row.hidden_at })),
          ...
          // v1.265 preference sync (FOURTEENTH-strike carrier: the namespace
          // rides the bundle in the table's birth commit). Keys are the
          // exec-plan allowlist; instance-agnostic by construction.
          prefs: sql.prepare('SELECT key, value, updated_at FROM user_prefs WHERE user_id = ? ORDER BY key').all(r.id)
    ...
    replaceAllUsersRaw(users, nowMs = Date.now()) {
      ...
      const insertWithId = sql.prepare(`
        INSERT INTO users (id, username, display_name, password_hash, role, can_manage_subscriptions, can_modify_library, settings_json, token_version, disabled, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      for (const u of users) {
        ...
        // v1.97 "Hide from feed" (THIRTEENTH-strike carrier). Absent in
        // pre-v1.97 bundles -> restores nothing (legal). Without this loop the
        // DELETE FROM users above cascades away every feed-hide row and never
        // rebuilds it -- the silent-erase the gate caught.
        for (const fh of Array.isArray(u.feedHidden) ? u.feedHidden : []) {
          if (fh && typeof fh.mediaId === 'string' && fh.mediaId) s.addFeedHidden.run(u.id, fh.mediaId, typeof fh.hiddenAt === 'string' ? fh.hiddenAt : null);
        }
        ...
        // v1.265 preference sync (FOURTEENTH-strike carrier). Absent in
        // pre-v1.265 bundles - legal, restores nothing (localStorage caches
        // repopulate the server on the next mirrored write). Adversarial
        // W-A/W-C: this loop enforces the SAME defenses as the route - the
        // seat planted an off-list 100KB row served forever, and a far-future
        // stamp that out-ranked every clamped route write (a 285,000-year
        // wedge riding a restore). One shared module, every ingress.
        for (const pref of Array.isArray(u.prefs) ? u.prefs : []) {
          if (!pref || typeof pref.key !== 'string' || typeof pref.value !== 'string' || !Number.isFinite(Number(pref.updatedAt))) continue;
          if (!PREF_KEYS.has(pref.key)) continue;
          if (Buffer.byteLength(pref.value, 'utf8') > PREF_VALUE_MAX_BYTES) continue;
          s.upsertPrefLWW.run(u.id, pref.key, pref.value, Math.min(Number(pref.updatedAt), nowMs + PREF_CLOCK_SLACK_MS));
        }
    ...
    __clearUserStateForTests() {
      ...
      adapter.sql.exec('DELETE FROM user_search_history'); // v1.85 #1: search history joins the reset
      adapter.sql.exec('DELETE FROM user_feed_hidden'); // v1.97: feed-hide state must not bleed across cases
      ...
      adapter.sql.exec('DELETE FROM user_prefs'); // v1.265 (the reset is a hand-maintained sibling list - this line was caught MISSING by the suite's own leak)
