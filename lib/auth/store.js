...
module.exports = function createUserStore(adapter) {
  ...
  return {
    ...
    getQueue(userId) {
      ...
      const entries = st().getQueueEntries.all(userId).map((r) => ({ uid: r.entry_uid, mediaId: r.media_id, kind: r.entry_kind === 'podcast' ? 'podcast' : 'media' }));
    ...
    setQueue(userId, entries, pointerUid, updatedAt) {
      ...
      try {
        ...
        list.forEach((e, i) => {
          if (e && e.uid && e.mediaId) s.upsertQueueEntry.run(userId, e.uid, e.mediaId, i, e.kind === 'podcast' ? 'podcast' : 'media');
        });
        ...
        if (qb) {
          ...
          qEntries.forEach((e, i) => {
            if (e && typeof e.uid === 'string' && e.uid && typeof e.mediaId === 'string' && e.mediaId) {
              ...
              s.upsertQueueEntry.run(u.id, e.uid, e.mediaId, i, e.kind === 'podcast' ? 'podcast' : 'media');
            }
          });
