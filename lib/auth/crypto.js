...
function verifySession(token, secret, { nowSeconds = Math.floor(Date.now() / 1000) } = {}) {
  ...
  // Compare the received mac STRING against the canonical base64url digest
  // (design-delta SUGGESTION-7): Buffer.from(x,'base64url') is
  // padding-tolerant, so a decode-then-compare accepted `mac=` and other
  // non-canonical variants as a second valid string for the same session.
  // A stateless HMAC cookie makes that benign today, but if v1.44+ ever
  // keys server-side state or a revocation list on the token string,
  // malleability silently breaks it. The canonical sha256 digest is exactly
  // 43 base64url chars; anything else cannot be the signature.
  const expectedB64 = crypto.createHmac('sha256', secret).update(payloadB64).digest('base64url');
  if (macB64.length !== expectedB64.length) return null;
