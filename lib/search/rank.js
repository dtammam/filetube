...
// Normalize a store's `addedAt`/`pubDateMs` into a comparable recency NUMBER
// (higher = newer). Video/podcast store epoch-ms NUMBERS; music/books/tv store
// ISO-8601 STRINGS (new Date().toISOString()). A bare Number("2026-01-...") is
// NaN, which silently zeroed the recency tiebreak for those three types (gate
// WARNING) - Date.parse handles the ISO form, and a finite number passes
// through. Anything unparseable -> 0 (sorts oldest, deterministic).
function toRecency(v) {
...
module.exports = { matchTier, TYPE_PRIORITY, rankResults, toRecency };
