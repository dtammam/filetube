...
function searchMetadata(q, req, deps, mediaType) {
  ...
  for (const item of items) {
    ...
    pushIfMatch(out, q, {
      ...
      // v1.338 (plan first-class-any-site D5): a download from another site's saved page link.
      sourceShareUrl: deps.sourceShareUrl ? deps.sourceShareUrl(item) : undefined,
      progressPercent: 0,
    });
  }
  return out;
...
// The music tracks (Wave 5: music_tracks behind server.js's musicDb, handed
// in as the `musicTracks` dep - a snapshot map). RBAC: trackVisibleTo. Client kind 'track'.
function searchMusic(q, req, deps) {
  const tracks = Object.values(typeof deps.musicTracks === 'function' ? (deps.musicTracks() || {}) : {});
  const out = [];
  ...
  // v1.221: the OPTED-IN downloaded library audio (already RBAC-filtered inside
  // musicLibraryTracks), chaptered files expanded into per-chapter tracks - so a
  // downloaded track, and each CHAPTER TITLE, is a findable song that plays via
  // the music player (the library/chapter markers ride the result for the client).
  const lib = (typeof deps.musicLibraryTracks === 'function') ? (deps.musicLibraryTracks() || []) : [];
  for (const t of lib) {
    pushIfMatch(out, q, {
      resultType: 'music', kind: 'track', id: t.id,
      title: t.title || '',
      identityText: [t.artist, t.album, t.albumArtist].filter(Boolean).join(' '),
      recency: rank.toRecency(t.addedAt),
      artist: t.artist, album: t.album,
      source: t.source, streamSrc: t.streamSrc, artUrl: t.artUrl, progressEndpoint: t.progressEndpoint,
      ...(t.source === 'library-chapter' ? { chapterStartSec: t.chapterStartSec } : {}),
    });
  }
  return out;
}

// The podcasts namespace reaches the registry through the `podcastsNs` dep
// (Wave 5: a snapshot of the feature store - the same shape readPodcasts gave).
function podcastsNsOf(deps) {
  const ns = typeof deps.podcastsNs === 'function' ? deps.podcastsNs() : null;
  return ns && typeof ns === 'object' ? ns : {};
}

// podcasts.subscriptions (shows). RBAC: podcastVisibleTo on a bare {subId}
// (the show-art route's form). Client kind 'podcast-show'.
function searchPodcastShows(q, req, deps) {
  const subs = podcastsNsOf(deps).subscriptions || [];
  const out = [];
...
// podcasts.episodes. RBAC: podcastVisibleTo({subId, filePath}). Client kind
// 'podcast' (reuses the existing cardKindPresentation podcast arm).
function searchPodcastEpisodes(q, req, deps) {
  const ns = podcastsNsOf(deps);
  const eps = Object.values(ns.episodes || {});
...
// The book items (Wave 5: books_items behind server.js's booksDb, handed in as
// the `booksItems` dep - a snapshot map). RBAC: bookVisibleTo. Client kind 'book'.
function searchBooks(q, req, deps) {
  const items = Object.values(typeof deps.booksItems === 'function' ? (deps.booksItems() || {}) : {});
  const out = [];
