...
function searchTvShows(q, req, deps) {
  const shows = tvParse.groupShows(deps.gates.tvVisibleEpisodes(req));
  const out = [];
  for (const s of shows) {
    pushIfMatch(out, q, {
      ...
      title: s.name || '',
      identityText: '',
      recency: Number(s.latestAddedAt) || 0,
