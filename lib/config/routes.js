...
function registerSettingsRoutes(app, deps) {
  const {
    ...
    chapterSnap, // chapter snap (2026-09-24): lib/media/chapterSnap - the lead-in validator + read clamp
  ...
  function settingsResponse(settings) {
    return {
      ...
      perfDiagnosticsEnabled: settings.perfDiagnosticsEnabled === true,
      // Chapter Snap (2026-09-24): the lead-in before a snapped chapter's first sound
      // (see DEFAULT_SETTINGS), clamped on the way out like the snap reader does.
      chapterSnapLeadInSec: chapterSnap.clampLeadIn(settings.chapterSnapLeadInSec),
  ...
  app.post('/api/settings', async (req, res) => {
    ...
    // perfDiagnosticsEnabled -- boolean, mirrors attributeControlEnabled exactly.
    if ('perfDiagnosticsEnabled' in body && typeof body.perfDiagnosticsEnabled !== 'boolean') {
      return res.status(400).json({ error: 'perfDiagnosticsEnabled must be a boolean' });
    }
    // Chapter Snap (2026-09-24): chapterSnapLeadInSec -- a number of seconds in
    // [0, 2], stored rounded to the hundredth (what the Setup select offers).
    if ('chapterSnapLeadInSec' in body) {
      if (!chapterSnap.isValidLeadIn(body.chapterSnapLeadInSec)) {
        return res.status(400).json({ error: 'chapterSnapLeadInSec must be a number of seconds from 0 to 2' });
      }
      body.chapterSnapLeadInSec = Math.round(body.chapterSnapLeadInSec * 100) / 100;
    }
