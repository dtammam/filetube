...
const MATRIX = Object.freeze({
  video: {
    ...
    reheat: sup(['player'], ['reheatModuleEnabled', 'reheat']),
  ...
  podcasts: {
    ...
    download: sup(['list'], ['?download=1']),
    share: todo('next-wave', 'universal Share via shareMediaFile; needs the factory generalization'),
    queue: sup(['list'], ['addToQueue']),
    delete: sup(['list'], ['Move to trash']),
    ...
    like: sup(['list'], ['icon-heart', 'toggleLike']),
    watched: sup(['list'], ['played']),
    listen: na('a podcast episode is already audio'),
    // The player menu needs createExtrasMenu generalized off the video/music model
    // (it fetches /api/videos/:id + deletes via that endpoint) - the next-wave refactor.
    playerMenu: todo('next-wave', 'generalize createExtrasMenu to be endpoint-driven, then wire podcast actions'),
  },
  books: {
    download: sup(['reader'], ['reader-download-btn']),
    share: todo('next-wave', 'universal Share extends to books (Dean: send a friend a PDF)'),
