...
const { partFileName } = require('./paths');
...
async function downloadEnclosure(enclosureUrl, destDir, finalName, opts = {}) {
  ...
  // ONE spelling of the temp-name convention (gate suggestion #11: this was
  // a hardcoded twin of paths.partFileName, with the test bound to the
  // unused copy - the divergent-fixture class in miniature).
  const partPath = path.resolve(root, partFileName(finalName));
  ...
  return await new Promise((resolve) => {
    ...
    res.on('data', (c) => {
      ...
      // v1.124 R1: honor backpressure. A fast origin over a slow disk (Dean's
      // NAS/SMB) would otherwise let Node buffer the write stream unbounded in
      // memory - up to the 2 GB size cap per concurrent enclosure, enough to OOM
      // a small box. When the write buffer is full, pause the read; the drain
      // handler below resumes it. Guarded by `settled` so a drain after a
      // failure/finish never resumes a destroyed response.
      if (!out.write(c) && !settled) res.pause();
    });
    out.on('drain', () => { if (!settled) res.resume(); });
    res.on('end', () => {
