...
function archiveOrphanedSecrets(dataDir, orphans) {
  ...
  let existing = {};
  let raw = null;
  try { raw = fs.readFileSync(archivePath, 'utf8'); } catch { /* ENOENT: first archive */ }
  if (raw !== null) {
    try {
      const parsed = JSON.parse(raw);
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('not an object');
      ...
      // Delta-round-2 E3: a corrupt recovery store gets the SAME preserve-
      // aside discipline as the live file - overwriting it would silently
      // destroy previously-archived credentials.
      try {
        fs.renameSync(archivePath, `${archivePath}.corrupt`);
        console.error(`Podcasts: orphaned-secrets archive was corrupt; moved aside to ${archivePath}.corrupt`);
      } catch { /* best-effort */ }
    }
  }
  const merged = {};
