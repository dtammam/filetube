...
async function processSubscription(deps, subId) {
  ...
  // Gate fix (adversarial #2): feed-derived text is REDACTED at this
  // boundary, before anything derived from it is persisted or even used as
  // an identity. The load-bearing case is the parser's guid fallback (a
  // guid-less item adopts its ENCLOSURE URL, which on Patreon carries the
  // token in its path) - but description/author/link/title get the same
  // treatment, because a feed echoing its own tokened URL in prose was the
  // measured leak. redactSecretText is deterministic for a given generic
  // pattern set, so redacted guids stay stable archive keys across polls
  // and across token rotations (both /u/<old>/ and /u/<new>/ collapse to
  // /u/<redacted>/).
  ...
  // redactSecretText. Redaction depends on the GLOBAL secrets map, so an
  // unrelated subscription's ?format=podcast query value could rewrite THIS
  // show's episode identities (measured: 3 duplicate records + a full
  // re-download of an untouched show). guidKey is leak-free by construction
  // (a URL-shaped guid, tokened or not, becomes md5; a safe-charset guid
  // like Patreon's numeric post id stays verbatim) and depends on nothing
  // but the guid itself.
