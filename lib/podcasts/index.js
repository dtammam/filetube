 ...
 * a clean 500); never leaves a half-copy at the destination.
 */
...
async function sweepExpiredTrash(deps) {
  ...
  for (const id of expired) {
    ...
    if (ep && typeof ep.trashPath === 'string' && ep.trashPath.startsWith(root + path.sep)) {
...
function registerRoutes(app, deps) {
  ...
  app.delete('/api/podcasts/episodes/:id', async (req, res) => {
    ...
    if (!trashPath.startsWith(root + path.sep)) {
