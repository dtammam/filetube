...
async function processSubscription(deps, subId) {
  ...
  // stall the poll). Gated on the feature flag; a store failure is
  // best-effort (the episodes are downloaded either way).
  if (downloadedNotifications.length > 0 && deps.userStore
    && typeof deps.notificationsEnabled === 'function' && deps.notificationsEnabled()) {
    try {
...
function registerRoutes(app, deps) {
  ...
  // latest-first (the store's contract); the full episode LIST for the
  // Liked lane rides GET /api/podcasts/episodes?filter=liked below.
  ...
  // v1.71: cross-show episode selections. filter=liked feeds the Liked lane
  // (T4); filter=recent-listening feeds home's Continue-listening row (T5),
  // music's exact selection contract (position > 0, updatedAt desc,
  // server.js:/api/music recent-listening). Downloaded episodes only - both
  // lanes must be playable on tap. No filter = 400: this route is a
