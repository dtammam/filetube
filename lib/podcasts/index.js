...
async function processSubscription(deps, subId) {
  ...
  const cleanItems = parsed.items.map((it) => ({
    ...it,
    guid: redactSecretText(it.guid, allSecretUrls),
...
function sweepOrphanFeedSecrets(deps) {
  try {
    ...
    for (const subId of orphans) delete map[subId];
    secrets.saveFeedSecrets(deps.dataDir, map);
    console.warn(`Podcasts: reaped ${orphans.length} orphaned feed secret(s) with no owning subscription (likely a restored pre-podcasts backup).`);
  } catch { /* best-effort */ }
}
