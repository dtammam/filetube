...
async function sweepExpiredTrash(deps) {
  ...
  const ns = store.readPodcasts(db);
  const expired = store.selectExpiredTrashedEpisodes(ns.episodes, retentionDays, deps.now());
  ...
  for (const id of expired) {
    const ep = ns.episodes[id];
    // Confinement (unbound before the gate; now bound): only ever unlink
    // inside our own root, never a bundle-supplied path.
  ...
  await deps.updateDatabase((mdb) => {
    ...
    let changed = false;
    for (const id of expired) changed = store.reduceEpisodeStatus(mns, id, 'tombstone') || changed;
    return changed;
  });
  try { deps.userStore.removePodcastEpisodeState(expired); } catch (err) {
...
async function processSubscription(deps, subId) {
  ...
  if (parsed.channel.imageUrl
    ...
    try {
      ...
      if (!fs.existsSync(root)) throw new Error('podcasts root absent');
    ...
    } catch { coverFailure = 'unexpected'; /* downloadEnclosure never throws by contract; belt for a hostile impl */ }
  }
...
function registerRoutes(app, deps) {
  ...
  app.post('/api/podcasts/subscriptions', async (req, res) => {
    ...
    // Create the podcasts ROOT here, once, deliberately - the only place in
    // the module that may. Everywhere else (cover retry, reconcile, the
    // retention sweep) a missing root means MOUNT LOSS, and that ambiguity
    // is exactly what the gate's CRITICAL #2 turned destructive. Zero
    // subscriptions still creates nothing (the fresh-install guarantee):
    // this line only runs once an add has genuinely landed.
  ...
  app.post('/api/podcasts/episodes/:id/restore', async (req, res) => {
    ...
    try {
      fs.mkdirSync(path.dirname(ep.filePath), { recursive: true });
      moveFileAtomicish(ep.trashPath, ep.filePath);
    } catch {
      return res.status(500).json({ error: 'could not restore the file' });
    }
    await d.updateDatabase((mdb) => store.reduceEpisodeRestored(store.ensurePodcasts(mdb), ep.id));
