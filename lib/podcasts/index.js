...
//   updateDatabase(mutator)  - the serialized writer (the doc commit's tick)
//   podcastsDb               - the namespace's feature store (Wave 5 of the
//                              relational-migration arc): read() is the
//                              snapshot every read below takes; every write
//                              is `updateDatabase(() => podcastsDb.mutate(
//                              (mdb) => ...))` - the module's own reducers on
//                              a fresh holder, the row diff riding the commit
//   loadDatabase()           - the doc object (only the external yt-dlp show
//                              listers still take it)
//   getCachedDatabase()      - read-cache of the same (GET routes, same use)
 ...
 */
function resolvePodcastsRoot(_db, deps) { // the db argument is IGNORED by design (above); kept for the call shape
...
/** The namespace snapshot (the readPodcasts(db) view, from the tables). */
function readNs(deps) {
  return deps.podcastsDb.read();
}

function pollMinutesFrom(deps) {
  const v = readNs(deps).settings.pollMinutes;
...
async function sweepExpiredTrash(deps) {
  const root = resolvePodcastsRoot(null, deps);
  if (!fs.existsSync(root)) return;
  const retentionDays = (typeof deps.getSettings === 'function' ? deps.getSettings() : {}).trashRetentionDays; // Wave 4: app settings are a store, not a doc key (guarded like getLibraryFolders below)
  const ns = readNs(deps);
  ...
  // layer in.
  const tombstoned = [];
  await deps.updateDatabase(() => deps.podcastsDb.mutate((mdb) => {
...
async function processSubscription(deps, subId) {
  ...
  // transition check compares against this. (`sub` is a snapshot record now
  // - Wave 5 - but the loop re-reads the live record each iteration, and a
  // primitive keeps the "at cycle start" intent unmistakable.)
  ...
  if (!feedUrl) {
    await deps.updateDatabase(() => deps.podcastsDb.mutate((mdb) => store.reduceSetSubscriptionStatus(store.ensurePodcasts(mdb), subId, {
      lastCheckedAt: nowMs,
  ...
  const targets = store.episodesForSub(readNs(deps).episodes, subId)
  ...
  for (const ep of targets) {
    ...
    // must run with it).
    const liveSub = readNs(deps).subscriptions.find((s) => s && s.id === subId);
    ...
    if (result && result.ok) {
      downloadedCount += 1;
      await deps.updateDatabase(() => deps.podcastsDb.mutate((mdb) => store.reduceEpisodeDownloaded(store.ensurePodcasts(mdb), ep.id, {
      ...
      failedCount += 1;
      await deps.updateDatabase(() => deps.podcastsDb.mutate((mdb) => store.reduceEpisodeFailed(store.ensurePodcasts(mdb), ep.id,
  ...
  try {
    ...
    for (const sub of targets) {
      try {
        ...
        try {
          await deps.updateDatabase(() => deps.podcastsDb.mutate((mdb) => store.reduceSetSubscriptionStatus(store.ensurePodcasts(mdb), sub.id, {
            lastCheckedAt: deps.now(),
...
function sweepOrphanFeedSecrets(deps) {
  try {
    ...
    const live = new Set(readNs(deps).subscriptions.map((s) => s && s.id).filter(Boolean));
...
function armPodcastsTimer(deps) {
  ...
  const minutes = pollMinutesFrom(deps);
...
function startBackground(deps) {
  ...
  try {
    const root = resolvePodcastsRoot(null, deps);
    const others = [
      ...(typeof deps.getLibraryFolders === 'function' ? deps.getLibraryFolders() : []), // Wave 4: the root list is a table
      ...(typeof deps.getMusicFolders === 'function' ? deps.getMusicFolders() : []), // Wave 5: the music roots are a table
      ...(typeof deps.getBookFolders === 'function' ? deps.getBookFolders() : []), // Wave 5: the book roots are a table
...
function registerRoutes(app, deps) {
  ...
  // v1.128 Wave B (L10/L11): these are MEDIA items, so pass the requester's
  // media-visibility predicate so a restricted member sees no hidden yt-dlp
  // show/episode. Every real request (admin included) gets a predicate - for an
  // admin it just returns true for all their items, so the server-side
  // raw-vs-visible drop keeps their empty shows (byte-identical). `null` only
  // when there is no req (the health count) or the dep is absent (tests).
  const mediaItemVisible = (req) => (d.mediaVisibleTo && req ? (item) => d.mediaVisibleTo(req, item) : null);
  const externalShows = (db, req) => {
    try { return typeof d.listExternalShows === 'function' ? (d.listExternalShows(db, mediaItemVisible(req)) || []) : []; } catch { return []; }
  };
  ...
  app.get('/api/podcasts/subscriptions', (req, res) => {
    const ns = readNs(d);
    const subs = [...ns.subscriptions]
      .filter((s) => !d.episodeVisibleTo || d.episodeVisibleTo(req, { subId: s.id })) // v1.80 RBAC: hide restricted shows
      .sort((a, b) => (a.order || 0) - (b.order || 0));
    res.json({ subscriptions: subs.map((s) => publicSubscription(s, ns.episodes)) });
  });

  app.post('/api/podcasts/subscriptions', async (req, res) => {
    if (d.requireManageSubscriptions && !d.requireManageSubscriptions(req, res)) return; // v1.80 RBAC
    ...
    let added = false;
    try {
      await d.updateDatabase(() => d.podcastsDb.mutate((mdb) => {
        const mns = store.ensurePodcasts(mdb);
    ...
    // genuinely landed.
    try { fs.mkdirSync(resolvePodcastsRoot(null, d), { recursive: true }); } catch (err) {
  ...
  app.patch('/api/podcasts/subscriptions/:id', async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'could not save the subscription' }); // Wave 5: a failed commit is a 500, never a hang
    }
  ...
  app.delete('/api/podcasts/subscriptions/:id', async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ error: 'could not delete the subscription' }); // Wave 5: nothing was deleted (the commit failed whole)
    }
  ...
  app.post('/api/podcasts/subscriptions/:id/feed-url', async (req, res) => {
    ...
    try {
      await d.updateDatabase(() => d.podcastsDb.mutate((mdb) => store.reduceSetSubscriptionStatus(store.ensurePodcasts(mdb), sub.id, { secretMissing: false, lastStatus: 'feed URL updated' })));
    } catch {
      // Wave 5 (gate pass B): the secret is already stored; the record's flag
      // stays stale until the next poll clears it. A 500, never a hang.
      return res.status(500).json({ error: 'the feed URL was stored but the subscription record could not be updated - the next check will clear it' });
    }
  ...
  app.get('/api/podcasts/shows', (req, res) => {
    ...
    const subs = [...ns.subscriptions]
      .filter((s) => !d.episodeVisibleTo || d.episodeVisibleTo(req, { subId: s.id })) // v1.80 RBAC: hide restricted shows
      .sort((a, b) => (a.order || 0) - (b.order || 0));
    res.json({ shows: [...subs.map((s) => publicSubscription(s, ns.episodes)), ...externalShows(db, req)] });
  });
  ...
  app.get('/api/podcasts/episodes', (req, res) => {
    ...
    if (d.episodeVisibleTo) list = list.filter((ep) => d.episodeVisibleTo(req, ep)); // v1.80 RBAC
  ...
  app.get('/api/podcasts/episodes/:id', (req, res) => {
    ...
    if (d.episodeVisibleTo && !d.episodeVisibleTo(req, ep)) { // v1.80 RBAC
      return res.status(404).json({ error: 'no such episode' });
    }
  ...
  app.get('/api/podcasts/shows/:id/episodes', (req, res) => {
    ...
    if (req.params.id.startsWith('yt:')) {
      let payload = null;
      try { payload = typeof d.listExternalEpisodes === 'function' ? d.listExternalEpisodes(db, req.params.id, req.user.id, mediaItemVisible(req)) : null; } catch { payload = null; }
    ...
    if (d.episodeVisibleTo && !d.episodeVisibleTo(req, { subId: sub.id })) { // v1.80 RBAC: restricted show hidden
  ...
  app.get('/episode/:id', (req, res) => {
    ...
    if (d.episodeVisibleTo && !d.episodeVisibleTo(req, ep)) { // v1.80 RBAC: restricted show -> 404
      return res.status(404).json({ error: 'no such episode' });
    }
    ...
    const streamRoot = resolvePodcastsRoot(null, d);
  ...
  app.get('/podcastart/:subId', (req, res) => {
    const ns = readNs(d);
    if (d.episodeVisibleTo && !d.episodeVisibleTo(req, { subId: req.params.subId })) { // v1.80 RBAC
      return res.status(404).json({ error: 'not found' });
    }
    const sub = ns.subscriptions.find((s) => s && s.id === req.params.subId);
    if (sub && sub.showDirName) {
      const root = resolvePodcastsRoot(null, d);
      let showDir;
      ...
      if (showDir) {
        for (const name of ['cover.jpg', 'cover.png', 'folder.jpg', 'folder.png']) {
          ...
          if (fs.existsSync(p)) {
            // v1.123 T4 (security): `private` - /podcastart 404s per-user via
            // episodeVisibleTo, so a shared cache must not replay a restricted
            // show's art to another member. Matches the /thumbnail posture.
    ...
    res.set('Cache-Control', 'private, max-age=3600'); // v1.123 T4: uniform private axis behind the auth wall
  ...
  app.delete('/api/podcasts/episodes/:id', async (req, res) => {
    // v1.81 write-RBAC: an episode delete is a CONTENT deletion (a file leaves
    // disk), so it moves from the subscriptions gate to the library-modify gate.
    // A canManageSubscriptions-only member (who manages the registry but is not
    // trusted to delete media) is now 403 here (attack surface W4). Subscription
    // delete stays under requireManageSubscriptions (registry management).
    if (d.requireModifyLibrary && !d.requireModifyLibrary(req, res)) return; // v1.81 write-RBAC
    const ns = readNs(d);
    const ep = Object.prototype.hasOwnProperty.call(ns.episodes, req.params.id) ? ns.episodes[req.params.id] : null;
    if (!ep) return res.status(404).json({ error: 'no such episode' });
    // v1.123 T3 (security): the VISIBILITY axis. requireModifyLibrary gates the
    // capability, but a member holding it must still not trash an episode of a
    // show RESTRICTED from them - the "writer has capability but resource is
    // hidden" gap the v1.81 capability net does not cover. 404 (never confirm a
    // hidden resource exists), exactly as the read routes do, BEFORE the status
    // 400 below so a hidden episode's state never leaks.
    if (d.episodeVisibleTo && !d.episodeVisibleTo(req, ep)) return res.status(404).json({ error: 'no such episode' });
    ...
    if (!fs.existsSync(ep.filePath)) {
      ...
      await d.updateDatabase(() => d.podcastsDb.mutate((mdb) => store.reduceEpisodeStatus(store.ensurePodcasts(mdb), ep.id, 'deleted-on-disk')));
  ...
  app.post('/api/podcasts/episodes/:id/restore', async (req, res) => {
    // v1.81 write-RBAC (gate fix round): restore re-materializes a SHARED
    // episode - the inverse of the now-gated episode delete, so it takes the
    // same content-modify capability.
    if (d.requireModifyLibrary && !d.requireModifyLibrary(req, res)) return;
    const ns = readNs(d);
    const ep = Object.prototype.hasOwnProperty.call(ns.episodes, req.params.id) ? ns.episodes[req.params.id] : null;
    if (!ep) return res.status(404).json({ error: 'no such episode' });
    // v1.123 T3 (security): visibility axis - mirror of the delete route. A
    // capable-but-restricted member must not re-materialize an episode of a
    // show hidden from them. 404 before the status 400, same neutral posture.
    if (d.episodeVisibleTo && !d.episodeVisibleTo(req, ep)) return res.status(404).json({ error: 'no such episode' });
    ...
    const restoreRoot = resolvePodcastsRoot(null, d);
    ...
    if (typeof ep.trashPath !== 'string' || !fs.existsSync(ep.trashPath)) {
      await d.updateDatabase(() => d.podcastsDb.mutate((mdb) => store.reduceEpisodeStatus(store.ensurePodcasts(mdb), ep.id, 'tombstone')));
  ...
  app.get('/api/podcasts/settings', (req, res) => {
    res.json({
      pollMinutes: pollMinutesFrom(d),
      downloadDir: resolvePodcastsRoot(null, d),
    });
  });

  app.post('/api/podcasts/settings', async (req, res) => {
    // v1.81 write-RBAC (gate fix round): the global podcast poll interval is
    // registry/subscription config - same gate as the subscription-mutation
    // routes, not a per-user setting.
    if (d.requireManageSubscriptions && !d.requireManageSubscriptions(req, res)) return;
    const body = req.body || {};
    ...
    try {
      ...
      return res.status(500).json({ error: 'could not save the settings' }); // Wave 5: a failed commit is a 500, never a hang
    }
