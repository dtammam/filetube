 ...
 * definitions is dropped as opaque bytes. Gate fix (adversarial #7):
 * restricted to the region BEFORE the root element - a "<!DOCTYPE" string
 * inside the document body (e.g. inside CDATA show-notes) is CONTENT, and
 * the old whole-document search let it swallow real items.
 */
function stripDoctype(xml) {
  const rootAt = xml.search(/<(rss|feed|channel)[\s>]/i);
  const at = xml.search(/<!DOCTYPE/i);
  if (at === -1 || (rootAt !== -1 && at > rootAt)) return xml;
  let pos = at + 9;
  let depth = 0;
  ...
  return xml.slice(0, at); // unterminated prelude DOCTYPE: drop the rest
}

/**
 * Strip XML comments, CDATA-AWARE (gate fix, adversarial #7 - measured: a
 * '<!--' inside one item's CDATA show-notes plus a '-->' in a later item
 * deleted every item between them and glued the survivor onto another
 * episode's enclosure; the file then persisted under the wrong identity
 * FOREVER via the archive law). CDATA spans are copied through verbatim -
 * their content is literal by definition. An UNTERMINATED '<!--' outside
 * CDATA is kept as literal text rather than dropping the document tail:
 * mass silent deletion is the worse failure (the truncation-flags law), and
 * a stray '<!--' rendering inside one field is visible and harmless.
 */
function stripComments(xml) {
  if (xml.indexOf('<!--') === -1) return xml;
  let out = '';
  let pos = 0;
  // Delta-round CRITICAL D1: the CDATA index is HOISTED and re-searched only
  // once the cursor passes it. The first shipped shape re-ran
  // indexOf('<![CDATA[') from `pos` on EVERY comment iteration - O(comments
  // x length), measured at 9.8s for a 24MB in-cap document (92x regression,
  // unbounded at 375k comments) on a SYNCHRONOUS path that froze the whole
  // server each poll. This shape is linear: every character is scanned a
  // bounded number of times regardless of comment count.
  let cdataAt = xml.indexOf('<![CDATA[');
  while (pos < xml.length) {
    const comment = xml.indexOf('<!--', pos);
    if (comment === -1) { out += xml.slice(pos); break; }
    if (cdataAt !== -1 && cdataAt < pos) cdataAt = xml.indexOf('<![CDATA[', pos);
    if (cdataAt !== -1 && cdataAt < comment) {
      // Copy the CDATA span through untouched (unterminated: rest is literal).
      const cdataEnd = xml.indexOf(']]>', cdataAt + 9);
      const spanEnd = cdataEnd === -1 ? xml.length : cdataEnd + 3;
      out += xml.slice(pos, spanEnd);
      pos = spanEnd;
      continue;
    }
    out += xml.slice(pos, comment);
    const end = xml.indexOf('-->', comment + 4);
    if (end === -1) {
      out += xml.slice(comment); // unterminated comment-open: literal text
      break;
    }
    pos = end + 3;
  }
