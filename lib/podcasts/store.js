...
const VALID_EPISODE_STATUSES = new Set([
  ...
  'tombstone', // trash retention expired, restore found the trash file vanished (410), or legacy delete; never re-downloaded
]);
...
function reduceEpisodeStatus(ns, id, status, opts) {
  if (!VALID_EPISODE_STATUSES.has(status)) return false;
  const ep = Object.prototype.hasOwnProperty.call(ns.episodes, id) ? ns.episodes[id] : null;
  if (!ep) return false;
  // v1.70 delta round 2 (defence-in-depth, the seat's SUGGESTION #2): an
  // optional FROM-state guard, matching reduceEpisodeTrashed/Restored. The
  // retention sweep passes { from: 'trashed' } so a record that changed
  // state between the sweep's selection and its mutation - a restore, say -
  // can never be tombstoned out from under the user. (The seat could not
  // construct that interleaving today; both paths are synchronous. This
  // makes it structurally impossible rather than incidentally safe.)
  if (opts && typeof opts.from === 'string' && ep.status !== opts.from) return false;
  ep.status = status;
...
// ---- v1.72: show pins (the Playlists surface - Dean's intake ruling 5) -----
// Pure reducers mirroring lib/books/store.js's shelf-pin trio byte-for-
// byte in semantics: idempotent add, order-gap-tolerant numbering (removal
// never renumbers survivors), FIFO cap, and the reduceReorder algorithm
// (leading ids in requested order, stable-partitioned tail). pin_id = the
// show's SUBSCRIPTION id.

const MAX_SHOW_PINS = 100;

function reduceAddShowPin(pins, { id, label, pinnedAt }) {
  ...
  const record = { id, label, pinnedAt, order };
  const next = [...list, record];
  const bounded = next.length > MAX_SHOW_PINS ? next.slice(next.length - MAX_SHOW_PINS) : next;
  return { pins: bounded, record, changed: true };
}

function reduceRemoveShowPin(pins, id) {
  const list = Array.isArray(pins) ? pins : [];
  const next = list.filter((p) => !p || p.id !== id);
  return { pins: next, changed: next.length !== list.length };
}

function reduceReorderShowPins(pins, orderedIds) {
  const list = Array.isArray(pins) ? pins : [];
...
// Wave 5 of the relational-migration arc: the namespace as three tables behind
// a feature store (lib/db/featureStore.js). `subscriptions` is an ORDERED
// record list (podcasts_subscriptions: id, position, json - the array order is
// the `order` the routes sort by, kept verbatim in the json), `episodes` one
// row per episode (podcasts_episodes: episode_id, json - the download ARCHIVE,
// tombstones included), `settings` one row per key (podcasts_settings). The
// module's reducers keep running on the holder (`ensurePodcasts(holder)`)
// inside `podcastsDb.mutate(...)`; `read()` is the `readPodcasts(db)` snapshot.
...
module.exports = {
  FEATURE,
  createPodcastsStore: (adapter, opts) => FEATURE.createStore(adapter, opts),
  ensurePodcasts,
  readPodcasts,
  subscriptionIdFor,
  episodeIdFor,
