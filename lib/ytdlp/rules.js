...
/**
 * Pure, fail-open Shorts detector: the `/shorts/` URL marker, plus (v1.36.1)
 * a SHAPE fallback for entries that lose the marker.
 *
 * Layer 1 (v1.15.0): `true` when one of `videoMeta.webpage_url` /
 * `videoMeta.original_url` / `videoMeta.url` is a STRING containing the
 * substring `/shorts/` -- the shape yt-dlp uses when YouTube's renderer
 * metadata marks the entry as a Short.
 *
 * Layer 2 (v1.36.1, Dean's on-device confirmation: a skipShorts=true sub
 * downloaded Shorts after v1.36): the URL marker is NOT reliable on the UU
 * uploads-feed listing -- yt-dlp only rewrites a playlist entry's URL to
 * `/shorts/<id>` form when YouTube's playlist renderer carries the SHORTS
 * overlay, and `webpage_url` is always canonicalized to `watch?v=` form
 * after full extraction. The fallback detects the SHAPE that defines a
 * Short: duration <= 180s (YouTube's current Shorts ceiling) AND a
 * vertical-or-square frame (`aspect_ratio` <= 1, or width <= height when
 * yt-dlp's precomputed ratio is absent). A conventional landscape video can
 * never trip it; a <=3-minute vertical/square video on a sub whose owner
 * explicitly opted into Shorts exclusion is precisely what that owner asked
 * to skip.
 *
 * Missing/odd fields still resolve to `false` (NOT a short -- no duration,
 * or no dimensions AND no ratio, means no shape verdict): this fails open
 * toward "download it," matching the feature's "default off = download
 * everything" posture. The caller only acts on this when
 * `sub.skipShorts === true` anyway. Pure/synchronous, no I/O.
 * @param {object} videoMeta a parsed video metadata object (may be missing/
 *   odd fields -- handled defensively)
 * @returns {boolean} true = this video is a YouTube Short (by URL marker or
 *   by shape)
 */
...
function isShort(videoMeta) {
  ...
  if (candidates.some((value) => typeof value === 'string' && value.includes('/shorts/'))) return true;
  // v1.36.1 shape fallback -- see the doc comment above.
  const duration = videoMeta.duration;
  if (typeof duration !== 'number' || !Number.isFinite(duration) || duration <= 0 || duration > SHORTS_MAX_DURATION_SECONDS) {
    return false;
  }
  const aspect = videoMeta.aspect_ratio;
