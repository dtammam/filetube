...
// restricted to stdout because that JSON payload gets PERSISTED),
...
// persisted, never used to make a download/skip/delete decision) --
...
// and global ERROR lines.
//
// v1.47.4 (found while fixing gate CRITICAL 1, NOT reported by either seat):
// this used to also match the BARE NOUNS `subtitl|caption|transcript`, on the
// v1.36.2 reasoning that a per-video line is id-scoped and therefore safe.
// Id-scoping does not help: the REASON text embeds creator-controlled titles.
// Demonstrated against this very parser --
//
//   ERROR: [youtube] abc: Merging failed for video How To Add Subtitles To Your Videos
//     -> subtitleOnly: true
//
// -- a MERGE failure misread as a subtitle failure. That was inert while
// run.js dropped the flag (CRITICAL 1), but fixing CRITICAL 1 makes it live,
// and its consequence is the worst kind: `computeDownloadOutcome` would
// DISCOUNT that failure and report a genuinely broken download as a success.
// That is exactly the failure mode this wave refused `--ignore-errors` for, so
// it must not be reintroduced through the back door.
//
// The bare nouns are therefore gone, and both the per-video and global paths
// now share ONE strict set: unambiguous flag names, file extensions, and full
// ANCHORED PHRASES lifted verbatim from yt-dlp's source. A hostile title can no
// longer forge a match, because it would have to reproduce a whole error
// sentence.
//
// Tightening is the SAFE direction, and v1.36.2's own comment says so: an
// unmatched subtitle phrasing "degrades to the old (blocking) classification,
// never the other way around" -- i.e. a real failure stays counted as a real
// failure. Over-matching is what silently loses videos.
//
// The two trailing alternatives are yt-dlp SOURCE LITERALS (`YoutubeDL.py:4498`
// and `:4488`) -- see GLOBAL_SUBTITLE_REASON_RE below, which is now an alias of
// this same set, for why they are required and how they were verified live.
//
// THE SIDECAR-EXTENSION TOKENS ARE LANGUAGE-TAGGED, and that matters (v1.47.4
// gate delta, adversarial seat). They used to be bare `\.vtt\b|\.srt\b`, which
// the seat forged from ordinary creator-controlled text:
//
//   ERROR: [youtube] abc: Video unavailable: My Video About .srt Files
//     -> subtitleOnly: true   (a REAL failure, misread as subtitle-only)
//
// "How to make .srt files" is a normal YouTube genre, not a contrived attack.
// Requiring a language tag immediately before the extension matches the shape
// our OWN sidecars actually have (`args.js` emits `--sub-langs en.*
// --convert-subs vtt`, so files land as `Title [id].en.vtt` /
// `Title [id].en-US.vtt`), while a bare ".srt" mentioned in prose does not.
//
// Dropping the extensions ENTIRELY was the other option offered. Rejected: the
// `--convert-subs` postprocessing failure (`Postprocessing: Error opening
// output file <path>.en.vtt`) is a genuine subtitle-only failure with no source
// literal of its own, and losing it would make a caption-conversion error
// blocking again -- reviving the exact "no transcripts must be non-blocking"
// complaint v1.36.2 exists to answer.
//
// HONEST RESIDUAL (the previous version of this comment claimed forgery was
// impossible; the adversarial seat disproved that, so the claim is corrected
// rather than restated). A reason that reproduces a full source literal --
// e.g. a title containing "Unable to download video subtitles for" -- still
// forges a match. That is contrived rather than incidental, and the blast
// radius is bounded and NOT data loss: accounting is protected by
// `computeDownloadOutcome`'s `completedCount` corroboration guard (a phantom
// success needs completed >= target, and completedCount comes from `after_move:`
// FTCHMETA, which cannot include a video that never landed), the outcome stays
// partial/error, and `recordFailureLog` still writes a run-level row. The cost
// is one wasted subtitle-stripped retry spawn and the loss of PER-ITEM
// attribution on that failure.
const SUBTITLE_REASON_RE = /--write-subs|--convert-subs|\.[A-Za-z]{2,3}(?:-[A-Za-z0-9]+)?\.(?:vtt|srt)\b|Unable to download video subtitles for|Cannot write video subtitles file/i;

// v1.36.2 gate hardening: GLOBAL (id-less) lines use a STRICTER marker set
// than per-video lines -- only the subtitle FLAG names and artifact
// extensions, never the bare nouns. A global postprocessing message embeds
// arbitrary creator-controlled text (a merge failure naming a video titled
// "How To Add Subtitles..." must not read as a subtitle failure); a
// per-video line is id-scoped so the broader nouns are safe there.
//
// v1.47.4 gate CRITICAL 2 (adversarial seat): the two SOURCE LITERALS below
// were missing, and their absence meant the single most common real-world
// subtitle failure was not classified as one at all.
//
// yt-dlp raises a plain `DownloadError` from `_write_subtitles` (not an
// `ExtractorError`), so the line carries NO `[extractor] <id>:` prefix and can
// never match `ITEM_ERROR_RE`. It falls to this global branch, where the
// deliberately-narrow marker set matched none of it. Verbatim, reproduced live
// against yt-dlp 2026.07.04 (this is Dean's exact reported failure):
//
//   ERROR: Unable to download video subtitles for 'en-en-US': HTTP Error 429: Too Many Requests
//
// No `--write-subs`, no `.vtt` -- so `parseItemFailureLine` returned `null`,
// the failure was invisible to every subtitle-aware consumer, and the video was
// lost exactly as before.
//
// These two additions are full phrases lifted verbatim from yt-dlp's own source
// (`YoutubeDL.py:4498` and `:4488`), not bare nouns, so a hostile video title
// merely containing the word "subtitles" cannot forge a match -- it would have
// to reproduce an entire error sentence. See SUBTITLE_REASON_RE above for the
// HONEST RESIDUAL: that is substantially harder to forge, not impossible, and
// the earlier claim that it was impossible was wrong.
// v1.47.4: now the SAME set as SUBTITLE_REASON_RE above. The two used to
// differ -- the per-video path also accepted bare nouns -- and unifying them on
// the strict set is what removes the title-spoofing hole (see that constant's
// comment). Kept as a separate NAME because the two call sites below are
// genuinely different branches and the distinction is worth reading at a
// glance, even though the sets are now identical.
const GLOBAL_SUBTITLE_REASON_RE = SUBTITLE_REASON_RE;
...
function parseItemFailureLine(line, knownIds) {
  ...
  if (!match) {
    ...
    if (globalMatch) {
      ...
      if (globalReason !== '' && GLOBAL_SUBTITLE_REASON_RE.test(globalReason)) {
 ...
 */
function computeDownloadOutcome({ ok, itemFailures, targetIds, completedCount }) {
  ...
  // transcript fetch/convert failed. They are partitioned out here, and a
  // non-zero exit caused PURELY by subtitle errors is an honest SUCCESS
  // (pre-fix it read as a channel-level failure: outcome 'error', nothing
  // credited, a red history row for a completed download).
  //
  // v1.36.2 gate hardening (adversarial WARNING): the subtitle-only grant
  // is CORROBORATED, never taken on faith. `completedCount` is positive
  // per-download evidence the call site already has (the FTCHMETA
  // channel-meta print each completed download emits exactly once) -- an
  // UNPARSED real fatal alongside a subtitle line must not read as full
  // success and advance the cutoff past videos that never downloaded (the
  // permanent-loss class). Credit exactly what the evidence supports:
  // all-completed -> success; some -> partial; none -> the old
  // channel-level error (nothing credited, safe retry).
  ...
  if (failures.length === 0 && subtitleOnlyCount > 0) {
    const completed = Number.isInteger(completedCount) && completedCount > 0
      ? Math.min(completedCount, target)
      : 0;
    if (completed >= target && target > 0) return { outcome: 'success', succeeded: target, failed: 0 };
    if (completed > 0) return { outcome: 'partial', succeeded: completed, failed: target - completed };
    return { outcome: 'error', succeeded: 0, failed: target };
  }
