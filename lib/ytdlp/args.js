...
function buildMatchFilterArg({ skipShorts, maxDurationSeconds, minDurationSeconds } = {}) {
  ...
  // v1.285: the FLOOR clause is emitted before the ceiling so a window reads
  // `duration >= min & duration < max`. `0`/unset/malformed contributes nothing
  // (mirroring the ceiling), so a min-only or max-only bound is just its clause.
  if (typeof minDurationSeconds === 'number' && Number.isInteger(minDurationSeconds) && minDurationSeconds > 0) {
    clauses.push(buildMinDurationClause(minDurationSeconds));
  }
