...
function buildYtdlpDownloadArgs(sub, config, targetIds, opts = {}) {
  ...
  if (format === 'audio') {
    ...
    // v1.111 (Dean, streaming Tier 1): make every new VIDEO download born with a
    // FRONT-LOADED moov atom (+faststart), so the browser can start playback and
    // seek after buffering only the start instead of fetching deep into the file
    // for a trailing moov ("slow to start on click"). This is a container remux
    // only -- LOSSLESS, no re-encode, no re-download. `ffmpeg:` targets every
    // ffmpeg-based postprocessor yt-dlp runs on the final file (the Merger for
    // multi-stream formats AND the metadata/thumbnail/chapters embed that
    // re-muxes EVERY video download below), so it covers merged and single-file
    // downloads alike. Fixed literal (no interpolation); one array element, so
    // yt-dlp shlex-splits the value itself under the shell-less argv spawn.
    // Video-only -- audio (mp3) has no moov to move. VERIFICATION is Dean's
    // device pass: `node scripts/probe-faststart.js --list` must NOT list a
    // freshly-downloaded file (the flag CANNOT be proven non-inert without a real
    // yt-dlp/ffmpeg download, which the test env lacks).
