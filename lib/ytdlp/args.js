 ...
 * means unlimited. Also bounded to items shorter than `--match-filter
 * "duration < <n>"` (v1.22.0 FR-6), where `<n>` is `sub.maxDurationSeconds`
 * when set, else the global `config.maxDurationSeconds` -- `0` (from either
 * source) means unbounded -- AND-joined with the skip-Shorts filter into a
 * single `--match-filter` arg by `buildMatchFilterArg` (see its doc comment
 * above for why multiple `--match-filter` flags cannot be used instead).
 */
function buildYtdlpListArgs(sub, config) {
  ...
  // precedence over the global `config.maxDurationSeconds`, mirroring
  // `effectiveMaxVideos` above EXACTLY -- `sub.maxDurationSeconds` is only
  // consulted when it is not null/undefined, so an explicit per-sub `0`
  // (unbounded) is honored, and an unset/missing per-sub value falls back to
  // the global unchanged.
