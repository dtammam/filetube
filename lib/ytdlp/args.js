...
// or a subtly-corrupted persisted record), both are RE-ASSERTED here,
...
// The title is CAPPED to 100 chars (Dean's on-device ENAMETOOLONG): a non-
// YouTube "title" is often the full video DESCRIPTION -- hundreds of chars
// (a Facebook post) -- which overruns the 255-byte filename limit. yt-dlp's
// `%(title).100s` slices to the first 100 CHARACTERS (never mid-codepoint) and
// the `[id]` bracket + ext still follow intact (extractMediaRef parses the
// trailing bracket). The DISPLAYED title is unaffected -- it comes from the
// captured sourceTitle (the full title), not the on-disk filename. YouTube's
// own OUTPUT_TEMPLATE is left uncapped (YouTube caps titles ~100 chars; keeping
// it byte-identical), so this cap is universal-lane only.
const UNIVERSAL_OUTPUT_TEMPLATE = '%(title).100s [%(extractor_key)s=%(id)s].%(ext)s';
...
// v1.285: the per-channel duration FLOOR clause. Same fixed-shape/validated-
// integer safety as buildDurationClause above. DELIBERATE (mirroring the
// ceiling's strict `<`): the operator is the strict `>=`, NOT the `>=?`
// "match-if-field-absent" form -- an item whose duration is UNKNOWN at list time
// (live/ongoing streams, extractors that omit duration) is therefore EXCLUDED,
// exactly like the ceiling excludes them (Dean's locked intake: unknown-length
// items are excluded when a bound is set). Do not add a `?` suffix without an
// explicit product decision to reverse this.
function buildMinDurationClause(minDurationSeconds) {
 ...
 * re-assert-at-build-time posture); `maxDurationSeconds`/`minDurationSeconds`
 * each contribute a clause only when a positive integer (`0`/unset/malformed
 * means "no bound on that side" -- no clause, mirroring `playlistEndArgs`'s own
 * "0/malformed = omit" semantics). v1.285 (data-safety): when BOTH bounds are
 * active and the floor is >= the ceiling, the FLOOR is DROPPED so the emitted
 * window can never be empty (which would silently match nothing) -- this is the
 * one place the TRUE effective window is known (the ceiling may be the global
 * default while the floor is per-sub), so it fails safe toward downloading MORE.
 * @param {{skipShorts?: boolean, maxDurationSeconds?: number, minDurationSeconds?: number}} opts
 * @returns {string[]} either `[]` or `['--match-filter', <clause-string>]`
 */
function buildMatchFilterArg({ skipShorts, maxDurationSeconds, minDurationSeconds } = {}) {
  const clauses = [];
  if (skipShorts === true) clauses.push(SHORTS_MATCH_FILTER);
  const minActive = typeof minDurationSeconds === 'number' && Number.isInteger(minDurationSeconds) && minDurationSeconds > 0;
  const maxActive = typeof maxDurationSeconds === 'number' && Number.isInteger(maxDurationSeconds) && maxDurationSeconds > 0;
  // v1.285 (data-safety, QA WARNING): the FLOOR is emitted before the ceiling so
  // a window reads `duration >= min & duration < max` -- BUT it is dropped when it
  // would EMPTY the effective window (floor >= active ceiling). The store's
  // cross-guard only sees the two PER-SUB values; a per-sub floor paired with the
  // GLOBAL ceiling (no per-sub max) is invisible to it and can only be caught
  // here, where the effective ceiling is resolved.
  const floorActive = minActive && !(maxActive && minDurationSeconds >= maxDurationSeconds);
  if (floorActive) clauses.push(buildMinDurationClause(minDurationSeconds));
  if (maxActive) clauses.push(buildDurationClause(maxDurationSeconds));
...
// v1.48 item 2 (Dean, "view counts taken from the content the day of"): the
// selector grew `view_count`. It stays inside the SAME fixed-literal, JSON-
// escaped `.{...}j` construct as every other key -- no new `%(field)s`-style
// interpolation is introduced, which is the property the args tests lock. The
// value is bounded downstream by `store.parseCapturedViewCount` (non-negative
// integer, plausibility ceiling) and is never trusted raw.
// v1.54 (Dean's real subscriber counts): `channel_follower_count` joins both
// selectors -- same fixed-literal, JSON-escaped `.{...}j` posture as
// view_count (v1.48), bounded downstream by store.parseCapturedFollowerCount,
// never trusted raw. Verified against yt-dlp 2026.07.04 primary source:
// yt_dlp/extractor/youtube/_video.py:4468 sets it on the PER-VIDEO info dict
// (approximate -- "24K" -> 24000); absent on many extractors -> mock fallback.
const CHANNEL_META_PRINT_TEMPLATE = `after_move:${CHANNEL_META_SENTINEL} %(.{id,title,channel_url,channel_id,uploader_url,channel,upload_date,release_date,view_count,channel_follower_count})j`;
// v1.41.13 (universal one-offs, design D2 touch #1): the UNIVERSAL-lane print
// template adds `extractor_key` (which source this is -> the composite bridge
// key, and the D1a proxy-host youtubeId adoption), `uploader` (the D7
// pseudo-channel label fallback when `channel` is absent), and `filepath` (the
// rendered on-disk path -> the sanitized composite lookup key at consume time,
// so we never re-implement yt-dlp's filename sanitizer). Used ONLY on the
// universal one-off spawn; the subscription/YouTube template above stays
// byte-for-byte unchanged (a byte-parity lock guards this).
// v1.338 (Dean: "anything that can and is grabbed should be kind of treated and formed the same way";
// plan 2026-09-26-first-class-any-site D2): `webpage_url` joins the UNIVERSAL selector only - the page
// the download came from, saved on the item as `sourceUrl` so Share works everywhere (a YouTube link is
// rebuilt from its id; another site's cannot be). Same fixed-literal `.{...}j` posture; bounded
// downstream by source-share.js sanitizeSourceShareUrl, never trusted raw.
const UNIVERSAL_CHANNEL_META_PRINT_TEMPLATE = `after_move:${CHANNEL_META_SENTINEL} %(.{id,title,channel_url,channel_id,uploader_url,channel,uploader,extractor_key,upload_date,release_date,view_count,channel_follower_count,webpage_url,filepath})j`;
...
// v1.145: same boundary re-validation posture as resolvePlayerClient - the
// argv builder never trusts the upstream parse alone.
function resolveJsRuntimes(config) {
  return ytdlpConfig.parseJsRuntimes(config && config.jsRuntimes);
}
...
function resiliencePacingArgs(config, opts = {}) {
  ...
  //
  // v1.47.4 item 1: `skip=translated_subs` now shares this SAME argv pair, and
  // that sharing is load-bearing, not stylistic. yt-dlp wires --extractor-args
  // through `_dict_from_options_callback` with `append=False`
  // (options.py:1927-1934), so a SECOND `--extractor-args youtube:...` element
  // REPLACES the first outright instead of merging with it. Verified against
  // the real parser, not inferred:
  //
  //   youtube:player_client=android + youtube:skip=translated_subs (two flags)
  //     -> {'youtube': {'skip': ['translated_subs']}}        <-- player_client GONE
  //   youtube:skip=translated_subs;player_client=android     (one flag)
  //     -> {'youtube': {'skip': [...], 'player_client': [...]}}
  //
  // So emitting these as two separate flags would have silently disabled the
  // v1.31 bot-check player_client mitigation for every operator who configured
  // it -- an invisible regression, since nothing would error, downloads would
  // just start failing bot checks again. They are joined with `;` instead.
  //
  // WHY skip=translated_subs at all (see the v1.47.4 exec plan for the full
  // source-verified chain): `--write-auto-subs` (pushed by
  // buildYtdlpDownloadArgs) is exactly what switches ON YouTube's translated-
  // subtitle construction (_video.py:4224), which synthesizes keys shaped
  // `<target>-<source>` (_video.py:4287) such as `en-en-US` -- a machine
  // translation of an `en-US` track back into English, redundant when we
  // already fetch `en-US` itself. Our `--sub-langs en.*` is REGEX-matched
  // (YoutubeDL.py:3182), so it drags that redundant track in, YouTube 429s the
  // translated-track request, and -- because subtitles are written BEFORE the
  // media download (YoutubeDL.py:3392 vs 3536) and a subtitle error raises
  // DownloadError under our effective ignoreerrors='only_download'
  // (YoutubeDL.py:4498-4501) -- the ENTIRE video download dies with it. That is
  // why specific videos failed on every single retry rather than intermittently.
  //
  // Confirmed on Dean's repro (8Z2tmZy_Vj0): `en.*` matched
  // en/en-US/en-en-US/en-orig before, and en/en-US/en-orig after -- the
  // offending track gone, every genuine track kept.
  //
  // Emitted unconditionally (the `youtube:` prefix scopes it to that extractor,
  // so it is inert for every other site the universal lane can reach). Fixed
  // literal; the only interpolated value remains the re-validated playerClient.
  const youtubeExtractorArgs = ['skip=translated_subs'];
  const playerClient = resolvePlayerClient(config);
  if (playerClient) {
    youtubeExtractorArgs.push(`player_client=${playerClient}`);
  }
  args.push('--extractor-args', `youtube:${youtubeExtractorArgs.join(';')}`);
  // v1.145 (gate W2, the nightly-pin wave): opt yt-dlp into a JavaScript
  // runtime. Upstream deprecated running without one - every extraction
  // prints "No supported JavaScript runtime could be found... some formats
  // may be missing" (the format-not-available failure shape from Dean's
  // device) - yet only deno is auto-enabled, and the Docker image ships
  // NODE, a supported runtime yt-dlp will not use unless told. Mirrors the
  // playerClient posture exactly: unset-by-default (the image's ENV
  // defaults it to `node`; bare-metal stays unset so an OLDER binary that
  // rejects unknown flags never sees it), re-validated at THIS boundary via
  // the same charset/length allowlist that gated the upstream parse, and
  // emitted as two fixed-shape argv elements only when set.
  const jsRuntimes = resolveJsRuntimes(config);
  if (jsRuntimes) {
    // Gate W3 (round 2): yt-dlp does NOT comma-split this flag's value - it
    // is registered with no delimiter (options.py:466, verified against the
    // pinned nightly), so `--js-runtimes deno,node` is "Ignoring unsupported
    // JavaScript runtime(s)" and silently degrades to NO runtime - the exact
    // condition this flag exists to fix. Multi-runtime is expressed by
    // REPEATING the flag: split the validated comma list here, one
    // fixed-shape pair per runtime name.
    for (const runtime of jsRuntimes.split(',')) {
...
function buildYtdlpListArgs(sub, config) {
  ...
  // v1.285: the per-channel duration FLOOR. Unlike the ceiling it has NO global
  // fallback (per-channel only, Dean's intake) -- so it is the per-sub value or
  // nothing; `0`/unset contributes no clause via buildMatchFilterArg.
  const effectiveMinDurationSeconds = sub ? sub.minDurationSeconds : undefined;
  const args = [
    '--dump-json',
    '--no-download',
    '--no-warnings',
    ...
    // target).
    ...buildMatchFilterArg({ skipShorts: sub && sub.skipShorts === true, maxDurationSeconds: effectiveMaxDurationSeconds, minDurationSeconds: effectiveMinDurationSeconds }),
    ...cookiesArgs(config),
...
function buildYtdlpDownloadArgs(sub, config, targetIds, opts = {}) {
  ...
  // v1.47.4 item 1 (L2): strict `=== true`, so a bare/forged/partial opts object
  // can only ever fail CLOSED (subtitles kept, today's behavior) -- never
  // silently strip captions from every download. Same posture as `oneOff`.
  const skipSubtitles = Boolean(opts && opts.skipSubtitles === true);
  ...
  // v1.41.13 folder fix (Dean's on-device report -- universal downloads all
  // landed in one "Uncategorized" folder): a universal one-off with NO explicit
  // folder folds by %(uploader)s (fallback uploader_id, then extractor_key) so
  // items GROUP by their creator as a pseudo-channel (D7), exactly the design's
  // intent. yt-dlp renders the subfolder from the download's own metadata and
  // sanitizes each path component (--windows-filenames -> no `/` traversal), and
  // the post-download confinement (run.js) checks against the download ROOT, so
  // downloadRoot/<uploader>/file is safe. An EXPLICIT folder override still wins
  // (autoUploaderFolder false -> the confined channelDir). YouTube unchanged.
  const autoUploaderFolder = universal && opts.autoUploaderFolder === true;
  let outputTemplate;
  if (autoUploaderFolder) {
    const root = path.resolve(config && config.downloadDir);
    outputTemplate = path.join(root, '%(uploader,uploader_id,extractor_key)s', UNIVERSAL_OUTPUT_TEMPLATE);
  } else {
    outputTemplate = path.join(channelDir, universal ? UNIVERSAL_OUTPUT_TEMPLATE : OUTPUT_TEMPLATE);
  }
  ...
  // = mtime) on every deployment; it is a no-op on any pinned Docker binary
  // since the 2025.07 default flip (currently the 2026.8 nightly - gate W1,
  // v1.145: verified still the default there), whose default this already is.
  args.push('--no-mtime');
  ...
  //
  // v1.47.4 item 1 (L2, containment): `opts.skipSubtitles` omits this whole
  // group. yt-dlp writes subtitles BEFORE it downloads the media
  // (YoutubeDL.py:3392 vs 3536) and raises DownloadError on a subtitle failure
  // under our effective ignoreerrors='only_download' (YoutubeDL.py:4498-4501),
  // so ANY subtitle-side error -- a 429, a withdrawn track, a transient blip --
  // aborts the item with no media file at all. L1 (skip=translated_subs, see
  // resiliencePacingArgs) removes the KNOWN trigger; this flag is the general
  // containment: on a subtitle-attributed failure the caller retries the item
  // once with `skipSubtitles: true`, so the video always lands even when the
  // captions cannot. The detection half already exists: v1.36.2's
  // `subtitleOnly` tagging in lib/ytdlp/failures.js. NOTE what that tagging did
  // and did NOT do -- it made a subtitle failure non-blocking for ACCOUNTING
  // (computeDownloadOutcome discounts it, so the run is not scored failed), but
  // yt-dlp had still aborted before fetching the media, so the video was simply
  // missing. That accounting fix is why this bug looked benign in the logs while
  // costing Dean real downloads. The retry in lib/ytdlp/run.js is what actually
  // lands the file.
  //
  // Deliberately NOT solved with yt-dlp's own `-i/--ignore-errors`: per
  // YoutubeDL.py:4499 that WOULD downgrade this error to a warning, but it also
  // downgrades genuine download AND postprocessing failures ("the download will
  // be considered successful even if the postprocessing fails"), which would
  // make FileTube report broken downloads as successes. That trades a visible
  // bug for an invisible one, which is strictly worse.
  if (!skipSubtitles) {
  ...
  args.push('--print', universal ? UNIVERSAL_CHANNEL_META_PRINT_TEMPLATE : CHANNEL_META_PRINT_TEMPLATE);
...
module.exports = {
  ...
  UNIVERSAL_CHANNEL_META_PRINT_TEMPLATE,
