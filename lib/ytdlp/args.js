...
//
// Deliberately NO count cap and NO break-early logic here -- `--dateafter`
// is the ONLY scoping mechanism on the list pass now. This is safe because
// `--download-archive` (already applied, unconditionally, to every list AND
// download pass -- see `resolveArchivePath`/its call sites above) is the
// sole dedup mechanism: any video that has already surfaced once is
// permanently recorded in the archive and is skipped on every subsequent
// poll no matter how wide the `--dateafter` window is, so there is NO
// re-download storm from removing the count cap.
//
...
//
// v1.24.0 C5-ytdlp/C6 (T11, Wave 3): the field-selector grew THREE more keys
// -- `upload_date`, `release_date` (both a `YYYYMMDD` string per yt-dlp's own
// documented convention, feeding C5's release-date capture) and
// `channel_thumbnail` (feeding C6's channel-avatar capture). Exactly the SAME
...
// `release_date`, which is normally only set for premieres/livestreams, and
// possibly for `channel_thumbnail` depending on extractor/version) renders as
// JSON `null`, the SAME "absent, never literal data" convention already
// documented above -- there is no new failure mode, only a wider,
// best-effort field set.
//
// KNOWN LIMITATION (flagged for verification, not a security concern):
// `channel_thumbnail` is a best-effort field-name choice -- unlike
// `channel_url`/`channel_id`/`uploader_url`/`channel`, yt-dlp's documented
// common `InfoExtractor` fields do not guarantee a single, stable "channel
// avatar" key across every extractor/version. If a live yt-dlp does not
// populate this exact key for a downloaded video's info dict, the `%(...)j`
// selector simply renders `null` for it (the SAME safe-degrade behavior as
// any other absent field) and `channelAvatarUrl` is never captured -- this
// degrades to "no real avatar, generated fallback stays in effect"
// (`common.js`'s `resolveAvatarSource`), never a crash or a security issue.
// Confirm the exact field name against a live `yt-dlp --dump-json` output
// before relying on this in production; swapping in the correct key name
// later is a one-line change to this same literal.
const CHANNEL_META_SENTINEL = 'FTCHMETA';
const CHANNEL_META_PRINT_TEMPLATE = `after_move:${CHANNEL_META_SENTINEL} %(.{id,channel_url,channel_id,uploader_url,channel,upload_date,release_date,channel_thumbnail})j`;
 ...
 * see `dateAfterArgs` above; REPLACES the old `--playlist-end` count cap,
 * now dormant -- there is deliberately NO count cap on the list pass
 * anymore, since `--download-archive` dedup already prevents re-downloads
 * regardless of window width). Also bounded to items shorter than
