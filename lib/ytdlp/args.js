...
// ---- v1.25 QoL T2 (now DORMANT, v1.36 F1 fix round): --dateafter ----------
//
...
// to keeping this flag on the argv at all:
...
//
// The list pass therefore no longer emits `--dateafter` when the break
// filter is active (see `breakEarlyArgs` below -- the breaking filter both
// rejects the pre-cutoff entry AND stops the listing, subsuming this flag's
// job); the authoritative per-video date gate now lives in JS
// (`rules.isBeforeCutoff`, applied in lib/ytdlp/index.js's survivor loop,
// where it also covers the slack window the break filter deliberately lets
// through). This function is kept exported/functional standalone EXACTLY
// like `playlistEndArgs` above (same dormant-not-deleted posture, same
// validation contract: 8-digit re-assert, own argv elements, fail-safe []).
...
function buildYtdlpListArgs(sub, config) {
  ...
  const args = [
    ...
    // v1.36 F1 (fix round, adversarial gate CRITICAL): `--dateafter` is NO
    // LONGER emitted here at all -- yt-dlp's daterange check runs before
    // match filters and its rejection is non-breaking, so a co-present
    // `--dateafter` masks the breaking filter below entirely and the
    // break-early stop never fires (see breakEarlyArgs's and dateAfterArgs's
    // doc comments for the verified-against-source mechanics). Both builders
    // share the same validated-cutoff condition, so a conditional fallback
    // to `--dateafter` would be a dead branch: no valid cutoff disables
    // both, leaving the pre-v1.25 no-date-bound shape (+ the cap below).
    // The authoritative per-video date gate is `rules.isBeforeCutoff` in
    // the JS survivor loop (lib/ytdlp/index.js).
    // v1.36 F1: break-early listing -- stop enumerating at the first
    // genuinely-pre-cutoff video (slack-tolerant) instead of full-extracting
    // the entire unarchived back catalog every poll (the "chronic burner"
    // root cause; see breakEarlyArgs's doc comment above).
