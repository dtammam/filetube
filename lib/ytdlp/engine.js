...
let runtime = { dataDir: null, onAutoRevert: null };
...
function _resetForTests() {
  ...
  runtime = { dataDir: null, onAutoRevert: null };
 ...
 * `python3`. Cached for the process lifetime (interpreters do not appear
 * mid-run; a fresh check is one `_resetForTests()` away in tests).
 * Bare metal without python3 = `{ supported: false, reason }` - the Setup
 * section renders the honest message and the feature degrades; boot is
 * never blocked (nothing here runs at import).
 *
 * @param {object} [deps] { } (spawn comes from the module seam)
 ...
 * auto-revert callback for the bell). Idempotent; called from both deps
 * bundles like every other dataDir threading in this module family.
 *
 * @param {object} opts { dataDir, onAutoRevert? }
 */
...
//      ModuleNotFoundError/ImportError means the venv's site-packages no
//      longer match its interpreter - the engine cannot start.
// Everything else - 403s, "video unavailable", timeouts, stalls, even a
// mid-extraction traceback on one weird video - is a DOWNLOAD failure and
// must never revert the channel choice.
const ENGINE_CRASH_PATTERN = /Traceback \(most recent call last\)[\s\S]*(?:ModuleNotFoundError|ImportError)/;
 ...
 * renders as the pre-wave plain version row.
 *
...
function runtimeStateSummary() {
  ...
  const state = readState(runtime.dataDir);
  return { channel: state.channel, active: state.active };
}
