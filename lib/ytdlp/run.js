...
function spawnYtdlpDownload(args, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const stdoutLineSplitter = makeLineSplitter(handleProgressLine);
    const stderrLineSplitter = makeLineSplitter(handleProgressLine);
    ...
    if (child.stdout) {
      child.stdout.on('data', (chunk) => {
        // Parsed-and-DISCARDED: only fed to the line-splitter for progress
        // signal extraction, never appended to any accumulator (SF3).
        stdoutLineSplitter.push(stdoutDecoder.write(chunk));
      });
      child.stdout.on('error', (err) => {
        if (settled) return;
        settled = true;
 ...
 * @param {{onProgress?: (patch: object) => void}} [opts]
 */
