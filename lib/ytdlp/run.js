...
// v1.146 T3 (downloader-engine): the binary is no longer a hardcoded
// literal - both spawn sites resolve it through `engine.activeBinaryPath()`
// at spawn time (bundled 'yt-dlp' unless the admin activated a channel
// engine that exists on disk), and every settled result is screened by the
// engine's runtime failure net (spawn-level/import-time crashes of the venv
// engine auto-revert to bundled; download failures never do).
...
// would be for a date.
const { parseCapturedReleaseDate, parseCapturedViewCount, parseCapturedFollowerCount, sanitizeCapturedTitle, sanitizeChannelAvatarUrl, selectChannelAvatarUrl, sanitizeCapturedChannelMeta, CHANNEL_ID_PATTERN } = require('./store');
...
const { validateChannelUrl, classifySingleVideo, isPlausibleMediaUrl } = require('./url');
// v1.338 (plan first-class-any-site D9): the DNS resolve-then-check (the shortlink resolver's per-hop
// guard) for a universal re-pull's stored page link.
const { guardHop } = require('./shortlink');
 ...
 * @returns {{videoId: (string|null), channelUrl: (string|null), channelId: (string|null), uploaderUrl: (string|null), channelName: (string|null), uploadDate: (string|null), releaseDate: (string|null), sourceViewCount: (number|string|null)} | null}
 */
function parseChannelMetaLine(line) {
  ...
  return {
    ...
    // v1.41.13 (universal one-offs, design D2 touch #2): only present on a
    // UNIVERSAL-lane capture (the extended print template). `source` (the
    // extractor_key) drives the composite bridge key + the D1a proxy-host
    // check; `uploader` is the D7 pseudo-channel label fallback; `filePath`
    // (the rendered on-disk path) yields the sanitized composite lookup key
    // at consume time. All null for a YouTube/subscription capture (legacy
    // template) -> sanitizeCapturedChannelMeta's YouTube branch, unchanged.
    source: present(parsed.extractor_key),
    uploader: present(parsed.uploader),
    filePath: present(parsed.filepath),
    // v1.338 (plan first-class-any-site D2): the page the universal download came from, raw here -
    // bounded downstream by sanitizeCapturedChannelMeta's universal arm (sanitizeSourceShareUrl).
    // Null on a YouTube/subscription capture (the legacy template has no webpage_url).
    webpageUrl: present(parsed.webpage_url),
    ...
    // v1.48 item 2: the day-of view count. Named `sourceViewCount` even here,
    // at the raw-parser boundary (delta gate fix, adversarial SUGGESTION S-D3):
    // this was the last place in the bridge carrying the collision-prone bare
    // `viewCount`, and the whole premise of the rename is that the name is
    // load-bearing -- anything that ever spread a parseChannelMetaLine result
    // onto a library item would otherwise write the LEGACY local-watch-counter
    // key. Deliberately NOT run through
    // `present` above -- yt-dlp emits `view_count` as a JSON NUMBER, and
    // `present` only passes non-empty STRINGS, so routing it there would null
    // every real count and silently disable the whole feature (the exact
    // dead-on-arrival class the v1.47.4 gate caught). Numbers pass through
    // untouched and a numeric string is tolerated too, because the `%(...)j`
    // selector has rendered fields as strings before; both shapes are bounded
    // downstream by `store.parseCapturedViewCount`, never here.
    sourceViewCount: presentNumeric(parsed.view_count),
    // v1.54: the channel's subscriber count, same presentNumeric carrier as
    // sourceViewCount above (the `present` string-gate would null every real
    // number -- the exact dead-on-arrival class v1.47.4 caught). Bounded
    // downstream by store.parseCapturedFollowerCount, never here.
    sourceFollowerCount: presentNumeric(parsed.channel_follower_count),
  };
}

// Presence-check for a field yt-dlp reports as a NUMBER (see `sourceViewCount`
// above). Mirrors `present`'s contract -- return the raw value or null, do no
// validation -- but for numbers, and it treats yt-dlp's 'NA' placeholder and
// the empty string as absent exactly like `present` does. A genuine 0 is
// PRESERVED (`0` is a real view count, not an absence), which a truthiness
// check would have dropped.
function presentNumeric(value) {
  if (typeof value === 'number') return Number.isFinite(value) ? value : null;
  if (typeof value === 'string' && value !== '' && value !== 'NA') return value;
  return null;
}
...
function spawnYtdlp(args, opts = {}) {
  ...
  return new Promise((resolveRaw) => {
    // T3: the binary is resolved ONCE per spawn (a swap mid-flight can
    // never retarget a live child), and every settle path funnels through
    // the engine's failure net. The wrapper shadows `resolve` so every
    // existing settle site below participates without modification.
    const engineBinary = engine.activeBinaryPath();
    const resolve = (result) => {
      try {
        if (engine.looksLikeEngineCrash(result)) {
          engine.reportEngineFailure({ binaryUsed: engineBinary, reason: result.error || 'yt-dlp engine crashed' });
        }
      } catch (err) {
        console.error('Engine failure report threw (ignored):', err && err.message);
      }
      resolveRaw(result);
    };
    let child;
    try {
      child = cp.spawn(engineBinary, args, {
        cwd: opts.cwd,
    ...
    child.on('close', (code, signal) => {
      ...
      resolve({
        ...
        // would persist / expose via GET /api/subscriptions.
...
function spawnYtdlpDownload(args, opts = {}) {
  ...
  return new Promise((resolveRaw) => {
    // T3: same seam + failure-net wrapper as `spawnYtdlp` above - the
    // binary resolves once per spawn, and every settle site below funnels
    // through the engine crash screen via the shadowed `resolve`.
    const engineBinary = engine.activeBinaryPath();
    const resolve = (result) => {
      try {
        if (engine.looksLikeEngineCrash(result)) {
    ...
    function handleStderrLine(line) {
      ...
      if (failure && itemFailures.length < MAX_CAPTURED_META) {
        // v1.47.4 gate CRITICAL 1 (adversarial seat): this whitelist used to be
        // `{videoId, reason}` ONLY, silently dropping `parseItemFailureLine`'s
        // third field. `subtitleOnly` is the classification EVERY downstream
        // subtitle decision keys off, and this is the sole producer of
        // `itemFailures` -- so dropping it here made all of them unreachable:
        //
        //   - v1.47.4's own L2 retry (`shouldRetryWithoutSubtitles`) could
        //     never fire for any input. The containment layer was dead code.
        //   - v1.36.2's "no transcripts must be non-blocking" fix has been
        //     INERT SINCE IT SHIPPED: `mapItemFailuresForActivity`'s
        //     `subtitleOnly !== true` filter (index.js) and
        //     `computeDownloadOutcome`'s discount (failures.js) have never
        //     filtered anything, because the flag never arrived.
        //
        // Preserved conditionally so a non-subtitle failure's object shape is
        // byte-identical to before (no stray `subtitleOnly: false` key on
        // entries that are persisted and rendered).
        itemFailures.push(failure.subtitleOnly === true
          ? { videoId: failure.videoId, reason: failure.reason, subtitleOnly: true }
          : { videoId: failure.videoId, reason: failure.reason });
      }
      safeOnProgress(parseProgressLine(line));
    }
 ...
 */
// v1.115 (Dean, A1): the PURE parse of a channel `--dump-single-json` payload
// into `{ avatarUrl, channelId, channelUrl, channelName }`. Extracted from
// probeChannelAvatar so it is unit-testable without a live spawn (spawnYtdlp is
// module-internal). Behaviour-identical for the pre-existing avatar/id/url fields
// -- and it now ALSO surfaces the canonical channel NAME (`channel` preferred,
// then `uploader`; trimmed, NA-guarded) that rides the SAME payload. Keeps the
// original "total miss" contract: `null` unless an avatar OR a channelId survived
// (so an `if (probed)` avatar caller is unaffected). Never throws.
function channelProbeResultFromParsed(parsed) {
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return null;
  const avatarUrl = sanitizeChannelAvatarUrl(selectChannelAvatarUrl(parsed.thumbnails));
  ...
  // v1.115 gate fix (both seats): STRIP control chars/NUL from the name, matching
  // every other channelName writer (sanitizeCapturedChannelMeta, the manual-
  // attribute path, the pin-label reducer) -- the repo's "never persist a raw
  // control char" invariant. Stripping at the source covers both write paths
  // (item name + pin label); the writers strip again defense-in-depth.
  const pick = (v) => {
  ...
  const channelName = pick(parsed.channel) || pick(parsed.uploader) || null;
  return { avatarUrl, channelId, channelUrl, channelName };
}
...
function probeChannelAvatar(channelUrl, config) {
  ...
  return spawnYtdlp(probeArgs, {
    ...
    .then((result) => {
      ...
      return channelProbeResultFromParsed(parsed);
    })
    .catch(() => null);
}

/**
 * v1.56 (Dean's bulk subscriber-count reheat): the CHANNEL-LEVEL follower
 * count probe -- `probeChannelAvatar`'s exact spawn shape (`--dump-single-json
 * --playlist-items 0` against the channel endpoint, `PROBE_TIMEOUT_MS`,
 * cookiesArgs, `--` separator, no shell), but its unit of value is
 * `channel_follower_count` instead of `thumbnails[]`. Deliberately a SIBLING
 * rather than a new field on `probeChannelAvatar`'s return: that function's
 * "total miss" contract (`null` unless an avatar or channelId survived) is
 * load-bearing for the avatar registry and must not grow a third success
 * condition, and its callers all sit behind `ensureChannelAvatar`'s registry
 * freshness gate, which has no business throttling a count refresh.
 *
 * yt-dlp sets `channel_follower_count` on the PLAYLIST-LEVEL info dict in
 * `_extract_metadata_from_tabs` (verified against current yt-dlp master,
 * 2026-07-30 -- derived from the channel header's subscriber text,
 * independent of entry extraction), so `--playlist-items 0` carries it
 * without listing a single video. Non-YouTube extractors that report no
 * such field resolve `null` ("nothing usable found"), never a throw.
 *
 * The count is bounded HERE by `parseCapturedFollowerCount` -- the SAME
 * standalone validator the download-capture and repull paths use, never a
 * fork -- because this probe's one job is producing a count; a dict without
 * a usable one is a total miss. `channelId`/`channelUrl` ride along
 * (validated the same way `probeChannelAvatar` validates them) so the
 * caller's fan-out can match items by the canonical id even when the target
 * only knew a handle URL.
 * @param {string} channelUrl an already-validated channel URL
 * @param {object} config parsed yt-dlp config (`{ cookiesFile, ... }`)
 * @returns {Promise<{followerCount: number, channelId: string|null, channelUrl: string|null} | null>}
 */
function probeChannelFollowerCount(channelUrl, config) {
  if (typeof channelUrl !== 'string' || channelUrl === '') return Promise.resolve(null);
  const probeArgs = [
    '--dump-single-json',
    '--playlist-items',
  ...
  return spawnYtdlp(probeArgs, {
    ...
    .then((result) => {
      ...
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return null;
      const followerCount = parseCapturedFollowerCount(parsed.channel_follower_count);
      ...
      return { followerCount, channelId, channelUrl: probedChannelUrl };
    })
    .catch(() => null);
}
 ...
 * as `probeChannel`/`runList` above -- never built into a shell string here.
 * In the default (YouTube) mode `watchUrl` is an ALREADY-VALIDATED single-video
 * watch URL (e.g. `classifySingleVideo(...).watchUrl`); in UNIVERSAL mode
 * (v1.338 D9) it is a stored page link, guarded below before any spawn.
 * @param {string} watchUrl a validated YouTube watch URL, or (universal) a stored page link
 ...
 * @returns {Promise<{releaseDate?: number, sourceTitle?: string, sourceViewCount?: number, chapters?: Array, channel?: {channelUrl: string, channelHandleUrl?: string, channelId?: string, channelName?: string}, wroteSubs: boolean, subsSkipped?: true} | null>}
 */
// v1.338 gate (adversary r1 W1): does the id yt-dlp returned name the item's own video? Exact, or equal
// once reduced to letters and digits - the item's `sourceId` may be the filename bracket's sanitized
// rendering of the raw id (yt-dlp's filename sanitizing replaces `/` and similar), the D8c residual.
function sameSourceId(returned, expected) {
  if (typeof returned !== 'string' || typeof expected !== 'string' || returned === '' || expected === '') return false;
  if (returned === expected) return true;
  const squash = (s) => s.normalize('NFKC').replace(/[^A-Za-z0-9]/g, '');
  const a = squash(returned);
  return a !== '' && a === squash(expected);
}

async function repullItemMetaAndSubs(watchUrl, mediaFilePath, config, opts) {
  if (typeof watchUrl !== 'string' || watchUrl === '') return null;
  if (typeof mediaFilePath !== 'string' || mediaFilePath === '') return null;
  // v1.338 (Dean: "Reheat is fine" for downloads from any site; plan first-class-any-site D9):
  // UNIVERSAL mode re-pulls a download from another site from its stored page link. That link is data
  // (a scan, a capture, a backup restore wrote it), so before yt-dlp sees it: the download lane's own
  // intake check (isPlausibleMediaUrl: http(s), no credentials, no private / local literal host), a DNS
  // resolve-then-check (a public name that resolves internally is refused - shortlink.guardHop), and on
  // both passes the lane's named-extractor gate (`--use-extractors default,-generic`: no generic
  // scrape-any-page fallback) plus the one-item bound. The YouTube-only channel capture is skipped.
  // RESIDUAL (disclosed; the download lane carries the same one - url.js isPlausibleMediaUrl, shortlink.js
  // guardHop): these checks run BEFORE yt-dlp, which then resolves the host again and follows redirects
  // with no address check of its own, so a public host that passes can still 302 or DNS-rebind to a
  // private address. What D9 adds is REPLAY: a stored link is fetched again later (the per-video Reheat,
  // the library batch), not only at download time.
  const universal = !!(opts && opts.universal);
  let target = watchUrl;
  if (universal) {
    const plausible = isPlausibleMediaUrl(watchUrl);
    // A refusal of the link ITSELF is structural (the same string is refused every time), so it is
    // reported as `refused` for the caller to stop retrying; a guardHop miss below may be a DNS blip
    // and stays a plain retryable null.
    if (!plausible.ok) return { refused: 'implausible-link', wroteSubs: false };
    const guarded = await guardHop(plausible.url, { lookup: opts && opts.lookup });
    if (!guarded.ok) return null;
    target = guarded.url;
  }
  const universalGate = universal ? ['--use-extractors', 'default,-generic', '--playlist-items', '1'] : [];
  // v1.338 gate (adversary r1 W1): a saved page link is item-specific only if the site makes it so - a
  // Twitch live recording saves the CHANNEL url (TwitchStreamIE sets no webpage_url), an Instagram
  // stories download the stories feed - so today's answer may be a DIFFERENT video. In universal mode
  // Pass A must return the item's own id (`opts.expectSourceId`) or nothing is kept and Pass B (which
  // would save the other video's captions as this file's sidecar) never runs.
  const expectSourceId = universal && opts && typeof opts.expectSourceId === 'string' ? opts.expectSourceId : null;
  let idVerified = !universal;
  ...
  let chapters;
  let channel;
  let viewCount; // v1.48 item 2: re-snapshotted on every reheat (Dean: "re-heatable if pulled later")
  let followerCount; // v1.54: the channel subscriber count, re-snapshotted the same way
  try {
    const metaArgs = [
      '--dump-json',
      '--skip-download',
    ...
    if (metaResult && metaResult.ok && typeof metaResult.stdout === 'string' && metaResult.stdout.trim() !== '') {
      ...
      if (universal && parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        if (!sameSourceId(parsed.id, expectSourceId)) return { refused: 'different-video', wroteSubs: false };
        idVerified = true;
      }
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        ...
        // v1.48 item 2: re-snapshot the view count. This is the half of Dean's
        // ask that makes an OLD download's count catch up ("have it re-heatable
        // if pulled later"), and the reason the value is a fresh snapshot each
        // time rather than a gap-fill. Bounded by the SAME standalone validator
        // the download-capture path uses, called directly here for the same
        // reason parseCapturedReleaseDate is above: a good view count must
        // never be dropped because some OTHER field failed validation.
        const parsedViews = parseCapturedViewCount(parsed.view_count);
        if (parsedViews !== null) viewCount = parsedViews;
        // v1.54: the subscriber count, through the SAME bounded standalone
        // validator posture -- a good count never dropped because another
        // field failed.
        const parsedFollowers = parseCapturedFollowerCount(parsed.channel_follower_count);
        if (parsedFollowers !== null) followerCount = parsedFollowers;
        ...
        const classified = classifySingleVideo(target);
        const captured = universal ? null : sanitizeCapturedChannelMeta({
  ...
  try {
    ...
    if (!idVerified) {
      // Universal mode and Pass A could not confirm the id (a timeout, a 429, bad output): no captions
      // from an unverified page. Retryable (wroteSubs false, no subsSkipped).
    } else if (typeof root === 'string' && root !== '' && realpathUnderChannelDir(mediaFilePath, root)) {
  ...
  // v1.48 item 2: `viewCount` joins the "did this run produce ANYTHING usable"
  // test. Without it, a reheat whose ONLY new information was a fresh view
  // count would resolve null -- "nothing to persist" -- and silently discard
  // the very value it was run to refresh.
  if (releaseDate === undefined && sourceTitle === undefined && chapters === undefined
    && channel === undefined && viewCount === undefined && followerCount === undefined && !wroteSubs) return null;

  return {
    ...(releaseDate !== undefined ? { releaseDate } : {}),
    ...(sourceTitle !== undefined ? { sourceTitle } : {}),
    ...(viewCount !== undefined ? { sourceViewCount: viewCount } : {}),
    ...(followerCount !== undefined ? { sourceFollowerCount: followerCount } : {}),
 ...
 */
async function runDownload(sub, config, targetIds, opts = {}) {
  const spawnOnce = (skipSubtitles) => spawnYtdlpDownload(
    ...
    buildYtdlpDownloadArgs(sub, config, targetIds, {
      oneOff: opts && opts.oneOff,
      sourceUrl: opts && opts.sourceUrl,
      autoUploaderFolder: opts && opts.autoUploaderFolder,
      skipSubtitles,
    }),
    {
  ...
  const first = await spawnOnce(false);
  if (!shouldRetryWithoutSubtitles(first)) return first;

  // v1.47.4 item 1 (L2): exactly ONE retry, subtitles stripped. Not recursive
  // and not configurable -- `spawnOnce(true)`'s argv cannot produce another
  // subtitle failure (the flags are gone), so there is no second retry to guard
  // against, but the single flat call keeps that structural rather than relying
  // on the argv staying that way.
  //
  // Re-running the WHOLE target set is correct, not wasteful, for both lanes:
  // a subscription carries `--download-archive`, so already-downloaded siblings
  // are skipped and only the genuinely-missing ids are re-attempted; a one-off
  // carries `--no-download-archive --force-overwrites` but has exactly one
  // target, which is the one that just failed.
  //
  // The caller's `onProgress` will therefore see a second 0->100 ramp. That is
  // deliberate and honest (a real second download IS happening); it is not a
  // reset of the first one.
  const retried = await spawnOnce(true);
  return {
    ...retried,
    // `subtitleFallback` means one specific thing: THE MEDIA LANDED, but
    // without its captions. It is therefore gated on the retry actually
    // succeeding -- gate fix (v1.47.4 delta): setting it unconditionally
    // labelled a retry that ALSO failed as "saved without captions", which
    // would tell the user a video was safely on disk when nothing had been
    // downloaded at all. That is precisely the class of dishonest success this
    // wave refused `--ignore-errors` for.
    ...(retried.ok === true ? { subtitleFallback: true } : {}),
    // Kept on BOTH outcomes: the original attributed reasons (e.g. the 429) are
    // the diagnostic payload, and they matter most when the retry also failed.
    subtitleFallbackFrom: Array.isArray(first.itemFailures) ? first.itemFailures : [],
  };
}

/**
 * v1.47.4 item 1 (L2): should this failed download be retried with subtitles
 * stripped? True only for a genuine yt-dlp exit (a NUMERIC exit code) that
 * reported at least one subtitle-attributed failure.
 *
 * The numeric-code requirement is the load-bearing guard. `spawnYtdlpDownload`
 * signals our OWN interventions with non-numeric codes -- `'ETIMEDOUT'` (the
 * absolute ceiling), `'ESTALLED'` (the idle watchdog), and `null` (a spawn error
 * or a child we killed, which is how job CANCELLATION lands here). Retrying any
 * of those would re-spawn work the user or a watchdog just stopped, so a
 * non-numeric code never retries no matter what the stderr happens to contain.
 *
 * Uses v1.36.2's existing `subtitleOnly` tagging (lib/ytdlp/failures.js) rather
 * than re-sniffing stderr here, so there is exactly one definition of "this was
 * a subtitle failure" in the codebase. `.some` (not `.every`) is deliberate: in
 * a batch where video A failed on subtitles and video B is genuinely
 * unavailable, the retry still rescues A while B fails again structurally --
 * strictly better than abandoning both.
 *
 * Never throws on a malformed/partial result object (returns false).
 */
function shouldRetryWithoutSubtitles(result) {
  if (!result || typeof result !== 'object' || result.ok === true) return false;
  if (typeof result.code !== 'number') return false;
  if (!Array.isArray(result.itemFailures)) return false;
  return result.itemFailures.some((f) => f && f.subtitleOnly === true);
}
...
module.exports = {
  ...
  probeChannelAvatar,
  channelProbeResultFromParsed,
  // v1.56 (Dean's bulk subscriber-count reheat): the channel-level follower
  // count probe -- see its own doc comment for why it is a sibling of
  // `probeChannelAvatar`, not a new field on it.
  probeChannelFollowerCount,
  ...
  pickStderrReason,
  // v1.47.4 item 1 (L2): the pure retry decision, exported so tests can assert
  // the cancellation/timeout guards directly rather than only through a spawn.
  shouldRetryWithoutSubtitles,
};
