...
// ./url, never ./run.
const { parseCapturedReleaseDate, sanitizeChannelAvatarUrl } = require('./store');
 ...
 * bounded upstream instead -- NOT by a count cap anymore (v1.25 QoL T2
 * REMOVED the old `--playlist-end`/`FILETUBE_YTDLP_MAX_VIDEOS` count-based
 * bound entirely): the LIST pass is scoped by `--dateafter <sub.cutoffDate>`
 * (`buildYtdlpListArgs`/`dateAfterArgs`, lib/ytdlp/args.js) instead, so only
 * videos published on/after that date are even considered. `cutoffDate`
 * itself is what stays BOUNDED, not the count: `sub.cutoffDate` starts at
 * either a user-supplied date or "yesterday" (T1), and a subsequent
 * cleanly-successful poll cycle (two-reviewer-gate FIX 1,
 * `lib/ytdlp/index.js`'s `processSubscription`) advances it forward to
 * "today" -- so after the initial history pull, every later poll's
 * `--dateafter` window narrows to "since the last successful poll," never
 * re-listing a channel's entire back catalog forever. Removing the
 * artificial `maxBuffer` ceiling here remains defense-in-depth on top of
 * that date-scoped bound.
 *
...
function spawnYtdlpDownload(args, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (code, signal) => {
      ...
      if (timedOut) {
        console.error('yt-dlp timed out:', describeFailure(args, { code: 'ETIMEDOUT', signal }));
        resolve({ ok: false, code: 'ETIMEDOUT', stdout: '', stderr: safeStderr, error: 'yt-dlp timed out and was killed', channelMeta: capturedMeta, itemFailures });
        return;
      }
      if (code === 0) {
...
/**
 * Re-pull backfill: refresh RELEASE-DATE/AVATAR metadata and best-effort
 * SUBTITLES for a SINGLE already-downloaded video, without ever downloading
 * the video itself again. Used by a backfill loop over existing yt-dlp
 * downloads (never wired to a route/scheduler here -- this file stays a thin
 ...
 * and derives `releaseDate`/`channelAvatarUrl` INDEPENDENTLY of each other via
 * `store.parseCapturedReleaseDate` (`release_date`, falling back to
 * `upload_date`) and `store.sanitizeChannelAvatarUrl` (`channel_thumbnail`) --
 * the SAME two standalone validators `store.sanitizeCapturedChannelMeta`
 * itself calls internally, reused directly rather than through that combined
 * validator. This is deliberate: `sanitizeCapturedChannelMeta` requires a
 * `channelUrl`/`uploaderUrl` that survives `validateChannelUrl` or it drops
 * the ENTIRE entry -- including a perfectly good `release_date` -- which is
 * unacceptable for a backfill whose primary goal is an accurate release date.
 * Calling the two field-level validators directly means a video with a valid
 * date but a missing/odd channel URL still yields `{releaseDate}` (with
 * `channelAvatarUrl` simply absent). No date parsing / URL validation is
 * reimplemented here.
 *
 ...
 * still returns `{releaseDate, channelAvatarUrl, wroteSubs: false}` if Pass A
 * succeeded. Only when NEITHER pass produced anything usable does this
 * resolve `null` -- "nothing to persist for this item" -- so a caller can
 * treat one bad video exactly like any other structured no-op, never a thing
 * that wedges the rest of a batch.
 *
 ...
 * @param {string} mediaFilePath the EXISTING on-disk media file this backfill targets
 * @param {object} config `{ downloadDir, cookiesFile, ... }`
 * @returns {Promise<{releaseDate?: number, channelAvatarUrl?: string, wroteSubs: boolean} | null>}
 */
async function repullItemMetaAndSubs(watchUrl, mediaFilePath, config) {
  ...
  try {
    ...
    if (metaResult && metaResult.ok && typeof metaResult.stdout === 'string' && metaResult.stdout.trim() !== '') {
      ...
      if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
        // Each field is derived INDEPENDENTLY (see this function's own doc
        // comment above for why `sanitizeCapturedChannelMeta` -- which
        // requires a valid channelUrl/uploaderUrl or drops EVERYTHING,
        // including the date -- is the wrong tool here). `?? undefined`
        // normalizes `parseCapturedReleaseDate`'s/`sanitizeChannelAvatarUrl`'s
        // own `null` ("absent/invalid") into `undefined`, matching this
        // function's own "present iff the key exists" return contract below.
        ...
        const parsedAvatarUrl = sanitizeChannelAvatarUrl(parsed.channel_thumbnail);
        if (parsedAvatarUrl !== null) channelAvatarUrl = parsedAvatarUrl;
      }
    }
  } catch {
  ...
  try {
    ...
    // `mediaFilePath` resolves outside `config.downloadDir` (or doesn't exist
    // at all -- `realpathUnderChannelDir` fails closed): Pass B is skipped
    // entirely, no spawn is ever attempted, and `wroteSubs` stays `false`.
  } catch {
    wroteSubs = false;
  }

  if (releaseDate === undefined && channelAvatarUrl === undefined && !wroteSubs) return null;
