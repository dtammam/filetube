...
// would be for a date.
const { parseCapturedReleaseDate, parseCapturedViewCount, sanitizeCapturedTitle, sanitizeChannelAvatarUrl, selectChannelAvatarUrl, sanitizeCapturedChannelMeta, CHANNEL_ID_PATTERN } = require('./store');
...
async function repullItemMetaAndSubs(watchUrl, mediaFilePath, config) {
  ...
  if (releaseDate === undefined && sourceTitle === undefined && chapters === undefined
    && channel === undefined && viewCount === undefined && !wroteSubs) return null;
