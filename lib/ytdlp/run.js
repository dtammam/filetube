...
const { parseCapturedReleaseDate, sanitizeCapturedTitle, sanitizeChannelAvatarUrl, selectChannelAvatarUrl, CHANNEL_ID_PATTERN } = require('./store');
...
// produced twenty identical 5-minute timeout kills (H0). `maxVideos: 0`
// ("unlimited") is scaled as UNLIMITED_SCALING_STAND_IN entries -- a stand-in,
// not a promise -- and the whole budget is capped at MAX_LIST_TIMEOUT_MS so a
// pathological config can never make a hung listing effectively unbounded
// (SF2 still holds: the budget is always finite).
...
function resolveListTimeoutMs(sub, config) {
  ...
  const rawMax = sub && Number.isInteger(sub.maxVideos) && sub.maxVideos >= 0
    ? sub.maxVideos
    : ytdlpConfig.parseMaxVideos(config && config.maxVideos);
  const entries = rawMax > 0 ? rawMax : UNLIMITED_SCALING_STAND_IN;
...
function runList(sub, config) {
  return spawnYtdlp(buildYtdlpListArgs(sub, config), {
    ...
    // v1.36 F1: the list argv carries --break-match-filters (args.js's
    // breakEarlyArgs) -- a break-early stop exits 101 and is a SUCCESS here.
    breakExitOk: true,
  });
}
 ...
 * @returns {Promise<{releaseDate?: number, wroteSubs: boolean} | null>}
 */
async function repullItemMetaAndSubs(watchUrl, mediaFilePath, config) {
  ...
  if (releaseDate === undefined && sourceTitle === undefined && chapters === undefined && !wroteSubs) return null;
