...
async function runDownload(sub, config, targetIds, opts = {}) {
  ...
  return {
    ...retried,
    // Preserved so the caller and the v1.47.4 failure log can report WHY this
    // item has no captions, instead of silently shipping a caption-less file.
    // `subtitleFallback` is the flag consumers branch on; `subtitleFallbackFrom`
    // keeps the original attributed reasons (e.g. the 429) verbatim.
