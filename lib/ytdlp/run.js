...
const { parseCapturedReleaseDate, sanitizeChannelAvatarUrl, selectChannelAvatarUrl } = require('./store');
 ...
 * never returns a raw, unvalidated URL, unlike `probeChannel` above (whose
 * caller does its own validation downstream via `resolveChannelDir`).
  ...
  return spawnYtdlp(probeArgs, {
    ...
    .then((result) => {
      ...
      try {
        ...
        return null; // malformed/non-JSON stdout -- never throw, just report "no avatar found"
      }
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return null;
      const rawAvatarUrl = selectChannelAvatarUrl(parsed.thumbnails);
      return sanitizeChannelAvatarUrl(rawAvatarUrl);
    })
    .catch(() => null);
}
