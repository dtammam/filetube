...
/**
 * FIX-6 (two-reviewer gate): guard the ORCHESTRATOR's own TERMINAL (done/
 * error) activity write against a subscription that was DELETED mid-cycle.
 * `DELETE /api/subscriptions/:id` calls `activity.clearSubscription(id)`
 * exactly once -- but a `runSubscriptionCycle`/`processSubscription` call
 * already in flight for that id (it captured its own `sub` object before the
 * delete) keeps running right through its own terminal state transition,
 * which would otherwise call `activity.setSubscription(id, ...)` and silently
 * RE-CREATE a permanent, stale activity entry for an id that no longer has a
 * subscription row (nothing else ever clears it again -- subscription
 * entries, unlike one-shots, are never TTL-pruned). FIX-3's settled-guard (in
 * lib/ytdlp/run.js) closes the analogous window for late onProgress
 * dispatch, but the orchestrator's OWN terminal `setSubscription` call here
 * is the main path that resurrects a deleted id, so it needs its own guard.
 *
 * Checked ONCE, right before a terminal write (not on every progress patch --
 * a cheap `loadDatabase`-backed existence check is negligible compared to the
 * `updateDatabase`/`scanDirectories` calls this same cycle already makes
 * immediately afterward). If the subscription is confirmed gone by then, the
 * terminal write is replaced with an explicit `clearSubscription` so the
 * snapshot can never resurrect it; a `loadDatabase` failure fails OPEN (still
 * writes the status) so a transient read error can never suppress a
 * legitimate status update for a subscription that's still very much alive.
 */
function setTerminalSubscriptionActivity(deps, subId, patch) {
...
async function runSubscriptionCycle(deps, config, sub) {
  ...
  if (!listResult.ok) {
    ...
    // FIX-6: terminal write guarded -- see setTerminalSubscriptionActivity's
    // doc comment.
    setTerminalSubscriptionActivity(deps, sub.id, { state: 'error', error: status });
    return status;
  }
  ...
  if (survivorIds.length === 0) {
    ...
    setTerminalSubscriptionActivity(deps, sub.id, { state: 'done', percent: 100 });
...
async function processSubscription(deps, config, sub) {
  ...
  try {
    ...
    // FIX-6: terminal write guarded.
    if (sub && sub.id) setTerminalSubscriptionActivity(deps, sub.id, { state: 'error', error: lastStatus });
