...
/**
 * The refresh-avatars batch's background worker: iterates EVERY current
 * subscription (`subs`, an already-resolved snapshot the route below
 * captured synchronously via `store.listSubscriptions` before ever
 * responding) ONE AT A TIME, probing `run.probeChannelAvatar(sub.channelUrl,
 * config)` for each -- wrapped in `runExclusive` so it shares the SAME
 * global FIFO gate the poll loop, one-shot downloads, and the reheat batch
 * already serialize against (NFR2): this batch can never storm the network,
 * and never runs a probe spawn concurrently with any other spawn.
 *
 * Refreshes EVERY subscription with a `channelUrl`, not just ones missing an
 * avatar -- doubling as a "refresh" (a channel's avatar image can
 * legitimately change over time; `store.recordSubscriptionChannelAvatar`
 * already deliberately overwrites a previously-captured one, see its own doc
 * comment). A subscription with no `channelUrl` at all is counted `skipped`
 * (never probed -- there is no channel identity to probe).
 *
 * One bad subscription never wedges the batch: `run.probeChannelAvatar`
 * itself never throws/rejects (see its own doc comment) -- the per-item
 * try/catch here is defense-in-depth against a future regression of that
 * guarantee, matching `runRepullMetadataBatch`'s identical posture
 * immediately above. A probe that resolves `null` (no avatar found for this
 * channel) is counted `failed`, same as an unexpected throw -- both simply
 * mean "nothing usable to persist for this subscription."
 *
 * NEVER triggers a scan and NEVER re-keys anything:
 * `store.recordSubscriptionChannelAvatar` writes directly into the matching
 * subscription's own record.
 * @param {object} deps the SAME deps object `registerRoutes` received
 * @param {object} config parsed yt-dlp config
 * @param {Array<object>} subs a `store.listSubscriptions`-shaped snapshot, captured at route time
 */
async function runRefreshAvatarsBatch(deps, config, subs) {
  ...
  try {
    for (const sub of (Array.isArray(subs) ? subs : [])) {
      ...
      if (!sub || typeof sub.channelUrl !== 'string' || sub.channelUrl === '') {
        skipped += 1;
        activity.setOneShot(REFRESH_AVATARS_ACTIVITY_ID, { skipped, current: (sub && sub.id) || null });
        continue;
      }

      activity.setOneShot(REFRESH_AVATARS_ACTIVITY_ID, { current: sub.id });
      try {
        // NFR2: shares the SAME shared FIFO gate every other spawn in this
        // module already serializes against -- one probe in flight at a
        // time, never concurrently with a poll/one-shot/reheat spawn.
        const avatarUrl = await runExclusive(() => run.probeChannelAvatar(sub.channelUrl, config));
        if (avatarUrl) {
          await store.recordSubscriptionChannelAvatar(deps, sub.channelUrl, avatarUrl);
          done += 1;
          activity.setOneShot(REFRESH_AVATARS_ACTIVITY_ID, { done, current: sub.id });
        } else {
          failed += 1;
          activity.setOneShot(REFRESH_AVATARS_ACTIVITY_ID, { failed, current: sub.id });
        }
      } catch (err) {
        failed += 1;
        console.error(`yt-dlp: refresh-avatars item ${sub.id} failed unexpectedly:`, err && err.message);
        activity.setOneShot(REFRESH_AVATARS_ACTIVITY_ID, { failed, current: sub.id });
      }
    }
...
async function processSubscription(deps, config, sub, nowMs = Date.now(), avatarBudget = { remaining: Infinity }) {
  ...
  if (
    ...
    try {
      const avatarUrl = await run.probeChannelAvatar(sub.channelUrl, config);
      if (avatarUrl) await store.recordSubscriptionChannelAvatar(deps, sub.channelUrl, avatarUrl);
    } catch (err) {
  ...
  try {
    ...
    await runExclusive(async () => {
      for (const sub of targets) {
        ...
        await processSubscription(deps, config, sub, nowMs, avatarBudget);
      }
    });
  } finally {
    ...
    if (!folderIsExplicit && Array.isArray(downloadResult.channelMeta)) {
      ...
      if (capturedEntry) {
        ...
        if (sanitizedForAvatar && sanitizedForAvatar.channelUrl && !sanitizedForAvatar.channelAvatarUrl) {
          try {
            const avatarUrl = await run.probeChannelAvatar(sanitizedForAvatar.channelUrl, config);
            if (avatarUrl) capturedEntry.channelThumbnail = avatarUrl;
          } catch (err) {
...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  app.post('/api/subscriptions', async (req, res) => {
    ...
    try {
      ...
      if (record && !record.channelAvatarUrl) {
        Promise.resolve()
          .then(() => run.probeChannelAvatar(record.channelUrl, config))
          .then((avatarUrl) => (avatarUrl ? store.recordSubscriptionChannelAvatar(deps, record.channelUrl, avatarUrl) : undefined))
  ...
  app.post('/api/ytdlp/download', (req, res) => {
    ...
    resolveOneOffFolder(classification.watchUrl, explicitFolder, config, jobId)
      .then((folder) => runExclusive(() => runOneShot(deps, config, {
        ...
        folderIsExplicit: explicitFolder !== null,
        jobId,
      })))
      .catch((err) => {
  ...
  app.post('/api/ytdlp/refresh-avatars', (req, res) => {
    ...
    // 202 immediately, reporting the blast radius (total subscription count)
    // up front -- never blocks on the network pass (mirrors every other
    // background-batch route in this file).
    res.status(202).json({ started: true, total });

    refreshAvatarsInProgress = true;
    refreshAvatarsCancelled = false;
    runRefreshAvatarsBatch(deps, config, subs).catch((err) => {
