...
function launchOneShotJob(deps, config, { jobId, videoId, watchUrl, sourceUrl, format, quality, filetype, explicitFolder }) {
  ...
  const laneReady = (typeof sourceUrl === 'string' && sourceUrl !== '')
    ? shortlink.resolveShortlink(sourceUrl).then((r) => {
      ...
      if (!reclassified.ok) return { videoId, watchUrl, sourceUrl: r.url }; // let yt-dlp report no-match on the resolved url
