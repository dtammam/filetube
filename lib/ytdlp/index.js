...
async function reheatOneItem(deps, config, item, opts) {
  ...
  if (watchUrl) {
  ...
  } else if (item.universal === true && item.inDownloadRoot === true) {
    ...
    const sourcePage = sanitizeSourceShareUrl(item.sourceUrl) || sanitizeSourceShareUrl(local && local.sourceUrl);
    if (sourcePage) {
...
async function runSingleItemReheat(deps, config, item, before) {
  ...
  try {
    ...
    activity.setOneShot(REPULL_ITEM_ACTIVITY_ID, {
      ...
      // genuine home video with no YouTube identity anywhere. The client turns
      // it into "No YouTube identity found, nothing to refresh" rather than a
