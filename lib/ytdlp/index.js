...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  // initiates ad-hoc downloads. Subscription-poll cancellation is explicitly
  // OUT OF SCOPE (see the exec plan's A3 design section: a subscription's
  // `runDownload` targets a WHOLE batch of survivor videos in one spawn, and
  // cancelling mid-batch raises coalescing/partial-batch questions this
  // round deliberately defers).
  //
