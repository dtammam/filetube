...
function applyBackfilledChannelName(metadata, target, probedName) {
  ...
  const name = typeof probedName === 'string' ? probedName.trim() : '';
  if (name === '') return 0;
...
async function runChannelNameBackfillBatch(deps, config, targets) {
  ...
  let failed = 0;
  let itemsUpdated = 0;
  // Dedupe the FAN-OUT on the probe's canonical channelId, mirroring the
  // reheat-subs batch: two targets can resolve to the same real channel (a
  // channelId target + a url-only target for the same channel), and each was
  // already fanned out to ALL its items -- a second fan-out would rescan for
  // nothing. The probe still counts `done`.
  const fannedOutChannelIds = new Set();
