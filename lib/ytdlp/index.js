...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  app.post('/api/ytdlp/repull-metadata/item/:mediaId/relocate', async (req, res) => {
    ...
    // from them.
    const body = req.body && typeof req.body === 'object' ? req.body : {};
    const expect = (body.expect && typeof body.expect === 'object'
      && typeof body.expect.currentPath === 'string'
      && typeof body.expect.destinationPath === 'string')
      ? { currentPath: body.expect.currentPath, destinationPath: body.expect.destinationPath }
      : null;
