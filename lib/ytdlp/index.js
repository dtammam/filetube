...
function tripPollBreaker(deps, config, skippedSubs, consecutiveFailures) {
  ...
  breakerResumeTimer = setTimeout(() => {
    breakerResumeTimer = null;
    runPoll(deps, config, deferredIds).catch((err) => {
  ...
  try {
    for (const item of (Array.isArray(items) ? items : [])) {
      ...
      try {
        ...
        // bracket, no embedded purl/comment) -- there is nothing a future
        // reheat could ever add beyond what the local pass just produced, so
        // the item is marked complete to keep it from being retried on every
        // later non-force reheat forever. `force` still re-processes it.
        const exhausted = !watchUrl;
...
async function runPoll(deps, config, subId, nowMs = Date.now(), onDecision) {
  ...
  if (Array.isArray(subId)) {
    ...
    // repull overrides it).
    const subset = allSubs.filter((s) => wanted.has(s.id) && !s.paused);
    if (subset.length === 0) {
