...
// v1.38.0 TTS defer coordination (Dean's "less-spiky" choice): the authoritative
// "a download/poll is in flight or queued" signal. `ytdlpQueueLength` is
// incremented for the ENTIRE lifetime of every runExclusive job (channel polls
// AND one-shot downloads) and decremented only on settle, so this is a strictly
// more precise gate than the meta-tagged `getGateQueueSnapshot().length`. The
// server's TTS worker reads it (a single read-only counter, atomic under JS
// single-threadedness) at each dequeue and defers synthesis while it is true, so
// TTS never hammers CPU/disk alongside a download. One-directional: downloads
// never wait for TTS.
function isHeavyJobActive() {
  return ytdlpQueueLength > 0;
}
