...
// v1.338 (plan first-class-any-site D9): the ONE strict link check, for a reheat's saved page link.
const { sanitizeSourceShareUrl } = require('../media/source-share');

// Wave 5 of the relational-migration arc: the ytdlp namespace is a feature
// store handed in as `deps.ytdlpDb`; the doc object (`deps.loadDatabase()`)
// no longer carries `db.ytdlp`. The helpers in this file that read the
// namespace through a db-shaped argument (ensureYtdlp(db) / readYtdlpNamespace
// (db)) take a HOLDER - `{ ytdlp: snapshot }` - or, when they read db.metadata
// as well, a VIEW: the doc snapshot with the namespace snapshot attached.
function ytdlpHolder(deps, only) {
  if (!deps || !deps.ytdlpDb) throw new Error('yt-dlp: deps.ytdlpDb is required (the namespace is a feature store since Wave 5)');
  return deps.ytdlpDb.holder(only);
}
function withYtdlp(deps, db) {
  return Object.assign({}, db, { ytdlp: deps.ytdlpDb.read() });
}
const rules = require('./rules');
const args = require('./args');
const url = require('./url');
const shortlink = require('./shortlink');
// FR-E: the ephemeral in-process activity map. Never imports store.js, never persists
...
// v1.146 (downloader-engine): the runtime engine selector - state, installs,
// health gate, spawn seam, and the bell-event vocabulary server.js renders.
const engineLib = require('./engine');
...
// ---- v1.47.4 item 7: the durable failure log ------------------------------
//
// Dean: "If any downloads fail I'd like it to be explicitly logged and have the
// error so one can look in posterity."
//
// The outcomes that mean something actually failed. `cancelled` is deliberately
// EXCLUDED -- a user pressing cancel is an intent, not a failure, and logging it
// would bury the real failures in noise. `success`/`requeued` are likewise not
// failures. See `runlog.recordRun`'s call sites for where each outcome is set.
const FAILURE_LOG_OUTCOMES = new Set(['error', 'partial', 'dropped']);

/**
 * Mirror a failing run's per-item detail into the durable failure log.
 *
 * Deliberately called from the SAME choke points that already write the run log
 * (`emitRunLog` for one-shots, the terminal `recordRun` for subscription
 * cycles), rather than from each individual failure branch -- this repo's
 * recurring bug class is "the seat that forgot to CALL the shared helper"
 * (v1.41.4), and there is exactly one place per lane that every terminal
 * outcome already funnels through.
 *
 * Falls back to a single run-level row when a failing run reported no per-item
 * failures at all (e.g. a channel-level timeout, or a `dropped` requeue): a
 * failure with no attributable item is still a failure Dean needs to see, and
 * silently writing nothing would under-report exactly what this log is for.
 *
 * Never throws -- a diagnostics write must never break terminal-status handling.
 */
function recordFailureLog(deps, { source, outcome, reason, failures, name, url }) {
  if (!FAILURE_LOG_OUTCOMES.has(outcome)) return;
  try {
    ...
    const rows = list.length > 0
      ? list.map((f) => ({
        source,
        videoId: f.videoId,
        // Prefer the item's own title; fall back to the run's name so a row is
        // never anonymous.
        title: f.title || name,
        url,
        reason: f.reason || reason,
      }))
      : [{ source, title: name, url, reason }];
    faillog.recordFailures(deps && deps.dataDir, rows);
  } catch (err) {
    console.error('yt-dlp: failed to record failure-log entries (continuing):', err && err.message);
  }
}

/**
 * v1.47.4 item 1 (L3): record that an item downloaded successfully but WITHOUT
 * its captions, because the subtitle-stripped retry was what rescued it.
 *
 * This is logged even though the download SUCCEEDED, and that is the point: the
 * pre-v1.47.4 behavior was that a caption problem silently cost you the whole
 * video, and the fix now silently costs you only the captions. Silently is
 * still the wrong word for it -- Dean gets a row saying so, flagged distinctly
 * so it never reads as a failed download.
 */
function recordSubtitleFallback(deps, source, downloadResult, { name, url }) {
  if (!downloadResult || downloadResult.subtitleFallback !== true) return;
  try {
    const originals = Array.isArray(downloadResult.subtitleFallbackFrom) ? downloadResult.subtitleFallbackFrom : [];
    const rows = originals.length > 0
      ? originals.filter((f) => f && typeof f === 'object').map((f) => ({
        source,
        videoId: f.videoId,
        title: f.title || name,
        url,
      ...
      : [{ source, title: name, url, reason: 'Subtitles could not be fetched', subtitleFallback: true }];
    faillog.recordFailures(deps && deps.dataDir, rows);
  } catch (err) {
    console.error('yt-dlp: failed to record subtitle-fallback entries (continuing):', err && err.message);
  }
}
...
// pathological/very long redacted error must not bloat the database indefinitely.
...
// v1.49: WHICH kind of reheat currently holds `repullMetadataInProgress` --
// `'batch'`, `'item'`, or `null` when idle. Exists purely so the cancel route
// can stay HONEST. Cancellation is implemented as "stop before the NEXT item",
// which is meaningful for a library batch and meaningless for a single-video
// run (there is no next item, and the in-flight one always runs to completion).
// Without this the cancel route would cheerfully answer `{cancelled: true}` to
// a user cancelling a per-video reheat and then not cancel anything.
let repullMetadataJobKind = null;
...
// v1.49: the SINGLE-VIDEO reheat's activity entry, deliberately a DIFFERENT
// one-shot id from the library batch above. Both surface in the same status
// chip (`GET /api/subscriptions/status` -> `oneShots`), but they are separate
// jobs with separate lifecycles, and collapsing them onto one id would let a
// per-video run's terminal write stomp a library batch's counters (or vice
// versa) in the snapshot the client renders.
const REPULL_ITEM_ACTIVITY_ID = 'repull-metadata-item';

/**
 * v1.49 (Dean: "force a reheat on a specific video"): ONE item's reheat, the
 * whole of it -- local ffprobe tags pass, watch-URL derivation, network pass,
 * the merge-precedence rules, the avatar probe, and the single
 * `recordRepulledItemMeta` persist.
 *
 * THIS FUNCTION IS THE ANTI-DRIFT SEAM for the reheat's metadata half, and it
 * exists for the same reason `planImportRelocation` exists for the relocation
 * half (v1.41.7): there are now TWO callers -- the library-wide batch
 * (`runRepullMetadataBatch`, below) and the per-video route (`POST
 * /api/ytdlp/repull-metadata/item/:mediaId`) -- and a second, forked copy of
 * "what a reheat does to an item" would be free to disagree with the first.
 * Every precedence rule, trust gate and completion-marker decision below was
 * paid for by a gate finding at some point between v1.33 and v1.48; they are
 * not re-derivable from the field list, so they live in exactly one place.
 *
 * DELIBERATELY NOT INCLUDED: the relocation. A reheat does two things per item
 * (hydrate metadata, maybe move the file), and only the first belongs here --
 * the batch relocates automatically right after this returns, while the
 * per-video route PROPOSES the move and waits for the user's confirm (Dean has
 * no media backup; the irreversible half never happens as a side effect of the
 * reversible half). Both callers therefore need `youtubeId` and `persisted`
 * back, which is why they are on the return shape.
 *
 * Does NOT touch `activity` and does NOT count outcomes -- the caller owns its
 * own reporting surface (the batch has running totals; the single-item route
 * has a per-field diff). This function only reports what happened.
 *
 * @param {object} deps the deps bundle `registerRoutes` received
 * @param {object} config parsed yt-dlp config
 * @param {{mediaId: string, filePath: string, videoId: string|null, watchUrl: string|null, inDownloadRoot?: boolean}} item an `enumerateRepullableItems`-shaped entry
 * @param {{avatarMemo?: Map<string, string|null>, allowViewCountDecrease?: boolean, nowMs?: number}} [opts]
 *   - `avatarMemo`: channelUrl -> avatarUrl|null. The batch passes its
 *     batch-scoped map so a channel is probed ONCE across the whole run; a
 *     single-item caller passes nothing and gets a throwaway map (one probe).
 *   - `allowViewCountDecrease`: v1.49 decision 2 -- threaded to
 *     `recordRepulledItemMeta`, true ONLY on the explicit per-video force. See
 *     that function's monotonicity-guard comment (server.js) for why the batch
 *     must keep refusing a decrease.
 *   - `nowMs`: injectable clock for the persist, same posture as
 *     `recordRepulledItemMeta`'s own.
 * @returns {Promise<{persisted: boolean, attemptedPersist: boolean, hasAnythingToPersist: boolean, exhausted: boolean, markComplete: boolean, youtubeId: string|null, watchUrl: string|null, networkRan: boolean, changed: object}>}
 */
async function reheatOneItem(deps, config, item, opts) {
  const o = opts || {};
  const avatarByChannelUrl = o.avatarMemo instanceof Map ? o.avatarMemo : new Map();
  ...
  // as before.
  // v1.338 (plan first-class-any-site D9): a download from another site re-pulls from its saved page
  // link (else the page link in its own tags), in the re-pull's UNIVERSAL mode - the download lane's
  // guards (isPlausibleMediaUrl, a DNS resolve-then-check, `--use-extractors default,-generic`).
  // Hoisted: `exhausted` below also reads it.
  const sourcePage = (!watchUrl && item.universal === true && item.inDownloadRoot === true)
    ? (sanitizeSourceShareUrl(item.sourceUrl) || sanitizeSourceShareUrl(local && local.sourceUrl))
    : null;
  let result = null;
  // v1.338 gate (adversary r1 W1 + S3): a universal re-pull is REFUSED (structurally, every time) when
  // the link fails the lane's intake check or today's page is a DIFFERENT video than this item's own
  // `sourceId` (a Twitch channel url, an Instagram stories feed). Nothing from it is kept, and the item
  // is exhausted rather than retried forever. An item with no usable sourceId cannot be verified, so it
  // is refused the same way.
  let refusedLink = false;
  if (watchUrl) {
    result = await runExclusive(() => run.repullItemMetaAndSubs(watchUrl, item.filePath, config));
  } else if (sourcePage) {
    const expectSourceId = typeof item.sourceId === 'string' && item.sourceId !== '' ? item.sourceId : null;
    if (expectSourceId === null) {
      refusedLink = true;
    } else {
      result = await runExclusive(() => run.repullItemMetaAndSubs(sourcePage, item.filePath, config, { universal: true, expectSourceId }));
      if (result && result.refused) { refusedLink = true; result = null; }
    }
  }

  // v1.41.5 gate fix (adversarial CRITICAL): may we trust this file's own
  // EMBEDDED tags as YouTube provenance? Yes iff the item is a YouTube
  // video (a watch URL / discovered id -- from a filename bracket, a
  // persisted youtubeId, or the purl this very probe just surfaced) OR it
  // physically lives under the module's own download dir (a file in there
  // was put there by yt-dlp/metube, tags and all -- the v1.33 case, where
  // an in-root download with no bracket and no purl still legitimately
  // yields its embedded date/title).
  //
  // Before the widening this was IMPLICIT: only in-root items were ever
  // enumerated, so "has embedded tags" and "came from YouTube" were the
  // same thing. The root-agnostic gate breaks that equivalence -- it now
  // enumerates Dean's ripped CDs, home videos and movie rips too
  // (deliberately: the local probe is the only way a pre-v1.33 purl-only
  // import is ever discovered). Ungated, those files would have their
  // DISPLAYED TITLE superseded by whatever generic string sits in their
  // ID3/container `title` tag ("Track 05") and their releaseDate rewritten
  // from an embedded `date` tag -- reordering the library-wide default
  // sort, with no title-edit endpoint to undo it, and the scan's own
  // carry-forward would then re-prefer the damaged sourceTitle forever.
  // A plain library file yields NOTHING here: it is exhausted, marked,
  // and never probed again.
  const trustEmbeddedTags = Boolean(watchUrl) || Boolean(youtubeId) || item.inDownloadRoot === true;
  ...
  const releaseDate = (result && result.releaseDate !== undefined)
    ? result.releaseDate
    : (trustEmbeddedTags && local && Number.isFinite(local.releaseDateMs) ? local.releaseDateMs : undefined);
  const sourceTitle = (result && result.sourceTitle !== undefined)
    ? result.sourceTitle
    : (trustEmbeddedTags && local && typeof local.title === 'string' && local.title !== '' ? local.title : undefined);
  // v1.48 item 2: the re-snapshotted view count. NETWORK-ONLY on purpose,
  // with no embedded-tag fallback beside it -- a view count is a property
  // of the SOURCE at a moment in time, and nothing in a local container
  // tag can carry it. `undefined` means "leave the item's current value
  // alone", so a reheat that could not reach the network never destroys a
  // previously captured count.
  const sourceViewCount = (result && result.sourceViewCount !== undefined) ? result.sourceViewCount : undefined;
  // v1.54: the follower count rides the same seam.
  const sourceFollowerCount = (result && result.sourceFollowerCount !== undefined) ? result.sourceFollowerCount : undefined;
  ...
  // stale list), which is correct for a successful probe. Same
  // provenance gate as the two fields above (v1.41.5): a home video's
  // container chapters are not ours to rewrite.
  const chapters = (result && result.chapters !== undefined)
    ? result.chapters
    : (trustEmbeddedTags && local && Array.isArray(local.chapters) ? local.chapters : undefined);
  const wroteSubs = Boolean(result && result.wroteSubs);
  // v1.41.5: Pass B could never even be ATTEMPTED for this file (it
  // lives outside `config.downloadDir` -- a MeTube-era import). A
  // structural skip, not a transient failure; see the `markComplete`
  // gate below and run.js's own comment.
  const subsSkipped = Boolean(result && result.subsSkipped);

  // v1.41.5: the channel identity Pass A discovered (network-only --
  // the embedded local tags carry no channel URL). `undefined` means
  // "leave the item's identity alone", same convention as above.
  const channel = (result && result.channel && typeof result.channel === 'object')
    ? result.channel
    : undefined;

  // The avatar for that channel -- probed ONCE PER DISTINCT channelUrl
  // across the whole batch (memoized, misses included), never once per
  // item. `ensureChannelAvatar` registers what it finds into the
  // channelId-keyed registry, which is ALSO what
  // `store.resolveItemChannelAvatarUrl` serves from -- so even an item
  // whose own `channelAvatarUrl` write is declined by the
  // never-overwrite guard still renders the right avatar.
  let channelAvatarUrl;
  if (channel && typeof channel.channelUrl === 'string' && channel.channelUrl !== '') {
    if (!avatarByChannelUrl.has(channel.channelUrl)) {
      const probed = await ensureChannelAvatar(deps, config, {
        channelId: channel.channelId,
        channelUrl: channel.channelUrl,
      });
      avatarByChannelUrl.set(channel.channelUrl, (probed && probed.avatarUrl) ? probed.avatarUrl : null);
    }
    const memoized = avatarByChannelUrl.get(channel.channelUrl);
    if (memoized) channelAvatarUrl = memoized;
  }

  // `exhausted`: NO watch URL is derivable from anywhere (no filename
  // bracket, no embedded purl/comment) AND - v1.338 D9 - no page link for
  // a download from another site (a universal item WITH one ran the
  // network pass above, so it is completed like a YouTube item: by Pass B,
  // and a failed pass - a 429, a timeout - leaves it retryable; a REFUSED
  // link - implausible, or now a different video - counts as none) AND the local probe genuinely
  ...
  const exhausted = !watchUrl && (!sourcePage || refusedLink) && local !== null;
  // Gate the idempotency marker on the SUBTITLE pass actually
  // completing (see the `done` vs `failed` doc comment above) -- OR on
  // the item being exhausted (nothing more will ever be derivable) -- OR
  // (v1.41.5) on Pass B having been STRUCTURALLY skipped for a file
  // outside the download root, where "completing" is not a thing that
  // can ever happen and Pass A's metadata IS the whole deliverable.
  const markComplete = wroteSubs === true || subsSkipped === true || exhausted;
  ...
  // failed. v1.41.5: `channel` joins it for exactly the same reason (an
  // import whose ONLY new fact is its channel identity is real work).
  // v1.48 item 2: `sourceViewCount` joins the OR-chain for the same reason
  // `chapters` and `channel` had to (the persist-gate bug class's fifth
  // and sixth strikes) -- an item whose only new fact this run is a fresh
  // view count is real work, and omitting it here would drop that count
  // AND count the item failed.
  const hasAnythingToPersist = releaseDate !== undefined || sourceTitle !== undefined ||
    chapters !== undefined || channel !== undefined || sourceViewCount !== undefined ||
    sourceFollowerCount !== undefined ||
    (youtubeId !== null && youtubeId !== undefined) || wroteSubs;

  let persisted = false;
  const attemptedPersist = hasAnythingToPersist || exhausted;
  // v1.53: the persist meta is captured in a local so the write side's
  // OUT-FIELD (`attributionConflict`, set by recordRepulledItemMeta when a
  // manual attribution declined a conflicting network identity) can be read
  // back after the await -- the deps-bridge style, no return-shape change.
  const persistMeta = {
    releaseDate,
    sourceTitle,
    sourceViewCount,
    ...
    markComplete,
    // v1.49 decision 2: OFF unless the caller is the explicit per-video
    // force. Passing it unconditionally (or defaulting it true) would
    // silently retire the v1.48 monotonicity guard for the library batch,
    // which is the exact case that guard was written for.
    allowViewCountDecrease: o.allowViewCountDecrease === true,
  };
  if (attemptedPersist) {
    persisted = await deps.recordRepulledItemMeta(deps, item.mediaId, persistMeta, typeof o.nowMs === 'number' ? o.nowMs : Date.now());
  }

  return {
    // v1.53 (Dean's decision 3): a kept-but-conflicting manual attribution,
    // surfaced -- null when none.
    attributionConflict: persistMeta.attributionConflict || null,
    persisted,
    attemptedPersist,
    hasAnythingToPersist,
    exhausted,
    markComplete,
    youtubeId,
    watchUrl,
    networkRan: result !== null,
    // What this run actually produced, for a caller that wants to tell the
    // user what changed (the per-video route's toast). `undefined` entries
    // mean "not refreshed this run" and are meaningful -- callers must not
    // collapse them into a falsy check.
    changed: { releaseDate, sourceTitle, sourceViewCount, chapters, channel, channelAvatarUrl, wroteSubs, subsSkipped },
  };
}
 ...
 *   - `result.subsSkipped === true` (v1.41.5: the media file is not under
 *     `config.downloadDir`, so Pass B STRUCTURALLY cannot run -- the normal
 *     case for a MeTube-era import in a plain library root) with Pass A's
 *     metadata persisted -> `done`, marker set. This is NOT the retryable
 *     `wroteSubs: false` case above: no future reheat could ever make that
 *     subtitle spawn happen, so leaving the item un-marked would re-fetch it
 *     over the network on EVERY later reheat and report it `failed` forever.
 ...
 * `releaseDate`/`channelAvatarUrl`/`channel identity`/`hasSubtitles` directly
 * into `db.metadata`, so the UI reflects a reheated item immediately without a
 ...
 *
 * v1.41.5 (Dean's MeTube-import hydration) -- CHANNEL IDENTITY + AVATAR:
 * Pass A now returns the video's channel (`result.channel`, see
 * run.repullItemMetaAndSubs). Two things happen with it here:
 *   1. it is threaded into `recordRepulledItemMeta`'s new never-overwrite
 *      identity branch (server.js), which is what finally gives an imported
 *      item a real channel name + a working Subscribe button; and
 *   2. its AVATAR is probed via `ensureChannelAvatar` -- ONCE PER DISTINCT
 *      `channelUrl` in this batch, memoized in `avatarByChannelUrl` below.
 *      Dean's library is ~100 videos across a handful of channels: probing
 *      per ITEM would mean ~100 channel-endpoint spawns for ~5 real channels.
 *      The memo caches the MISS too (`null`), so a channel whose avatar can't
 *      be probed is not retried once per item either. `ensureChannelAvatar` is
 *      called unforced, so its own registry freshness gate (90d TTL) means a
 *      channel FileTube already knows costs zero spawns; and it shares the
 *      same `runExclusive` FIFO gate as everything else here (NFR2).
 *
 * v1.41.6 (Dean) -- IMPORT RELOCATION, the other half of that hydration: once
 * an item's identity has PERSISTED, its file is moved into that channel's
 * folder under the yt-dlp download dir (native `<title> [<id>].<ext>` name), so
 * a MeTube import becomes indistinguishable from a download -- it shows up in
 * the channel's sidebar folder, can be pinned, and its channel link resolves.
 * The whole decision (eligibility, never touching genuine local media, the
 * default-ON settings toggle, confinement, no-clobber, the `getMediaId` re-key)
 * lives behind ONE deps seam, `deps.relocateHydratedImport` (server.js's
 * `relocateHydratedImportIntoChannelFolder`) -- this file only counts the
 * outcome. Absent from a stubbed/minimal deps bundle => no relocation at all,
 * and the reheat behaves exactly as it did in v1.41.5.
 *
 * The batch's activity entry therefore carries TWO extra counters, `moved` and
 * `moveFailed`, alongside done/skipped/failed -- a relocation that fails while
 * the metadata pass succeeded is neither a "done" nor a "failed" reheat, and
 * the summary line says so out loud rather than burying it.
 *
 * @param {object} deps the SAME deps object `registerRoutes` received
 * @param {object} config parsed yt-dlp config
 * @param {Array<{mediaId: string, filePath: string, videoId: string, watchUrl: string, inDownloadRoot?: boolean, alreadyRepulled: boolean}>} items
...
async function runRepullMetadataBatch(deps, config, items, force) {
  ...
  let skipped = 0;
  let failed = 0;
  // v1.41.6 (import relocation): move accounting, kept SEPARATE from
  // done/skipped/failed -- a relocation is a distinct outcome from a metadata
  // reheat, and conflating them would let "12 done" hide "3 files could not be
  // moved". `moved` counts files physically relocated into a channel folder;
  // `moveFailed` counts items whose move was attempted and failed (the file and
  // its db entry are untouched in that case). Items that were never candidates
  // (local media, no identity, already in a download root, setting off) are not
  // counted at all -- they are not "skipped moves", they are simply not moves.
  let moved = 0;
  let moveFailed = 0;
  // v1.41.6 gate fix (QA WARNING): relocations that landed but could NOT be
  // written into `.ytdlp-archive.txt`. The file is fine and correctly filed --
  // but a subscription poll of that channel may now re-download it, and the
  // relocated filename is not the one yt-dlp would write, so yt-dlp's own
  // "file already exists" skip cannot save us. That is a user-visible
  // consequence, so it is surfaced, not buried in stderr.
  let moveArchiveWarnings = 0;
  // v1.41.5: batch-scoped avatar memo -- `channelUrl -> avatarUrl|null` (see
  // this function's doc comment). One probe per DISTINCT channel, never one
  // per item.
  const avatarByChannelUrl = new Map();
  ...
  activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, {
    ...
    moveFailed,
    moveArchiveWarnings,
    // v1.53 gate W3: manual attributions the batch KEPT against a
    // conflicting network identity -- reset with the siblings (setOneShot
    // merges), counted below, surfaced so decision 3's "never silent"
    // holds for the BATCH path too, not just the single-item flame button.
  ...
  /**
   * v1.41.6: relocate ONE item's file into its channel folder, and count the
   * outcome. Called from BOTH paths through the loop below -- the freshly
   * hydrated item, and the already-hydrated one a non-force reheat skips (see
   * that branch's comment: that is Dean's whole existing library).
   *
   * The PRE-FILTER here is pure, zero-cost enumeration data, and it exists so a
   * library-wide reheat does not pay a database read per home video:
   *   - `inDownloadRoot` -- an item already under the module's own download dir
   *     is already home; there is nothing to relocate. (Absent/undefined from an
   *     older stubbed enumerator -> falsy -> the seam decides, as before.)
   *   - a YouTube id -- either the enumerator's (`item.videoId`: a filename
   *     bracket, or a persisted `youtubeId`) or the one THIS pass just
   *     discovered from an embedded purl tag. No id, no move, ever: that is the
   *     rule that keeps genuine local media untouched, and it is re-asserted
   *     independently inside `relocateHydratedImport` itself.
   * Everything else -- the settings toggle, the identity re-validation,
   * confinement, no-clobber, the id re-key -- belongs to the seam (server.js's
   * `relocateHydratedImportIntoChannelFolder`), which re-reads the item from the
   * FRESH db rather than trusting this loop's snapshot.
   */
  const maybeRelocate = async (item, itemLabel, discoveredVideoId) => {
    if (typeof deps.relocateHydratedImport !== 'function') return;
    if (item.inDownloadRoot === true) return;
    if (!item.videoId && !discoveredVideoId) return;
    try {
      const relocation = await deps.relocateHydratedImport(deps, config, item.mediaId);
      if (relocation && relocation.status === 'moved') {
        moved += 1;
        if (relocation.archived === false) moveArchiveWarnings += 1;
        activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, { moved, moveArchiveWarnings, current: itemLabel });
      } else if (relocation && relocation.status === 'failed') {
        // Honest: the move was attempted and did not happen. The file and its db
        // entry are untouched (see the relocator's contract), so the item is
        // still perfectly usable where it is -- which is why this does NOT also
        // count as a `failed` reheat: the metadata pass, the thing the user
        // actually asked for, succeeded.
        moveFailed += 1;
        console.error(`yt-dlp: repull-metadata could not relocate ${itemLabel}: ${relocation.reason}`);
        activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, { moveFailed, current: itemLabel });
      }
      // `skipped` relocations (not a candidate: local media, no identity,
      // already in a download root, toggle off) are silent by design -- most of a
      // widened, library-wide reheat is exactly that, and counting them would
      // drown the real numbers.
    } catch (relocErr) {
      // A relocation must never wedge, or fail, a metadata reheat that already
      // succeeded for this item.
      moveFailed += 1;
      console.error(`yt-dlp: repull-metadata relocation of ${itemLabel} threw unexpectedly:`, relocErr && relocErr.message);
      activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, { moveFailed, current: itemLabel });
    }
  };
  ...
  try {
    for (const item of (Array.isArray(items) ? items : [])) {
      ...
      if (item.alreadyRepulled && !force) {
        // v1.41.6 -- Dean's ACTUAL library state, and the literal wording of the
        // ask ("if a reheat finds a hydrated file NOT in such a folder, fix
        // it"). Every import v1.41.5's reheat already hydrated carries
        // `metadataRepulledAt`, so it lands HERE, on the skip path, and would
        // never have been relocated at all without this: the user would press
        // Reheat, watch it skip its way through the whole library, and see
        // nothing move. The relocation is deliberately NOT gated on the reheat
        // marker -- it is a separate, idempotent piece of work about WHERE the
        // file lives, not about whether its metadata is fresh.
        //
        // Costs nothing for the items that are not candidates: the pre-filter
        // below is pure, already-computed enumeration data (no db read, no FS,
        // no spawn), and `relocateHydratedImport` is only consulted for a file
        // that is OUTSIDE the download root AND carries a YouTube id -- which is
        // exactly Dean's imports, and never his home videos.
        await maybeRelocate(item, itemLabel, item.videoId);
        skipped += 1;
      ...
      try {
        // v1.49: the whole per-item metadata pass now lives in ONE function
        // this batch SHARES with the per-video route (`reheatOneItem`, above)
        // -- see its header for why a second copy of these rules would be a
        // liability. Everything this loop still owns is batch-specific: the
        // avatar memo's SCOPE (one map across the whole run, so a channel is
        // probed once, not once per item), the running counters, the activity
        // writes, and the relocation.
        //
        // `allowViewCountDecrease` is deliberately absent (i.e. false): the
        // v1.48 monotonicity guard stays exactly as shipped for the batch, and
        // only the explicit single-video force overrides it.
        const outcome = await reheatOneItem(deps, config, item, { avatarMemo: avatarByChannelUrl });

        // v1.53 gate W3: a kept-but-conflicting manual attribution is
        // COUNTED and logged on the batch path (the single-item path toasts
        // it; a library-wide reheat must not swallow N of them silently).
        if (outcome.attributionConflict && outcome.attributionConflict.kept) {
          attributionConflicts += 1;
          console.error(`yt-dlp: reheat kept the manual attribution "${outcome.attributionConflict.kept}" for ${itemLabel}; the source reports "${outcome.attributionConflict.discovered}".`);
          activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, { attributionConflicts, current: itemLabel });
        }
        if (outcome.attemptedPersist) {
          // v1.41.6 (Dean's "make an import indistinguishable from a native
          // download"): the item's identity is now PERSISTED -- so this is the
          // moment its FILE can be relocated into that channel's folder under
          // the download dir.
          //
          // Gated on `outcome.persisted` -- an item that vanished mid-batch
          // (`recordRepulledItemMeta` resolved false, its own safe no-op) must
          // never have its file moved on the strength of a write that did not
          // land.
          //
          // NOTE the ordering consequence: on a successful move the item's
          // mediaId is now STALE (the id is a hash of the path). Nothing below
          // this point in the iteration touches it -- and nothing may be added
          // that does.
          if (outcome.persisted) await maybeRelocate(item, itemLabel, outcome.youtubeId);

          if (outcome.exhausted && !outcome.hasAnythingToPersist) {
            ...
            skipped += 1;
            activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, { skipped, current: itemLabel });
          } else if (outcome.markComplete && outcome.persisted) {
            done += 1;
            activity.setOneShot(REPULL_METADATA_ACTIVITY_ID, { done, current: itemLabel });
          } else {
            // Either the subs pass didn't complete (retryable -- see
            // `markComplete` in reheatOneItem) or the item vanished mid-batch
            // (`persisted === false`, a safe no-op) -- honestly `failed`,
    ...
    repullMetadataJobKind = null;
  }
}

/**
 * v1.49: the fields a reheat can change, lifted off an item for a BEFORE/AFTER
 * diff. Pure, tiny, and deliberately a fixed list rather than a whole-item
 * clone: the diff exists to tell the user what a click actually did, and a
 * whole-item snapshot would both bloat the activity entry and start reporting
 * churn in fields the reheat never touches.
 *
 * Chapters collapse to a COUNT. The list itself can be long, the user only
 * needs to know it was refreshed, and holding two full chapter arrays in an
 * activity entry that lives in memory for the TTL window is waste.
 *
 * @param {object|undefined} item a `db.metadata` entry (or nothing)
 * @returns {{title: string|null, channelName: string|null, sourceViewCount: number|null, releaseDate: number|null, hasSubtitles: boolean, chapterCount: number, youtubeId: string|null}}
 */
function snapshotReheatableFields(item) {
  const it = item && typeof item === 'object' ? item : {};
  const str = (v) => (typeof v === 'string' && v !== '' ? v : null);
  const num = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : null);
  return {
    title: str(it.sourceTitle) || str(it.title),
    channelName: str(it.channelName),
    sourceViewCount: num(it.sourceViewCount),
    releaseDate: num(it.releaseDate),
    hasSubtitles: it.hasSubtitles === true,
    chapterCount: Array.isArray(it.chapters) ? it.chapters.length : 0,
    youtubeId: str(it.youtubeId),
  };
}

/**
 * v1.49: the per-video reheat's background worker -- the single-item sibling of
 * `runRepullMetadataBatch`, sharing its whole metadata pass via `reheatOneItem`.
 *
 * Runs entirely OUTSIDE the HTTP request/response cycle (the route has already
 * answered 202), so like every other background worker in this module it owns a
 * top-level try/finally and must never let a rejection escape.
 *
 * `force` is implicit and unconditional: the user pressed a button on THIS
 * video, so the `alreadyRepulled` skip the batch applies would defeat the whole
 * feature. `allowViewCountDecrease` rides along for the same reason (intake
 * decision 2) -- and ONLY from here.
 *
 * Ends by writing a terminal activity entry carrying (a) the BEFORE/AFTER diff
 * and (b) the relocation PROPOSAL. It never moves the file: see the route's own
 * comment. The proposal is computed AFTER the persist deliberately -- hydration
 * is frequently what makes an item relocatable in the first place (it is the
 * step that gives a MeTube import its channel identity), so computing it first
 * would report "not a candidate" for exactly the items this feature exists for.
 */
async function runSingleItemReheat(deps, config, item, before) {
  const label = item.videoId || path.basename(item.filePath || '');
  // `activity.setOneShot` MERGES a patch into whatever entry already sits under
  // this key, and this key is fixed and only TTL-pruned after minutes -- so a
  // previous per-video reheat's TERMINAL fields survive underneath this
  // 'running' write unless they are explicitly cleared. That is not cosmetic:
  // a client polling this entry would read the PREVIOUS video's `relocation`
  // proposal while the current one is still running, and offer to move a file
  // that has nothing to do with the item on screen. Every result field is
  // therefore reset here, and `mediaId` is stamped from the START (not just on
  // completion) so a poller can always tell whose run it is looking at.
  activity.setOneShot(REPULL_ITEM_ACTIVITY_ID, {
    kind: 'repull-item',
    state: 'running',
    total: 1,
    done: 0,
    ...
    after: null,
    relocation: null,
    // v1.53: reset with its siblings -- setOneShot MERGES, and a stale
    // conflict from a previous run must never ride a new run's entry.
  ...
  try {
    let outcome;
    try {
      outcome = await reheatOneItem(deps, config, item, {
        allowViewCountDecrease: true,
      });
    } catch (err) {
      console.error(`yt-dlp: per-video reheat of ${label} failed:`, err && err.message);
      // GATE FIX (QA SUGGESTION): `outcome` is reset here too, for the same
      // reason the 'running' write resets before/after/relocation -- setOneShot
      // MERGES, and leaving a prior run's `outcome: 'done'` sitting under an
      // `error` state is a contradiction waiting for the first consumer that
      // reads the two fields in the other order. Harmless for today's client,
      // which checks `state` first; that is luck, not design.
      activity.setOneShot(REPULL_ITEM_ACTIVITY_ID, { state: 'error', outcome: 'failed', failed: 1, current: null });
      return;
    }

    // The AFTER snapshot comes from a FRESH read, never from what the pass
    // believed it wrote: `recordRepulledItemMeta` has guards of its own (the
    // never-overwrite channel identity, the view-count validator) that can
    // decline a value the pass produced, and a diff built from the pass's
    // intent rather than the db's reality would tell the user their channel
    // was re-pointed when the guard correctly refused to.
    let after = null;
    try {
      const freshDb = deps.loadDatabase();
      after = snapshotReheatableFields(freshDb.metadata && freshDb.metadata[item.mediaId]);
    } catch (err) {
      console.error('yt-dlp: per-video reheat could not re-read the database for its diff:', err && err.message);
    }

    // The relocation PROPOSAL -- read-only, no file touched (see
    // `planImportRelocation`'s "PURE + READ-ONLY" contract). Absent from a
    // minimal/stubbed deps bundle => simply no proposal, and the metadata half
    // is unaffected.
    let relocation = { available: false, reason: 'unavailable' };
    if (typeof deps.planImportRelocation === 'function') {
      try {
        // v1.49 GATE FIX (adversarial CRITICAL 1): `allowRecentlyWatched`. The
        // watch page streams the video on mount, which marks the file
        // live-watched for ten minutes -- so without this the proposal ALWAYS
        // came back 'recently-watched' and the confirm dialog could never open
        // from the one entry point this feature has. See the clause's own
        // comment in server.js's `planImportRelocation` for why lifting it is
        // correct for an attended per-video action and wrong for the batch.
        const plan = deps.planImportRelocation(deps, config, item.mediaId, null, { allowRecentlyWatched: true });
        relocation = (plan && plan.action === 'move')
          ? {
            available: true,
          ...
          : { available: false, reason: (plan && plan.reason) || 'not-a-candidate' };
      } catch (err) {
        console.error('yt-dlp: per-video reheat could not compute a relocation proposal:', err && err.message);
        relocation = { available: false, reason: 'plan-failed' };
      }
    }

    // Honest outcome bucketing, the SAME three buckets and the same rules the
    // batch uses (see `runRepullMetadataBatch`'s loop): an item that yielded
    // nothing is `skipped`, never `done` -- a button that reports success for
    // fetching nothing is the kind of lie this repo's release notes exist to
    // prevent.
    let done = 0;
    let skipped = 0;
    let failed = 0;
    if (!outcome.attemptedPersist) {
      failed = 1;
    } else if (outcome.exhausted && !outcome.hasAnythingToPersist) {
      skipped = 1;
    } else if (outcome.markComplete && outcome.persisted) {
      done = 1;
    } else {
    ...
    activity.setOneShot(REPULL_ITEM_ACTIVITY_ID, {
      // `state` is the LIFECYCLE marker the shared activity surface reads (it
      // drives TERMINAL_STATES/TTL pruning), and it stays 'done' for "this job
      // finished" exactly like the batch's own terminal write -- 'error' is
      // reserved for the job THROWING, not for an item that legitimately came
      // back empty or incomplete.
      state: 'done',
      // ...which is precisely why the per-item RESULT needs its own field. A
      // single-video reheat has one outcome, and a client that inferred success
      // from `state: 'done'` would cheerfully report "Reheated." for an item
      // that actually failed. The counters below say the same thing; this is
      // the one a caller should branch on.
      outcome: failed ? 'failed' : (skipped ? 'skipped' : 'done'),
      done,
      skipped,
      failed,
      current: null,
      mediaId: item.mediaId,
      // `networkRan: false` is the honest signal for decision 3's case -- a
      // genuine home video with no source link anywhere. The client turns
      // it into "No source link found for this video, so there was nothing to refresh" rather than a
      // success toast for work that never happened.
      networkRan: outcome.networkRan,
      before,
      after,
      relocation,
      // v1.53 (Dean's decision 3): a manual attribution that declined a
      // CONFLICTING network identity -- the client toasts it specifically;
      // without this the diff reads "everything was already up to date",
      // which is actively false.
      attributionConflict: outcome.attributionConflict || null,
    });
  } finally {
    repullMetadataInProgress = false;
    repullMetadataJobKind = null;
...
function resetRepullMetadataStateForTests() {
  ...
  // v1.49: the third piece of this singleton's state. Leaving it set would
  // leak a prior test's job kind into the next file sharing this module and
  // silently change the cancel route's answer.
  repullMetadataJobKind = null;
}
...
// v1.116 (Dean): the name backfill + local heal cover downloaded MEDIA -- video
// AND audio (the music library, where the NESTALGIA-class fragmentation lives).
// Other types (books/podcasts) carry their own identity model and no yt-dlp
// channel handle. ONE predicate shared by every enumerator/writer so the covered
// set can never drift.
function isChannelBearingMediaType(item) {
  return !!item && (item.type === 'video' || item.type === 'audio');
}

// v1.115 (Dean, A1): a channelName is "bad" (shows a handle/folder instead of the
// real name) when it is EMPTY (the card falls back to the folder) or itself an
// "@handle". Pure; the SINGLE source of truth the enumerator + writer + tests
// share, so "which names get backfilled" can never drift between them.
function isBadChannelName(channelName) {
  const n = typeof channelName === 'string' ? channelName.trim() : '';
  return n === '' || n.charAt(0) === '@';
}

// v1.115 (Dean, A1): the name backfill's UNIT OF WORK -- one entry per DISTINCT
// channel that has at least one BAD-NAME, NON-manually-attributed item carrying a
// channel identity (channelId, else channelUrl/handleUrl) to probe. Dedups by
// channelId (canonical) then channelUrl. Pure, never throws, returns a NEW array,
// never mutates db. A channel with only manually-attributed or already-good-name
// items yields NO target (nothing to fix); an item with no identity at all yields
// none (nothing to probe -- the honest ceiling).
function collectDistinctChannelNameTargets(db) {
  if (!db || typeof db !== 'object' || !db.metadata || typeof db.metadata !== 'object') return [];
  const targets = [];
  const seenIds = new Set();
  const seenUrls = new Set();
  for (const item of Object.values(db.metadata)) {
    if (!isChannelBearingMediaType(item)) continue; // v1.116: video + audio (music)
    if (item.channelAttributedManually) continue; // attribution wins -- never backfill over it
    if (!isBadChannelName(item.channelName)) continue; // already shows a real name
    const channelId = typeof item.channelId === 'string' && item.channelId !== '' ? item.channelId : null;
    const channelUrl = typeof item.channelUrl === 'string' && item.channelUrl !== ''
      ? item.channelUrl
      : (typeof item.channelHandleUrl === 'string' && item.channelHandleUrl !== '' ? item.channelHandleUrl : null);
    if (!channelId && !channelUrl) continue; // no identity -> unprobeable (needs a re-download)
    if (channelId && seenIds.has(channelId)) continue;
    if (!channelId && channelUrl && seenUrls.has(channelUrl)) continue;
    if (channelId) seenIds.add(channelId);
    if (channelUrl) seenUrls.add(channelUrl);
    targets.push({ channelId, channelUrl });
  }
  return targets;
}

// v1.115 (Dean, A1): apply a probed canonical name onto EVERY video item of the
// target channel that still shows a BAD name and is NOT manually attributed.
// MUTATES the passed `metadata` map in place (the caller runs this INSIDE
// `deps.updateDatabase`'s serialized mutator, holding the lock). Returns the
// count written. GUARDS (all load-bearing -- the data-safety gate destroys these):
//   - match on channelId when the target has one (never cross channel A's name
//     onto channel B's item); else the channelUrl/handleUrl.
//   - NEVER overwrite a manually-attributed item, NEVER overwrite an
//     already-good name (only a bad one -- so re-running is idempotent and a
//     later good capture is never clobbered).
//   - bound the write to 200 chars (a name is a title, not a payload) and reject
//     a blank/whitespace probed name (no-op, count 0).
function applyBackfilledChannelName(metadata, target, probedName) {
  if (!metadata || typeof metadata !== 'object') return 0;
  if (!target || (!target.channelId && !target.channelUrl)) return 0;
  // v1.115 gate fix (both seats): strip control chars/NUL at the WRITE point too
  // (defense-in-depth over the probe's own strip), matching every other
  // channelName writer's "never persist a raw control char" posture. Strip
  // first, then a strip-to-empty name is a no-op (never writes a blank).
  // eslint-disable-next-line no-control-regex
  const name = typeof probedName === 'string' ? probedName.replace(/[\x00-\x1f\x7f]/g, '').trim() : '';
  if (name === '') return 0;
  const bounded = name.slice(0, 200);
  let written = 0;
  for (const item of Object.values(metadata)) {
    if (!isChannelBearingMediaType(item)) continue; // v1.116: video + audio (music)
    if (item.channelAttributedManually) continue;
    if (!isBadChannelName(item.channelName)) continue; // only ever replace a bad name
    const matches = target.channelId
      ? item.channelId === target.channelId
      : (!!target.channelUrl && (item.channelUrl === target.channelUrl || item.channelHandleUrl === target.channelUrl));
    if (!matches) continue;
    item.channelName = bounded;
    written++;
  }
  return written;
}

// v1.116 (Dean): LOCAL channel-identity reconciliation. Some channels are
// FRAGMENTED across their own folder: newer pulls captured the real channelId +
// name ("NESTALGIA" / UC-6oT0…), older ones only the "@handle" with a null
// channelId. The item that carries the channelId is LOCAL GROUND TRUTH -- heal
// the fragments from it, no network. Bucketed by the physical channel FOLDER
// (dirname of filePath, never the basename -- a same-basename folder in another
// root would collide, the exact v1.115 pin lesson).
//
// A bucket yields ONE target IFF it has EXACTLY ONE canonical member (non-empty
// channelId AND a non-bad name) and at least one bad, non-manual item. A bucket
// with >1 distinct canonical channelId is a CONFLICT (a junk-drawer folder like
// "Miscellaneous from SLS" holding 12 channels) -> skipped wholesale, never
// guessed. Pure, never throws, never mutates db. Returns a NEW array.
function folderKeyOf(item) {
  const fp = item && typeof item.filePath === 'string' ? item.filePath : '';
  return fp !== '' ? path.dirname(fp) : null;
}
function collectLocalChannelHealTargets(db) {
  if (!db || typeof db !== 'object' || !db.metadata || typeof db.metadata !== 'object') return [];
  const buckets = new Map(); // folderKey -> { canon: Map<channelId, identity>, urls:Set, badCount }
  for (const item of Object.values(db.metadata)) {
    if (!isChannelBearingMediaType(item)) continue;
    const key = folderKeyOf(item);
    if (!key) continue;
    let b = buckets.get(key);
    if (!b) { b = { canon: new Map(), urls: new Set(), badCount: 0 }; buckets.set(key, b); }
    const channelId = typeof item.channelId === 'string' && item.channelId !== '' ? item.channelId : null;
    if (channelId && !isBadChannelName(item.channelName)) {
      // A canonical member: real id + real name. Snapshot its identity UNIT and
      // the urls it is known by (so a fragment's url can be corroborated).
      if (!b.canon.has(channelId)) {
        b.canon.set(channelId, {
          channelId,
          channelName: item.channelName,
          channelUrl: typeof item.channelUrl === 'string' && item.channelUrl !== '' ? item.channelUrl : null,
          channelHandleUrl: typeof item.channelHandleUrl === 'string' && item.channelHandleUrl !== '' ? item.channelHandleUrl : null,
          channelAvatarUrl: typeof item.channelAvatarUrl === 'string' && item.channelAvatarUrl !== '' ? item.channelAvatarUrl : null,
        });
      }
      const c = b.canon.get(channelId);
      if (c.channelUrl) b.urls.add(c.channelUrl);
      if (c.channelHandleUrl) b.urls.add(c.channelHandleUrl);
    } else if (!item.channelAttributedManually && isBadChannelName(item.channelName)) {
      b.badCount += 1;
    }
  }
  const targets = [];
  for (const [folderKey, b] of buckets) {
    if (b.canon.size !== 1 || b.badCount === 0) continue; // no single truth, or nothing to fix
    const identity = [...b.canon.values()][0];
    targets.push({ folderKey, identity, urls: b.urls });
  }
  return targets;
}

// v1.116: apply a local-heal target onto EVERY bad, non-manual sibling in the
// target's folder. Corroboration: heal an item only if it has NO channel url at
// all OR one of its urls is in the canonical url set -- never adopt the
// canonical identity onto a foreign channel's item that merely shares the folder.
// Adopts the identity as a UNIT (id/name/url/handleUrl together, never mixed),
// and the avatar only when the canonical has one and the item lacks one. MUTATES
// `metadata` in place (caller holds the updateDatabase lock). Returns count.
function applyLocalChannelHeal(metadata, target) {
  if (!metadata || typeof metadata !== 'object') return 0;
  if (!target || !target.folderKey || !target.identity || !target.identity.channelId) return 0;
  const id = target.identity;
  // eslint-disable-next-line no-control-regex
  const name = typeof id.channelName === 'string' ? id.channelName.replace(/[\x00-\x1f\x7f]/g, '').trim().slice(0, 200) : '';
  if (name === '' || isBadChannelName(name)) return 0; // never heal TO a bad name
  const urls = target.urls instanceof Set ? target.urls : new Set();
  let written = 0;
  for (const item of Object.values(metadata)) {
    if (!isChannelBearingMediaType(item)) continue;
    if (item.channelAttributedManually) continue;
    if (!isBadChannelName(item.channelName)) continue; // only bad-name fragments
    if (folderKeyOf(item) !== target.folderKey) continue;
    const itemUrls = [item.channelUrl, item.channelHandleUrl].filter(u => typeof u === 'string' && u !== '');
    const corroborated = itemUrls.length === 0 || itemUrls.some(u => urls.has(u));
    if (!corroborated) continue; // a foreign-handle item sharing the folder -- skip
    item.channelName = name;
    item.channelId = id.channelId;
    if (id.channelUrl) item.channelUrl = id.channelUrl;
    if (id.channelHandleUrl) item.channelHandleUrl = id.channelHandleUrl;
    if (id.channelAvatarUrl && (typeof item.channelAvatarUrl !== 'string' || item.channelAvatarUrl === '')) {
      item.channelAvatarUrl = id.channelAvatarUrl;
    }
    written++;
  }
  return written;
...
async function runRefreshAvatarsBatch(deps, config, targets) {
  ...
  try {
    for (const target of list) {
      ...
      // v1.142 (Dean's one-off avatar gap): a target with a bare `UC…`
      // channelId but no captured channelUrl is PROBEABLE now - the canonical
      // channel URL derives from the id (store.deriveChannelUrlFromId; also
      // wired into ensureChannelAvatar itself). Only a target with NEITHER a
      // URL nor a derivable id is truly unprobeable and counted `skipped`.
      const targetProbeUrl = (target && typeof target.channelUrl === 'string' && target.channelUrl !== '')
        ? target.channelUrl
        : store.deriveChannelUrlFromId(target && target.channelId);
      if (!targetProbeUrl) {
        skipped += 1;
      ...
      try {
        ...
        const probed = await ensureChannelAvatar(deps, config, { channelId: target.channelId, channelUrl: targetProbeUrl }, { force: true });
...
// ---- v1.56 (Dean's bulk subscriber-count reheat) ---------------------------
//
// "Reheat sub counts": a single, user-triggered batch that re-probes EVERY
// distinct channel's subscriber count and fans it out to every library item
// attributed to that channel -- the refresh-avatars batch's shape verbatim
// (module-level single-flight latch, durable cooperative-cancel, activity
// one-shot progress, same distinct-channel unit of work via
// `collectDistinctChannelAvatarTargets`), with a different per-target action:
// `run.probeChannelFollowerCount` (one cheap `--playlist-items 0` channel
// dump) followed by `deps.recordChannelFollowerCountFanout` (server.js's ONE
// channel->items writer, deps-injected like `recordRepulledItemMeta`).
// Before this batch the only refresh paths were per-VIDEO (download capture,
// the per-video flame reheat, the library-wide repull) -- one full metadata
// dump per video; catching a whole channel up meant reheating its videos one
// at a time.
let reheatSubsInProgress = false;
let reheatSubsCancelled = false;
const REHEAT_SUBS_ACTIVITY_ID = 'reheat-subs';

/**
 * The reheat-subs batch's background worker: iterates a `targets` snapshot
 * (`collectDistinctChannelAvatarTargets`'s output, captured synchronously by
 * the route below before ever responding) ONE AT A TIME. Each probe goes
 * through the SAME shared `runExclusive` FIFO gate every other spawn in this
 * module serializes against (NFR2): this batch can never storm the network,
 * and never runs a probe concurrently with a poll/one-shot/reheat/avatar
 * spawn.
 *
 * Per-target accounting (the refresh-avatars vocabulary, one addition):
 * - no `channelUrl` at all -> `skipped` (no channel identity to probe).
 * - probe yields a usable count -> `done`, and the fan-out's stamped-item
 *   count accumulates into `itemsUpdated` (additive activity field -- the
 *   client label can say "N channels / M videos"). A probe success whose
 *   fan-out matches 0 items is still `done`: a subscribed channel with no
 *   downloaded items yet is legitimate, not a failure.
 * - probe yields nothing usable (`null` -- non-YouTube extractors that
 *   report no follower count land here by design, DISCLOSED in the release
 *   notes) or throws -> `failed`; existing counts for that channel stay
 *   exactly as they were (the fan-out never runs).
 *
 * One bad target never wedges the batch (per-item try/catch, the identical
 * defense-in-depth posture as `runRefreshAvatarsBatch`). The cancel latch is
 * checked at the TOP of every iteration: an item mid-flight runs to
 * completion, only the NEXT one is ever skipped.
 *
 * NEVER triggers a scan, NEVER re-keys anything, and NEVER auto-runs (the
 * route below is its only call site -- structurally locked by the endpoint
 * test, mirroring refresh-avatars' own).
 * @param {object} deps the SAME deps object `registerRoutes` received
 *   (`recordChannelFollowerCountFanout` rides on it)
 * @param {object} config parsed yt-dlp config
 * @param {Array<{channelId: string|null, channelUrl: string|null, subId: string|null}>} targets
 */
async function runReheatSubsBatch(deps, config, targets) {
  const list = Array.isArray(targets) ? targets : [];
  const total = list.length;
  let done = 0;
  let skipped = 0;
  let failed = 0;
  let itemsUpdated = 0;
  // Gate fix (adversarial S2, repro'd): enumeration can yield the SAME real
  // channel twice while a subscription's channelId is still unpopulated
  // (pre-first-poll) and its handle URL differs from an item's canonical URL
  // -- the collector's idless-target upgrade keys on URL equality and never
  // fires across the two spellings. The probes themselves resolve the
  // canonical channel_id, so dedupe HERE, on what the probe returned: a
  // second target resolving to an already-fanned-out channelId still counts
  // `done` (the target was genuinely probed and is fresh) but skips the
  // duplicate fan-out, keeping `itemsUpdated` an honest count of distinct
  // stamped videos. The enumeration-level double-probe itself is inherited
  // verbatim from refresh-avatars (since v1.25) and tracked in
  // docs/exec-plans/tech-debt-tracker.md.
  const fannedOutChannelIds = new Set();

  activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, {
    kind: 'reheat-subs',
    state: 'running',
    total,
    done,
  ...
  try {
    for (const target of list) {
      if (reheatSubsCancelled) {
        activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { state: 'cancelled', current: null });
        return;
      }
      ...
      if (!target || typeof target.channelUrl !== 'string' || target.channelUrl === '') {
        skipped += 1;
        activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { skipped, current: label });
        continue;
      }

      activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { current: label });
      try {
        const probed = await runExclusive(() => run.probeChannelFollowerCount(target.channelUrl, config));
        if (probed && typeof deps.recordChannelFollowerCountFanout === 'function') {
          const alreadyFannedOut = typeof probed.channelId === 'string' && probed.channelId !== ''
            && fannedOutChannelIds.has(probed.channelId);
          if (!alreadyFannedOut) {
            itemsUpdated += await deps.recordChannelFollowerCountFanout(deps, target, probed);
            if (typeof probed.channelId === 'string' && probed.channelId !== '') fannedOutChannelIds.add(probed.channelId);
          }
          done += 1;
          activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { done, itemsUpdated, current: label });
        } else {
          failed += 1;
          activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { failed, current: label });
        }
      } catch (err) {
        failed += 1;
        console.error(`yt-dlp: reheat-subs item ${label} failed unexpectedly:`, err && err.message);
        activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { failed, current: label });
      }
    }

    activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { state: 'done', current: null });
  } finally {
    // Cleared unconditionally once the batch has fully run its course --
    // mirroring runRefreshAvatarsBatch's identical finally.
    reheatSubsInProgress = false;
  }
}

/**
 * Test-only: reset the reheat-subs batch's module-level singleton state back
 * to its initial idle values, mirroring `resetRefreshAvatarsStateForTests`
 * immediately above.
 */
function resetReheatSubsStateForTests() {
  reheatSubsInProgress = false;
  reheatSubsCancelled = false;
}

// ---- v1.115 (Dean, A1): per-channel NAME backfill --------------------------
// The reheat-subs batch's shape VERBATIM (module-level single-flight latch,
// durable cooperative-cancel, activity one-shot progress), with a different
// unit of work and per-target action: `collectDistinctChannelNameTargets` (only
// channels with a bad-name item to fix) -> probe each via `ensureChannelAvatar`
// (which returns the canonical NAME the extended probe now surfaces, AND
// refreshes the avatar as a bonus) -> `deps.recordChannelNameBackfillFanout`
// (server.js's ONE channel->items name writer + pin-label refresh).
let channelNameBackfillInProgress = false;
let channelNameBackfillCancelled = false;
const CHANNEL_NAME_BACKFILL_ACTIVITY_ID = 'backfill-channel-names';

async function runChannelNameBackfillBatch(deps, config, initialTargets) {
  let done = 0;
  let skipped = 0;
  let failed = 0;
  let itemsUpdated = 0;
  let healedItems = 0;
  let healedChannels = 0;
  // v1.115 gate fix (adversarial SUGGESTION-2): NO fan-out dedup on channelId.
  // Two targets can resolve to the same real channel via DIFFERENT url spellings
  // (a /@handle vs a /channel/UC… target), and each keys a DIFFERENT set of items
  // -- deduping on the probe's channelId would skip the second target's items,
  // leaving them bad-name. applyBackfilledChannelName is IDEMPOTENT (only ever
  // replaces a bad name), so re-fanning a channel already handled is a cheap
  // no-op and itemsUpdated stays an honest count of DISTINCT items renamed.

  activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, {
    kind: 'backfill-channel-names',
    state: 'running',
    phase: 'heal',
    total: 0,
    done,
  ...
  try {
    // Phase 1 (v1.116): LOCAL identity reconciliation -- instant, no network.
    // Heal every folder that has a single canonical channelId sibling BEFORE
    // probing, so the network phase only re-probes what is STILL bad. Uses the
    // library's own ground truth (a same-folder item that already carries the
    // real id+name), so it fixes even channels with NO probeable url.
    let healDb = null;
    try { healDb = typeof deps.loadDatabase === 'function' ? deps.loadDatabase() : null; } catch { healDb = null; }
    for (const ht of (healDb ? collectLocalChannelHealTargets(healDb) : [])) {
      if (channelNameBackfillCancelled) {
        activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { state: 'cancelled', current: null });
        return;
      }
      if (typeof deps.recordLocalChannelHealFanout !== 'function') break;
      let n = 0;
      try { n = await deps.recordLocalChannelHealFanout(deps, ht); }
      catch (err) { console.error('yt-dlp: local channel heal failed:', err && err.message); n = 0; }
      if (n > 0) { healedItems += n; itemsUpdated += n; healedChannels += 1; }
      activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, {
        itemsUpdated, healedItems, healedChannels,
        current: (ht.identity && ht.identity.channelName) || null,
      });
    }

    // Phase 2: NETWORK probe for whatever is STILL bad (recompute on the healed
    // db so a locally-healed channel is not needlessly re-probed).
    let list;
    try {
      const freshDb = typeof deps.loadDatabase === 'function' ? deps.loadDatabase() : null;
      list = freshDb ? collectDistinctChannelNameTargets(freshDb) : (Array.isArray(initialTargets) ? initialTargets : []);
    } catch { list = Array.isArray(initialTargets) ? initialTargets : []; }
    const total = list.length;
    activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { phase: 'probe', total, current: null });
    ...
    for (const target of list) {
      ...
      const label = (target && (target.channelId || target.channelUrl)) || null;
      // Derive a probeable channel URL: the target's own url, else the canonical
      // /channel/<id> form from its channelId (an item enumerated by channelId
      // alone still gets probed).
      const channelUrl = target && typeof target.channelUrl === 'string' && target.channelUrl !== ''
        ? target.channelUrl
        : (target && typeof target.channelId === 'string' && target.channelId !== '' ? `https://www.youtube.com/channel/${target.channelId}` : null);
      if (!channelUrl) {
        skipped += 1;
        activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { skipped, current: label });
        continue;
      }
      activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { current: label });
      try {
        // ensureChannelAvatar shares the SAME shared FIFO gate (NFR2); force:true
        // -- this is the user's explicit "fix names now" action.
        const probed = await ensureChannelAvatar(deps, config, { channelId: target.channelId, channelUrl }, { force: true });
        if (probed && typeof probed.channelName === 'string' && probed.channelName !== '' && typeof deps.recordChannelNameBackfillFanout === 'function') {
          // No channelId dedup (see the note at itemsUpdated's declaration):
          // applyBackfilledChannelName is idempotent, so always fan out -- a
          // divergent-url same-channel target keeps its OWN items covered.
          itemsUpdated += await deps.recordChannelNameBackfillFanout(deps, target, probed);
          done += 1;
          activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { done, itemsUpdated, current: label });
        } else {
          // Probed but no usable name (private/removed channel, or the probe
          // resolved nothing) -- counted failed, same as the reheat batches.
          failed += 1;
          activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { failed, current: label });
        }
      } catch (err) {
        failed += 1;
        console.error(`yt-dlp: backfill-channel-names item ${label} failed unexpectedly:`, err && err.message);
        activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { failed, current: label });
      }
    }

    activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { state: 'done', current: null });
  } finally {
    channelNameBackfillInProgress = false;
  }
}

function resetChannelNameBackfillStateForTests() {
  channelNameBackfillInProgress = false;
  channelNameBackfillCancelled = false;
}
 ...
 * `lastStatus` is what gets PERSISTED (the subscription record) and served back verbatim via
...
// v1.37.5 (Dean's "Skip" action): the skip-list twin of `readArchiveTextSafely`
// above. Missing/unreadable (nothing skipped yet, or a permissions hiccup) is
// treated as "nothing on the skip list" -- the poll proceeds normally,
// identical to the archive-read contract. The file shares the archive's
// `<extractor> <id>` line format, so `rules.isArchived` doubles as the
// membership check against this text (a generic "(extractor, id) present in
// this text" predicate -- see the survivor loop and the skip route).
function readSkiplistTextSafely(config) {
  try {
    return fs.readFileSync(args.resolveSkiplistPath(config), 'utf8');
  } catch {
    return '';
  }
}

// v1.37.5 (Dean's "Skip" action): append `videoId` to the permanent skip list
// so no future subscription poll re-attempts it. Idempotent (a second skip of
// the same id is a no-op, via the SAME `rules.isArchived` membership check the
// survivor loop uses). UNLIKE `recordOneShotInArchive`, this DOES surface a
// failure to its caller (throws): a user explicitly clicked Skip and is owed
// an honest "it didn't stick" rather than a silent success. Returns true when
// a new line was written, false when the id was already on the list.
function recordSkip(config, videoId) {
  const skiplistPath = args.resolveSkiplistPath(config);
  const existingText = readSkiplistTextSafely(config);
  if (rules.isArchived(existingText, 'youtube', videoId)) return false; // already skipped -- idempotent
  fs.mkdirSync(path.dirname(skiplistPath), { recursive: true });
  const needsLeadingNewline = existingText.length > 0 && !existingText.endsWith('\n');
  fs.appendFileSync(skiplistPath, `${needsLeadingNewline ? '\n' : ''}youtube ${videoId}\n`, 'utf8');
  return true;
}
 ...
 */
// v1.338 (plan first-class-any-site D2): the fallback for the saved page link. yt-dlp prints
// `webpage_url` for a universal capture; if it printed none, the job's own validated source URL stands
// in (the universal lane is ONE item - `--no-playlist --playlist-items 1` - so it is that item's page).
// Only a UNIVERSAL capture (one with an extractor `source`) is touched; the value is still sanitized in
// store.sanitizeCapturedChannelMeta. Pure.
function withUniversalPageUrl(channelMeta, universalSourceUrl) {
  if (!Array.isArray(channelMeta)) return channelMeta;
  if (typeof universalSourceUrl !== 'string' || universalSourceUrl === '') return channelMeta;
  return channelMeta.map((m) => ((m && typeof m === 'object' && typeof m.source === 'string' && m.source !== '' && !m.webpageUrl)
    ? { ...m, webpageUrl: universalSourceUrl }
    : m));
}
 ...
 *   NEITHER a `channelUrl` nor a derivable `UC…` `channelId` present
 *   (v1.142: a bare valid channelId now derives the canonical channel URL
 *   instead of bailing - see the probeUrl derivation below) / an
 *   unexpected error.
 */
async function ensureChannelAvatar(deps, config, target, options) {
  ...
  try {
    if (!force && typeof channelId === 'string' && channelId !== '') {
      const db = ytdlpHolder(deps, ['channelAvatars']);
      if (store.hasFreshChannelAvatar(db, channelId, store.AVATAR_TTL_MS)) return null; // already fresh -- skip the probe entirely
    }
    // v1.142 (Dean's one-off avatar gap): a target with a KNOWN `UC…`
    // channelId but no captured channelUrl used to bail right here - which
    // silently made every bare-id channel (a one-off whose FTCHMETA capture
    // carried the id but not the URL, an item-derived refresh-batch target)
    // permanently unprobeable. The id alone determines the canonical channel
    // endpoint, so derive it (store.deriveChannelUrlFromId - pattern-gated,
    // never built from unvalidated input) and probe that. This is THE shared
    // choke point, so every populate call site inherits the fix at once.
    const probeUrl = (typeof channelUrl === 'string' && channelUrl !== '')
      ? channelUrl
      : store.deriveChannelUrlFromId(channelId);
    if (!probeUrl) return null;
    const probed = await runExclusive(() => run.probeChannelAvatar(probeUrl, config));
    ...
    if (resolvedChannelId && probed.avatarUrl) {
      await store.registerChannelAvatar(deps, {
        ...
        channelUrl: probed.channelUrl || probeUrl,
      });
    }
    return probed;
  } catch (err) {
    console.error(`yt-dlp: ensureChannelAvatar probe failed unexpectedly for ${channelUrl || channelId} (continuing):`, err && err.message);
    return null;
  }
}

// v1.142 (Dean's one-off avatar gap): in-process memo of ITEM-derived
// channels the automatic sweep below has already attempted this server run --
// keyed by channelId (falling back to channelUrl). A probe that fails leaves
// NO registry entry, so without this memo the sweep would re-attempt the
// same permanently-unprobeable channel on EVERY poll cycle forever. One
// attempt per channel per process lifetime; a server restart (or the user's
// explicit "Refresh channel avatars" button, which bypasses this sweep
// entirely) retries. Bounded by the number of distinct channels in the
// library - the same magnitude the registry's own MAX_CHANNEL_AVATARS cap
// already accepts.
const itemAvatarSweepAttempted = new Set();

// Test hook mirroring resetPollRerunStateForTests/resetRefreshAvatarsStateForTests:
// clears the sweep's in-process attempted-memo so runPoll-driving tests are
// isolated from one another (gate S2, v1.142 fix round). Never called in
// production.
function resetItemAvatarSweepStateForTests() {
  itemAvatarSweepAttempted.clear();
}

/**
 * v1.142 (Dean's one-off avatar gap): the AUTOMATIC self-heal for channels
 * known only from library ITEMS - the fourth avatar-populate limb, closing
 * the loop the other three leave open: the subscription poll self-heal
 * (processSubscription) only covers SUBSCRIBED channels, the one-off fold
 * only fires at download time (a probe that failed then never retried), and
 * the refresh batch needs an explicit button press. This sweep runs at the
 * tail of every poll cycle and spends whatever `avatarBudget` the
 * subscription self-heal left over (never its own budget - the
 * AVATAR_SELFHEAL_PER_POLL contract stays "at most N probes per poll cycle,
 * total") probing item-derived channels that lack a fresh registry entry.
 *
 * Targets come from the SAME pure helper the refresh batch uses
 * (`collectDistinctChannelAvatarTargets`), filtered to item-only entries
 * (`subId === null` - subscription entries are the poll self-heal's job).
 * Each probe routes through `ensureChannelAvatar` NON-forced - the freshness
 * gate's first real production caller - so a channel that gained a registry
 * entry between snapshot and probe costs nothing. Runs OUTSIDE the
 * per-channel `runExclusive` jobs (ensureChannelAvatar takes the gate per
 * probe itself); `runPoll`'s own overlap guard means no concurrent sweep.
 * Never throws (the "one bad thing never wedges the rest" posture).
 * @param {object} deps `{ updateDatabase, loadDatabase }`
 * @param {object} config parsed yt-dlp config
 * @param {{remaining: number}} avatarBudget the poll cycle's shared budget
 */
async function sweepItemChannelAvatars(deps, config, avatarBudget) {
  try {
    if (!avatarBudget || avatarBudget.remaining <= 0) return;
    const db = withYtdlp(deps, deps.loadDatabase());
    const targets = collectDistinctChannelAvatarTargets(db).filter((t) => t && t.subId === null);
    for (const target of targets) {
      if (avatarBudget.remaining <= 0) return;
      const key = target.channelId || target.channelUrl;
      if (!key || itemAvatarSweepAttempted.has(key)) continue;
      if (target.channelId && store.hasFreshChannelAvatar(db, target.channelId, store.AVATAR_TTL_MS)) continue;
      const probeUrl = target.channelUrl || store.deriveChannelUrlFromId(target.channelId);
      if (!probeUrl) continue;
      // Budget counts probe ATTEMPTS, decremented at selection time - the
      // same contract as processSubscription's own self-heal.
      itemAvatarSweepAttempted.add(key);
      avatarBudget.remaining -= 1;
      await ensureChannelAvatar(deps, config, { channelId: target.channelId, channelUrl: probeUrl });
    }
  } catch (err) {
    console.error('yt-dlp: item-channel avatar sweep failed (continuing):', err && err.message);
  }
}
...
async function runSubscriptionCycle(deps, config, sub) {
  ...
  // v1.37.5 (Dean's "Skip" action): the permanent per-video skip list, loaded
  // once per cycle exactly like `archiveText`. A skipped id is filtered out of
  // the survivor set below so it is never re-attempted by ANY channel poll.
  const skiplistText = readSkiplistTextSafely(config);
  ...
  // v1.188 (Dean, diagnostic): the survivor loop had THREE silent drop paths
  // (archive dedup, the date gate, premiere-defer) that removed a video with no
  // log line at all - so a subscription that "succeeds" but downloads nothing
  // was a black box (his re-pull of a channel silently skipped a 5-day-old
  // upload). These counters + the one-line summary logged after the loop make
  // every drop reason visible without flooding: the per-video skip logs already
  // present (skip-list / availability / shorts / unsafe-id) stay, the two
  // silent-but-rare paths (before-cutoff, premiere-defer) gain a per-video line,
  // and the archive drop - the common, high-volume case - is COUNTED only (never
  // per-video, to avoid flooding a poll of a fully-downloaded channel).
  const dropCounts = { archived: 0, beforeCutoff: 0, skipList: 0, availability: 0, premiere: 0, shorts: 0, unsafeId: 0, duplicate: 0 };
  const listedCount = videos.length;
  for (const video of videos) {
    ...
    if (id && rules.isArchived(archiveText, extractor, id)) { dropCounts.archived += 1; continue; }
    // v1.37.5 (Dean's "Skip" action): a user permanently skipped this video
    // from a failed-download row -- drop it from the survivor set so no poll
    // re-attempts it. Same `<extractor> <id>` line format as the archive, so
    // `rules.isArchived` is the membership check against the skip-list text too.
    if (id && rules.isArchived(skiplistText, extractor, id)) {
      console.log(`yt-dlp: skipping video ${id} for subscription ${sub.id}: on the permanent skip list`);
      dropCounts.skipList += 1;
      continue;
    }
    ...
    if (rules.isBeforeCutoff(video, sub && sub.cutoffDate)) {
      dropCounts.beforeCutoff += 1;
      // v1.188 diagnostic: the date gate is the prime SILENT suspect for "a
      // recent upload never downloaded" (the cutoff advances to today on every
      // successful poll). Log the video's OWN upload_date against the cutoff.
      console.log(`yt-dlp: skipping video ${id || '(unknown id)'} for subscription ${sub.id}: uploaded ${video && video.upload_date} is before the cutoff ${sub && sub.cutoffDate}`);
      continue;
    }
    ...
    if (decision.skip) {
      ...
      dropCounts.availability += 1;
      continue;
    }
    if (rules.shouldDeferPremiere(video, Date.now())) {
      dropCounts.premiere += 1;
      // v1.188 diagnostic: previously silent - a premiere/live inside its defer
      // window looks identical to "nothing new" until the window elapses.
      console.log(`yt-dlp: deferring video ${id || '(unknown id)'} for subscription ${sub.id}: premiere/live still inside its defer window`);
      continue;
    }
    ...
    if (!id || !url.isSafeVideoId(id)) {
      ...
      dropCounts.unsafeId += 1;
      continue;
    }
    if (seenSurvivorIds.has(id)) { dropCounts.duplicate += 1; continue; } // D5: de-dupe -- already accepted this id this cycle
    seenSurvivorIds.add(id);
    survivorIds.push(id);
  }

  // v1.188 (Dean, diagnostic): the money line. ONE summary per list pass tying
  // the raw listed count to the survivor count and every drop reason - so a
  // subscription that "succeeds" but downloads nothing is no longer a black box.
  // A short `listed` count vs. what the channel actually holds points at an
  // early --break / listing cap; a non-zero `before-cutoff` with a low survivor
  // count points at the date gate; and so on. Emitted for EVERY poll (one line).
  console.log(`yt-dlp: subscription ${sub.id} list pass -> ${listedCount} listed, ${survivorIds.length} new to download `
    + `(dropped: ${dropCounts.archived} archived, ${dropCounts.beforeCutoff} before-cutoff, ${dropCounts.skipList} skip-list, `
    + `${dropCounts.availability} availability, ${dropCounts.premiere} premiere-defer, ${dropCounts.shorts} shorts, `
    + `${dropCounts.unsafeId} unsafe-id, ${dropCounts.duplicate} duplicate)`);
  ...
  if (!downloadResult.ok) {
    ...
    const outcomeResult = computeDownloadOutcome({
      ...
      // v1.36.2 gate hardening: positive per-download evidence -- each
      // completed download prints exactly one FTCHMETA line (channelMeta
      // entry), corroborating the subtitle-only success/partial grant.
      completedCount: Array.isArray(downloadResult.channelMeta) ? downloadResult.channelMeta.length : 0,
    });
  ...
  // v1.47.4 gate WARNING (BOTH seats): the subscription lane had no L3
  // subtitle-fallback record at all -- `recordSubtitleFallback` was wired only
  // into `runOneShot`. A subscribed channel's video rescued by the
  // subtitle-stripped retry therefore landed caption-less with NO row anywhere,
  // which is the exact silent outcome L3 exists to prevent, on the lane that
  // actually produced Dean's original report. Recorded here, at the success
  // return, where `downloadResult` is still in scope -- no return-value
  // threading needed, and `subtitleFallback` is only ever set when the media
  // genuinely landed (see run.js's gating).
  recordSubtitleFallback(deps, 'subscription', downloadResult, {
    name: sanitizeFailureTitle(sub && sub.name) || (sub && sub.name) || '',
    url: sub && sub.channelUrl,
  });
...
async function processSubscription(deps, config, sub, nowMs = Date.now(), avatarBudget = { remaining: Infinity }) {
  ...
  // v1.47.4 item 7: the subscription lane's failure-log mirror, on the SAME
  // single terminal choke point the run log already uses (this function has
  // exactly one emit site by design -- see the comment above).
  recordFailureLog(deps, {
    source: 'subscription',
    outcome: result.outcome,
    reason: result.outcome === 'success' ? '' : lastStatus,
    failures: Array.isArray(result.failures) ? result.failures : [],
...
async function runPoll(deps, config, subId, nowMs = Date.now(), onDecision) {
  ...
  try {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      const oneShotsOnGate = getGateQueueSnapshot().length;
    ...
    // v1.142 (Dean's one-off avatar gap): spend whatever avatar self-heal
    // budget this cycle has LEFT on channels known only from library items
    // (a one-off download whose probe failed/skipped at download time, a
    // MeTube import). Skipped entirely when the breaker tripped - a
    // throttled/bot-checked session must not be hammered with avatar probes
    // either. See sweepItemChannelAvatars' own doc comment.
    if (!breakerTripped) {
      await sweepItemChannelAvatars(deps, config, avatarBudget);
    }
  } finally {
...
// `pollBusy`).
//
// The FIFO gate itself moved to lib/heavyGate.js (v1.69.0 T2) so the
// podcasts module can serialize its enclosure downloads through the SAME
// server-wide gate without importing this (possibly-disabled) module's
// internals. Semantics are unchanged and documented there: never-wedge on
// rejection, the uncontended synchronous fast path, whole-lifetime queue
// counting for isHeavyJobActive, and the FIFO-ordered snapshot. Re-exported
// from this module unchanged (see module.exports), so every existing caller
// and test keeps its binding.
const { runExclusive, isHeavyJobActive, getHeavyQueueLength, getGateQueueSnapshot } = require('../heavyGate');
 ...
 * a completed download as an error.
 *
 * v1.41.6 (gate fix, QA WARNING): it still never throws -- but it now REPORTS,
 * returning `true` when the id is on record (freshly appended, or already there)
 * and `false` when it could not be. Every pre-existing caller ignores the return
 * value and is therefore byte-identical in behavior; the import-relocation checks
 * it, because for a RELOCATED file this append is the only thing preventing a
 * duplicate re-download (unlike a real download, yt-dlp's own "file exists" skip
 * cannot back it up -- our filename is not the one yt-dlp would have written).
 * @returns {boolean} whether `videoId` is now recorded in the archive
 */
function recordOneShotInArchive(config, videoId, source = 'youtube') {
  // v1.41.13: `source` is the yt-dlp extractor (lowercased) -- the archive
  // line is native `<extractor> <id>` (make_archive_id format). Defaults to
  // 'youtube' so every existing YouTube caller is byte-for-byte unchanged; a
  // universal one-off passes its own sourceExtractor. `rules.isArchived`
  // already compares the extractor case-insensitively + the id exactly.
  const extractor = (typeof source === 'string' && source !== '' ? source : 'youtube').toLowerCase();
  try {
    const archivePath = args.resolveArchivePath(config);
    const existingText = readArchiveTextSafely(config);
    if (rules.isArchived(existingText, extractor, videoId)) return true; // already recorded -- idempotent no-op
 ...
 */
function launchOneShotJob(deps, config, { jobId, videoId, watchUrl, sourceUrl, format, quality, filetype, explicitFolder }) {
  ...
  // v1.41.13: the universal lane has no YouTube channel to probe -- its real
  // per-uploader folder is resolved later from the captured channel/uploader
  // (P2 bridge + relocation, exactly like a MeTube import). At download time it
  // uses the explicit folder if given, else the fallback; resolveOneOffFolder's
  // YouTube channel probe is only meaningful for a real watchUrl.
  // v1.41.13 (Decision D6): a UNIVERSAL sourceUrl may be a share SHORTLINK that
  // no named extractor matches (facebook.com/share/r/<id>) -- yt-dlp only
  // handles those via the generic extractor, which we block. Resolve its
  // redirects OURSELVES first (bounded + SSRF-guarded, UNLOCKED -- runs here,
  // strictly before the download gate) and re-enter the lane classifier with
  // the FINAL url, so a shortlink that resolves to a named-extractor URL (or
  // even a YouTube URL) is handled correctly. A direct URL resolves to itself
  // in one hop. On a resolver refusal (SSRF/timeout/hop-cap) the sourceUrl is
  // left unchanged -- yt-dlp then surfaces its own no-match failure honestly.
  const laneReady = (typeof sourceUrl === 'string' && sourceUrl !== '' && shortlink.isLikelyShortlink(sourceUrl))
    ? shortlink.resolveShortlink(sourceUrl).then((r) => {
      if (!r.ok || r.url === sourceUrl) return { videoId, watchUrl, sourceUrl };
      const reclassified = url.classifyOneOffUrl(r.url);
      // gate W1: if the resolved url doesn't classify (e.g. it redirected to a
      // login/consent interstitial), NEVER hand yt-dlp that dead end -- fall
      // back to the ORIGINAL pasted url, which yt-dlp may still handle with its
      // own cookies-file/API path.
      if (!reclassified.ok) return { videoId, watchUrl, sourceUrl };
      if (reclassified.lane === 'youtube') return { videoId: reclassified.videoId, watchUrl: reclassified.watchUrl, sourceUrl: undefined };
      return { videoId: null, watchUrl: null, sourceUrl: reclassified.sourceUrl };
    })
    : Promise.resolve({ videoId, watchUrl, sourceUrl });

  return laneReady.then(({ videoId: effVideoId, watchUrl: effWatchUrl, sourceUrl: effSourceUrl }) => {
  const folderPromise = (typeof effSourceUrl === 'string' && effSourceUrl !== '' && !explicitFolder)
    ? Promise.resolve(ONE_OFF_FALLBACK_FOLDER)
    : resolveOneOffFolder(effWatchUrl, explicitFolder, config, jobId);
  return folderPromise
    .then((folder) => runExclusive(() => runOneShot(deps, config, {
      videoId: effVideoId,
      watchUrl: effWatchUrl,
      sourceUrl: effSourceUrl,
      format,
      quality,
      filetype,
      folder,
      ...
      jobId,
    }), { kind: 'oneshot', label: effWatchUrl || effSourceUrl, jobId }));
  })
    .then(() => {
 ...
 * untrusted-at-rest, exactly like the persisted ytdlp records): the URL back
...
function requeuePendingOneShots(deps, config) {
  ...
  for (const entry of entries) {
    // v1.41.13 (design S3): re-classify through the SAME lane gate the route
    // uses, so an accepted UNIVERSAL job survives a restart instead of being
    // dropped by the YouTube-only classifier. A durable universal entry stored
    // its raw URL as `entry.url` (== entry.sourceUrl); classifyOneOffUrl
    // re-derives the lane.
    const classification = url.classifyOneOffUrl(entry && entry.url);
    const isUniversal = classification.ok && classification.lane === 'universal';
    ...
    activity.setOneShot(entry.jobId, {
      ...
      url: isUniversal ? classification.sourceUrl : classification.watchUrl,
      format,
      quality,
    ...
    runlog.recordRun(deps && deps.dataDir, {
      ...
      kind: 'one-shot',
      id: entry.jobId,
      name: isUniversal ? classification.sourceUrl : classification.watchUrl,
      outcome: 'requeued',
      succeeded: 0,
      failed: 0,
    ...
    launchOneShotJob(deps, config, {
      jobId: entry.jobId,
      videoId: isUniversal ? null : classification.videoId,
      watchUrl: isUniversal ? null : classification.watchUrl,
      sourceUrl: isUniversal ? classification.sourceUrl : undefined,
      format,
      quality,
      filetype: filetypeResult.value,
      explicitFolder,
 ...
 * avatar off of).
 */
async function runOneShot(deps, config, { videoId, watchUrl, sourceUrl, format, quality, filetype, folder, folderIsExplicit, jobId }) {
  ...
  const emitRunLog = (outcome, succeeded, failed, reason, failures) => {
    ...
    // v1.47.4 item 7: the failure log rides this same choke point, so every
    // terminal one-shot outcome is covered by construction rather than by
    // remembering to add a call to each branch.
    recordFailureLog(deps, {
      source: 'one-off',
      outcome,
      reason,
      failures,
      name: sanitizeFailureTitle(effectiveFolder) || effectiveFolder,
      // The universal lane carries `sourceUrl`; the YouTube lane carries
      // `watchUrl`. Exactly one is set per job (see the route's classification).
  ...
  try {
    ...
    // pre-validated `sourceUrl` (isPlausibleMediaUrl at the route) and no
    // videoId. buildYtdlpDownloadArgs (via run.runDownload) targets that URL
    // directly with the universal template + extractor gate. A YouTube one-off
    // leaves `sourceUrl` undefined and keeps its per-id watch?v= target.
    const universalSourceUrl = typeof sourceUrl === 'string' && sourceUrl !== '' ? sourceUrl : undefined;
    const downloadTargetIds = universalSourceUrl ? [] : [videoId];
    ...
    let downloadResult;
    try {
      downloadResult = await run.runDownload(syntheticSub, config, downloadTargetIds, {
        onProgress,
        oneOff: true,
        sourceUrl: universalSourceUrl,
        // v1.41.13 folder fix: a universal one-off with no EXPLICIT folder folds
        // by %(uploader)s (the pseudo-channel), instead of dumping every
        // non-YouTube download into one "Uncategorized" folder. An explicit
        // folder override (folderIsExplicit) still wins.
        autoUploaderFolder: !!universalSourceUrl && !folderIsExplicit,
    ...
    if (!downloadResult.ok) {
      ...
      if (cancelledOneShotJobs.has(jobId)) {
        ...
        return; // v1.36.2: explicit -- the else arm below can now FALL THROUGH to success, so the shared post-branch return is gone
      } else {
        const status = safeErrorStatus(downloadResult.error, config);
        // v1.29.0 T3: classify via the SAME shared outcome function the
        // subscription path uses, keeping the counting logic single-sourced.
        // v1.36.2 gate follow-up (adversarial WARNING): the old comment here
        // claimed a single-target job "can only ever resolve to error" --
        // FALSE since the subtitle-only arm landed: a one-off whose MEDIA
        // downloaded fine (corroborated by its FTCHMETA line) but whose
        // subtitle fetch/convert exit-1'd now resolves outcome 'success',
        // and must get the SUCCESS handling -- fall through to the archive
        // append + channel-meta/avatar persist below instead of a red chip
        // that contradicts its own run-log line and leaves the id
        // un-archived (a later subscription poll would re-download a
        // duplicate). That is exactly the "transcripts must be non-blocking"
        // complaint, surviving on the one-shot surface.
        const outcomeResult = computeDownloadOutcome({
          ok: false,
          itemFailures: downloadResult.itemFailures,
          targetIds: [videoId],
          // v1.36.2 gate hardening: same corroboration as the subscription
          // path (see that call site's comment).
          completedCount: Array.isArray(downloadResult.channelMeta) ? downloadResult.channelMeta.length : 0,
        });
        ...
        console.log(`yt-dlp: one-off ${videoId} exited non-zero but only subtitle errors were found and the download completed -- treating as success`);
        // Deliberate FALL THROUGH to the success path below.
      }
    }
    ...
    // succeeded.
    // gate S1: only the YouTube lane has a videoId here; a universal one-off's
    // videoId is null, and writing `youtube null` would pollute the shared
    // archive with an un-deduping junk line. A universal item's archive line
    // is recorded (with its real extractor + RAW id) by the DELETE route
    // instead, which is where "deleted stays gone" actually matters -- the
    // archive is only ever consulted by subscription polls, which are
    // YouTube-only, so a universal item never needs a success-time entry.
    if (videoId) recordOneShotInArchive(config, videoId);
    ...
    //
    // v1.142 (Dean's one-off avatar gap): TWO of this fold's original skips
    // were over-broad and are now gone. (1) The `folderIsExplicit` skip -- its
    // rationale ("no probed channel identity to key off") conflated the
    // FOLDER probe with the FTCHMETA capture: the capture runs during the
    // download regardless of folder choice, so an explicit-folder one-off has
    // exactly the same probeable identity as an auto-routed one. (2) The
    // captured-`channelUrl` requirement -- a capture carrying a bare `UC…`
    // channelId now derives the canonical channel URL
    // (store.deriveChannelUrlFromId) instead of silently leaving the item
    // avatar-less forever (Dean's on-device repro: one-off item with
    // channelId present, avatar permanently blank). Still skipped when the
    // capture already carries a usable avatar (defense-in-depth against a
    // future producer change), or when NEITHER a channelUrl nor a derivable
    // channelId survived sanitization (nothing to probe).
    //
    ...
    if (Array.isArray(downloadResult.channelMeta)) {
      ...
      if (capturedEntry) {
        ...
        const avatarProbeUrl = sanitizedForAvatar
          ? (sanitizedForAvatar.channelUrl || store.deriveChannelUrlFromId(sanitizedForAvatar.channelId))
          : null;
        if (sanitizedForAvatar && avatarProbeUrl && !sanitizedForAvatar.channelAvatarUrl) {
          try {
            // v1.25.x QoL bugfix: `probeChannelAvatar` now returns
            ...
            // a subscribed one).
            const probed = await run.probeChannelAvatar(avatarProbeUrl, config);
    ...
    try {
      await Promise.race([
        persistCapturedChannelMeta(deps, withUniversalPageUrl(downloadResult.channelMeta, universalSourceUrl)),
        new Promise((resolve) => {
          const t = setTimeout(() => {
    ...
    // v1.47.4 item 1 (L3): the download succeeded, but if it only succeeded
    // because the subtitle-stripped retry rescued it, the file has no captions.
    // Recorded here rather than left silent -- see recordSubtitleFallback.
    recordSubtitleFallback(deps, 'one-off', downloadResult, {
      name: sanitizeFailureTitle(effectiveFolder) || effectiveFolder,
      url: sourceUrl || watchUrl,
    });
  } catch (err) {
...
/**
 * v1.41.6 (import relocation): WHERE does a video by channel X belong on disk?
 *
 * There is a trap here, and it is the whole reason this function exists rather
 * than a bare `args.resolveChannelDir(config, {name: item.channelName})` at the
 * call site. A SUBSCRIPTION's download folder is `resolveChannelDir(config, sub)`
 * -- which folds `sub.name`, the name the user subscribed under (typically the
 * `@handle`; no channel-name probe happens at subscribe time). An item's
 * `channelName` is yt-dlp's REAL channel display name. Those two routinely
 * sanitize to DIFFERENT folder names ("-RickAstley" vs "Rick Astley"), so
 * relocating a subscribed channel's import by `channelName` would file it into a
 * PARALLEL folder -- next to, but not the same as, the folder that
 * subscription's own downloads keep landing in. One channel, two sidebar
 * folders, forever. (The same mismatch is what the v1.25 gate caught for
 * `migrateOneOffsIntoChannelFolders`; see that function's header in server.js.)
 *
 * So: if this channel IS subscribed, hand back THAT subscription's own channel
 * dir, computed by the SAME `resolveChannelDir(config, sub)` call its downloads
 * use -- byte-identical, never a re-derivation. Matched on `channelId` when both
 * sides know one (yt-dlp returns the canonical `/channel/UC...` form while a
 * subscription may carry the `/@handle` form -- a bare URL compare would miss),
 * else on the normalized channel URL (`store.normalizeChannelUrl`, the same
 * normalization `addSubscription` keys its own id off). No subscription => the
 * channel's display name, exactly like a one-shot download of an unsubscribed
 * channel (`resolveOneOffFolder` -> `run.probeChannel`) already does.
 *
 * The subscription JOIN stays INSIDE this module (server.js reads the
 * namespace's tables through `ytdlpDb` since Wave 5 but never joins a channel
 * to a subscription itself -- see store.js's pins invariant), which is why
 * this is a module export rather than a few lines in server.js.
 *
 * Throws (via `resolveChannelDir`) if the result cannot be confined under
 * `config.downloadDir` -- the caller treats that as a hard failure, never a
 * fallback to some other directory.
 *
 * THE MATCH ITSELF (gate fix, CRITICAL, found independently by both reviewers).
 * The first cut matched on channelId "when both sides know one", else on the
 * normalized channel URL. That is broken in the single most common shape there
 * is: **`addSubscription` NEVER writes a `channelId`** -- it is only backfilled
 * later, by a successful poll/probe (`store.recordSubscriptionChannelId`). So a
 * subscription added by @handle and not yet polled (just added; a manual poll
 * interval; the breaker open; the network down) has NO id, the id branch cannot
 * fire, and the fallback compared yt-dlp's canonical `/channel/UC...` (what the
 * ITEM carries) against `/@RickAstley` (what the SUB carries) -- no match, and
 * every import of that channel filed into a PARALLEL folder, permanently.
 *
 * So the join is on ALL the identities either side actually has: ids when both
 * know one, else the sub's normalized URL against EITHER of the item's normalized
 * URL FORMS -- its canonical `channelUrl` and its `channelHandleUrl` (v1.41.5
 * persists both; `sanitizeCapturedChannelMeta` emits both). This is precisely
 * the join `store.resolveItemChannelAvatarUrl` already performs; the folder
 * matcher was the only join in the codebase that ignored the handle form.
 *
 * @param {object} db a `loadDatabase()`-shaped snapshot
 * @param {object} config parsed yt-dlp config
 * @param {{channelUrl: string, channelHandleUrl?: string, channelId?: string, channelName?: string}} channel
 * @returns {string} the absolute, confined per-channel directory
 */
function resolveChannelDirForChannel(db, config, channel) {
  const c = channel && typeof channel === 'object' ? channel : {};
  const match = findSubscriptionForChannel(db, c);
  // The subscription's own dir -- the SAME expression its downloads use.
  if (match) return args.resolveChannelDir(config, match);
  // Unsubscribed: the channel's display name (falling back to the URL inside
  // `resolveChannelDir` itself when there is no name).
  return args.resolveChannelDir(config, { name: c.channelName, channelUrl: c.channelUrl });
}
...
/**
 * Every normalized URL form an item's channel identity can be recognized by --
 * its canonical `/channel/UC...` URL and its `/@handle` URL. See
 * `resolveChannelDirForChannel`'s own comment for why BOTH are load-bearing.
 */
function channelUrlForms(channel) {
  const forms = new Set();
  for (const raw of [channel && channel.channelUrl, channel && channel.channelHandleUrl]) {
    const value = nonEmpty(raw);
    if (value) forms.add(store.normalizeChannelUrl(value));
  }
  return forms;
}

/**
 * The channel id a `/channel/UC.../` URL LITERALLY CONTAINS, or `null`.
 *
 * v1.41.6 gate fix round 3 (QA CRITICAL): the `channelId` FIELD is optional by
 * construction -- `store.sanitizeCapturedChannelMeta` emits it only when yt-dlp
 * supplied one (`...(channelId ? { channelId } : {})`) -- so an item can perfectly
 * well carry the canonical URL and no id field. But the canonical URL is
 * `https://www.youtube.com/channel/<the id>`: the id is right there. Reading it
 * out turns a whole class of "we cannot tell" into an exact match, on both sides
 * of the join (a subscription added by canonical URL is in the same position).
 *
 * Re-validated through `store.CHANNEL_ID_PATTERN` -- the SAME shape gate
 * `recordSubscriptionChannelId`/`sanitizeCapturedChannelMeta` use -- so a garbage
 * path segment can never be mistaken for an id. Never throws.
 */
function extractChannelIdFromUrl(rawUrl) {
  const value = nonEmpty(rawUrl);
  ...
  if (!m) return null;
  return store.CHANNEL_ID_PATTERN.test(m[1]) ? m[1] : null;
}

/**
 * The channel id for one side of the join: the explicit field when present, else
 * the one its own canonical URL carries (see `extractChannelIdFromUrl`). Used for
 * BOTH the item and the subscription, so neither side has to have been polled for
 * the id branch to work.
 */
function effectiveChannelId(channelish) {
  if (!channelish) return null;
  return nonEmpty(channelish.channelId) || extractChannelIdFromUrl(channelish.channelUrl);
}

/**
 * The subscription this channel identity belongs to, or `null`. Matches on
 * channelId when BOTH sides have one (explicit OR derived from a canonical URL),
 * else on the subscription's normalized URL against either of the item's URL
 * forms.
 */
function findSubscriptionForChannel(db, channel) {
  const subs = store.ensureYtdlp(db).subscriptions;
  if (!Array.isArray(subs) || subs.length === 0) return null;
  const wantId = effectiveChannelId(channel);
  const forms = channelUrlForms(channel);
  return subs.find((sub) => {
    const subUrl = nonEmpty(sub && sub.channelUrl);
    if (!subUrl) return false;
    const subId = effectiveChannelId(sub);
    if (wantId && subId) return wantId === subId;
    return forms.has(store.normalizeChannelUrl(subUrl));
  }) || null;
}

/**
 * Could a subscription be this channel WITHOUT us being able to tell? (v1.41.6,
 * gate fix: "skip when the folder can't be resolved confidently -- a skipped file
 * is recoverable, a split library is not".)
 *
 * The blind spot is COMPARABILITY, not any one URL shape. A subscription with no
 * `channelId` yet (the id is only ever backfilled by a successful poll/probe) can
 * only be identified by its URL -- and `validateChannelUrl` accepts FOUR channel
 * URL shapes (`url.js` CHANNEL_PATH_PATTERNS): `/@handle`, `/channel/UC...`,
 * `/c/Name` and `/user/Name`. Only `/channel/UC...` is derivable from what an
 * item carries; a `/c/` or `/user/` URL is exactly as uncomparable to an item's
 * canonical `/channel/UC...` as a bare handle is. So "not subscribed" is a GUESS
 * against ANY id-less, non-`/channel/` subscription whose URL is not one of the
 * item's own forms -- and guessing wrong splits that channel's library across two
 * folders permanently (eligibility clause 3 means the files are never re-moved).
 *
 * v1.41.6 gate fix, round 2 (adversarial WARNING with a repro): the first cut
 * tested `subUrl.includes('/@')`, which missed `/c/` and `/user/` subs entirely,
 * AND early-returned `false` whenever the ITEM had a `channelHandleUrl` -- but
 * possessing the item's handle tells you nothing about a `/c/` sub. Both are gone:
 * the question is now asked per-SUBSCRIPTION, and it is asked about whether that
 * subscription's URL is comparable to anything this item knows.
 *
 * Returns `false` (i.e. "go ahead") whenever the question is actually decidable:
 * a subscription matched; BOTH sides carry a channel id (explicit or derived from
 * a canonical URL), so the id branch genuinely decided; the subscription's URL is
 * one of the item's own forms (a real, decided miss); or the subscription's URL is
 * `/channel/`-shaped and therefore directly comparable. A subscription URL that
 * cannot even be PARSED is treated as ambiguous -- fail toward not moving the
 * file, never toward a split.
 */
function hasAmbiguousChannelSubscription(db, channel) {
  const c = channel && typeof channel === 'object' ? channel : {};
  if (findSubscriptionForChannel(db, c)) return false; // decided: it IS this sub
  const subs = store.ensureYtdlp(db).subscriptions;
  if (!Array.isArray(subs)) return false;
  const itemUrls = channelUrlForms(c); // every normalized URL form the ITEM knows
  // v1.41.6 gate fix round 3 (QA CRITICAL -- the MIRROR of the shape bug fixed in
  // round 2). An id decides NOTHING on its own: `findSubscriptionForChannel`'s id
  // branch fires only when BOTH sides have one. The previous guard asked only
  // whether the SUBSCRIPTION had an id, and waved the move through on that -- so
  // an item with NO channelId (a shape `sanitizeCapturedChannelMeta` produces BY
  // DESIGN: the field is emitted only when yt-dlp supplied one) against a polled,
  // id-bearing @handle subscription was declared "decidable", missed, and filed
  // into a parallel folder. Permanently.
  const wantId = effectiveChannelId(c);
  return subs.some((sub) => {
    const subUrl = nonEmpty(sub && sub.channelUrl);
    if (!subUrl) return false;
    if (wantId && effectiveChannelId(sub)) return false; // BOTH sides have ids -> the id branch DID decide (and it said "not this one")
    if (itemUrls.has(store.normalizeChannelUrl(subUrl))) return false; // comparable, and the join above already missed it -- a REAL miss
    // v1.41.6 gate round 3 (QA SUGGESTION 1 -- the SYMMETRIC MIRROR of the round-3
    // CRITICAL, applied because "skip a file" is always cheaper than "split a
    // library"). Reaching here means: no id could decide (clause 1), and this
    // sub's URL is none of the item's own forms (clause 2). A previous version
    // carved out `/channel/UC...` subs as "directly comparable" and returned
    // NOT-ambiguous -- but that carve-out is only sound if the ITEM carries a
    // canonical URL to compare against. When `wantId` is absent (an item holding
    // ONLY a handle URL -- reachable via sanitizeCapturedChannelMeta's
    // uploaderUrl fallback), there is nothing to compare a `/channel/` sub TO,
    // so the carve-out declared a genuinely-undecidable pair "decided" and filed
    // the file into a parallel folder, permanently. And when `wantId` IS present,
    // clause 1 has already settled every id-bearing sub -- so the carve-out could
    // never have helped a decidable case anyway. It is therefore unreachable-or-
    // wrong: ambiguity is the only correct answer here. (An unparseable stored URL
    // lands here too, and is likewise ambiguous -- no `new URL` parse needed at
    // all now, so no throw to guard.)
    return true; // uncomparable -> we cannot decide -> do not move
  });
}

/**
 * Opportunistic, write-once backfill: if this channel is subscribed and we hold a
 * validated `channelId` its subscription record lacks, record it (via the
 * existing `store.recordSubscriptionChannelId`, which re-validates the id shape
 * and never reassigns an id already there). Makes every FUTURE join -- avatars,
 * folder matching, `hasAmbiguousChannelSubscription` -- exact, so the ambiguity
 * above can only ever cost one deferral per subscription.
 *
 * Never throws; resolves `false` when there was nothing to do.
 */
function backfillSubscriptionChannelIdForChannel(deps, channel) {
  const c = channel && typeof channel === 'object' ? channel : {};
  const channelId = nonEmpty(c.channelId);
  if (!channelId || !deps || !deps.ytdlpDb || typeof deps.updateDatabase !== 'function') { // Wave 5: the store, not the doc
    return Promise.resolve(false);
  }
  const sub = findSubscriptionForChannel(ytdlpHolder(deps, ['subscriptions']), c);
  if (!sub || nonEmpty(sub.channelId)) return Promise.resolve(false);
  // Keyed on the SUBSCRIPTION's own channelUrl -- that is the exact string
  // `recordSubscriptionChannelId` looks the record up by.
  return store.recordSubscriptionChannelId(deps, sub.channelUrl, channelId);
}
 ...
 */
// ---------------------------------------------------------------------------
// v1.146 downloader-engine (T4): the gated install/update job + the status
// shape both engine routes and the Setup UI consume.
// ---------------------------------------------------------------------------

// At most ONE engine job exists at a time (queued or installing) - a second
// request while one is pending is answered with the busy status instead of
// stacking another gated job (the FIFO gate would serialize them anyway;
// refusing keeps "what is happening" honest and single-valued).
let engineJob = null;

function currentEngineJob() {
  return engineJob;
}

// The runtime bindings both deps bundles pass to engineLib.initRuntime:
// the auto-revert side effects (bell + version-cache refresh) and the
// install-phase probe (gate round 1 W4: while a job is rewriting the venv,
// an ungated spawn failing against the half-written binary must not be
// read as engine death - downloads are all gated, so nothing legitimate
// can crash-report during that phase).
function engineInstallActive() {
  return Boolean(engineJob && engineJob.status === 'installing');
}

function buildEngineAutoRevertHandler(deps) {
  return (info) => {
    try {
      if (typeof deps.recordEngineEvent === 'function') {
        deps.recordEngineEvent('reverted', info && info.fromVersion);
      }
    } catch (err) {
      console.error('Engine revert bell failed (continuing):', err && err.message);
    }
...
/**
 * The update flow both triggers share (channel switch, Update now - and the
 * T6 daily tick): force-fresh PyPI read -> resolve the channel's target ->
 * short-circuit when already current -> install + health-probe -> bell the
 * outcome. Runs INSIDE the FIFO gate (queueEngineInstall wraps it); never
 * throws - every dead end lands in state.lastResult honestly.
 *
 * `opts.force` (explicit user actions: channel switch, Update now) pushes
 * past the anti-flap revert guard below; the unattended triggers (daily
 * tick, boot recovery) leave it standing - they will NOT reinstall the
 * exact build that was auto-reverted, only a newer one.
 *
 * @param {object} deps the routes/background deps bundle (dataDir, recordEngineEvent)
 * @param {string} channel 'stable' | 'nightly'
 * @param {object} [opts] { force? }
 * @returns {Promise<object>} the installEngine-shaped outcome
 */
async function executeEngineUpdate(deps, channel, opts = {}) {
  const dataDir = deps.dataDir;
  const finishCheck = (patch) => {
    const state = engineLib.readState(dataDir);
    state.lastCheckAt = Date.now();
    if (patch) state.lastResult = { at: state.lastCheckAt, ...patch };
    engineLib.writeState(dataDir, state);
  };
  let latest;
  try {
    latest = await engineLib.fetchChannelLatest({ force: true });
  } catch (err) {
    latest = { stable: null, nightly: null, error: err && err.message };
  }
  const target = channel === 'stable' ? latest.stable : latest.nightly;
  if (!target) {
    const message = latest.error || 'PyPI is unreachable - could not look up the latest version';
    finishCheck({ ok: false, action: 'check', version: null, message });
    return { ok: false, message, becameActive: false, reverted: false };
  }
  const pre = engineLib.readState(dataDir);
  if (pre.active === 'venv' && pre.installed && pre.installed.channel === channel &&
      !engineLib.isUpdateAvailable(pre.installed.version, target)) {
    const message = `already on the latest ${channel} engine (${pre.installed.reported || pre.installed.version})`;
    finishCheck({ ok: true, action: 'check', version: pre.installed.version, message });
    return { ok: true, message, becameActive: false, reverted: false, alreadyCurrent: true };
  }
  // The anti-flap guard (attack surface #4): an UNATTENDED trigger never
  // reinstalls the exact build the runtime net reverted - a bad nightly
  // would otherwise bounce install/revert once per day (or per boot) with
  // nobody at the wheel. Explicit user actions force past this.
  if (!opts.force && pre.revert && pre.revert.fromVersion &&
      !engineLib.isUpdateAvailable(pre.revert.fromVersion, target)) {
    const message = `the latest ${channel} engine (${target}) is the same build that was reverted - waiting for a newer one`;
    finishCheck({ ok: false, action: 'check', version: target, message });
    return { ok: false, message, becameActive: false, reverted: false, heldByRevert: true };
  }
  const result = await engineLib.installEngine({ dataDir, version: target, channel });
  finishCheck(null); // installEngine already wrote lastResult; stamp the check time
  try {
    if (result.ok) {
      // The active binary just changed under the 6h version cache - the
      // About/Stats surface must reflect the NEW engine now, not in 6h.
      // AWAITED (gate round 1 S1): still inside the gated job, bounded by
      // the probe timeout - and awaiting makes "job done => version cache
      // reports the new engine" a real ordering a test can bind.
      await refreshYtdlpVersionCache({ force: true });
      if (typeof deps.recordEngineEvent === 'function') {
        deps.recordEngineEvent('updated', result.reported || target);
      }
    } else if (typeof deps.recordEngineEvent === 'function') {
      // Install/probe failures bell as 'update-failed' whatever the prior
      // active engine was; 'reverted' is reserved for the RUNTIME failure
      // net (a working engine that stopped working).
      deps.recordEngineEvent('update-failed', target);
    }
  } catch (err) {
...
/**
 * Queue one engine job through the FIFO gate (a swap must never race a
 * gated download - the brief's named attack surface #2). Refuses (busy)
 * when a job is already pending.
 */
function queueEngineInstall(deps, channel, trigger) {
  if (engineJob) return { queued: false, busy: true };
  // Explicit user actions force past the anti-flap revert guard; the
  // unattended triggers (daily-check, boot-recovery) respect it.
  const force = trigger === 'channel-switch' || trigger === 'manual-update';
  const job = { status: 'queued', channel, trigger };
  engineJob = job;
  job.promise = runExclusive(async () => {
    job.status = 'installing';
    try {
      await executeEngineUpdate(deps, channel, { force });
    } finally {
      engineJob = null;
    }
  }, { kind: 'engine', label: `downloader engine ${trigger} (${channel})` }).catch((err) => {
    // Belt to executeEngineUpdate's own never-throws posture.
    engineJob = null;
    console.error('Engine job failed unexpectedly:', err && err.message);
  });
  return { queued: true };
}

// ---------------------------------------------------------------------------
// v1.146 T6: the daily auto-update gate + boot reconcile.
// ---------------------------------------------------------------------------

// Hourly wake-ups gating on a 24h ledger (state.lastCheckAt), instead of a
// raw 24h interval: a container that restarts nightly at 03:00 would reset
// a naive interval forever and never check. The timer is unref'd and OFF
// in tests unless armed explicitly (the armScanTimer discipline).
let engineTimer = null;
const ENGINE_TICK_MS = 60 * 60 * 1000;
const ENGINE_AUTO_UPDATE_INTERVAL_MS = 24 * 60 * 60 * 1000;

function armEngineTimer(deps) {
  if (engineTimer) { clearInterval(engineTimer); engineTimer = null; }
  engineTimer = setInterval(() => {
    engineAutoUpdateTick(deps).catch((err) => {
      console.error('Engine auto-update tick failed (continuing):', err && err.message);
    });
  }, ENGINE_TICK_MS);
...
function currentEngineTimer() {
  return engineTimer;
}

/**
 * One tick of the daily gate (the ruling's term 4). All four skips are
 * cheap state reads - the tick spawns/fetches NOTHING unless a check is
 * genuinely due; the queued job bells its own outcome either way.
 *
 * @param {object} deps { dataDir, recordEngineEvent }
 * @param {number} [nowMs] injectable clock for tests
 * @returns {Promise<object>} { skipped } or { queued: true }
 */
async function engineAutoUpdateTick(deps, nowMs = Date.now()) {
  const state = engineLib.readState(deps.dataDir);
  if (state.autoUpdate !== true) return { skipped: 'opt-out' };
  if (state.channel !== 'stable' && state.channel !== 'nightly') return { skipped: 'bundled' };
  if (state.lastCheckAt && nowMs - state.lastCheckAt < ENGINE_AUTO_UPDATE_INTERVAL_MS) return { skipped: 'not-due' };
  const q = queueEngineInstall(deps, state.channel, 'daily-check');
  return q.queued ? { queued: true } : { skipped: 'busy' };
}

/**
 * Boot reconcile (the brief: "check/reinstall at boot if the venv is
 * absent"). If the channel intent wants an engine that is not actually
 * usable on disk, queue an async recovery install - boot NEVER blocks, and
 * downloads run on bundled until the recovery lands. The queued job is the
 * non-forced kind, so a standing revert holds it back from reinstalling
 * the exact reverted build (the anti-flap guard in executeEngineUpdate).
 *
 * @param {object} deps { dataDir, recordEngineEvent }
 * @returns {object} { action: 'none' | 'queued' | 'busy' | 'error' }
 */
function reconcileEngineAtBoot(deps) {
  try {
    const state = engineLib.readState(deps.dataDir);
    if (state.channel !== 'stable' && state.channel !== 'nightly') return { action: 'none' };
    const binaryPresent = fs.existsSync(engineLib.resolveVenvBinaryPath(deps.dataDir));
    // Healthy requires the installed engine to be the INTENDED channel's
    // (gate round 1, both seats: a channel switched while an install job
    // was busy could leave channel=stable / installed=nightly forever -
    // the boot heal is the belt under the route's 409 suspenders).
    const healthy = state.active === 'venv' && state.installed &&
      state.installed.channel === state.channel && binaryPresent;
    if (healthy) return { action: 'none' };
    // State honesty BEFORE the async recovery (gate round 1 QA W2): if the
    // venv binary is gone, the seam already spawns bundled - the persisted
    // state must say so NOW, not after a recovery that may dead-end on an
    // unreachable PyPI. Not a revert (nothing failed at runtime); a plain
    // repair, so no revert record and no bell.
    if (state.active === 'venv' && !binaryPresent) {
      state.active = 'bundled';
      state.installed = null;
      engineLib.writeState(deps.dataDir, state);
    }
    const q = queueEngineInstall(deps, state.channel, 'boot-recovery');
    return { action: q.queued ? 'queued' : 'busy' };
  } catch (err) {
    console.error('Engine boot reconcile failed (continuing):', err && err.message);
    return { action: 'error' };
  }
}

/**
 * The one status shape (GET response, POST responses, the Setup UI's poll).
 * `skipNetwork` keeps POST responses instant (cached PyPI data only);
 * `refresh` forces a fresh PyPI read on an explicit GET ?refresh=1.
 */
async function buildEngineStatus(deps, opts = {}) {
  const state = engineLib.readState(deps.dataDir);
  const support = await engineLib.getSupportInfo();
  const bundledVersion = await engineLib.getBundledVersion();
  let latest = null;
  if (support.supported) {
    latest = opts.skipNetwork
      ? engineLib.peekChannelLatest()
      : await engineLib.fetchChannelLatest({ force: opts.refresh === true });
  }
  return {
    supported: support.supported,
    reason: support.reason,
    channel: state.channel,
    // The EFFECTIVE engine (the seam's own existence check) - a status
    // surface must never claim a venv whose binary is gone (QA W2).
    active: engineLib.effectiveActive(deps.dataDir, state),
    autoUpdate: state.autoUpdate,
    bundledVersion,
    installed: state.installed,
    latest: latest
      ? { stable: latest.stable, nightly: latest.nightly, checkedAt: latest.checkedAt, error: latest.error }
      : null,
    busy: engineJob ? { status: engineJob.status, channel: engineJob.channel, trigger: engineJob.trigger } : null,
...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  // v1.146 (downloader-engine T4): bind the spawn seam the moment routes
  // exist - from here on run.js resolves its binary through the persisted
  // engine state, and a venv-engine runtime failure auto-reverts + bells.
  engineLib.initRuntime({
    dataDir: deps.dataDir,
    onAutoRevert: buildEngineAutoRevertHandler(deps),
    isInstallActive: engineInstallActive,
  });
  ...
  app.get('/subscriptions', (req, res) => {
    // v1.41.18 (Dean): render via the server's shared shell helper so a
    // configured custom logo is baked onto <html> before paint (no text-logo
    // FOUC on refresh), identical to every other header shell. Falls back to a
    // plain sendFile when the helper wasn't injected (older/standalone wiring).
    const abs = path.join(__dirname, 'views', 'subscriptions.html');
    if (typeof deps.sendShellHtml === 'function' && deps.sendShellHtml(res, abs)) return;
    res.sendFile(abs);
  });
  ...
  app.post('/api/subscriptions', async (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC
  ...
  // `db.folders`/`db.folderSettings`. v1.43 (chunk 4b): pins are PER-USER --
  // these handlers run store.js's PURE reducers (`reduceAddPin`/
  // `reduceRemovePin`/`reducePinReorder`, semantics unchanged: idempotent
  // re-pin, max(order)+1 tail append, FIFO cap, stable reorder partition)
  // against the signed-in user's pin list and persist through
  // `deps.userChannelPins` (user_channel_pins rows -- server.js's bridge).
  // The frozen pre-auth `db.ytdlp.pins` record is no longer read or written
  // here (it was adopted into the first admin at /welcome).
  app.get('/api/subscriptions/pins', (req, res) => {
    res.json(enrichPinsWithChannelAvatar(config, deps, deps.userChannelPins.list(req.user.id)));
  });

  app.post('/api/subscriptions/pins', (req, res) => {
    ...
    try {
      const { channelDir, label } = validation.value;
      const result = store.reduceAddPin(deps.userChannelPins.list(req.user.id), {
        id: deps.getMediaId(channelDir), channelDir, label, pinnedAt: new Date().toISOString(),
      });
      if (result.changed) deps.userChannelPins.replace(req.user.id, result.pins);
  ...
  app.delete('/api/subscriptions/pins/:id', (req, res) => {
    try {
      const result = store.reduceRemovePin(deps.userChannelPins.list(req.user.id), req.params.id);
      if (!result.changed) return res.status(404).json({ error: 'Pin not found' });
      deps.userChannelPins.replace(req.user.id, result.pins);
      res.json({ success: true });
    } catch (err) {
  ...
  app.post('/api/subscriptions/pins/reorder', (req, res) => {
    const validation = validatePinReorderRequest(req.body);
    if (!validation.ok) {
      return res.status(400).json({ error: validation.error });
    }
    try {
      const reordered = store.reducePinReorder(deps.userChannelPins.list(req.user.id), validation.value);
      deps.userChannelPins.replace(req.user.id, reordered);
      res.json(enrichPinsWithChannelAvatar(config, deps, deps.userChannelPins.list(req.user.id)));
    } catch (err) {
  ...
  // ---- v1.146 downloader-engine selector (T4) ----------------------------
  // ADMIN-ONLY, all three routes: these cause pip to execute code from PyPI
  // (RCE-adjacent - the wave's attack surface #1). The gate FAILS CLOSED: a
  // deployment that somehow wires these routes without requireAdmin refuses
  // everyone rather than admitting everyone (deliberately NOT the
  // requireManageSubscriptions fail-open idiom above - the stakes differ).
  // Both POSTs are classified 'admin' in the route-write forcing net.
  const engineAdminGate = (req, res) => {
    if (typeof deps.requireAdmin === 'function') return deps.requireAdmin(req, res);
    res.status(403).json({ error: 'Only an admin can manage the downloader engine.' });
    return false;
  };

  app.get('/api/ytdlp/engine', async (req, res) => {
    if (!engineAdminGate(req, res)) return;
    try {
      res.json(await buildEngineStatus(deps, { refresh: req.query.refresh === '1' }));
    } catch (err) {
      console.error('Engine status failed:', err && err.message);
      res.status(500).json({ error: 'Could not read the downloader engine status' });
    }
  });

  app.post('/api/ytdlp/engine', async (req, res) => {
    if (!engineAdminGate(req, res)) return;
    const body = req.body || {};
    for (const key of Object.keys(body)) {
      if (key !== 'channel' && key !== 'autoUpdate') {
        return res.status(400).json({ error: `unknown engine settings key: ${key}` });
      }
    }
    if ('channel' in body && !engineLib.CHANNELS.includes(body.channel)) {
      return res.status(400).json({ error: 'channel must be bundled, stable, or nightly' });
    }
    if ('autoUpdate' in body && typeof body.autoUpdate !== 'boolean') {
      return res.status(400).json({ error: 'autoUpdate must be a boolean' });
    }
    // Gate round 1 (both seats, independently): a channel write while an
    // engine job is pending must be REFUSED WHOLE, before any persist - a
    // half-applied switch (intent persisted, install silently dropped, or
    // a bundled flip racing a pending activation) is the measured wrong
    // outcome. autoUpdate-only writes stay fine while busy.
    if ('channel' in body && engineJob) {
      return res.status(409).json({ error: 'An engine install is already in progress - try again when it finishes.' });
    }
    if (body.channel === 'stable' || body.channel === 'nightly') {
      const support = await engineLib.getSupportInfo();
      if (!support.supported) return res.status(400).json({ error: support.reason });
    }
    if ('autoUpdate' in body) {
      engineLib.setAutoUpdate({ dataDir: deps.dataDir, enabled: body.autoUpdate });
    }
    if ('channel' in body) {
      // Gate round 2 (QA residual): re-check the busy flag AFTER the
      // awaited support probe above - two channel POSTs interleaving
      // across that microtask hop could otherwise land one intent with no
      // job. No await sits between this check and the persist+queue below,
      // so the pair is atomic on the event loop.
      if (engineJob) {
        return res.status(409).json({ error: 'An engine install is already in progress - try again when it finishes.' });
      }
      engineLib.setChannelIntent({ dataDir: deps.dataDir, channel: body.channel });
      if (body.channel === 'bundled') {
        // Immediate: the intent setter already flipped active - the version
        // cache must stop reporting the venv engine right now.
        refreshYtdlpVersionCache({ force: true });
      } else {
        queueEngineInstall(deps, body.channel, 'channel-switch');
      }
    }
    res.json(await buildEngineStatus(deps, { skipNetwork: true }));
  });

  app.post('/api/ytdlp/engine/update', async (req, res) => {
    if (!engineAdminGate(req, res)) return;
    const state = engineLib.readState(deps.dataDir);
    if (state.channel === 'bundled') {
      return res.status(400).json({ error: 'The bundled engine updates with the app image - pick the stable or nightly channel first.' });
    }
    const support = await engineLib.getSupportInfo();
    if (!support.supported) return res.status(400).json({ error: support.reason });
    queueEngineInstall(deps, state.channel, 'manual-update');
    res.json(await buildEngineStatus(deps, { skipNetwork: true }));
  });
  ...
  app.post('/api/subscriptions/repull', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC
    if (refuseIfReadOnlyMedia(config, res)) return; // v1.42 safe-mode lever
    let responded = false;
  ...
  // no future poll re-attempts it. The `:id` mirrors the repull route's shape
  // (a synchronous 404 when the subscription is unknown, and it anchors the UI
  // affordance to the row the user clicked) -- but the effect is GLOBAL: the
  // skip list lives once under the download root, so a skipped id is excluded
  // for every channel, exactly like the shared download archive. `videoId` is
  // re-validated with the SAME `url.isSafeVideoId` predicate the survivor loop
  // applies before a survivor can be individually targeted. Unlike the
  // fire-and-forget repull, `recordSkip` surfaces a write failure as a 500 --
  // the user is owed an honest result for a deliberate action.
  app.post('/api/subscriptions/:id/skip', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC
    if (refuseIfReadOnlyMedia(config, res)) return; // v1.42: writes the SHARED .ytdlp-skiplist.txt
    const exists = store.listSubscriptions(deps).some((sub) => sub.id === req.params.id);
    if (!exists) {
      return res.status(404).json({ error: 'Subscription not found' });
    }
    const videoId = req.body && typeof req.body.videoId === 'string' ? req.body.videoId.trim() : '';
    if (!videoId || !url.isSafeVideoId(videoId)) {
      return res.status(400).json({ error: 'A valid videoId is required' });
    }
    try {
      const added = recordSkip(config, videoId);
      return res.json({ success: true, videoId, added });
    } catch (err) {
      console.error(`yt-dlp: failed to record skip for ${videoId}:`, err && err.message);
      return res.status(500).json({ error: `Could not record the skip: ${err.message}` });
    }
  });
  ...
  app.post(
    ...
    (req, res) => {
    // v1.80 RBAC (security-gate W2): a plain member must not pull arbitrary
    // internet media into the SHARED library. The Shortcut API-token caller
    // (req.auditActor==='api-token', session-less by design) is exempt; a
    // SESSION caller needs admin OR canManageSubscriptions.
    if (req.auditActor !== 'api-token' && deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return;
    if (refuseIfReadOnlyMedia(config, res)) return; // v1.42 safe-mode lever
    const body = typeof req.body === 'string' ? { url: req.body } : (req.body && typeof req.body === 'object' ? req.body : {});

    // v1.41.13: the one-off lane classifier -- YouTube single-video URLs take
    // the legacy lane (videoId/watchUrl, byte-for-byte unchanged); any other
    // named-extractor-supported URL takes the universal lane (a validated raw
    // sourceUrl). A YouTube channel/playlist/handle is still a hard 400.
    // `classifyOneOffUrl` is the single source of truth (composes
    // classifySingleVideo + isPlausibleMediaUrl), never a forked validator;
    // its error is neutral (no internal field name leaks -- the "channelUrl
    // host..." screenshot bug).
    const classification = url.classifyOneOffUrl(body.url);
    if (!classification.ok) {
      return res.status(400).json({ error: classification.error });
    }
    const isUniversal = classification.lane === 'universal';
    ...
    if (getHeavyQueueLength() >= MAX_ONESHOT_QUEUE_LENGTH) {
    ...
    pending.addPending(deps && deps.dataDir, {
      jobId,
      // v1.41.13: for a universal one-off, the durable record's `url` IS the
      // raw sourceUrl (there is no watchUrl); `sourceUrl` is stored explicitly
      // so requeuePendingOneShots can re-classify it through the SAME lane gate
      // on restart (design S3 -- never drop an accepted universal job).
      url: isUniversal ? classification.sourceUrl : classification.watchUrl,
      videoId: isUniversal ? null : classification.videoId,
      sourceUrl: isUniversal ? classification.sourceUrl : undefined,
      format,
  ...
  app.post('/api/ytdlp/download/:jobId/cancel', (req, res) => {
    // v1.81 write-RBAC (forcing-net find): cancelling a one-shot download is a
    // download control - same gate as the download itself (canManageSubscriptions).
    // FIRST guard, before the jobId existence check (no cross-user job oracle).
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return;
  ...
  app.post('/api/ytdlp/repull-metadata', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    // v1.42 safe-mode lever: the reheat writes .vtt sidecars into shared
    // media dirs, appends the shared .ytdlp-archive.txt, and can RELOCATE
    // files (relocateHydratedImports) — all shared-state mutation. (The
    // /cancel and /preview siblings stay open: cancel stops work, preview
    // is read-only.)
    if (refuseIfReadOnlyMedia(config, res)) return;
    ...
    // v1.127 Wave A (external review round 2): the library-wide reheat used to
    // enumerate EVERY item - the per-item sibling below gained its visibility
    // axis in v1.123, this batch did not - so a manage-subs member restricted
    // from a folder could reheat and even RELOCATE its files. The predicate is
    // captured req-FREE (deps.mediaVisiblePredicate) because the batch runs on
    // after the 202; the enumerator applies it so hidden items enter neither
    // the worklist nor the reported counts.
    const itemVisible = typeof deps.mediaVisiblePredicate === 'function' ? deps.mediaVisiblePredicate(req) : null;
    const { items, eligible, ineligible, withSourceId } = deps.enumerateRepullableItems(db, config, itemVisible);
    ...
    //
    // v1.41.5: `withSourceId` (how many of the eligible items actually have a
    // YouTube id, i.e. will get a NETWORK pass) is reported alongside the raw
    // eligible count, because the widened, root-agnostic gate now enumerates
    // ordinary library files too -- they cost only a local, network-free
    // ffprobe. Without this the summary line would imply FileTube is about to
    // fetch every home video in the library. `?? 0` keeps a stubbed
    // enumerator in older tests from rendering `undefined`.
    res.status(202).json({ started: true, eligible, ineligible, withSourceId: withSourceId ?? 0 });

    repullMetadataInProgress = true;
    repullMetadataCancelled = false;
    repullMetadataJobKind = 'batch';
  ...
  app.post('/api/ytdlp/repull-metadata/cancel', (req, res) => {
    ...
    // v1.49: a per-video reheat is ONE item, and an in-flight item always runs
    // to completion (there is no per-item spawn handle to signal -- see this
    // route's own comment above). "Stop before the next item" therefore cancels
    // nothing at all here, so say so instead of claiming a cancellation that
    // will not happen.
    if (repullMetadataJobKind === 'item' || repullMetadataJobKind === 'relocate') {
      // A relocation joins the single-item run on the honest-answer side for a
      // stronger reason than "there is no next item": a file move is mid-flight
      // (possibly mid-copy), and there is no safe point to abandon it. Claiming
      // a cancellation here would be worse than useless.
      return res.json({ cancelled: false, reason: repullMetadataJobKind === 'relocate' ? 'relocating' : 'single-item' });
    }
    repullMetadataCancelled = true;
    res.json({ cancelled: true });
  });

  // ---- v1.41.7 (Dean has NO media backup): DRY-RUN preview of the reheat's ----
  // import relocation -----------------------------------------------------------
  //
  // Dean cannot back up his media, so before he runs the bulk, irreversible
  // relocation (the reheat above physically MOVES hydrated imports into channel
  // folders -- and across filesystems that means copy-then-delete-the-original)
  // he needs to SEE exactly what it will do. This route computes the FULL plan
  // from EXISTING db state ONLY and returns it -- it moves nothing, writes
  // nothing, spawns no yt-dlp, and makes NO network call.
  //
  // STRUCTURALLY WRITE-FREE: the whole computation is `deps.previewImportRelocations`
  // (server.js's `buildImportRelocationPreview`), which only reads the db + the
  // filesystem via the SAME `planImportRelocation` predicate the executor uses --
  // so the preview can never disagree with the real op. This route adds no
  // `updateDatabase`, no `runExclusive`, no spawn of its own.
  //
  // Read-only, so it is deliberately NOT gated on the single-flight
  // `repullMetadataInProgress` latch (unlike the reheat trigger above): a preview
  // during a running batch simply reflects the db as it currently stands. GET
  // would be equally valid; POST mirrors the rest of this route family and keeps
  // it off the caches/prefetchers a GET can attract.
  app.post('/api/ytdlp/repull-metadata/preview', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    if (!deps || typeof deps.previewImportRelocations !== 'function') {
      // Defense-in-depth only -- server.js always wires it (see the
      // `registerRoutes` call site). A deployment missing it gets a clean 503.
      return res.status(503).json({ error: 'Relocation preview is unavailable (missing server wiring)' });
    }
    let preview;
    try {
      // v1.127 Wave A: same visibility axis as the batch route above - the
      // preview renders CURRENT and DESTINATION paths per item, which must
      // never include an item the requester cannot see.
      const itemVisible = typeof deps.mediaVisiblePredicate === 'function' ? deps.mediaVisiblePredicate(req) : null;
      preview = deps.previewImportRelocations(deps, config, itemVisible);
    } catch (err) {
      console.error('yt-dlp: repull-metadata preview failed:', err && err.message);
      return res.status(500).json({ error: 'Could not compute the relocation preview' });
    }
    res.json(preview);
  });
  ...
  //
  // "I'd want the ability to, on a specific video, force a reheat." The
  // library-wide route above is all-or-nothing: it enumerates every item, and
  // a non-force run skips everything already marked, so there has never been a
  // way to refresh ONE video's channel identity, title, chapters, subtitles or
  // view count. This is that way.
  //
  // THREE properties this route is built around, each the residue of a past
  // gate finding:
  //
  //  1. NO FORKED ELIGIBILITY. The candidate is found by running the SAME
  //     `enumerateRepullableItems` the batch runs and filtering to one id --
  //     an O(N) pure pass with no I/O, paid once per click. A per-item
  //     eligibility copy would be free to disagree with the batch about what
  //     is reheatable, and "the button says no but the batch says yes" is
  //     exactly the class of drift v1.41.7 spent three gate rounds killing.
  //  2. NO METADATA PASS FORK either -- `reheatOneItem` (above) is shared with
  //     the batch verbatim.
  //  3. THE FILE IS NEVER MOVED HERE. A reheat does two things per item, and
  //     the batch does both back-to-back; on a per-video click only the
  //     reversible half runs. The irreversible half is PROPOSED (destination +
  //     hard-link-vs-copy, straight out of `planImportRelocation`) and happens
  //     only if the user confirms it on the sibling route below. Dean has no
  //     media backup: nothing irreversible may ever be a side effect of
  //     something reversible.
  app.post('/api/ytdlp/repull-metadata/item/:mediaId', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    // Same safe-mode gate as the library-wide trigger: this writes .vtt
    // sidecars into shared media dirs and mutates shared db state.
    if (refuseIfReadOnlyMedia(config, res)) return;
    // ONE latch across both reheat shapes (decision 5). A per-video run and a
    // library batch touch the same items through the same single serialized
    // writer, and letting them interleave would mean two `recordRepulledItemMeta`
    // passes racing over one item's identity.
    if (repullMetadataInProgress) {
      return res.status(409).json({ started: false, alreadyRunning: true });
    }
    if (!deps || typeof deps.loadDatabase !== 'function'
    ...
    const mediaId = typeof req.params.mediaId === 'string' ? req.params.mediaId : '';
    if (mediaId === '') return res.status(400).json({ error: 'A media id is required' });

    let db;
    try {
      db = deps.loadDatabase();
    } catch (err) {
      console.error('yt-dlp: per-video reheat could not read the database:', err && err.message);
      return res.status(500).json({ error: 'Could not read the database' });
    }

    // v1.123 T3 (gate, adversarial W3): the VISIBILITY axis. This route is
    // requireManageSubscriptions-gated (capability), but a manage-subs member
    // RESTRICTED from this item's folder must still not repull it. Neutral 404
    // (identical shape to the not-eligible path below), before any work.
    const visItem = db.metadata && Object.prototype.hasOwnProperty.call(db.metadata, mediaId) ? db.metadata[mediaId] : null;
    if (deps.mediaVisibleTo && visItem && !deps.mediaVisibleTo(req, visItem)) {
      return res.status(404).json({ error: 'That video is not eligible for a reheat', mediaId });
    }

    // v1.127 Wave A: thread the same predicate the batch route threads - the
    // explicit check above already refused a hidden TARGET; this keeps the
    // enumeration itself consistent with the batch (a hidden sibling can never
    // even appear in `items` to be matched).
    const itemVisible = typeof deps.mediaVisiblePredicate === 'function' ? deps.mediaVisiblePredicate(req) : null;
    const { items } = deps.enumerateRepullableItems(db, config, itemVisible);
    const target = (Array.isArray(items) ? items : []).find((it) => it && it.mediaId === mediaId);
    if (!target) {
      // Either the id is not in the library at all, or it is an item the
      // enumerator classifies as ineligible (no filePath). Honest 404 rather
      // than a 202 that would resolve to a silent no-op.
      return res.status(404).json({ error: 'That video is not eligible for a reheat', mediaId });
    }

    // The BEFORE snapshot, taken from the db read we already hold, so the
    // completion can tell the user what actually changed instead of asserting
    // that something did. Fields only -- never the whole item (it is large and
    // most of it is irrelevant to a diff).
    const before = snapshotReheatableFields(db.metadata && db.metadata[mediaId]);

    res.status(202).json({ started: true, mediaId });

    repullMetadataInProgress = true;
    // Symmetry with the batch trigger: clear the cooperative-cancel latch at
    // the START of a run, so a cancel left set by a previously cancelled batch
    // can never bleed into this one.
    repullMetadataCancelled = false;
    repullMetadataJobKind = 'item';
    runSingleItemReheat(deps, config, target, before).catch((err) => {
      // Defense-in-depth only -- `runSingleItemReheat` has its own try/finally.
      console.error('yt-dlp: per-video reheat failed unexpectedly:', err && err.message);
      repullMetadataInProgress = false;
      repullMetadataJobKind = null;
      activity.setOneShot(REPULL_ITEM_ACTIVITY_ID, { state: 'error', current: null });
    });
  });

  // ---- v1.49: the CONFIRM step -- actually move the file ----------------------
  //
  // Separate route, separate click, separate decision. The route above only
  // ever PROPOSES a relocation; this is the one that performs it, and it is the
  // only per-video path that can touch a byte on disk.
  //
  // Deliberately re-runs the whole decision inside
  // `relocateHydratedImportIntoChannelFolder` (which calls `planImportRelocation`
  // itself, against a FRESH db read) rather than trusting anything the proposal
  // said. The proposal may be seconds or minutes old: the destination could
  // have become occupied, the setting could have been switched off, a scan
  // could have re-keyed the item. A confirm dialog is permission to attempt the
  // move, never a cached authorisation to skip its preconditions.
  app.post('/api/ytdlp/repull-metadata/item/:mediaId/relocate', async (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    if (refuseIfReadOnlyMedia(config, res)) return;
    if (repullMetadataInProgress) {
      return res.status(409).json({ moved: false, alreadyRunning: true });
    }
    if (!deps || typeof deps.relocateHydratedImport !== 'function') {
      return res.status(503).json({ error: 'Relocation is unavailable (missing server wiring)' });
    }
    const mediaId = typeof req.params.mediaId === 'string' ? req.params.mediaId : '';
    if (mediaId === '') return res.status(400).json({ error: 'A media id is required' });

    // v1.123 T3 (gate, adversarial W3): the VISIBILITY axis. relocate MOVES the
    // file, so a manage-subs member restricted from this item's folder must not
    // reach it. A fresh read just for the guard; neutral 404, before the latch.
    const visDb = deps.loadDatabase ? deps.loadDatabase() : null;
    const visItem = visDb && visDb.metadata && Object.prototype.hasOwnProperty.call(visDb.metadata, mediaId) ? visDb.metadata[mediaId] : null;
    if (deps.mediaVisibleTo && visItem && !deps.mediaVisibleTo(req, visItem)) {
      return res.status(404).json({ error: 'That video is not eligible for relocation', mediaId });
    }

    // GATE FIX (QA WARNING): this route CHECKED the shared latch but never HELD
    // it, so decision 5's "409 both ways" only held in one direction -- a
    // library-wide reheat (or a second per-video one) could start while this
    // relocation was still mid-copy on the same file. That race resolves safely
    // today, but only because of machinery this feature does not own:
    // `moveItemToFolder`'s atomic exclusive-create and the re-key mutator's
    // vanished-item no-op. Safety that is emergent from unrelated code is not
    // safety this feature established, and on a file move with no backup that
    // distinction is the whole point. The latch is now HELD for the duration,
    // released in a finally so a throw can never wedge every reheat route.
    // v1.49 GATE FIX (adversarial CRITICAL 3): the client echoes back the
    // destination it actually showed the user, and the executor refuses the move
    // if the fresh plan disagrees (see the PROPOSAL BINDING block in server.js).
    // Optional on the wire: a scripted caller with no `expect` at all gets the
    // pre-fix behaviour rather than a hard error, and the check is simply not
    // applied. Every echoed value is compared as an OPAQUE token -- string
    // equality for the two paths and `transfer`, numeric equality for
    // `sizeBytes`. Nothing is ever derived from them: they are never joined to
    // a path, resolved, or used to locate anything, so a hostile echo can only
    // ever cause the caller's OWN move to be refused.
    const body = req.body && typeof req.body === 'object' ? req.body : {};
    // GATE FIX (adversarial delta, condition 2): ALL-OR-NOTHING.
    //
    // The first cut accepted a PARTIAL `expect` and silently degraded to
    // binding the two paths alone, justified as kindness to "an older caller".
    // No such caller exists or can exist: `expect` and `transfer` were
    // introduced in the same unreleased wave, so nothing deployed knows this
    // route but not that field. The branch protected nobody, and the first
    // thing that would ever reach it is a client refactor that drops a field --
    // which is this repo's single most-repeated bug class (v1.41.9, v1.41.13,
    // v1.47.4), and silent degradation is exactly how that class survives a
    // release.
    //
    // So: omitting `expect` entirely is still a supported escape hatch (it is
    // what the batch and the whole-library preview do, by passing no opts at
    // all). Sending a partial one is a client bug and is refused loudly.
    let expect = null;
    if (body.expect !== undefined && body.expect !== null) {
      const e = body.expect;
      // `sizeBytes: null` is ACCEPTED (gate fix, QA SUGGESTION 2): the planner
      // legitimately yields `null` when its own `statSync` loses a TOCTOU race
      // with the `existsSync` above it, and `transfer` legitimately yields
      // 'unknown' the same way. A client honestly echoing back the proposal it
      // was given must not be 400'd for faithfully reporting what it was shown.
      // The executor's `typeof e.sizeBytes === 'number'` guard then simply does
      // not compare that field, which is the right graceful degrade -- a size
      // nobody could measure is not a promise anyone can break.
      const wellFormed = e && typeof e === 'object'
        && typeof e.currentPath === 'string'
        && typeof e.destinationPath === 'string'
        && typeof e.transfer === 'string'
        && (typeof e.sizeBytes === 'number' || e.sizeBytes === null);
      if (!wellFormed) {
        return res.status(400).json({
          error: 'expect must carry currentPath, destinationPath, transfer and sizeBytes',
        });
      }
      expect = {
        ...
        transfer: e.transfer,
        // Forwarded only when it is a real number -- a `null` echo means the
        // proposal itself could not measure the file (see above), and the
        // executor must skip that comparison rather than fail it.
    ...
    let relocation;
    repullMetadataInProgress = true;
    repullMetadataJobKind = 'relocate';
    try {
      relocation = await deps.relocateHydratedImport(deps, config, mediaId, {
        allowRecentlyWatched: true,
        ...(expect ? { expect } : {}),
      });
    } catch (err) {
      console.error(`yt-dlp: per-video relocation of ${mediaId} threw unexpectedly:`, err && err.message);
      return res.status(500).json({ status: 'failed', reason: 'relocation threw' });
    } finally {
      repullMetadataInProgress = false;
      repullMetadataJobKind = null;
    }

    const status = (relocation && relocation.status) || 'skipped';
    if (status === 'moved') {
      // `newId` is LOAD-BEARING for the client: the id is a hash of the file
      // path, so the move re-keyed this item and the caller's current watch
      // page is now pointing at an id that no longer resolves.
      //
      // `archived: false` is surfaced, never buried: the file is fine and
      // correctly filed, but it is NOT in `.ytdlp-archive.txt`, so a
      // subscription poll of that channel may re-download it -- and the
      // relocated filename is not the one yt-dlp would write, so yt-dlp's own
      // "already exists" skip cannot save us either.
      return res.json({
        status: 'moved',
        newId: relocation.newId,
        newPath: relocation.newPath,
        archived: relocation.archived !== false,
      });
    }
    // v1.49 (CRITICAL 3): the plan moved under the user's feet. 409, carrying
    // the destination that is on the table NOW so the client can re-ask rather
    // than the server quietly doing a move nobody approved.
    if (status === 'stale') {
      return res.status(409).json({
        ...
        relocation: {
          ...
          sameDevice: relocation.sameDevice,
          sizeBytes: relocation.sizeBytes,
        },
      });
    }
    // A skip is not an error -- 'destination-occupied' means the user has two
    // copies of one video and gets to decide, which is information, not failure.
    // v1.49 gate fix (adversarial WARNING 3): `failed` is NOT the same kind of
    // news as `skipped`. A cross-device checksum mismatch is an integrity
    // failure about an irreplaceable file and must not reach the user wearing
    // the same neutral wording as "you have two copies of this video".
    return res.json({
      status,
      failed: status === 'failed',
      reason: (relocation && relocation.reason) || 'not a relocation candidate',
    });
  });
  ...
  app.post('/api/ytdlp/refresh-avatars', (req, res) => {
    ...
    let db;
    try {
      db = withYtdlp(deps, deps.loadDatabase());
    } catch (err) {
  ...
  // ---- v1.56: "Reheat sub counts" -- bulk subscriber-count refresh ---------
  //
  // Re-probes EVERY distinct channel's subscriber count (one cheap channel-
  // level dump each, never per-video) and fans it out to all attributed
  // library items -- see `runReheatSubsBatch`'s own doc comment for the full
  // design. Mirrors the refresh-avatars route pair verbatim: hard
  // single-flight 409, 202-with-total posture, durable cooperative cancel.
  //
  // NEVER auto-run (thumbnail-backfill-regression lesson, same posture as
  // the reheat/refresh-avatars batches): this route is the ONLY trigger --
  // no boot path, no scan path, nothing on a timer (see the structural
  // regression test asserting exactly this).
  app.post('/api/ytdlp/reheat-sub-counts', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    if (reheatSubsInProgress) {
      return res.status(409).json({ started: false, alreadyRunning: true });
    }
    ...
    try {
      ...
      console.error('yt-dlp: reheat-sub-counts could not read the database:', err && err.message);
      return res.status(500).json({ error: 'Could not read the database' });
    }
    // SAME unit of work as refresh-avatars: every distinct channel drawn
    // from subscriptions AND db.metadata items (Dean's ruling 1: any channel
    // with content qualifies -- counts render on every watch page).
    const targets = collectDistinctChannelAvatarTargets(db);
    const total = targets.length;

    res.status(202).json({ started: true, total });

    reheatSubsInProgress = true;
    reheatSubsCancelled = false;
    runReheatSubsBatch(deps, config, targets).catch((err) => {
      // Defense-in-depth only -- runReheatSubsBatch already has its own
      // top-level try/finally and should never actually reject.
      console.error('yt-dlp: reheat-sub-counts batch failed unexpectedly:', err && err.message);
      reheatSubsInProgress = false;
      activity.setOneShot(REHEAT_SUBS_ACTIVITY_ID, { state: 'error', current: null });
    });
  });

  // Cancel an in-progress reheat-subs batch -- the refresh-avatars cancel's
  // posture exactly (durable latch, idempotent no-op when idle; no per-item
  // ChildProcess handle to SIGKILL -- `run.probeChannelFollowerCount` owns
  // its own spawn internally and never exposes it).
  app.post('/api/ytdlp/reheat-sub-counts/cancel', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    if (!reheatSubsInProgress) {
      return res.json({ cancelled: false });
    }
    reheatSubsCancelled = true;
    res.json({ cancelled: true });
  });

  // ---- v1.115 (Dean, A1): "Refresh channel names" backfill -------------------
  // The reheat-sub-counts route's posture exactly: RBAC-gated, single-flight,
  // 202-with-total, background batch. It WRITES db.metadata channel names +
  // pin labels, so (unlike the read-only sub-count reheat) it also honors the
  // read-only-media safe mode. `total` is the number of DISTINCT bad-name
  // channels that have an identity to probe (the honest reach).
  app.post('/api/ytdlp/backfill-channel-names', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    if (refuseIfReadOnlyMedia(config, res)) return; // it mutates db.metadata (channel names) + pins
    if (channelNameBackfillInProgress) {
      return res.status(409).json({ started: false, alreadyRunning: true });
    }
    let db;
    try {
      ...
      console.error('yt-dlp: backfill-channel-names could not read the database:', err && err.message);
      return res.status(500).json({ error: 'Could not read the database' });
    }
    const targets = collectDistinctChannelNameTargets(db);
    const total = targets.length;
    // v1.116: the batch ALSO runs a local heal pass first; surface how many
    // channels that will fix so the client can message it (instant, no network).
    const healChannels = collectLocalChannelHealTargets(db).length;

    res.status(202).json({ started: true, total, healChannels });

    channelNameBackfillInProgress = true;
    channelNameBackfillCancelled = false;
    runChannelNameBackfillBatch(deps, config, targets).catch((err) => {
      console.error('yt-dlp: backfill-channel-names batch failed unexpectedly:', err && err.message);
      channelNameBackfillInProgress = false;
      activity.setOneShot(CHANNEL_NAME_BACKFILL_ACTIVITY_ID, { state: 'error', current: null });
    });
  });

  app.post('/api/ytdlp/backfill-channel-names/cancel', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (W2)
    if (!channelNameBackfillInProgress) {
      return res.json({ cancelled: false });
    }
    channelNameBackfillCancelled = true;
    res.json({ cancelled: true });
  });
  ...
  // -- never persisted, never touches the database. Registered inside this same
  ...
  // ---- v1.47.4 item 7: the download failure log ---------------------------
  //
  // Dean: "If any downloads fail I'd like it to be explicitly logged and have
  // the error so one can look in posterity. It should be able to be
  // cleared/deleted as well. It's different than subscription downloads."
  //
  // ONE unified log covering BOTH lanes, with a `source` field the client
  // filters on, rather than two parallel logs -- a failure is a failure, and
  // splitting the store would mean two retention policies, two clear buttons,
  // and two places to look for "why is this video missing".
  //
  // Registered inside the SAME `isEnabled` gate as every route above, so all
  // three are equally absent (native Express 404) when the module is disabled,
  // and none of them ever creates the log file (only a `recordFailures` write
  // does, from the two terminal call sites above).
  //
  // Degrade, never crash: `faillog` already never throws, but each handler is
  // wrapped anyway so a future contract change cannot take the route down.
  app.get('/api/subscriptions/failures', (req, res) => {
    try {
      const requestedLimit = Number.parseInt(req.query && req.query.limit, 10);
      const limit = Number.isFinite(requestedLimit) && requestedLimit > 0
        ? requestedLimit
        : faillog.YTDLP_FAILLOG_MAX_ENTRIES;
      // `readFailures` already returns NEWEST-first and caps at its own
      // maximum regardless of what `limit` asks for, so this handler neither
      // reverses nor re-slices -- it cannot accidentally widen the cap.
      res.json({ entries: faillog.readFailures(deps && deps.dataDir, limit) });
    } catch (err) {
      console.error('yt-dlp: failed to read failure log (degrading to empty):', err && err.message);
      res.json({ entries: [] });
    }
  });

  // ROUTE-SHADOW WARNING (caught by this feature's own integration test, and
  // the reason both delete paths carry TWO segments after `/failures`):
  // `app.delete('/api/subscriptions/:id')` is registered far above, so a
  // single-segment `DELETE /api/subscriptions/failures` never reaches this
  // block at all -- Express matches it as "delete the subscription whose id is
  // 'failures'" and 404s from that handler instead. The existing
  // `/api/subscriptions/pins/:id` route already dodges the same shadow the same
  // way. GET is unaffected (there is no `GET /api/subscriptions/:id`), which is
  // exactly what made this easy to miss by hand.
  //
  // Clear-all is registered BEFORE the per-entry route so the literal `all`
  // wins over `:id`. That ordering is safe by construction, not by luck: ids
  // are `crypto.randomUUID()` values, which can never be the string 'all'.
  //
  // Reports the REAL number removed (never an assumed one) so the client's
  // confirmation cannot overstate what happened.
  app.delete('/api/subscriptions/failures/all', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (closes #54)
    try {
      return res.json({ ok: true, removed: faillog.clearFailures(deps && deps.dataDir) });
    } catch (err) {
      console.error('yt-dlp: failed to clear failure log:', err && err.message);
      return res.status(500).json({ error: 'Could not clear the failure log.' });
    }
  });

  // Per-entry delete. A 404 for an unknown id is the honest answer (the entry
  // is not there to delete) and is what a double-click or a stale tab gets --
  // `faillog.deleteFailure` guarantees a no-match rewrites nothing at all, so
  // a stale id can never disturb the rest of the log.
  app.delete('/api/subscriptions/failures/:id', (req, res) => {
    if (deps.requireManageSubscriptions && !deps.requireManageSubscriptions(req, res)) return; // v1.80 RBAC (closes #54)
    try {
      const removed = faillog.deleteFailure(deps && deps.dataDir, req.params && req.params.id);
      if (!removed) return res.status(404).json({ error: 'No such failure entry.' });
      return res.json({ ok: true });
    } catch (err) {
      console.error('yt-dlp: failed to delete failure entry:', err && err.message);
      return res.status(500).json({ error: 'Could not delete that entry.' });
    }
  });
}
 ...
 */
// v1.42 safe-mode lever (exec plan AC8): one honest refusal for every route
// that would mutate SHARED state -- media dirs or the yt-dlp archive/skip
// sidecar files -- while FILETUBE_READ_ONLY_MEDIA=1. Returns true when the
// request was refused (caller returns immediately).
function refuseIfReadOnlyMedia(config, res) {
  if (!config || config.readOnlyMedia !== true) return false;
  res.status(403).json({
    error: 'read-only media mode: this instance runs with FILETUBE_READ_ONLY_MEDIA=1 (parallel-run beta protection) — downloads, re-pulls, reheat, and skip-list writes are disabled here. Run them from the primary instance instead.',
    readOnlyMedia: true,
  });
  return true;
}

// Log the scheduled-poll no-op ONCE per process, not once per tick — the
// tick fires forever at pollMinutes cadence and a per-tick line would be
// pure log noise for the whole beta window.
let readOnlyPollSkipLogged = false;

function scheduledPollTick(deps, config) {
  if (config && config.readOnlyMedia === true) {
    if (!readOnlyPollSkipLogged) {
      readOnlyPollSkipLogged = true;
      console.log('yt-dlp: scheduled polls are DISABLED for this process — FILETUBE_READ_ONLY_MEDIA=1 (beta safe mode). This line logs once; every tick is a silent no-op.');
    }
    return;
  }
  if (breakerResumeTimer) {
 ...
 * database (the common case; every fresh/never-affected install) never
...
function migrateStaleDownloadDirFromFolders(deps, config) {
  ...
  // Wave 4: the root list is a table behind server.js's folderStore; the
  // deps seam reads it and prunes the stale entry INSIDE the doc commit's
  // transaction (inSaveTransaction) so a failed save leaves it in place.
  if (typeof deps.getLibraryFolders !== 'function' || typeof deps.removeLibraryFolder !== 'function' || typeof deps.inSaveTransaction !== 'function') {
    return Promise.resolve();
  }
  let existingFolders;
  try {
    existingFolders = deps.getLibraryFolders();
  } catch (err) {
    console.error('yt-dlp: could not read the library folders to check for a stale downloadDir entry:', err && err.message);
    return Promise.resolve();
  }
  ...
  let updateResult;
  try {
    updateResult = deps.updateDatabase(() => {
      const stale = deps.getLibraryFolders().filter(isStaleEntry);
      if (stale.length === 0) return false; // already gone by the time the lock was acquired
      // The prune rides the doc commit's transaction - never a direct write
      // (a failed save must leave the entry in place, the "try again next
      // start" contract).
      deps.inSaveTransaction(() => { for (const f of stale) deps.removeLibraryFolder(f); });
      console.log('yt-dlp: migrated a stale downloadDir entry out of the library folders (the module now owns this root via extraScanRoots)');
      return true;
    });
  } catch (err) {
    console.error('yt-dlp: failed to migrate a stale downloadDir entry out of the library folders:', err && err.message);
    return Promise.resolve();
  }
...
function refreshYtdlpVersionCache(opts = {}) {
  ...
  if (!opts.force && !ytdlpVersionCache) return Promise.resolve();
  // v1.146 gate round 2 (adversarial W-S1): `force` must beat the TTL
  // guard too - the engine selector swaps the ACTIVE binary under this
  // cache, and before this fix an activation/revert-time "forced" refresh
  // inside a primed 6h window was a measured NO-OP (the About row kept the
  // OLD engine's version labeled with the NEW channel). The probe promise
  // is RETURNED (every path resolves) so the install job's await is a real
  // ordering, not decoration.
  if (!opts.force && ytdlpVersionCache && now - ytdlpVersionCache.checkedAt < YTDLP_VERSION_TTL_MS) {
    return Promise.resolve();
  }
...
// v1.41.0: read-only accessor for the cached yt-dlp version, for the Stats
// page's About section (the yt-dlp version moved there from Subscriptions).
// Nudges a TTL-guarded refresh (fire-and-forget, spawn-free unless the cache
// was primed by startBackground) and returns the current cached value (null
// until the first probe resolves, or if yt-dlp isn't on PATH).
function getCachedYtdlpVersion() {
  refreshYtdlpVersionCache();
  return ytdlpVersionCache ? ytdlpVersionCache.version : null;
}

function startBackground(deps = {}, config = parseYtdlpConfig()) {
  if (!isEnabled(config)) return;
  ensureDownloadDir(config);
  // 2026-07-31 (capture-profile gate finding): the read-only-media belt
  // was REQUEST-scoped only - these two boot-time writers ran before any
  // request existed, so a data dir carrying pending one-shots (e.g.
  // restored from the primary's backup onto a capture/beta instance)
  // would START DOWNLOADING at process start with every request-level
  // protection blind to it. Safe mode now covers boot work too.
  if (config.readOnlyMedia !== true) {
    migrateStaleDownloadDirFromFolders(deps, config);
  } else {
    console.log('yt-dlp: read-only media mode - skipping boot-time folder migration.');
  }
  armYtdlpTimer(config, deps);
  // v1.146 T6 (downloader-engine): bind the spawn seam for the background
  // side (startBackground's deps bundle is its OWN copy - the v1.29 lesson;
  // last init wins and both bundles carry identical closures), reconcile a
  // wanted-but-missing engine asynchronously, and arm the daily gate. All
  // BEFORE the version-cache prime below, so the primed probe already
  ...
  if (config.readOnlyMedia !== true) {
    ...
    console.log('yt-dlp: read-only media mode - pending one-shots left queued, not launched.');
  }
}

module.exports = {
  registerRoutes,
  // v1.338 (plan first-class-any-site D2): the saved-page-link fallback (pure; tested directly).
  withUniversalPageUrl,
  getCachedYtdlpVersion,
  // v1.146 (downloader-engine T5): the bell-event vocabulary - server.js's
  // notification list route parses/describes engine feed rows through these
  // (defensive parse; malformed ids render nothing).
  parseEngineNotificationId: engineLib.parseEngineNotificationId,
  describeEngineEvent: engineLib.describeEngineEvent,
  buildEngineNotificationId: engineLib.buildEngineNotificationId,
  // v1.146 T4: the engine job's observability seam (tests await the queued
  // job's promise; buildEngineStatus embeds the same object as `busy`) and
  // the shared update flow (the T6 daily tick's body).
  currentEngineJob,
  executeEngineUpdate,
  // v1.146 T8: the About/Stats channel summary (null when unbound - the
  // pre-wave plain version row).
  getEngineSummary: engineLib.runtimeStateSummary,
  // v1.146 T6: the daily gate + boot reconcile (tick/reconcile exported so
  // tests fire them without waiting out real intervals - the
  // scheduledPollTick posture; the timer accessor mirrors
  // currentYtdlpPollTimer).
  armEngineTimer,
  currentEngineTimer,
  engineAutoUpdateTick,
  reconcileEngineAtBoot,
  ENGINE_AUTO_UPDATE_INTERVAL_MS,
  armYtdlpTimer,
  currentYtdlpPollTimer,
  startBackground,
  runPoll,
  ...
  // v1.38.0 TTS defer coordination: read-only "download/poll active" signal
  // (+ the FIFO gate itself, exported so the accessor's true-state is testable).
  isHeavyJobActive,
  runExclusive,
  getPollBreakerState,
  clearPollBreakerState,
  ...
  // v1.41.6 (import relocation): "which folder does a video by channel X belong
  // in" -- reusing an EXISTING subscription's own `resolveChannelDir(config, sub)`
  // folder when the channel is subscribed (a subscription folds `sub.name`, an
  // item carries the real `channelName`, and the two routinely differ), else the
  // channel's display name. Exported for server.js's relocator + direct coverage.
  resolveChannelDirForChannel,
  // v1.41.6 gate fix (CRITICAL): the id/handle-aware subscription join behind it,
  // the "we cannot tell -- do not guess" predicate the relocator refuses to move
  // on, and the opportunistic write-once channelId backfill. All exported for the
  // direct unit coverage the gate found missing.
  findSubscriptionForChannel,
  hasAmbiguousChannelSubscription,
  backfillSubscriptionChannelIdForChannel,
  // v1.41.6 gate fix round 3 (QA CRITICAL): a canonical `/channel/UC…` URL
  // literally CONTAINS the channel id -- reading it out makes the id branch work
  // even when neither side ever stored the field. Exported for direct coverage.
  extractChannelIdFromUrl,
  // v1.37.5 (Dean's "Skip" action): permanent per-video skip-list read/append,
  // exported for direct unit coverage (mirrors recordOneShotInArchive above).
  readSkiplistTextSafely,
  recordSkip,
  ...
  // v1.41.13 (universal one-offs): the non-YouTube scan bridge read, re-exported
  // for server.js's scan mutator (same layering as consumeDownloadChannelMeta).
  consumeUniversalDownloadMeta: store.consumeUniversalDownloadMeta,
  ...
  // v1.48 item 2: re-exported so server.js's recordRepulledItemMeta can
  // re-validate a view count at its own write boundary through the SAME gate
  // the capture path used, never a forked second check.
  parseCapturedViewCount: store.parseCapturedViewCount,
  // v1.54: the follower-count validator, same write-boundary layering.
  parseCapturedFollowerCount: store.parseCapturedFollowerCount,
  // v1.53 (manual attribution): the avatar sanitizer, re-exported for the
  // attribute endpoint's write boundary -- same single-gate layering as
  // sanitizeCapturedTitle above.
  sanitizeChannelAvatarUrl: store.sanitizeChannelAvatarUrl,
  // v1.53: the UC... shape check, for the attribute endpoint's channelId
  // validation through the SAME pattern the capture path uses.
  CHANNEL_ID_PATTERN: store.CHANNEL_ID_PATTERN,
  ...
  // v1.49 (per-video reheat): the sibling one-shot key, the shared per-item
  // pass, and the field-snapshot helper -- exported for the same reason as the
  // batch's own key above (tests read the activity entry and drive the shared
  // pass directly, without booting an HTTP server or hardcoding literals).
  REPULL_ITEM_ACTIVITY_ID,
  reheatOneItem,
  snapshotReheatableFields,
  ...
  // v1.56 (bulk subscriber-count reheat): the "Reheat sub counts" batch's
  // one-shot key + test-only state reset -- same posture as
  // REFRESH_AVATARS_ACTIVITY_ID/resetRefreshAvatarsStateForTests above.
  REHEAT_SUBS_ACTIVITY_ID,
  CHANNEL_NAME_BACKFILL_ACTIVITY_ID,
  resetReheatSubsStateForTests,
  runChannelNameBackfillBatch,
  resetChannelNameBackfillStateForTests,
  ...
  sweepItemChannelAvatars,
  resetItemAvatarSweepStateForTests,
  isBadChannelName,
  isChannelBearingMediaType,
  collectDistinctChannelNameTargets,
  applyBackfilledChannelName,
  collectLocalChannelHealTargets,
  folderKeyOf, // v1.126: the heal fanout's folder-display-name write keys on it
