...
async function runSingleItemReheat(deps, config, item, before) {
  ...
  try {
    ...
    try {
      ...
      activity.setOneShot(REPULL_ITEM_ACTIVITY_ID, { state: 'error', failed: 1, current: null });
      return;
    }
...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  app.post('/api/ytdlp/repull-metadata/cancel', (req, res) => {
    ...
    // will not happen.
    if (repullMetadataJobKind === 'item') {
      return res.json({ cancelled: false, reason: 'single-item' });
    }
