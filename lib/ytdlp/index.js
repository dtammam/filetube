...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  // Clear-all. Reports the REAL number removed (never an assumed one) so the
  // client's confirmation can't overstate what happened.
  app.delete('/api/subscriptions/failures', (req, res) => {
    try {
