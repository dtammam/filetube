...
// The auto-revert side effects shared by the runtime failure net and the
// install flows: bell the admin, and force the version cache off the
// now-stale binary reading.
...
function reconcileEngineAtBoot(deps) {
  try {
    ...
    const healthy = state.active === 'venv' && state.installed &&
      fs.existsSync(engineLib.resolveVenvBinaryPath(deps.dataDir));
...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  engineLib.initRuntime({ dataDir: deps.dataDir, onAutoRevert: buildEngineAutoRevertHandler(deps) });
