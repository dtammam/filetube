...
async function runSubscriptionCycle(deps, config, sub) {
  ...
  if (!listResult.ok) {
    ...
    // doc comment.
    guardedSetSubscriptionActivity(deps, sub.id, { state: 'error', error: status });
    return status;
  }
...
async function processSubscription(deps, config, sub) {
  ...
  try {
    ...
    // FIX-6: terminal write guarded.
    if (sub && sub.id) guardedSetSubscriptionActivity(deps, sub.id, { state: 'error', error: lastStatus });
...
async function runOneShot(deps, config, { videoId, watchUrl, format, quality, filetype, folder, jobId }) {
  ...
  try {
    ...
    const syntheticSub = { id: jobId, name: folder, format, quality, filetype };
    ...
    const downloadResult = await run.runDownload(syntheticSub, config, [videoId], { onProgress, oneOff: true });
...
function registerRoutes(app, deps = {}, config = parseYtdlpConfig()) {
  ...
  app.get('/api/subscriptions/pins', (req, res) => {
    // v1.24 C6 (render hop): a pin record is a bare `{ id, channelDir, label,
    // pinnedAt }` snapshot and NEVER stores an avatar (see store.js's pin
    // schema). The captured `channelAvatarUrl` lives on the matching
    // SUBSCRIPTION record (written by `recordSubscriptionChannelAvatar` at
    // download time). Enrich here at READ time -- not stored on the pin -- so a
    // later-captured avatar shows up on an already-existing pin, and a pin with
    // no matching subscription simply falls through to the generated glyph
    // (client-side `derivePinnedPlaylistEntries` reads `channelAvatarUrl`).
    // Matched by resolved `channelDir` (the SAME key a pin is built from), and
    // re-validated through `sanitizeChannelAvatarUrl` again here (defense in
    // depth -- never trust a persisted value straight into an `<img src>`).
    ...
    const pins = store.listPins(deps).map((pin) => {
