...
async function runSubscriptionCycle(deps, config, sub) {
  ...
  if (!listResult.ok) {
    ...
    guardedSetSubscriptionActivity(deps, sub.id, { state: 'error', error: status, failures: [], warning: cookieWarning });
  ...
  if (survivorIds.length === 0) {
    ...
    return { status: 'ok: no new videos', outcome: 'success', succeeded: 0, failed: 0, failures: [], cookieWarning };
  }
  ...
  if (!downloadResult.ok) {
    ...
    if (cancelledSubscriptionIds.has(sub.id)) {
      ...
      guardedSetSubscriptionActivity(deps, sub.id, { state: 'error', error: 'cancelled', failures: [], warning: cookieWarning });
      return { status: 'cancelled', outcome: 'cancelled', succeeded: 0, failed: 0, failures: [], cookieWarning };
    }
    ...
    if (outcomeResult.outcome === 'partial') {
      ...
      return { status, outcome: 'partial', succeeded: outcomeResult.succeeded, failed: outcomeResult.failed, failures, cookieWarning };
    }
    ...
    return { status, outcome: 'error', succeeded: outcomeResult.succeeded, failed: outcomeResult.failed, failures, cookieWarning };
  }
  ...
  return { status, outcome: 'success', succeeded: survivorIds.length, failed: 0, failures: [], cookieWarning };
}
...
async function processSubscription(deps, config, sub, nowMs = Date.now(), avatarBudget = { remaining: Infinity }) {
  ...
  try {
    ...
    if (sub && sub.id) guardedSetSubscriptionActivity(deps, sub.id, { state: 'error', error: status, failures: [], warning: cookieWarning });
