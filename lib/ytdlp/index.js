...
function migrateStaleDownloadDirFromFolders(deps, config) {
  ...
  if (typeof deps.getLibraryFolders !== 'function' || typeof deps.removeLibraryFolder !== 'function') {
    return Promise.resolve();
  }
  let existingFolders;
  try {
  ...
  try {
    updateResult = deps.updateDatabase(() => {
      ...
      const prune = () => { for (const f of stale) deps.removeLibraryFolder(f); };
      if (typeof deps.inSaveTransaction === 'function') deps.inSaveTransaction(prune); else prune();
