...
async function reheatOneItem(deps, config, item, opts) {
  ...
  if (watchUrl) {
    ...
    result = await runExclusive(() => run.repullItemMetaAndSubs(sourcePage, item.filePath, config, { universal: true }));
  }
  ...
  // and a failed pass - a 429, a timeout - leaves it retryable) AND the local probe genuinely
  ...
  const exhausted = !watchUrl && !sourcePage && local !== null;
