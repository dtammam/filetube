 ...
 *
 * @returns {number} how many entries were actually persisted (0 on any failure)
 */
function recordFailures(dataDir, failures, nowMs) {
  ...
  const ok = writeAllEntries(dataDir, existing.concat(normalized));
  return ok ? normalized.length : 0;
}
