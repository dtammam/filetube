...
// v1.31 P2: the poll-run circuit breaker. After this many CONSECUTIVE
// channel failures within one poll run, the remainder of the run is aborted
// (YouTube throttling/bot-checks are cumulative -- once tripped, hammering
// the remaining channels only digs the hole deeper and produced Dean's
// 20-channel cascade). Successes/partials reset the counter. 0 = breaker
// disabled (pre-v1.31 plow-through behavior).
const DEFAULT_BREAKER_FAILURES = 4;
const MIN_BREAKER_FAILURES = 0;
const MAX_BREAKER_FAILURES = 50;

// v1.31 P2: how long a tripped breaker waits before automatically retrying
// the poll (the skipped channels' cutoffs never advanced, so the retry
// naturally re-targets exactly the un-run work).
const DEFAULT_BREAKER_BACKOFF_MINUTES = 30;
const MIN_BREAKER_BACKOFF_MINUTES = 1;
const MAX_BREAKER_BACKOFF_MINUTES = 1440;

// v1.36 F1: hard cap on how many playlist entries the LIST pass may even
// ENUMERATE (`--playlist-end`), as a wall-clock backstop behind the
// break-early filter (see args.js's `breakEarlyArgs`). The break filter is
// the real bound (stop at the first pre-cutoff video); this cap only
// matters if break-early somehow never triggers (a playlist yt-dlp returns
// in a non-newest-first order, a channel where every video passes the date
// filter). 200 comfortably exceeds any real between-polls upload burst while
// still bounding a pathological listing to minutes, not hours. 0 = cap off.
const DEFAULT_LIST_SCAN_CAP = 200;
const MIN_LIST_SCAN_CAP = 0;
const MAX_LIST_SCAN_CAP = 10000;
...
// v1.31 P2: breaker threshold (0 = off) and backoff -- same bounded-integer
// posture as every parser above.
function parseBreakerFailures(raw) {
  if (raw === undefined || raw === null || raw === '') return DEFAULT_BREAKER_FAILURES;
  const n = Number(raw);
  if (!Number.isInteger(n) || n < MIN_BREAKER_FAILURES || n > MAX_BREAKER_FAILURES) {
    return DEFAULT_BREAKER_FAILURES;
  }
  return n;
}

function parseBreakerBackoffMinutes(raw) {
  if (raw === undefined || raw === null || raw === '') return DEFAULT_BREAKER_BACKOFF_MINUTES;
  const n = Number(raw);
  if (!Number.isInteger(n) || n < MIN_BREAKER_BACKOFF_MINUTES || n > MAX_BREAKER_BACKOFF_MINUTES) {
    return DEFAULT_BREAKER_BACKOFF_MINUTES;
  }
  return n;
}

// v1.36 F1: list-pass enumeration cap. 0 IS valid ("cap off"), same
// convention as maxVideos/stallMinutes; anything invalid falls back to the
// default, never throws.
function parseListScanCap(raw) {
  if (raw === undefined || raw === null || raw === '') return DEFAULT_LIST_SCAN_CAP;
  const n = Number(raw);
  if (!Number.isInteger(n) || n < MIN_LIST_SCAN_CAP || n > MAX_LIST_SCAN_CAP) {
    return DEFAULT_LIST_SCAN_CAP;
  }
  return n;
}
...
// v1.145 (gate W2, the nightly-pin wave): FILETUBE_YTDLP_JS_RUNTIMES is the
// unset-by-default operator lever for yt-dlp's `--js-runtimes <value>` flag.
// yt-dlp deprecated running with NO JavaScript runtime (it prints "some
// formats may be missing" on every extraction - exactly the
// format-not-available failure shape Dean hit on-device), auto-enables only
// deno, and will not use a present-but-unlisted runtime. The Docker image
// ships node and defaults this ENV to `node` (Dockerfile); bare-metal
// deployments leave it unset, which emits NO flag - load-bearing, because
// the flag is version-sensitive and an older binary rejects unknown flags.
// Same strict charset/length allowlist posture as parsePlayerClient above:
// operator config, still validated as if hostile before reaching an argv
// element. Comma-separated runtime names only (e.g. "node" or "deno,node")
// - and NOTE (gate W3): yt-dlp itself never comma-splits this flag's value,
// so the argv builder (lib/ytdlp/args.js) splits the validated list and
// emits one REPEATED `--js-runtimes <name>` pair per runtime. A value that
// fails either check is treated exactly like "unset".
const JS_RUNTIMES_PATTERN = /^[a-z0-9_,-]+$/;
const MAX_JS_RUNTIMES_LENGTH = 64;

function parseJsRuntimes(raw) {
  const value = parseOptionalString(raw);
  if (value === null) return null;
  if (value.length > MAX_JS_RUNTIMES_LENGTH) return null;
  if (!JS_RUNTIMES_PATTERN.test(value)) return null;
  return value;
}
...
// working directory) so downloads default to living next to filetube.db/
 ...
 *   retries: number, playerClient: (string|null), jsRuntimes: (string|null),
 *   listTimeoutMinutes: number, socketTimeoutSeconds: number,
 *   stallMinutes: number, breakerFailures: number,
 *   breakerBackoffMinutes: number}}
 */
function parseYtdlpConfig(env = process.env) {
  ...
  return {
    ...
    jsRuntimes: parseJsRuntimes(source.FILETUBE_YTDLP_JS_RUNTIMES),
    ...
    // v1.31 P2: circuit-breaker knobs -- see the constants' comments above.
    breakerFailures: parseBreakerFailures(source.FILETUBE_YTDLP_BREAKER_FAILURES),
    breakerBackoffMinutes: parseBreakerBackoffMinutes(source.FILETUBE_YTDLP_BREAKER_BACKOFF_MINUTES),
    // v1.36 F1: list-pass enumeration cap -- see DEFAULT_LIST_SCAN_CAP above.
    listScanCap: parseListScanCap(source.FILETUBE_YTDLP_LIST_SCAN_CAP),
    // v1.42: the beta safe-mode lever (exec plan, owner-approved). When on,
    // every operation that mutates SHARED state (media dirs, the yt-dlp
    // archive/skip sidecar files) refuses with an honest 403 and the
    // scheduled poll becomes a logged no-op -- so a beta instance sharing
    // prod's media folders during the parallel-run window can't touch them.
    // Deliberately NOT a yt-dlp-prefixed env: server.js reads the same
    // variable for delete/move/scan guards.
    readOnlyMedia: source.FILETUBE_READ_ONLY_MEDIA === '1',
  };
}
...
module.exports = {
  ...
  parseStallMinutes,
  // v1.31 P2: breaker knobs.
  DEFAULT_BREAKER_FAILURES,
  MIN_BREAKER_FAILURES,
  MAX_BREAKER_FAILURES,
  DEFAULT_BREAKER_BACKOFF_MINUTES,
  MIN_BREAKER_BACKOFF_MINUTES,
  MAX_BREAKER_BACKOFF_MINUTES,
  parseBreakerFailures,
  parseBreakerBackoffMinutes,
  // v1.36 F1: list-pass enumeration cap -- exported for lib/ytdlp/args.js's
  // defensive re-bounding and direct unit-test coverage of the bounds.
  DEFAULT_LIST_SCAN_CAP,
  MIN_LIST_SCAN_CAP,
  MAX_LIST_SCAN_CAP,
  parseListScanCap,
