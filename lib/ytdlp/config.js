...
// the original 25 to 3 in v1.17.0 ("25 is just aggro" -- Dean): 3 is a
// sane default scope for a fresh subscribe (recent uploads only) rather
// than silently attempting a channel's entire back-catalog; a user can
// still raise it per-subscription (see lib/ytdlp/store.js's
// `validateMaxVideos`) or globally via FILETUBE_YTDLP_MAX_VIDEOS. This is a
// default for NEW subscriptions only -- it has no effect on already-
// persisted subscriptions, which keep whatever `maxVideos` they were saved
// with (see lib/ytdlp/args.js's `subMaxVideos ?? config.maxVideos`
// resolution).
