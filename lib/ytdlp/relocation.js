...
// v1.338 D9: the saved page link's check (a leaf module, no deps bridge needed).
...
function createRelocation(deps) {
  ...
  function enumerateRepullableItems(db, config, itemVisible) {
    ...
    for (const id of Object.keys(metadata)) {
      ...
      if (videoId || (universal && inDownloadRoot && sanitizeSourceShareUrl(item.sourceUrl))) result.withSourceId++;
    }
