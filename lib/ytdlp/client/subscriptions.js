...
function initSubscriptionsView(root) {
  ...
  function wireSubRowDragAndDrop() {
    ...
    // Row-aligned, derived with the SAME predicate the row builder uses to
    // stamp `data-sub-id`. Indexing `currentSubs` by ROW index (as this did
    // before) silently misaligns every row after an id-less subscription.
    const rowSubs = (Array.isArray(currentSubs) ? currentSubs : [])
    ...
    window.wireReorderable(listContainer, {
      ...
      onReorder: (fromIndex, toIndex) => {
        const reordered = window.moveArrayItem(rowSubs, fromIndex, toIndex);
        persistReorder(reordered.map((sub) => sub && sub.id).filter((id) => typeof id === 'string' && id !== ''));
      },
      signal,
    });
  }
