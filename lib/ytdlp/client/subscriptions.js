...
function createHistorySectionElement(doc) {
  ...
  // wired by the mount site (wireCollapse), not here -- this builder stays
...
function initSubscriptionsView(root) {
  ...
  // v1.55 Track D: collapse persistence for the static details sections;
  // called AGAIN after each dynamic section mounts (the helper is
  // idempotent per element). typeof-guarded like actionStatus above.
  // Scoped to the view ROOT, not document (QA S1: the SPA swaps only
  // #view-root -- scoping to it is the defensive default per the repo's own
  // recurring-class memory), falling back to document when the router hands
  // us nothing.
  const wireCollapse = () => {
    if (typeof wireCollapsibleSections === 'function') wireCollapsibleSections('subscriptions', root || document, signal);
  };
  wireCollapse();
  ...
  function mountHistorySection() {
    ...
    historyListEl = list;
    wireCollapse(); // v1.55 Track D: persistence for the just-mounted card
  }
