...
function renderFailuresInto(container, entry, doc, handlers, subId) {
  ...
  const signature = items.map((it) => `${it.videoId} ${it.label} ${it.reason}`).join('');
...
function initSubscriptionsView(root) {
  ...
  function mountHardeningStrip() {
    ...
    // v1.32 (Dean): the version line lives at the TOP of the page now
    // (above the breaker banner, above the list) -- it was previously a
    // footer below 180+ rows.
    if (anchor && typeof anchor.insertAdjacentElement === 'function') {
      anchor.insertAdjacentElement('beforebegin', versionFooterEl);
      anchor.insertAdjacentElement('beforebegin', breakerBannerEl);
    } else if (anchor && anchor.parentNode) {
      anchor.parentNode.insertBefore(versionFooterEl, anchor);
