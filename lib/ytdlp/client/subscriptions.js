 ...
 * hand-edited/corrupted database, which is not expected for any
...
/**
 * v1.285: minutes <-> seconds for the duration-window inputs. The STORE keeps
 * SECONDS (matching `--match-filter "duration < N"`/`"duration >= N"`); the UI
 * presents whole MINUTES (Dean's units - "45 minute", "an hour"). Blank ->
 * `undefined` (omit the field: unchanged on a patch, default on an add); a valid
 * non-negative integer of minutes -> `minutes * 60`; `0` -> `0` (a distinct "no
 * bound on that side" value). Invalid (non-integer, negative, non-numeric) ->
 * `undefined` (omit), matching the fields' prior blank-means-omit posture -- the
 * server re-validates and is the authority.
 */
function minutesInputToSeconds(raw) {
  const s = typeof raw === 'string' ? raw.trim() : (typeof raw === 'number' ? String(raw) : '');
  if (s === '') return undefined;
  const mins = Number(s);
  if (!Number.isInteger(mins) || mins < 0) return undefined;
  return mins * 60;
}

/**
 * The inverse: a stored SECONDS value -> the minutes string an edit input shows.
 * Non-number/negative -> `''` (blank). A legacy second-value not divisible by 60
 * rounds to the nearest whole minute on display (disclosed residual: re-saving
 * then stores `roundedMinutes * 60`; Dean sets whole minutes, so this only ever
 * touches an odd pre-existing value).
 */
...
function formatYtdlpVersionText(version, nowMs) {
  ...
  if (m) {
    ...
    if (Number.isFinite(ageDays) && ageDays > STALE_YTDLP_DAYS) {
      text += ` — over ${STALE_YTDLP_DAYS} days old; YouTube changes frequently, so downloads may break until the engine is updated (Setup -> Downloads -> Downloader engine, or bump the Dockerfile ARG and rebuild)`;
    }
  }
  return text;
}
...
function createHistorySectionElement(doc) {
  const d = doc || document;
  // v1.55 Track D: a native collapsible details card (open by default),
  // the summary IS the heading, styled by the existing .sub-collapsible rules.
  // v1.156 (T3): this mounts inside the Activity slide-in panel now; the old
  // data-md-* master-detail attrs were dropped with the menu. Builder stays
  // pure DOM for its node:test coverage.
  const section = d.createElement('details');
  section.className = 'setup-box sub-collapsible';
  section.setAttribute('data-collapse-key', 'download-history');
  section.open = false; // v1.160 (Dean): default-collapsed

  const summary = d.createElement('summary');
  summary.textContent = 'Download history';
...
// ---- v1.47.4 item 7: the durable download-failure section ------------------
//
...
// as well. It's different than subscription downloads."
//
// ONE unified list across both lanes with a source filter, not two lists --
// see the route's own comment in lib/ytdlp/index.js for why splitting the store
// would mean two retention policies and two places to look.
//
// Deliberately built from the SAME `.setup-box`/`.sub-list-header`/`.sub-list`/
// `.sub-row` class families the history section above already reuses, so this
// section is visually indistinguishable from a server-rendered one with zero
// new CSS. `createElement`/`textContent` only -- never `innerHTML` -- because
// every field here originates in yt-dlp's UNTRUSTED stderr. `faillog.js`
// already strips control characters at the storage boundary; rendering as text
// is the second, independent layer.

// Source filter values. 'all' is the default so a failure can never be hidden
// by a filter the user did not set.
const FAILURE_SOURCE_FILTERS = ['all', 'one-off', 'subscription'];

/**
 * Pure: apply the source filter. An unrecognized/absent filter falls back to
 * showing EVERYTHING -- failing toward visibility, since silently hiding a
 * failure is the one outcome this feature exists to prevent.
 */
function filterFailureEntries(entries, sourceFilter) {
  const list = Array.isArray(entries) ? entries : [];
  if (sourceFilter !== 'one-off' && sourceFilter !== 'subscription') return list;
  return list.filter((e) => e && e.source === sourceFilter);
}

/**
 * Pure: the human label for a row's source + kind. A `subtitleFallback` row is
 * NOT a failed download -- the video landed, only its captions did not -- and
 * it must never be worded as if the download was lost.
 */
function formatFailureKindLabel(entry) {
  const source = entry && entry.source === 'subscription' ? 'Subscription' : 'One-off';
  if (entry && entry.subtitleFallback === true) return `${source} - saved without captions`;
  return `${source} - download failed`;
}

/**
 * Builds ONE failure row, including its own delete control. `onDelete` is
 * invoked with the entry id; the caller owns the actual request + re-render, so
 * this stays a pure builder.
 */
function createFailureRow(entry, doc, handlers) {
  const d = doc || document;
  const onDelete = handlers && typeof handlers.onDelete === 'function' ? handlers.onDelete : null;
  const row = d.createElement('div');
  row.className = 'sub-row';
  ...
  const videoId = entry && typeof entry.videoId === 'string' ? entry.videoId.trim() : '';
  // Never anonymous: title, else the video id, else an honest placeholder.
  nameEl.textContent = title || videoId || 'Unknown item';
  info.appendChild(nameEl);

  const metaEl = d.createElement('div');
  metaEl.className = 'sub-row-meta';
  metaEl.textContent = `${formatFailureKindLabel(entry)} - ${formatHistoryTimestamp(entry && entry.ts)}`;
  info.appendChild(metaEl);

  // The whole point of the feature: the VERBATIM yt-dlp error text.
  const reasonEl = d.createElement('div');
  reasonEl.className = 'sub-row-failures';
  reasonEl.textContent = (entry && typeof entry.reason === 'string' && entry.reason.trim() !== '')
    ? entry.reason.trim()
    : 'Unknown error';
  ...
  if (entry && typeof entry.url === 'string' && entry.url.trim() !== '') {
    ...
    urlEl.className = 'sub-row-meta';
    urlEl.textContent = entry.url.trim();
    info.appendChild(urlEl);
  }

  row.appendChild(info);

  if (onDelete && entry && typeof entry.id === 'string' && entry.id !== '') {
    const del = d.createElement('button');
    del.className = 'btn';
    del.type = 'button';
    del.textContent = 'Delete';
    del.setAttribute('aria-label', `Delete this failure record for ${nameEl.textContent}`);
...
/**
 * Builds the failure LIST container, mirroring `createHistoryListElement`'s
 * empty-state treatment verbatim. Entries are rendered in the EXACT order
 * given -- `faillog.readFailures` already returns newest-first, so this does no
 * re-ordering of its own.
 */
function createFailureListElement(entries, doc, handlers) {
  const d = doc || document;
  const container = d.createElement('div');
  const list = Array.isArray(entries) ? entries : [];
  if (list.length === 0) {
    const empty = d.createElement('div');
    empty.setAttribute('style', 'color:var(--text-secondary); font-style:italic; padding:8px 4px;');
    // Distinguishes "nothing has failed" from "your filter hides everything",
    // so an active filter can never read as a clean bill of health.
    empty.textContent = (handlers && handlers.filtered)
      ? 'No failures match this filter.'
      : 'No download failures recorded.';
    container.appendChild(empty);
    return container;
  }
  list.forEach((entry) => container.appendChild(createFailureRow(entry, d, handlers)));
  return container;
}

/**
 * Builds the failure section wrapper: heading, source filter, Clear all, and
 * the persistent `.sub-list` node the caller repopulates. Returns every node
 * the caller needs to wire, so this function itself performs no fetching.
 */
function createFailureSectionElement(doc, handlers) {
  const d = doc || document;
  const onClear = handlers && typeof handlers.onClear === 'function' ? handlers.onClear : null;
  const onFilterChange = handlers && typeof handlers.onFilterChange === 'function' ? handlers.onFilterChange : null;

  // v1.55 Track D: collapsible details card, same shape as the history
  // section above. The filter/clear CONTROLS stay in their own row inside
  // the details body (controls inside a <summary> would toggle the
  // disclosure on every click).
  const section = d.createElement('details');
  section.className = 'setup-box sub-collapsible';
  section.setAttribute('data-collapse-key', 'download-failures');
  // v1.156 (T3): mounts inside the Activity panel now; data-md-* dropped.
  section.open = false; // v1.160 (Dean): default-collapsed

  const summary = d.createElement('summary');
  summary.textContent = 'Download failures';
  section.appendChild(summary);

  // Gate round 1 (adversarial S1): with the heading now living in the
  // summary, this row holds ONLY the controls -- the -end modifier keeps
  // them on the right, where they sat when the h2 anchored the left side.
  const header = d.createElement('div');
  header.className = 'sub-list-header sub-list-header-end';

  const controls = d.createElement('div');

  const select = d.createElement('select');
  select.className = 'form-input';
  select.setAttribute('aria-label', 'Filter failures by source');
  [
    ['all', 'All sources'],
    ['one-off', 'One-off downloads'],
  ...
  if (onFilterChange) select.addEventListener('change', () => onFilterChange(select.value));
  ...
  if (onClear) clearBtn.addEventListener('click', () => onClear());
  ...
  return { section, list, select, clearBtn };
}
 ...
 *
 * v1.338 D9: a download from another site with a saved page link re-pulls too,
 * so the server counts it in `withSourceId` and the wording says "source link",
 * not "YouTube source link".
 */
function formatReheatSummary(eligible, ineligible, withSourceId) {
  ...
  if (typeof withSourceId === 'number' && Number.isFinite(withSourceId)) {
    parts.push(w === 0
      ? 'none have a source link yet (local tag check only)'
      : (w < e
        ? `${w} with a source link (the rest get a local tag check)`
        : 'all with a source link'));
  }
 ...
 */
// v1.55 Track C: the ONE feedback system (setActionStatus/setButtonBusy)
// lives in common.js, which every page serving this file loads first. The
// typeof guards keep this file requireable by Node unit tests (where
// common.js is not a global) with a faithful text/disabled fallback -- a
// test that wants the classList behavior stubs the globals before require.
function actionStatus(el, text, kind) {
  if (typeof setActionStatus === 'function') { setActionStatus(el, text, kind); return; }
  if (el && text !== null && text !== undefined) el.textContent = text;
}
function buttonBusy(btn, busy) {
  if (typeof setButtonBusy === 'function') { setButtonBusy(btn, busy); return; }
...
function applyReheatStateToControls(elements, entry) {
  ...
  buttonBusy(elements.button, running);
  if (elements.cancelButton) {
    elements.cancelButton.hidden = !running;
    // v1.55 Track B: the Cancel SWAPS IN PLACE of its trigger (they share
    // one .action-bar-cell) -- while running, the trigger hides instead of
    // sitting disabled beside a popped-in fifth button. Only when a Cancel
    // exists: a cancel-less action (repull-all) keeps its button visible.
    if (elements.button) elements.button.hidden = running;
  }
  if (elements.status) {
    const progressText = formatReheatProgressText(entry);
    // text: null when there is no fresh progress -- the summary line stays,
    // but the busy spinner still tracks `running` (Track C's contract).
    actionStatus(elements.status, progressText !== '' ? progressText : null, running ? 'busy' : undefined);
  }
}
...
function triggerReheat(elements, fetchImpl) {
  ...
  buttonBusy(elements.button, true);
  actionStatus(elements.status, 'Starting reheat…', 'busy');
  doFetch('/api/ytdlp/repull-metadata', { method: 'POST' })
    .then((r) => r.json().catch(() => ({})).then((data) => ({ ok: r.ok, status: r.status, data: data || {} })))
    .then(({ ok, status, data }) => {
      if (ok && data.started) {
        actionStatus(elements.status, formatReheatSummary(data.eligible, data.ineligible, data.withSourceId), 'busy');
        return;
      }
      if (status === 409 || data.alreadyRunning) {
        actionStatus(elements.status, 'A metadata reheat is already in progress.');
        return;
      }
      actionStatus(elements.status, 'Could not start metadata reheat.', 'error');
      buttonBusy(elements.button, false);
    })
    .catch((err) => {
      actionStatus(elements.status, 'Could not start metadata reheat (network error).', 'error');
      buttonBusy(elements.button, false);
...
function triggerReheatCancel(elements, fetchImpl) {
  ...
  doFetch('/api/ytdlp/repull-metadata/cancel', { method: 'POST' })
    .then(() => {
      actionStatus(elements.status, 'Cancelling reheat…', 'busy');
    })
...
function triggerReheatPreview(elements, fetchImpl) {
  ...
  buttonBusy(btn, true);
  actionStatus(elements.status, 'Computing preview…', 'busy');
  doFetch('/api/ytdlp/repull-metadata/preview', { method: 'POST' })
    .then((r) => r.json().catch(() => ({})).then((data) => ({ ok: r.ok, status: r.status, data: data || {} })))
    .then(({ ok, data }) => {
      buttonBusy(btn, false);
      if (!ok) {
        actionStatus(elements.status, 'Could not compute the preview.', 'error');
        return;
      }
      actionStatus(elements.status, '');
    ...
    .catch((err) => {
      buttonBusy(btn, false);
      actionStatus(elements.status, 'Could not compute the preview (network error).', 'error');
...
function applyRefreshAvatarsStateToControls(elements, entry) {
  ...
  if (elements.cancelButton) {
    elements.cancelButton.hidden = !running;
    // Track B in-cell swap -- see applyReheatStateToControls.
    if (elements.button) elements.button.hidden = running;
  }
  if (elements.status) {
...
function triggerRefreshAvatars(elements, fetchImpl) {
  ...
  if (typeof doFetch !== 'function' || !elements) return;
  buttonBusy(elements.button, true);
  actionStatus(elements.status, 'Starting avatar refresh…', 'busy');
  doFetch('/api/ytdlp/refresh-avatars', { method: 'POST' })
    .then((r) => r.json().catch(() => ({})).then((data) => ({ ok: r.ok, status: r.status, data: data || {} })))
    .then(({ ok, status, data }) => {
      if (ok && data.started) {
        actionStatus(elements.status, formatRefreshAvatarsSummary(data.total), 'busy');
        return;
      }
      if (status === 409 || data.alreadyRunning) {
        actionStatus(elements.status, 'An avatar refresh is already in progress.');
        return;
      }
      actionStatus(elements.status, 'Could not start avatar refresh.', 'error');
      buttonBusy(elements.button, false);
    })
    .catch((err) => {
      actionStatus(elements.status, 'Could not start avatar refresh (network error).', 'error');
      buttonBusy(elements.button, false);
...
function triggerRefreshAvatarsCancel(elements, fetchImpl) {
  ...
  doFetch('/api/ytdlp/refresh-avatars/cancel', { method: 'POST' })
    .then(() => {
      actionStatus(elements.status, 'Cancelling avatar refresh…', 'busy');
    })
...
// ---- v1.56 (Dean): "Reheat sub counts" -- bulk subscriber-count refresh ----
// over EVERY distinct channel with content in the library. Mirrors the
// refresh-avatars block above verbatim (same fixed-activity-id posture, same
// pure-formatter/DOM-applier/fetch-trigger trio riding the SAME ~2.5s status
// poll); backed by `POST /api/ytdlp/reheat-sub-counts`. The extra
// `itemsUpdated` field is the server's additive per-batch counter of how many
// VIDEOS actually got a fresh count stamped -- channels and videos are both
// worth saying because one channel probe fans out to many items.

const REHEAT_SUBS_ACTIVITY_ID = 'reheat-subs';

/**
 * Pure formatter for `POST /api/ytdlp/reheat-sub-counts`'s `202` response
 * body (`{started:true, total}`) -- `formatRefreshAvatarsSummary`'s exact
 * posture (clamped count, never throws), channel-flavored wording (the unit
 * of work is a distinct CHANNEL, not a subscription).
 */
function formatReheatSubsSummary(total) {
  const t = typeof total === 'number' && Number.isFinite(total) && total > 0 ? Math.floor(total) : 0;
  if (t === 0) {
    return 'No channels to reheat subscriber counts for.';
  }
  const channelWord = t === 1 ? 'channel' : 'channels';
  return `Reheating subscriber counts for ${t} ${channelWord}…`;
}

/**
 * Pure formatter for the reheat-subs batch's LIVE progress -- the
 * `REHEAT_SUBS_ACTIVITY_ID` one-shot `LiveEntry` (`{state, total, done,
 * skipped, failed, itemsUpdated, current}`, set by lib/ytdlp/index.js's
 * `runReheatSubsBatch`). Mirrors `formatRefreshAvatarsProgressText` exactly,
 * plus the videos-updated tally where it has something to say. Returns `''`
 * for a missing/malformed entry (or idle/unrecognized state) so callers can
 * leave the last-rendered text alone.
 */
function formatReheatSubsProgressText(entry) {
  if (!entry || typeof entry !== 'object') return '';
  const total = typeof entry.total === 'number' && Number.isFinite(entry.total) ? entry.total : 0;
  const done = typeof entry.done === 'number' && Number.isFinite(entry.done) ? entry.done : 0;
  const skipped = typeof entry.skipped === 'number' && Number.isFinite(entry.skipped) ? entry.skipped : 0;
  const failed = typeof entry.failed === 'number' && Number.isFinite(entry.failed) ? entry.failed : 0;
  const itemsUpdated = typeof entry.itemsUpdated === 'number' && Number.isFinite(entry.itemsUpdated) ? entry.itemsUpdated : 0;
  const current = typeof entry.current === 'string' && entry.current.trim() !== '' ? entry.current.trim() : '';

  if (entry.state === 'running') {
    const parts = [`Reheating sub counts: ${done} of ${total} channels done`];
    if (itemsUpdated > 0) parts.push(`${itemsUpdated} video${itemsUpdated === 1 ? '' : 's'} updated`);
    if (skipped > 0) parts.push(`${skipped} skipped`);
    if (failed > 0) parts.push(`${failed} failed`);
    if (current !== '') parts.push(`current: ${current}`);
    return parts.join(' · ');
  ...
  if (entry.state === 'done') {
    const parts = [`Sub-count reheat done: ${done} of ${total} channels refreshed`];
    parts.push(`${itemsUpdated} video${itemsUpdated === 1 ? '' : 's'} updated`);
    if (skipped > 0) parts.push(`${skipped} skipped`);
    if (failed > 0) parts.push(`${failed} failed`);
    return parts.join(' · ');
  }
  if (entry.state === 'cancelled') {
    return `Sub-count reheat cancelled — ${done} of ${total} channels refreshed before stopping`;
  }
  if (entry.state === 'error') {
    return 'Sub-count reheat failed unexpectedly.';
  }
  return ''; // idle / unrecognized state -- nothing live worth showing
}

/**
 * DOM-level (no fetch): mirrors `applyRefreshAvatarsStateToControls` exactly,
 * for the reheat-subs button/status/cancel refs and its own `LiveEntry`.
 */
function applyReheatSubsStateToControls(elements, entry) {
  if (!elements) return;
  const running = !!(entry && entry.state === 'running');
  buttonBusy(elements.button, running);
  if (elements.cancelButton) {
  ...
  if (elements.status) {
    const progressText = formatReheatSubsProgressText(entry);
    actionStatus(elements.status, progressText !== '' ? progressText : null, running ? 'busy' : undefined);
  }
}

/**
 * The "Reheat sub counts" button's click handler -- `POST`s
 * `/api/ytdlp/reheat-sub-counts` and renders the result. Mirrors
 * `triggerRefreshAvatars` exactly (instant-disable feedback, `202` renders
 * the summary, `409` is not a failure, a network failure re-enables the
 * button for retry).
 */
function triggerReheatSubs(elements, fetchImpl) {
  const doFetch = typeof fetchImpl === 'function' ? fetchImpl : (typeof fetch !== 'undefined' ? fetch : undefined);
  if (typeof doFetch !== 'function' || !elements) return;
  buttonBusy(elements.button, true);
  actionStatus(elements.status, 'Starting sub-count reheat…', 'busy');
  doFetch('/api/ytdlp/reheat-sub-counts', { method: 'POST' })
    .then((r) => r.json().catch(() => ({})).then((data) => ({ ok: r.ok, status: r.status, data: data || {} })))
    .then(({ ok, status, data }) => {
      if (ok && data.started) {
        actionStatus(elements.status, formatReheatSubsSummary(data.total), 'busy');
        return;
      }
      if (status === 409 || data.alreadyRunning) {
        actionStatus(elements.status, 'A sub-count reheat is already in progress.');
        return;
      }
      actionStatus(elements.status, 'Could not start the sub-count reheat.', 'error');
      buttonBusy(elements.button, false);
    })
    .catch((err) => {
      actionStatus(elements.status, 'Could not start the sub-count reheat (network error).', 'error');
      buttonBusy(elements.button, false);
      console.error('Reheat sub counts request failed:', err);
    });
}

/**
 * The "Reheat sub counts" cancel button's click handler -- `POST`s
 * `/api/ytdlp/reheat-sub-counts/cancel`. Mirrors
 * `triggerRefreshAvatarsCancel` exactly.
 */
function triggerReheatSubsCancel(elements, fetchImpl) {
  const doFetch = typeof fetchImpl === 'function' ? fetchImpl : (typeof fetch !== 'undefined' ? fetch : undefined);
  if (typeof doFetch !== 'function' || !elements) return;
  if (elements.cancelButton) elements.cancelButton.disabled = true;
  doFetch('/api/ytdlp/reheat-sub-counts/cancel', { method: 'POST' })
    .then(() => {
      actionStatus(elements.status, 'Cancelling sub-count reheat…', 'busy');
    })
    .catch((err) => console.error('Reheat sub counts cancel request failed:', err))
    .finally(() => {
      if (elements.cancelButton) elements.cancelButton.disabled = false;
    });
}

// ---- v1.115 (Dean, A1): "Refresh channel names" -- per-channel name backfill --
//
// The reheat-sub-counts block above VERBATIM (fixed-activity-id progress, the
// ~2.5s status poll, in-cell Cancel swap), backed by
// `POST /api/ytdlp/backfill-channel-names`. Different value: it re-pulls each
// bad-name channel's canonical name and fans it out to that channel's items.
const CHANNEL_NAME_BACKFILL_ACTIVITY_ID = 'backfill-channel-names';

/** Pure formatter for the 202 body ({started, total}) -- reheat's posture, name wording. */
function formatChannelNameBackfillSummary(total, healChannels) {
  const t = typeof total === 'number' && Number.isFinite(total) && total > 0 ? Math.floor(total) : 0;
  const h = typeof healChannels === 'number' && Number.isFinite(healChannels) && healChannels > 0 ? Math.floor(healChannels) : 0;
  // v1.116: the batch heals locally-recoverable channels first (no network),
  // then probes the rest -- reflect both in the kick-off message.
  if (t === 0 && h === 0) return 'No channels need a name refresh.';
  const parts = [];
  if (h > 0) parts.push(`repairing ${h} channel${h === 1 ? '' : 's'} locally`);
  if (t > 0) parts.push(`refreshing ${t} channel${t === 1 ? '' : 's'} online`);
  return parts.join(' + ') + '…';
}

/** Pure formatter for the batch's LIVE progress one-shot -- mirrors formatReheatSubsProgressText. */
function formatChannelNameBackfillProgressText(entry) {
  if (!entry || typeof entry !== 'object') return '';
  const total = typeof entry.total === 'number' && Number.isFinite(entry.total) ? entry.total : 0;
  const done = typeof entry.done === 'number' && Number.isFinite(entry.done) ? entry.done : 0;
  const skipped = typeof entry.skipped === 'number' && Number.isFinite(entry.skipped) ? entry.skipped : 0;
  const failed = typeof entry.failed === 'number' && Number.isFinite(entry.failed) ? entry.failed : 0;
  const itemsUpdated = typeof entry.itemsUpdated === 'number' && Number.isFinite(entry.itemsUpdated) ? entry.itemsUpdated : 0;
  const healedItems = typeof entry.healedItems === 'number' && Number.isFinite(entry.healedItems) ? entry.healedItems : 0;
  const healedChannels = typeof entry.healedChannels === 'number' && Number.isFinite(entry.healedChannels) ? entry.healedChannels : 0;
  const current = typeof entry.current === 'string' && entry.current.trim() !== '' ? entry.current.trim() : '';
  // v1.116 (QA SUGGESTION): "items", not "videos" -- the backfill now covers
  // audio too, and music channels (NESTALGIA) are the headline case.
  const items = (n) => `${n} item${n === 1 ? '' : 's'}`;
  if (entry.state === 'running') {
    // v1.116: the heal phase runs first (no `total`); the probe phase follows.
    if (entry.phase === 'heal') {
      const parts = ['Repairing channel identities locally'];
      if (healedItems > 0) parts.push(`${items(healedItems)} fixed`);
      if (current !== '') parts.push(`current: ${current}`);
      return parts.join(' · ');
    }
    const parts = [`Refreshing channel names: ${done} of ${total} channels done`];
    if (itemsUpdated > 0) parts.push(`${items(itemsUpdated)} updated`);
    if (skipped > 0) parts.push(`${skipped} skipped`);
    if (failed > 0) parts.push(`${failed} failed`);
    if (current !== '') parts.push(`current: ${current}`);
    return parts.join(' · ');
  ...
  if (entry.state === 'done') {
    const parts = [`Channel-name refresh done: ${items(itemsUpdated)} updated`];
    if (healedChannels > 0) parts.push(`${healedChannels} channel${healedChannels === 1 ? '' : 's'} repaired locally`);
    if (total > 0) parts.push(`${done} of ${total} probed`);
    if (skipped > 0) parts.push(`${skipped} skipped`);
    if (failed > 0) parts.push(`${failed} failed`);
    return parts.join(' · ');
  }
  if (entry.state === 'cancelled') {
    return `Channel-name refresh cancelled — ${done} of ${total} channels before stopping`;
  }
  if (entry.state === 'error') {
    return 'Channel-name refresh failed unexpectedly.';
  }
  return '';
}

/** DOM-level (no fetch): mirrors applyReheatSubsStateToControls exactly. */
function applyChannelNameBackfillStateToControls(elements, entry) {
  if (!elements) return;
  const running = !!(entry && entry.state === 'running');
  buttonBusy(elements.button, running);
  if (elements.cancelButton) {
  ...
  if (elements.status) {
    const progressText = formatChannelNameBackfillProgressText(entry);
    actionStatus(elements.status, progressText !== '' ? progressText : null, running ? 'busy' : undefined);
  }
}

/** "Refresh channel names" click handler -- mirrors triggerReheatSubs exactly. */
function triggerChannelNameBackfill(elements, fetchImpl) {
  const doFetch = typeof fetchImpl === 'function' ? fetchImpl : (typeof fetch !== 'undefined' ? fetch : undefined);
  if (typeof doFetch !== 'function' || !elements) return;
  buttonBusy(elements.button, true);
  actionStatus(elements.status, 'Starting channel-name refresh…', 'busy');
  doFetch('/api/ytdlp/backfill-channel-names', { method: 'POST' })
    .then((r) => r.json().catch(() => ({})).then((data) => ({ ok: r.ok, status: r.status, data: data || {} })))
    .then(({ ok, status, data }) => {
      if (ok && data.started) {
        actionStatus(elements.status, formatChannelNameBackfillSummary(data.total, data.healChannels), 'busy');
        return;
      }
      if (status === 409 || data.alreadyRunning) {
        actionStatus(elements.status, 'A channel-name refresh is already in progress.');
        return;
      }
      actionStatus(elements.status, 'Could not start the channel-name refresh.', 'error');
      buttonBusy(elements.button, false);
    })
    .catch((err) => {
      actionStatus(elements.status, 'Could not start the channel-name refresh (network error).', 'error');
      buttonBusy(elements.button, false);
      console.error('Refresh channel names request failed:', err);
    });
}

/** "Refresh channel names" cancel handler -- mirrors triggerReheatSubsCancel exactly. */
function triggerChannelNameBackfillCancel(elements, fetchImpl) {
  const doFetch = typeof fetchImpl === 'function' ? fetchImpl : (typeof fetch !== 'undefined' ? fetch : undefined);
  if (typeof doFetch !== 'function' || !elements) return;
  if (elements.cancelButton) elements.cancelButton.disabled = true;
  doFetch('/api/ytdlp/backfill-channel-names/cancel', { method: 'POST' })
    .then(() => { actionStatus(elements.status, 'Cancelling channel-name refresh…', 'busy'); })
    .catch((err) => console.error('Refresh channel names cancel request failed:', err))
    .finally(() => { if (elements.cancelButton) elements.cancelButton.disabled = false; });
}
 ...
 *      opens the settings panel (`onOpenSettings`), stopping propagation. The
 *      whole row body also opens it now (v1.155), so the kebab is an explicit
 *      redundant affordance.
 *
 * `handlers` = `{ onOpenSettings(sub), onTogglePin(sub, pinned), onToggleBell(sub) }`
 * decouples DOM construction from sheet-opening/pin-toggling so
 ...
 *
 * Row tap (v1.155): the row body -- everything OUTSIDE the inner buttons and
 * `<a>` links -- opens this channel's settings panel via `onOpenSettings`, for
 * EVERY row (a row whose `channelDir` did not resolve still has settings to
 * edit). The channel `<a>` link and the "View as Playlist" `<a>` (gated on a
 * resolved `channelDir`) are guarded so tapping them never also opens the
 * panel; the pin/retry/kebab `<button>`s both stopPropagation and are guarded.
 */
function createSubscriptionRow(sub, doc, handlers, liveEntry, pinned) {
  ...
  // v1.155: the row's stable id anchor. Once the reorder hook (its original
  // v1.24.0 purpose), `data-sub-id` now keys the id->element map the ~2.5s
  // poll's in-place updater reads: `renderSubscriptions` queries every
  // `.sub-row[data-sub-id]` after a build to rebuild `rowElementsById`, so the
  // poll can find and mutate each row's status line without re-rendering the
  // list (the FR-1 invariant) regardless of the A-Z section nesting. Unlike
  // the Playlist link/pin toggle (gated on a RESOLVED `channelDir`), this is
  // stamped for any real id -- attribute-only, no listeners: keeps this
  // builder pure/DOM-only.
  ...
  // v1.155 (Subscriptions redesign): the whole row is tappable and opens this
  // channel's settings PANEL -- the iOS list idiom (tap a row -> its detail).
  // Applies to EVERY row, not only navigable ones: a channel whose folder
  // could not be resolved still has settings to edit. Inner controls own their
  // clicks -- the pin/retry/kebab buttons `stopPropagation`, and the channel /
  // "View as Playlist" `<a>` links are guarded here so tapping a link (or any
  // inner button) never also opens the panel.
  //
  // (Before v1.155 a row tap SPA-navigated to the channel playlist. That moved
  // onto the "View as Playlist" link alone -- playlist access is preserved, the
  // whole-row gesture now belongs to settings, matching the approved redesign.)
  ...
  row.addEventListener('click', (event) => {
    ...
    if (target && typeof target.closest === 'function' && (target.closest('a') || target.closest('button'))) return;
  ...
  // research (docs/references/ui-research-2026-07.md §3), so this is a SEPARATE action
  ...
  // itself.
  // v1.316 (Dean, B2): every row chip (pin / bell / kebab) is a real `.btn`
  // (`btn btn-chip <role>`), so each era's control TREATMENT (the 2009 gloss,
  // the 2005 flat bevel, 2021's shadow) reaches it through the ONE `.btn` rule
  // family instead of a hand-copied per-chip bevel that only ever inherited
  // the era's colour tokens. The role class stays for the state colours + JS.
  if (navigable) {
    const pinBtn = d.createElement('button');
    pinBtn.type = 'button';
    pinBtn.className = pinned ? 'btn btn-chip sub-row-pin sub-row-pin-active' : 'btn btn-chip sub-row-pin';
  ...
  // v1.314 (Dean): the per-channel WEB PUSH bell - "which channels will notify
  // on downloads". Rendered for any row with a real id (the flag is a field of
  // the subscription record, so no channelDir gate); the glyph is the ONLY
  // thing `sub.pushBell` affects here - the truth is server-side. v1.316 (B1):
  // the rendered state is written by `applyBellState` - the SAME writer the
  // post-PATCH in-place update uses (`applyBellUpdateInPlace`), so "first
  // render" and "after a toggle" can never drift.
  if (sub && typeof sub.id === 'string' && sub.id !== '') {
    const bellBtn = d.createElement('button');
    bellBtn.type = 'button';
    applyBellState(bellBtn, sub.pushBell === true);
    bellBtn.addEventListener('click', (event) => {
      if (event && typeof event.stopPropagation === 'function') event.stopPropagation();
      if (typeof h.onToggleBell === 'function') h.onToggleBell(sub);
    });
  ...
  kebabBtn.className = 'btn btn-chip sub-row-kebab'; // v1.316 (B2): see the pin's comment above
 ...
 * NAME renders READ-ONLY in the header (v1.25 T3 coordinator decision:
...
function buildSettingsSheet(sub, doc, handlers) {
  ...
  // ---- iOS nav-bar: back arrow (pinned left) + centered channel name ----
  // v1.155 (Subscriptions redesign): the panel reads as an iOS detail page.
  // The header is a nav-bar -- a back chevron pinned left, the channel name
  // centered as the title -- mirroring the v1.154 master-detail `.md-detail-
  // head` treatment. The avatar + subscribed date move into a hero block below
  // it. The class stays `.sub-sheet-close` (its onClose wiring + the tests
  // that bind it are unchanged); only its glyph/label/position now read as
  // "Back".
  const header = d.createElement('div');
  ...
  closeBtn.textContent = '‹'; // single left-pointing angle quotation ('‹')
  ...
  // READ-ONLY by design -- no input control backs this field anywhere in the
  // sheet (see the coordinator-decision comment above the function). It is the
  // nav-bar's centered title now.
  nameEl.textContent = (sub && sub.name) || '(untitled subscription)';
  header.appendChild(nameEl);

  sheet.appendChild(header);

  // ---- hero: the channel avatar + subscribed-date subtext ---------------
  const hero = d.createElement('div');
  hero.className = 'sub-sheet-hero';
  ...
  // explicitly instead.
  // v1.285: a per-channel duration WINDOW, in MINUTES. The floor (min) filters
  // out short teaser clips; the ceiling (max) keeps out over-long items. Both are
  // stored as SECONDS (secondsToMinutesInput on display; minutesInputToSeconds on
  // save). The server rejects an empty window (min >= max).
  const minDurationInput = d.createElement('input');
  minDurationInput.type = 'number';
  minDurationInput.min = '0';
  minDurationInput.setAttribute('placeholder', 'blank = unchanged, 0 = no minimum');
  if (sub && typeof sub.minDurationSeconds === 'number') minDurationInput.value = secondsToMinutesInput(sub.minDurationSeconds);
  body.appendChild(buildSheetField(d, 'Min length (minutes, blank = unchanged, 0 = no minimum)', minDurationInput));
  ...
  if (sub && typeof sub.maxDurationSeconds === 'number') maxDurationInput.value = secondsToMinutesInput(sub.maxDurationSeconds);
  body.appendChild(buildSheetField(d, 'Max length (minutes, blank = 2h default, 0 = unlimited)', maxDurationInput));
  ...
  // v1.69 (D15): the file-under-Podcasts toggle. A bare <select> is covered
  // by the v1.68.3 base select rule (the styling-source law); no new class.
  const libraryPlaceSelect = d.createElement('select');
  for (const [value, label] of [['default', 'Home (default)'], ['podcasts', 'Podcasts']]) {
    const opt = d.createElement('option');
    opt.value = value;
    opt.textContent = label;
    libraryPlaceSelect.appendChild(opt);
  }
  libraryPlaceSelect.value = (sub && sub.libraryPlace === 'podcasts') ? 'podcasts' : 'default';
  body.appendChild(buildSheetField(d, 'File under', libraryPlaceSelect));
  ...
  saveBtn.addEventListener('click', () => {
    ...
    const patch = {
      ...
      // v1.69 (D15): a select always has a definite value - always included,
      // the skipShorts posture.
      libraryPlace: libraryPlaceSelect.value,
    };
    const cutoffDate = inputValueToCutoffDate(cutoffDateInput.value);
    if (cutoffDate !== undefined) patch.cutoffDate = cutoffDate;
    // v1.285: MINUTES in, SECONDS out. Blank -> omit (unchanged). Both bounds
    // travel together, so the server's both-present empty-window guard catches a
    // min >= max slip with a clean 400.
    const minSeconds = minutesInputToSeconds(minDurationInput.value);
    if (minSeconds !== undefined) patch.minDurationSeconds = minSeconds;
    const maxSeconds = minutesInputToSeconds(maxDurationInput.value);
    if (maxSeconds !== undefined) patch.maxDurationSeconds = maxSeconds;
...
// v1.316 (Dean, B1): ONE writer of the bell's rendered state - class (the
// `.btn.btn-chip` family + the role + the `-active` modifier), aria-label,
// aria-pressed and the glyph. Both the row builder (`createSubscriptionRow`)
// and the post-PATCH in-place update (`applyBellUpdateInPlace`) route through
// here, so the two moments can never drift apart (the INERT-SIBLING lesson:
// a second hand-copy of these four lines is exactly what would rot).
// SECURITY: fixed literal glyphs/labels only - never interpolates any
// server/user-derived string, so this needs no escaping discussion.
function applyBellState(bellBtn, on) {
  if (!bellBtn) return;
  const bellOn = on === true;
  bellBtn.className = bellOn ? 'btn btn-chip sub-row-bell sub-row-bell-active' : 'btn btn-chip sub-row-bell';
  bellBtn.setAttribute('aria-label', bellOn ? 'Push notifications on for this channel - turn off' : 'Push notifications off for this channel - turn on');
  bellBtn.setAttribute('aria-pressed', bellOn ? 'true' : 'false');
  bellBtn.textContent = bellOn ? '🔔' : '🔕';
}

// v1.316 (B1): the row's bell button, found by its ROLE class token (the
// className also carries `btn btn-chip` and, when on, `-active`, so an exact
// `findChildByClassName` match cannot see it). Works against a real element
// and the unit fake alike (both expose an iterable `.children` + `className`).
function findBellButton(rowEl) {
  if (!rowEl || !rowEl.children) return null;
  for (const child of rowEl.children) {
    const tokens = typeof child.className === 'string' ? child.className.split(/\s+/) : [];
    if (tokens.indexOf('sub-row-bell') !== -1) return child;
    const nested = findBellButton(child);
    if (nested) return nested;
  }
  return null;
}

// v1.316 (Dean, B1): update ONE row's bell in place after its PATCH resolved -
// the same posture as the ~2.5s poll's `applyStatusUpdatesInPlace` (keyed by
// id through `rowElementsById`, never a list rebuild), so the row element's
// identity survives a toggle and the page never flashes skeleton rows.
// Returns whether a bell was found (a row that re-rendered away mid-flight
// simply has nothing to update; the record itself was patched by the caller).
function applyBellUpdateInPlace(rowElementsById, subId, on) {
  const rowEl = rowElementsById && subId ? rowElementsById[subId] : null;
  const bellBtn = findBellButton(rowEl);
  if (!bellBtn) return false;
  applyBellState(bellBtn, on);
  return true;
}
 ...
 * but a plain boolean.
 */
// v1.155 (Subscriptions redesign): the sort key a channel is filed under in
// the A-Z list -- its display name, trimmed. Pure, exported for tests.
function subscriptionSortKey(sub) {
  return ((sub && sub.name) || '').trim();
}

// v1.155: the A-Z section a channel belongs to. A..Z for a leading Latin
// letter (case-folded), else '#' (digits, punctuation, non-Latin, blank) --
// exactly the iOS Contacts bucketing. Pure, exported for tests.
function subscriptionSectionLetter(sub) {
  const first = subscriptionSortKey(sub).charAt(0).toUpperCase();
  return first >= 'A' && first <= 'Z' ? first : '#';
}

// v1.155: group subscriptions into ordered A-Z sections (then a trailing '#'
// bucket), each section's rows sorted case-insensitively by name. Returns
// `[{ letter, subs }]`. Never mutates the input. Pure, exported for tests --
// this replaces the old server-`order` flat list now that manual reorder is
// gone (the list is purely alphabetical + search).
function groupSubscriptionsByLetter(subs) {
  const list = Array.isArray(subs) ? subs.slice() : [];
  list.sort((a, b) => {
    const ka = subscriptionSortKey(a).toLowerCase();
    const kb = subscriptionSortKey(b).toLowerCase();
    if (ka < kb) return -1;
    if (ka > kb) return 1;
    return 0;
  });
  ...
  list.forEach((sub) => {
    const letter = subscriptionSectionLetter(sub);
    if (!byLetter.has(letter)) byLetter.set(letter, []);
    byLetter.get(letter).push(sub);
  });
  const sections = [];
  for (let code = 65; code <= 90; code += 1) {
    const ch = String.fromCharCode(code);
    if (byLetter.has(ch)) sections.push({ letter: ch, subs: byLetter.get(ch) });
  }
  if (byLetter.has('#')) sections.push({ letter: '#', subs: byLetter.get('#') });
  return sections;
}

// v1.155: filter subscriptions by a search query -- case-insensitive
// substring over the channel NAME and its handle/URL (`channelUrl`). An empty
// query returns a shallow copy of every sub. Pure, exported for tests.
function filterSubscriptions(subs, query) {
  ...
  return list.filter((sub) => {
    ...
    const url = ((sub && sub.channelUrl) || '').toLowerCase();
    return name.indexOf(q) !== -1 || url.indexOf(q) !== -1;
  });
}

// v1.155: build the A-Z SECTIONED channel list. Each section is a
// `.sub-section[data-letter]` with a sticky `.sub-section-head` and its rows;
// every row still carries `data-sub-id`, so the ~2.5s poll's in-place updater
// (`applyStatusUpdatesInPlace`, keyed by id via `rowElementsById`) binds
// unchanged regardless of the section nesting -- the caller rebuilds that map
// by querying `.sub-row[data-sub-id]`, never by flat child index.
//
// `subs` is the ALREADY-FILTERED list the caller wants shown. When it is
// empty, an empty-state message is rendered instead; `options.emptyMessage`
// overrides the default so the caller can distinguish "no subscriptions yet"
// from "no channels match your search".
function createSubscriptionsListElement(subs, doc, handlers, statusSnapshot, pinnedChannelDirs, options) {
  const d = doc || document;
  const container = d.createElement('div');
  const opts = options || {};
  if (!subs || subs.length === 0) {
    const empty = d.createElement('div');
    empty.setAttribute('style', 'color:var(--text-secondary); font-style:italic; padding:8px 4px;');
    empty.textContent = opts.emptyMessage || 'No subscriptions yet. Add a channel below to get started.';
    container.appendChild(empty);
    return container;
  }
  const liveSubs = (statusSnapshot && statusSnapshot.subscriptions) || {};
  const pinnedSet = pinnedChannelDirs instanceof Set ? pinnedChannelDirs : new Set();
  groupSubscriptionsByLetter(subs).forEach(({ letter, subs: sectionSubs }) => {
    const section = d.createElement('div');
    section.className = 'sub-section';
    section.setAttribute('data-letter', letter);
    const head = d.createElement('div');
    head.className = 'sub-section-head';
    head.textContent = letter;
    // v1.155: the section header labels its run of channels. Hidden from
    // assistive tech so a screen reader hears the channel names, not a run of
    // single letters (the rows are the accessible navigation).
    ...
    sectionSubs.forEach((sub) => {
      ...
      section.appendChild(createSubscriptionRow(sub, d, handlers, liveEntry, pinned));
    });
...
// v1.155 (Subscriptions redesign, Q2): manual drag-to-reorder was REMOVED --
// the list is purely alphabetical (A-Z sections + search) now, so a
// user-curated `order` has no place to live in the UI. The old module-scope
// reorder decision (`deriveReorderRowSubs`/`buildSubscriptionReorderHandler`,
// v1.76) and the `wireSubRowDragAndDrop`/`persistReorder` wiring are gone with
// it. The server `order` field and `POST /api/subscriptions/reorder` route are
// left intact but simply unused by the client (a deliberate, disclosed
// residual -- see docs/exec-plans/completed/2026-08-20-subscriptions-redesign.md).

function initSubscriptionsView(root) {
  subscriptionsController = new AbortController();
  const { signal } = subscriptionsController;

  // v1.156 (T3): the master-detail menu (Following/Add/Activity tiles) is gone.
  // The pills toolbar opens One-off / Activity / + Add as iOS slide-in PANELS --
  // the SAME `.sub-sheet` overlay the per-channel settings panel uses (translateX
  // push-in, no desktop two-rail fork, so desktop = mobile). The panels are
  // STATIC in subscriptions.html (present at init), so every form/button id and
  // the ~2.5s status-poll bindings survive; opening just toggles `hidden` + the
  // `sheet-open` class rather than building/removing a subtree.
  let openPanelEl = null;
  function openPanel(panelEl) {
    if (!panelEl) return;
    if (openPanelEl && openPanelEl !== panelEl) closePanel();
    openPanelEl = panelEl;
    panelEl.hidden = false;
    const sheetEl = panelEl.querySelector ? panelEl.querySelector('.sub-sheet') : null;
    openOverlay(panelEl, 'sheet-open');
    if (sheetEl) openOverlay(sheetEl, 'sheet-open');
  }
  function closePanel() {
    const panelEl = openPanelEl;
    if (!panelEl) return;
    openPanelEl = null;
    const sheetEl = panelEl.querySelector ? panelEl.querySelector('.sub-sheet') : null;
    // Mirror closeSettingsSheet: animate out, then re-hide the static node (never
    // remove it -- it must stay in the DOM for its ids/wiring to persist).
    closeOverlayThen(sheetEl, 'sheet-open', () => { panelEl.hidden = true; });
    closeOverlayThen(panelEl, 'sheet-open', () => {});
  }
  function wirePanels() {
    const rootEl = root || document;
    if (!rootEl.querySelectorAll) return;
    // A pill with data-sub-panel="<key>" opens #sub-panel-<key>.
    Array.prototype.forEach.call(rootEl.querySelectorAll('[data-sub-panel]'), (btn) => {
      btn.addEventListener('click', () => {
        openPanel(document.getElementById('sub-panel-' + btn.getAttribute('data-sub-panel')));
      }, { signal });
    });
    // The nav-bar back chevron (and a click on the backdrop outside the sheet)
    // closes the open panel.
    Array.prototype.forEach.call(rootEl.querySelectorAll('[data-sub-panel-close]'), (btn) => {
      btn.addEventListener('click', () => closePanel(), { signal });
    });
    Array.prototype.forEach.call(rootEl.querySelectorAll('.sub-panel'), (panelEl) => {
      panelEl.addEventListener('click', (event) => {
        if (event && event.target === panelEl) closePanel();
      }, { signal });
    });
    document.addEventListener('keydown', (event) => {
      if (!event || event.key !== 'Escape') return;
      // QA gate: the reloc-preview modal (opened by the Activity panel's
      // "Preview changes") sits ABOVE the panel, so Esc must dismiss the
      // TOPMOST overlay first -- else Esc would close the Activity panel out
      // from under the still-open modal, leaving it floating over the list.
      const relocBackdrop = document.getElementById('reloc-preview-backdrop');
      if (relocBackdrop && relocBackdrop.hidden === false) {
        relocBackdrop.hidden = true;
        return;
      }
  ...
  // v1.155 (Subscriptions redesign): the live search field (static markup in
  // subscriptions.html). Null-guarded -- may be absent on an older cached shell.
  ...
  // v1.285: the per-channel duration FLOOR, add-subscription form (minutes).
  const addMinDurationInput = document.getElementById('sub-add-minduration');
  ...
  // v1.56 (Dean's bulk subscriber-count reheat): mirrors
  // `refreshAvatarsElements` immediately above -- same element-refs-in,
  // DOM-only/fetch-only functions posture.
  const reheatSubsElements = {
    button: document.getElementById('sub-reheat-subs-btn'),
    status: document.getElementById('sub-reheat-subs-status'),
    cancelButton: document.getElementById('sub-reheat-subs-cancel-btn'),
  };
  // v1.115 (Dean, A1): "Refresh channel names" controls.
  const channelNameBackfillElements = {
    button: document.getElementById('sub-backfill-names-btn'),
    status: document.getElementById('sub-backfill-names-status'),
    cancelButton: document.getElementById('sub-backfill-names-cancel-btn'),
  };
  ...
  let currentSubs = [];
  // v1.155 (Subscriptions redesign): the current channel-search query. Drives
  // `filterSubscriptions` in `renderSubscriptions`; unpersisted (resets on
  // every fresh view mount, like a native search bar).
  let subSearchQuery = '';
  ...
  function mountHistorySection() {
    ...
    // v1.156 (T3): history mounts INTO the Activity panel's static container
    // (was inserted afterend the list's setup-box + re-wired into the
    // master-detail menu). Its content is unchanged; only the mount site moved.
    const anchor = document.getElementById('sub-activity-history');
    if (!anchor) return;
  ...
  // v1.156 (T3, Dean Q1): the "Hide subscriptions (N)" collapse toggle was
  // REMOVED. Search + the A-Z section headers replace the need to collapse a
  // long list. The module-scope chooseCollapseToggleAnchor helper + its test
  // went with it.
  ...
  // ---- v1.47.4 item 7: failure-section state + wiring ---------------------
  //
  // `failureEntries` holds the last fetched (unfiltered) set so a filter change
  // is a pure re-render with no refetch -- the filter is a VIEW concern, and
  // refetching on every dropdown change would make a slow network feel like a
  // broken control.
  let failureListEl = null;
  let failureEntries = [];
  let failureSourceFilter = 'all';

  function renderFailureSection() {
    if (!failureListEl) return;
    const visible = filterFailureEntries(failureEntries, failureSourceFilter);
    clearChildren(failureListEl);
    failureListEl.appendChild(createFailureListElement(visible, document, {
      onDelete: deleteFailureEntry,
      // Lets the empty state distinguish "nothing failed" from "your filter
      // hides everything" -- an active filter must never read as all-clear.
      filtered: failureSourceFilter !== 'all' && failureEntries.length > 0,
    }));
  }

  function loadFailureSection() {
    if (!failureListEl) return;
    fetch('/api/subscriptions/failures')
      .then((r) => (r.ok ? r.json() : { entries: [] }))
      .catch(() => ({ entries: [] }))
      .then((data) => {
        if (signal.aborted) return; // view torn down mid-flight
        failureEntries = Array.isArray(data && data.entries) ? data.entries : [];
        renderFailureSection();
      });
  }

  // Destructive, so it re-reads from the server rather than optimistically
  // splicing the row out locally: the list must show what is actually stored,
  // never what we assumed the delete did.
  // Surfaces a failure-section error through common.js's shared toast when it
  // exists, degrading to a console warning otherwise (node:test, or a page that
  // somehow loaded this script without common.js). Never throws.
  function notifyFailureError(message) {
    // Reached via `window` rather than as a bare identifier: `typeof x` on an
    // undeclared name is runtime-safe but trips eslint's no-undef, and this
    // module is legitimately loaded in both a browser and node:test.
    const toast = (typeof window !== 'undefined') ? window.showToast : undefined;
    if (typeof toast === 'function') toast(message);
    else if (typeof console !== 'undefined') console.warn(message);
  }

  function deleteFailureEntry(id) {
    if (typeof id !== 'string' || id === '') return;
    fetch('/api/subscriptions/failures/' + encodeURIComponent(id), { method: 'DELETE' })
      .then((r) => {
        // v1.47.4 gate SUGGESTION (adversarial seat): the response used to be
        // discarded, so a 404 (stale tab / double-click) or a 500 (disk error)
        // silently re-rendered an unchanged list -- the user pressed Delete and
        // nothing happened, with no way to tell "already gone" from "failed".
        // A 404 needs no complaint (the row IS gone, which is what was wanted);
        // anything else genuinely failed and must say so.
        // `showToast` is a common.js global; guarded because this module is
        // also require()d by node:test, where it does not exist.
        if (!r.ok && r.status !== 404) notifyFailureError('Could not delete that entry.');
      })
      .catch(() => { if (!signal.aborted) notifyFailureError('Network error deleting that entry.'); })
      .then(() => {
  ...
  // Clear-all is irreversible and wipes records Dean explicitly keeps "for
  // posterity", so it confirms first. Per-row delete does not: it destroys one
  // visible row the user just pointed at, and a confirm on every row would
  // train the reflex that makes the clear-all confirm meaningless.
  function clearFailureSection() {
    const count = failureEntries.length;
    if (count === 0) return; // nothing to clear -- don't prompt about a no-op
    const ok = typeof window === 'undefined' || typeof window.confirm !== 'function'
      || window.confirm(`Permanently delete all ${count} recorded download failure${count === 1 ? '' : 's'}? This cannot be undone.`);
    if (!ok) return;
    // `/failures/all`, not `/failures` -- a single-segment DELETE here is
    // shadowed by `/api/subscriptions/:id`. See the route's own comment.
    fetch('/api/subscriptions/failures/all', { method: 'DELETE' })
      .catch(() => null)
      .then(() => {
        if (signal.aborted) return;
        loadFailureSection();
  ...
  function mountFailureSection() {
    if (failureListEl) return;
    // v1.156 (T3): failures mount INTO the Activity panel's static container
    // (was inserted afterend the list + re-wired into the master-detail menu).
    const anchor = document.getElementById('sub-activity-failures');
    if (!anchor) return;
    const { section, list } = createFailureSectionElement(document, {
      onClear: clearFailureSection,
      onFilterChange: (value) => {
        failureSourceFilter = FAILURE_SOURCE_FILTERS.includes(value) ? value : 'all';
        renderFailureSection(); // pure re-render -- see failureEntries' comment
      },
    });
    anchor.appendChild(section);
    failureListEl = list;
  ...
  // v1.155: `handleRowTap` was removed -- a row tap opens the settings panel
  // now (createSubscriptionRow's `onOpenSettings`), not the channel playlist.
  // Playlist access lives on the row's own "View as Playlist" link.
  ...
  // documented contract).
  // v1.156.1 (Dean device): the A-Z scrubber rail was REMOVED -- as built it was
  // a fat gapped column that scrolled away on a long list. Search + the A-Z
  // section headers are the navigation now.

  function renderSubscriptions() {
    if (!listContainer) return;
    clearChildren(listContainer);
    // v1.155: render the search-FILTERED library, grouped into A-Z sections. A
    // non-empty library that filters to nothing is a "no matches" state,
    // distinct from the first-run "no subscriptions yet" empty state.
    const filtered = filterSubscriptions(currentSubs, subSearchQuery);
    const hasQuery = !!(subSearchQuery && subSearchQuery.trim() !== '');
    const container = createSubscriptionsListElement(filtered, document, {
      onOpenSettings: openSettingsSheet,
      onTogglePin: togglePin,
      onToggleBell: toggleBell, // v1.314
      ...
      emptyMessage: hasQuery && currentSubs.length > 0 ? 'No channels match your search.' : undefined,
    });
    listContainer.appendChild(container);
    // v1.155: the rows are now nested inside `.sub-section` wrappers, so the
    // old flat `container.children[i] <-> currentSubs[i]` index alignment no
    // longer holds. Rebuild the id-keyed row map by QUERYING `data-sub-id`
    // instead -- the poll's in-place updater (`applyStatusUpdatesInPlace`) keys
    // strictly by id, so a query-built map is all it ever needed.
    rowElementsById = {};
    const rows = typeof container.querySelectorAll === 'function'
      ? container.querySelectorAll('.sub-row[data-sub-id]')
      : [];
    Array.prototype.forEach.call(rows, (rowEl) => {
      const id = rowEl && rowEl.getAttribute ? rowEl.getAttribute('data-sub-id') : null;
      if (id) rowElementsById[id] = rowEl;
    });
  }

  // v1.155 (Subscriptions redesign, Q2): `wireSubRowDragAndDrop` and
  // `persistReorder` were REMOVED with manual reordering -- the list is
  // alphabetical (A-Z sections + search) now, so there is no persisted user
  // order to write and the client no longer calls `POST /api/subscriptions/
  // reorder` at all (route left intact server-side).
  ...
  function repullOne(id) {
    ...
    return fetch('/api/subscriptions/' + encodeURIComponent(id) + '/repull', { method: 'POST' })
      .then((r) => {
        // Deliberately NOT 'busy': nothing ever clears this one-shot info
        // line (per-row status carries the live progress), and a spinner
        // with no clearing writer would spin forever.
        if (r.ok) actionStatus(repullStatus, 'Re-pull requested…');
        else actionStatus(repullStatus, 'Re-pull could not be started.', 'error');
      ...
      .catch((err) => {
        actionStatus(repullStatus, 'Re-pull request failed.', 'error');
  ...
  // v1.314: the row's bell -> PATCH { pushBell }. v1.316 (Dean, B1): then
  // update THAT row IN PLACE from the server's response - never
  // `loadSubscriptions()` (which cleared the list, painted skeleton rows,
  // re-fetched and rebuilt every row: the "page refresh" Dean saw on every
  // toggle) and never an optimistic flip (the glyph follows `data.pushBell`
  // from the RESPONSE, not the request). The ~2.5s poll's in-place updater is
  // the precedent (keyed by `data-sub-id` via `rowElementsById`, looked up at
  // RESPONSE time so a search-keystroke re-render mid-flight still lands on
  // the live row). The record is patched too - `sub` (the next tap reads
  // `sub.pushBell`) and the `currentSubs` entry by id (the next full re-render
  // rebuilds from it) - or the row would snap back to the OLD state. A non-2xx
  // (a 403 for a user without manage-subscriptions, a 400) is logged and
  // leaves the row exactly as it was - the FIX 4 lesson from togglePin. THAT
  // bell is disabled for the flight so a double-tap on it cannot race two
  // PATCHes; a list rebuild mid-flight (Pause / Retry / a search keystroke)
  // builds a fresh, enabled bell, and a tap on it starts a second flight -
  // both responses are server truth for the same id, so the end state
  // converges (the #233 race class, disclosed, not closed here).
  function toggleBell(sub) {
    if (!sub || !sub.id) return;
    const subId = sub.id;
    const inFlightBtn = findBellButton(rowElementsById[subId]);
    if (inFlightBtn) inFlightBtn.disabled = true;
    fetch('/api/subscriptions/' + encodeURIComponent(subId), {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pushBell: !(sub.pushBell === true) }),
    })
      .then((res) => {
        if (!res.ok) {
          return res.json().catch(() => ({})).then((data) => {
            console.error('Notification bell toggle failed:', (data && data.error) || res.status);
          });
        }
        return res.json().catch(() => ({})).then((data) => {
          const on = !!(data && data.pushBell === true);
          sub.pushBell = on;
          currentSubs.forEach((entry) => { if (entry && entry.id === subId) entry.pushBell = on; });
          applyBellUpdateInPlace(rowElementsById, subId, on);
        });
      })
      .catch((err) => console.error('Notification bell toggle failed:', err))
      .finally(() => { if (inFlightBtn) inFlightBtn.disabled = false; });
  }
  ...
  if (addBtn) {
    addBtn.addEventListener('click', () => {
      ...
      // v1.285: MINUTES in, SECONDS out (blank -> omit). The server rejects an
      // empty window (min >= max) at add time with a clean 400.
      const addMinSeconds = minutesInputToSeconds(addMinDurationInput ? addMinDurationInput.value : '');
      if (addMinSeconds !== undefined) body.minDurationSeconds = addMinSeconds;
      const addMaxSeconds = minutesInputToSeconds(addMaxDurationInput ? addMaxDurationInput.value : '');
      if (addMaxSeconds !== undefined) body.maxDurationSeconds = addMaxSeconds;
      ...
      fetch('/api/subscriptions', {
        ...
        .then(async (r) => {
          ...
          if (addMinDurationInput) addMinDurationInput.value = ''; // v1.285
          loadSubscriptions();
        })
        .catch((err) => {
  ...
  if (repullAllBtn) {
    repullAllBtn.addEventListener('click', () => {
      fetch('/api/subscriptions/repull', { method: 'POST' })
        .then((r) => {
          // Same no-'busy' rule as repullOne: no writer ever clears this
          // line, so a spinner here would never stop.
          if (r.ok) actionStatus(repullStatus, 'Re-pull-all requested…');
          else actionStatus(repullStatus, 'Re-pull-all could not be started.', 'error');
          scheduleRepullRefresh(1500);
        })
        .catch((err) => {
          actionStatus(repullStatus, 'Re-pull-all request failed.', 'error');
  ...
  // v1.56: wires the "Reheat sub counts" button/cancel control -- mirrors the
  // refresh-avatars wiring immediately above.
  if (reheatSubsElements.button) {
    reheatSubsElements.button.addEventListener('click', () => triggerReheatSubs(reheatSubsElements), { signal });
  }
  if (reheatSubsElements.cancelButton) {
    reheatSubsElements.cancelButton.addEventListener('click', () => triggerReheatSubsCancel(reheatSubsElements), { signal });
  }
  // v1.115 (Dean, A1): "Refresh channel names" wiring.
  if (channelNameBackfillElements.button) {
    channelNameBackfillElements.button.addEventListener('click', () => triggerChannelNameBackfill(channelNameBackfillElements), { signal });
  }
  if (channelNameBackfillElements.cancelButton) {
    channelNameBackfillElements.cancelButton.addEventListener('click', () => triggerChannelNameBackfillCancel(channelNameBackfillElements), { signal });
  }
    ...
    oneShotDownloadBtn.addEventListener('click', () => {
      ...
      fetch('/api/ytdlp/download', {
        ...
        .then(async (r) => {
          ...
          // Idle informational (no 'busy' -- the one-shot job list below
          // carries the live progress, and nothing clears this line).
          actionStatus(oneShotStatus, 'Download queued…');
  ...
  function pollStatusOnce() {
    ...
    fetch('/api/subscriptions/status')
      ...
      .then((snapshot) => {
        ...
        // v1.56 (bulk subscriber-count reheat): same posture, its own id.
        applyReheatSubsStateToControls(reheatSubsElements, latestSnapshot.oneShots[REHEAT_SUBS_ACTIVITY_ID]);
        applyChannelNameBackfillStateToControls(channelNameBackfillElements, latestSnapshot.oneShots[CHANNEL_NAME_BACKFILL_ACTIVITY_ID]);
        renderOneShots();
  ...
  // v1.155 (Subscriptions redesign): live channel search -- re-render the A-Z
  // list on every keystroke. Unpersisted; the input starts empty on each fresh
  // view mount, so nothing to restore here.
  if (subSearchInput) {
    subSearchInput.addEventListener('input', () => {
      subSearchQuery = subSearchInput.value || '';
      renderSubscriptions();
    }, { signal });
  }
  ...
  mountHardeningStrip();
  loadHistorySection();
  // v1.47.4 item 7: the failure section mounts + loads alongside history. It
  // deliberately does NOT ride the ~2.5s status poll the history section uses:
  // failures are a durable record read for posterity, not a live feed, and
  // re-fetching them every 2.5s would be pure noise. It refreshes on view load
  // and after any delete/clear -- the only moments its contents can change from
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    // v1.155 (Subscriptions redesign): the A-Z sectioning + search helpers,
    // exported so their bucketing/sort/filter are bound by EXECUTION. These
    // replace the removed reorder decision (deriveReorderRowSubs/
    // buildSubscriptionReorderHandler) now that the list is alphabetical.
    subscriptionSortKey,
    ...
    // v1.285: minutes<->seconds for the duration-window inputs.
    ...
    isPartialRowStatus,
    // v1.316 (B1): the bell's single state writer + the post-PATCH in-place
    // applier, exported so the one-writer contract is bound by execution.
    applyBellState,
    findBellButton,
    applyBellUpdateInPlace,
    ...
    // same pure-formatter/applier/trigger export posture as refresh-avatars.
    ...
    applyChannelNameBackfillStateToControls,
    triggerChannelNameBackfill,
    triggerChannelNameBackfillCancel,
    ...
    // v1.47.4 item 7: the failure section's pure builders + filter.
