...
// below, the two places all of it is rendered). v1.20.0 FR-4: the same
// discipline applies to the per-channel Playlist link's `href` -- the
// server-derived `channelDir` is ALWAYS passed through `encodeURIComponent`
// before being concatenated into the `/?root=` link, and is assigned via the
// anchor's `.href` property (never a template string handed to `innerHTML`);
// the link's own visible label is a fixed literal, never interpolated data.
//
...
function createSubscriptionRow(sub, doc, handlers, liveEntry) {
  ...
  // omitted, never null/empty-string, on a confinement failure). Reuses the
  // EXACT `/?root=<path>` pattern the sidebar/Playlists sheet already use
  // (public/js/common.js) -- `root=` is a pure path-prefix filter
  // (server.js's `underFolder`), so this link confines to that channel's own
  // subfolder and can never surface another channel's videos or the whole
  // Downloads root (AC19/AC20). Built via `createElement`/`.href`/
  // `textContent` only -- `channelDir` is `encodeURIComponent`-escaped into
  // the query string, and the link's own label is a fixed literal, never
  // interpolated -- so this can never become an innerHTML/XSS surface.
  ...
  if (sub && typeof sub.channelDir === 'string' && sub.channelDir !== '') {
