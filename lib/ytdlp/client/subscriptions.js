 ...
 * falls back to the real clock. See `ACTIVE_ENTRY_STALE_MS`'s comment for
 * the full staleness rationale (F4).
 */
...
//
// Pure precedence check: a non-blank, sanitized `channelAvatarUrl` wins over
// the generated first-letter fallback -- the SAME precedence
// public/js/common.js's `resolveAvatarSource`/`buildPinAvatarNode` already
// apply for the sidebar-pin avatars and the watch-page uploader avatar
// (`?real avatar -> img; else generated/letter?`). Reimplemented locally
// (rather than calling that global directly) because this file is a
// standalone browser script served from its own dedicated route
// (`GET /js/subscriptions.js`, lib/ytdlp/index.js), not bundled/built
// alongside public/js/, and its own Node-testability contract (this file's
// header comment) requires every helper it uses to be requirable/exercisable
// in isolation, without a real browser/DOM or a second script tag's globals.
// The PRECEDENCE itself is intentionally identical -- just not literally the
// same function reference. `channelAvatarUrl` is already validated
// (https-only, well-formed) server-side, at write time
// (`store.sanitizeChannelAvatarUrl`) -- this is a presence check only, the
// same trust boundary `resolveAvatarSource` itself applies.
function hasRealChannelAvatar(channelAvatarUrl) {
  return typeof channelAvatarUrl === 'string' && channelAvatarUrl.trim() !== '';
}
...
function createSubscriptionRow(sub, doc, handlers, liveEntry, pinned) {
  ...
  // v1.25.5 QoL follow-up (channel avatars, round 2): a real, sanitized
  // `sub.channelAvatarUrl` wins (rendered as an `<img>`, DOM-only, no
  // `innerHTML`, `alt=''` since the name is already shown as text alongside
  // it) -- see `hasRealChannelAvatar`'s own doc comment for the full
  // precedence rationale. The `.sub-row-avatar` CONTAINER itself is
  // unchanged either way (same class/size/shape); only its content differs.
  if (sub && hasRealChannelAvatar(sub.channelAvatarUrl)) {
    const avatarImg = d.createElement('img');
    avatarImg.alt = '';
    avatarImg.src = sub.channelAvatarUrl.trim();
...
function buildSettingsSheet(sub, doc, handlers) {
  ...
  const trimmedName = sub && typeof sub.name === 'string' ? sub.name.trim() : '';
  // v1.25.5 QoL follow-up (channel avatars, round 2): same real-avatar-vs-
  // letter precedence as `createSubscriptionRow`'s own `.sub-row-avatar`
  // above -- see `hasRealChannelAvatar`'s doc comment.
  if (sub && hasRealChannelAvatar(sub.channelAvatarUrl)) {
    const avatarImg = d.createElement('img');
    avatarImg.alt = '';
    avatarImg.src = sub.channelAvatarUrl.trim();
