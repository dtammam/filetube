...
function createHistorySectionElement(doc) {
  const d = doc || document;
  // v1.55 Track D: a collapsible details card (open by default -- exactly
  // yesterday's layout) instead of a div+h2; the summary IS the heading,
  // styled by the existing .sub-collapsible rules. Collapse persistence is
  // wired by the mount site (wireMenu -> wireMasterDetail), not here -- this
  // builder stays pure DOM for its node:test coverage.
  ...
  section.setAttribute('data-md-icon', 'clock');   // v1.152: master-detail Activity group
  section.setAttribute('data-md-group', 'Activity');
  section.open = true;
...
function createFailureSectionElement(doc, handlers) {
  ...
  section.setAttribute('data-md-icon', 'warning');  // v1.152: master-detail Activity group
  section.setAttribute('data-md-group', 'Activity');
  section.open = true;
...
// v1.155.1 (Dean device bug): choose where the "Hide subscriptions" collapse
// toggle is inserted. It must land ABOVE the list+scrubber flex row
// (`.sub-list-body`), never inside it -- inserted `beforebegin` the list
// (its historical anchor) it became a flex SIBLING of the list once T1 wrapped
// the list in `.sub-list-body`, stealing the row's width and crushing every
// channel name to one letter per line on Dean's 213-subscription library.
// Returns the `.sub-list-body` wrapper when the list is inside one, else the
// list itself (an older/un-wrapped shell). Exported so the anchor choice is
// bound by EXECUTION, not left to the closure.
function chooseCollapseToggleAnchor(listContainer) {
  if (listContainer && typeof listContainer.closest === 'function') {
...
function initSubscriptionsView(root) {
  ...
  // v1.152: the master-detail menu (was per-section <details> collapse). Called
  // AGAIN after each dynamic section (history/failures) mounts, so wireMasterDetail
  // adopts the new card into the menu (idempotent: a re-call just refreshes the
  // nav). typeof-guarded like actionStatus above. Scoped to the view ROOT, not
  // document (QA S1: the SPA swaps only #view-root -- scoping to it is the
  // defensive default), falling back to document when the router hands us nothing.
  const wireMenu = () => {
    if (typeof wireMasterDetail === 'function') wireMasterDetail('subscriptions', root || document, signal);
  };
  wireMenu();
  ...
  function mountHistorySection() {
    ...
    historyListEl = list;
    wireMenu(); // v1.152: adopt the just-mounted card into the master-detail menu
  }
  ...
  function mountCollapseToggle() {
    ...
    // v1.155.1 (Dean device bug): the collapse toggle must sit ABOVE the
    // list+scrubber flex row (`.sub-list-body`), NOT inside it -- inserted as a
    // flex sibling of the list it steals the row's width and crushes every
    // channel name to one letter per line. `chooseCollapseToggleAnchor`
    // (module scope, exported + test-bound) returns the wrapper so the toggle
    // lands above the whole row.
    const collapseAnchor = chooseCollapseToggleAnchor(listContainer);
    if (typeof collapseAnchor.insertAdjacentElement === 'function') {
      collapseAnchor.insertAdjacentElement('beforebegin', collapseToggleBtn);
    } else if (collapseAnchor.parentNode) {
      collapseAnchor.parentNode.insertBefore(collapseToggleBtn, collapseAnchor);
    }
  ...
  function mountFailureSection() {
    if (!listContainer || failureListEl) return;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    filterSubscriptions,
    // v1.155.1: the collapse-toggle anchor choice (Dean device-crush fix).
    chooseCollapseToggleAnchor,
