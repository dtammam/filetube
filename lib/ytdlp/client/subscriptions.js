 ...
 */
function formatYtdlpVersionText(version) {
  return typeof version === 'string' && version !== '' ? `yt-dlp ${version}` : '';
}

function formatLiveStatusText(entry) {
  if (!entry || typeof entry !== 'object') return null;
  const state = entry.state;
  // v1.31 P5: one-shot entries carry `queuedAhead` (gate jobs ahead on the
  // serial queue); subscriptions never do (the server only enriches
  // oneShots), so subscription rows keep the plain literal. Additive: an
  // older/absent field falls back unchanged.
  if (state === 'queued') {
