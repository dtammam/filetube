...
function createSubscriptionsListElement(subs, doc, handlers, statusSnapshot, pinnedChannelDirs, options) {
  ...
  groupSubscriptionsByLetter(subs).forEach(({ letter, subs: sectionSubs }) => {
    ...
    // v1.155: the section header is decorative repetition of the scrubber's
    // target; hide it from assistive tech so a screen reader hears the
    // channel names, not a run of single letters.
...
function initSubscriptionsView(root) {
  ...
  // v1.155 (Subscriptions redesign): the live search field + the A-Z scrubber
  // rail that flank the list (static markup in subscriptions.html). Either may
  // be absent on an older cached shell -- every use below is null-guarded.
  const subSearchInput = document.getElementById('sub-search-input');
  const subScrubberEl = document.getElementById('sub-scrubber');
  ...
  // REMOVED. Search + the A-Z scrubber replace the need to collapse a long
  // list, and removing it also kills the v1.155.1 residual where the A-Z rail
  // floated in empty space while the list was collapsed. The module-scope
  // chooseCollapseToggleAnchor helper + its test went with it.
  ...
  // documented contract).
  // v1.155 (Subscriptions redesign): (re)build the A-Z scrubber rail from the
  // sections currently rendered in the list -- the DOM is the source of truth
  // for which letters are reachable, so a filtered/sparse list only ever
  // offers letters that actually have a target. Tapping a letter scrolls its
  // section to the top of the scroll area. Hidden outright when the list has
  // no sections (empty library or no search matches), so an empty rail never
  // shows.
  function renderScrubber() {
    if (!subScrubberEl) return;
    clearChildren(subScrubberEl);
    const sections = listContainer && typeof listContainer.querySelectorAll === 'function'
      ? listContainer.querySelectorAll('.sub-section[data-letter]')
      : [];
    if (!sections || sections.length === 0) {
      subScrubberEl.setAttribute('hidden', '');
      return;
    }
    subScrubberEl.removeAttribute('hidden');
    Array.prototype.forEach.call(sections, (section) => {
      const letter = section.getAttribute && section.getAttribute('data-letter');
      if (!letter) return;
      const dot = document.createElement('button');
      dot.type = 'button';
      dot.className = 'sub-scrubber-letter';
      dot.textContent = letter;
      // v1.155: the rail is a pointer-only convenience mirroring the (already
      // AT-hidden) section headers -- the sections + channel rows are the
      // accessible navigation. Keep the dots out of the tab order so a
      // keyboard/AT user is not made to pass 27 jump buttons before the list;
      // the container carries aria-hidden in the markup.
  ...
  function renderSubscriptions() {
    ...
    // v1.155: refresh the A-Z scrubber rail to the letters actually present in
    // the (possibly filtered) list.
    renderScrubber();
  }

  // v1.155 (Subscriptions redesign, Q2): `wireSubRowDragAndDrop` and
  // `persistReorder` were REMOVED with manual reordering. The A-Z scrubber
  // (`renderScrubber`, below) is the list's new fast-navigation affordance;
  // there is no persisted user order to write, so the client no longer calls
  // `POST /api/subscriptions/reorder` at all (route left intact server-side).
