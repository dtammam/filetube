...
function createSubscriptionRow(sub, doc, handlers, liveEntry, pinned) {
  ...
  if (sub && typeof sub.id === 'string' && sub.id !== '') {
    // v1.76: no `draggable` attribute any more -- reordering is a POINTER
    // gesture now (see wireSubRowDragAndDrop), and leaving native HTML5 drag
    // enabled would let the browser start one out from under it.
...
// v1.76 (adversarial gate W1): the subscriptions reorder DECISION, hoisted to
// module scope so a test can execute it. Two things it owns, both of which
// silently survived the whole unit suite while they lived inside the view
// closure: the row-aligned subscription list, and the from/to argument order.
//
// `rowSubs` is derived with the SAME predicate the row builder uses to stamp
// `data-sub-id` (createSubscriptionRow). Indexing the FULL list by row index -
// what this did before v1.76 - silently misaligns every row after an id-less
// subscription, because such a subscription gets a row but no attribute and so
// is not a drag target.
//
// `move` is injected rather than reached for as a global: this file loads as a
// plain <script> beside common.js in the browser (so the caller passes
// `window.moveArrayItem`, the exact function common.js defines), while a test
// passes the same function required directly. No second implementation exists.
//
// COUPLED TO P3 - do not remove that guard thinking it is belt-and-braces.
// `getSubs` is read LIVE at drop time, not snapshotted at wire time, and the
// QA gate established that this is safe only because `wireReorderable`'s
// endGesture refuses any drop whose rows were rebuilt mid-gesture
// (`rows[from].isConnected === false`). Both writers of `currentSubs`
// (subscriptions.js ~:3813 and ~:3856) call `renderSubscriptions()`
// synchronously on the next line, and that detaches the rows - so there is no
// turn of the event loop in which the list is new while the dragged row is
// still connected. Delete P3 and that guarantee goes with it, turning a
// live-vs-snapshot subtlety into a silently wrong persisted order.
// Exported for node:test.
function deriveReorderRowSubs(subs) {
  return (Array.isArray(subs) ? subs : []).filter((sub) => sub && typeof sub.id === 'string' && sub.id !== '');
}

function buildSubscriptionReorderHandler({ getSubs, persist, move }) {
  return (fromIndex, toIndex) => {
    const rowSubs = deriveReorderRowSubs(typeof getSubs === 'function' ? getSubs() : []);
    const reordered = move(rowSubs, fromIndex, toIndex);
    persist(reordered.map((sub) => sub && sub.id).filter((id) => typeof id === 'string' && id !== ''));
  };
}
...
function initSubscriptionsView(root) {
  ...
  // B4 (v1.24.0, T6, FR-8), rewired in v1.76: drag-to-reorder for the
  // subscriptions management list. It used to carry its own copy of the
  // native HTML5 DnD wiring (five listeners, mirroring main.js's and
  // common.js's copies); it now calls the shared pointer gesture layer, so
  // this list can be reordered on a touch screen at all -- native HTML5 drag
  // never fired there.
  //
  // Unchanged: this page's OWN `.sub-row-dragging`/`.sub-row-drag-over-*`
  // class family (passed in rather than forked in CSS), and the immediate
  // persist on drop -- there is no Save button on this list.
  //
  // `window.`-qualified, as this file's reuse of `moveArrayItem` already was:
  // both load as plain non-module <script> tags on the same page (see
  // subscriptions.html's script order), so `window.wireReorderable` IS the
  // function common.js defines, and qualifying it avoids an eslint.config.js
  // globals edit outside this file's scope.
  //
  // The old comment here called drag events "untestable-by-necessity" - true
  // of HTML5 DataTransfer, which jsdom does not implement, but not of pointer
  // events: the gesture layer is bound end-to-end in
  // test/unit/reorder-gesture.test.js, and the server side stays proven by
  ...
  function wireSubRowDragAndDrop() {
    ...
    window.wireReorderable(listContainer, {
      rowSelector: '.sub-row[data-sub-id]',
      classes: {
        dragging: 'sub-row-dragging',
        before: 'sub-row-drag-over-before',
        after: 'sub-row-drag-over-after',
      },
      // The DECISION lives at module scope (below) so it can be executed by a
      // test. Adversarial gate W1 measured what this cost while it was inline:
      // swapping the two index arguments - so every drag on this list persists
      // a DIFFERENT order than the one dropped - survived all 4472 unit tests.
      // This closure is not reachable from a test (it is nested inside
      // initSubscriptionsView), and re-implementing its body in a fixture is
      // this repo's own divergent-fixture bug class.
      onReorder: buildSubscriptionReorderHandler({
        getSubs: () => currentSubs,
        persist: persistReorder,
        move: window.moveArrayItem,
      }),
      signal,
    });
  }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    // v1.76 (adversarial gate W1): the reorder decision, exported so its
    // argument order and row alignment are bound by EXECUTION rather than by
    // the census's presence check.
    deriveReorderRowSubs, buildSubscriptionReorderHandler,
