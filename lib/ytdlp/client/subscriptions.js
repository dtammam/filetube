...
function formatChannelNameBackfillSummary(total) {
  const t = typeof total === 'number' && Number.isFinite(total) && total > 0 ? Math.floor(total) : 0;
  if (t === 0) return 'No channels need a name refresh.';
  const channelWord = t === 1 ? 'channel' : 'channels';
  return `Refreshing names for ${t} ${channelWord}…`;
}
...
function formatChannelNameBackfillProgressText(entry) {
  ...
  if (entry.state === 'running') {
    ...
    if (itemsUpdated > 0) parts.push(`${itemsUpdated} video${itemsUpdated === 1 ? '' : 's'} renamed`);
    if (skipped > 0) parts.push(`${skipped} skipped`);
    if (failed > 0) parts.push(`${failed} failed`);
    if (current !== '') parts.push(`current: ${current}`);
    return parts.join(' · ');
  ...
  if (entry.state === 'done') {
    const parts = [`Channel-name refresh done: ${done} of ${total} channels`];
    parts.push(`${itemsUpdated} video${itemsUpdated === 1 ? '' : 's'} renamed`);
    if (skipped > 0) parts.push(`${skipped} skipped`);
    if (failed > 0) parts.push(`${failed} failed`);
    return parts.join(' · ');
  }
...
function triggerChannelNameBackfill(elements, fetchImpl) {
  ...
  doFetch('/api/ytdlp/backfill-channel-names', { method: 'POST' })
    ...
    .then(({ ok, status, data }) => {
      if (ok && data.started) {
        actionStatus(elements.status, formatChannelNameBackfillSummary(data.total), 'busy');
        return;
      }
      if (status === 409 || data.alreadyRunning) {
