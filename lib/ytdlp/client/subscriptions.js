...
function createSubscriptionRow(sub, doc, handlers, liveEntry, pinned) {
  ...
  // thing `sub.pushBell` affects here - the truth is server-side and the
  // handler re-fetches the list after its PATCH. Fixed literal glyphs only.
  if (sub && typeof sub.id === 'string' && sub.id !== '') {
    const bellOn = sub.pushBell === true;
    const bellBtn = d.createElement('button');
    bellBtn.type = 'button';
    bellBtn.className = bellOn ? 'sub-row-bell sub-row-bell-active' : 'sub-row-bell';
...
function initSubscriptionsView(root) {
  ...
  // v1.314: the row's bell -> PATCH { pushBell } then re-fetch the list so the
  // glyph reflects the server's record, never an optimistic flip. A non-2xx (a
  // 403 for a user without manage-subscriptions, a 400) is surfaced, not
  // swallowed - the FIX 4 lesson from togglePin.
