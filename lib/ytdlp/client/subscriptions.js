 ...
 *
 * Fields (AC21): quality/count(maxVideos)/type(format)/filetype/skipShorts/
...
function buildSettingsSheet(sub, doc, handlers) {
  ...
  maxVideosInput.setAttribute('placeholder', 'blank = unchanged, 0 = unlimited');
  if (sub && typeof sub.maxVideos === 'number') maxVideosInput.value = String(sub.maxVideos);
  body.appendChild(buildSheetField(d, 'Download last N videos', maxVideosInput));
  ...
  saveBtn.addEventListener('click', () => {
    ...
    // checked/unchecked state; maxVideos only sent when the input is
    // non-blank and a valid non-negative integer, so a blank field means
    // "leave unchanged" rather than clearing it to 0).
    const patch = {
    ...
    const rawMaxVideos = typeof maxVideosInput.value === 'string' ? maxVideosInput.value.trim() : '';
    if (rawMaxVideos !== '') {
      const parsed = Number(rawMaxVideos);
      if (Number.isInteger(parsed) && parsed >= 0) patch.maxVideos = parsed;
    }
    // v1.22.0 FR-6: same blank-means-unchanged posture as maxVideos above.
...
function applyStatusUpdatesInPlace(rowElementsById, subs, statusSnapshot) {
  ...
  subs.forEach((sub) => {
    ...
    if (!statusEl) return;
    statusEl.textContent = formatLiveStatusText(liveSubs[sub.id]) || formatSubStatus(sub);
  });
}
...
function initSubscriptionsView(root) {
  ...
  if (addBtn) {
    addBtn.addEventListener('click', () => {
      ...
      // v1.22.0 FR-6: same blank-means-omit posture as maxVideos above.
      if (rawMaxDuration !== '') {
        const parsed = Number(rawMaxDuration);
