...
function isPlausibleMediaUrl(raw) {
  ...
  if (FORBIDDEN_CHARS.test(trimmed)) return { ok: false, error: 'URL contains forbidden characters' };
  if (trimmed.startsWith('-')) return { ok: false, error: 'URL may not start with "-"' };
  let parsed;
  try { parsed = new URL(trimmed); } catch { return { ok: false, error: "This doesn't look like a media URL" }; }
