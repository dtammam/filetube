...
// never reach persistence (the database) or a child-process argv.
...
// v1.41.13: the same guard MINUS the query separator `&`, for the universal
// one-off lane -- a non-YouTube URL legitimately carries a multi-param query
// (`?p=3&t=5`), the URL is only ever a positional argv element (never a shell
// string), and searchParams canonicalization has already percent-encoded every
// value, so a literal `&` can only be a structural separator. Every other
// shell/control metacharacter is still rejected identically.
// eslint-disable-next-line no-control-regex
const UNIVERSAL_FORBIDDEN_CHARS = /[\s\x00-\x1f\x7f;|`$<>'"(){}\\]/;
...
// ---- v1.41.13 (universal one-offs): source-aware filename-bracket parser ----
//
// `extractMediaRef(baseName)` reads the yt-dlp `[id]` bracket at the END of a
// file's basename (extension already stripped by the caller) and reports WHICH
// SOURCE it belongs to. Two shapes, deliberately DISJOINT so every shipped
// YouTube code path keeps matching only YouTube files (see the design doc D1):
//   - LEGACY YouTube:  ` [<11-char id>]`  -> { source: 'youtube', id }
//       exactly `extractYtdlpVideoId`'s shape (`[A-Za-z0-9_-]{11}`); '=' can
//       never appear in that charset, so it can't be confused with the new
//       shape below.
//   - UNIVERSAL:       ` [<ExtractorKey>=<id>]` -> { source, id }
//       written by the non-YouTube one-off template `[%(extractor_key)s=%(id)s]`.
//       Key is `[A-Za-z0-9_]{1,64}` (yt-dlp's class-name charset -- a plugin
//       key's `+plugin` suffix is deliberately EXCLUDED, so plugin items are
//       metadata-only, no bracket key, per design S1). The bracketed id is the
//       SANITIZED on-disk form (yt-dlp already folded `/?:*` etc. per-OS), so
//       it is matched permissively: any run without `[`, `]`, or `/`, capped
//       at 128 (D5: the bracket is a same-folder RECOVERY HINT, never trusted
//       as authoritative -- callers cross-check against metadata/archive).
// Returns `null` for a name with no parseable trailing bracket. PURE, no I/O.
//
// TRUST (load-bearing): this only PARSES the shape. A title legitimately ending
// in ` [Word=x]` parses too -- so every caller confines the result to its own
// scope (delete resolver / SEAM-2: same-folder+ext confinement, exactly as the
// v1.41.9 machinery already does for the legacy bracket) and, where identity
// matters, only trusts a ref whose `source` matches a known extractor recorded
// in metadata or the archive. Wired ONLY to the source-aware sites; the legacy
// `extractYtdlpVideoId` call graph (Share URLs, the duplicates report, the
// relocation id-check) is untouched.
const LEGACY_YT_BRACKET = /[ _]\[([A-Za-z0-9_-]{11})\]$/;
const UNIVERSAL_BRACKET = /[ _]\[([A-Za-z0-9_]{1,64})=([^[\]/]{1,128})\]$/;

function extractMediaRef(baseName) {
  if (typeof baseName !== 'string' || baseName.length === 0) return null;
  const legacy = LEGACY_YT_BRACKET.exec(baseName);
  if (legacy) return { source: 'youtube', id: legacy[1] };
  const universal = UNIVERSAL_BRACKET.exec(baseName);
  if (universal) return { source: universal[1], id: universal[2] };
  return null;
}

// ---- v1.41.13: the loose one-off intake gate + its LOAD-BEARING SSRF guard --
//
// The subscription/channel surface keeps `validateChannelUrl`'s YouTube
// allowlist. A one-off download instead accepts any plausible media URL and
// lets yt-dlp (with `--use-extractors default,-generic`) be the real
// validator. But since NAMED extractors match arbitrary hosts by path shape
// (MediasiteIE et al -- design C2), the private-address reject below is the
// ONLY SSRF defense, not belt-and-suspenders: it must catch every literal form
// of an internal address. `http(s)` only, no embedded credentials, length-
// capped, reuses the RAW-string FORBIDDEN_CHARS check (shell/control-char
// option-injection guard) that validateChannelUrl uses. Never throws.
//
// Residual (disclosed, design C2): a PUBLIC hostname that RESOLVES to internal
// space is not caught here (that needs the D6 resolver's per-hop
// resolve-then-check); this literal guard is the synchronous, pure first line.

// Every literal shape of a loopback / private / link-local / unspecified host.
// IPv4 is normalized first (decimal/octal/hex/short forms all collapse to four
// octets) so `http://2130706433/`, `http://0177.0.0.1/`, `http://127.1/` can't
// smuggle loopback past a naive dotted-quad check.
function normalizeIpv4(host) {
  // Split into 1-4 parts; each part may be decimal, 0x-hex, or 0-octal.
  ...
  // Collapse per inet_aton: the last part absorbs all remaining low octets.
  let value;
  ...
  if (nums.length > 1 && nums.slice(0, -1).some((x) => x > 255)) return null;
  if (value > 0xffffffff || value < 0) return null;
  return [(value >>> 24) & 255, (value >>> 16) & 255, (value >>> 8) & 255, value & 255];
}

// Expand an IPv6 literal (brackets/zone already stripped, lowercased) to its
// 16 bytes, or null if not well-formed. Handles `::` compression and a
// trailing embedded IPv4. new URL() re-serializes an IPv4-mapped literal to
// HEX hextets (`::ffff:a9fe:a9fe`), which the old code never matched -- gate
// CRITICAL C1 (an SSRF-to-cloud-metadata bypass); both forms are parsed here.
function expandIpv6(host) {
  if (host.indexOf(':') === -1) return null;
  let s = host;
  const lastColon = s.lastIndexOf(':');
  const tail = s.slice(lastColon + 1);
  if (tail.indexOf('.') !== -1) { // trailing embedded IPv4 -> last two hextets
  ...
  const back = parts.length === 2 ? (parts[1] === '' ? [] : parts[1].split(':')) : null;
  const hextets = [];
  for (const h of head) { if (!/^[0-9a-f]{1,4}$/.test(h)) return null; hextets.push(parseInt(h, 16)); }
  if (back !== null) {
    const backVals = [];
    for (const h of back) { if (!/^[0-9a-f]{1,4}$/.test(h)) return null; backVals.push(parseInt(h, 16)); }
    const fill = 8 - hextets.length - backVals.length;
    if (fill < 0) return null;
    for (let i = 0; i < fill; i++) hextets.push(0);
    for (const v of backVals) hextets.push(v);
  }
  if (hextets.length !== 8) return null;
  const bytes = [];
  for (const h of hextets) bytes.push((h >> 8) & 255, h & 255);
  return bytes;
}

function isPrivateIpv6(host) {
  const b = expandIpv6(host);
  if (!b) return false; // not an IPv6 literal
  const zeroThrough = (n) => b.slice(0, n).every((x) => x === 0);
  if (zeroThrough(15) && b[15] === 1) return true; // ::1 loopback
  if (zeroThrough(16)) return true;                // :: unspecified
  if (b[0] === 0xfe && (b[1] & 0xc0) === 0x80) return true; // fe80::/10 link-local
  if ((b[0] & 0xfe) === 0xfc) return true;          // fc00::/7 ULA
  // IPv4-mapped ::ffff:0:0/96 and NAT64 64:ff9b::/96 -> classify the embedded v4.
  if (zeroThrough(10) && b[10] === 0xff && b[11] === 0xff) return isPrivateV4([b[12], b[13], b[14], b[15]]);
  if (b[0] === 0 && b[1] === 0x64 && b[2] === 0xff && b[3] === 0x9b && b.slice(4, 12).every((x) => x === 0)) {
    return isPrivateV4([b[12], b[13], b[14], b[15]]);
  }
  return false; // a real public IPv6 literal
}

function isPrivateOrLocalHost(rawHost) {
  if (typeof rawHost !== 'string' || rawHost === '') return true; // no host -> reject
  let host = rawHost.toLowerCase();
  // Bracketed IPv6 literal: strip brackets + any zone id (`%eth0`).
  ...
  // A bare (unbracketed) IPv6 literal reaches here from a raw-IP argument
  // (hostResolvesPublic passes dns's bare addresses) -- classify it too.
  if (host.indexOf(':') !== -1 && /^[0-9a-f:.]+$/.test(host)) return isPrivateIpv6(host);
  // A single trailing dot is the FQDN-root form (`127.0.0.1.`, `localhost.`)
  // -- strip it before classifying so it can't dodge the checks below.
  if (host.endsWith('.')) host = host.slice(0, -1);
  if (host === 'localhost' || host.endsWith('.localhost')) return true;
  const octets = normalizeIpv4(host);
  if (octets) return isPrivateV4(octets);
  return false; // a DNS name -> literal guard passes; D6 resolve-then-check covers resolution
}
...
function isPlausibleMediaUrl(raw) {
  if (typeof raw !== 'string' || raw.trim() === '') return { ok: false, error: 'A media URL is required' };
  if (raw.length > MAX_URL_LENGTH) return { ok: false, error: 'That URL is too long' };
  // Shortcut/share-sheet normalization: the SAME step sequence
  // validateChannelUrl runs (steps a-d) -- each transformation is one of the
  // module-level helpers below (single-sourced); only this ~6-line
  // orchestration is mirrored, deliberately, so the security-critical YouTube
  // path stays byte-for-byte untouched. A pasted `"Title\nhttps://vimeo.com/x"`
  // (Shortcut) therefore normalizes here exactly as a YouTube paste does.
  let candidate = stripSurroundingPunctuation(raw.trim());
  if (/\s/.test(candidate) || !/^https?:\/\//i.test(candidate)) {
    const match = candidate.match(/https?:\/\/\S+/i);
    if (match) candidate = stripSurroundingPunctuation(match[0]);
  }
  candidate = truncateAtBackslash(candidate);
  candidate = stripFragment(candidate);
  // gate CRITICAL C2: do NOT run rebuildQueryAllowlist here -- that is the
  // YOUTUBE share normalizer (keeps only `v`/`list`, drops everything else).
  // On the universal lane it would silently strip a non-YouTube resource
  // identity that lives in the query (Bilibili `?p=3` part selector, sites
  // with `?video_id=`/`?vid=`/`?clip=`), making yt-dlp download the WRONG item.
  // The universal lane must preserve the full query. FORBIDDEN_CHARS rejects a
  // raw `&`, so a multi-param query is re-encoded losslessly via new URL()'s
  // searchParams below (which normalizes `&`/`=` into a percent-safe form the
  // subsequent FORBIDDEN_CHARS pass accepts) rather than the identity allowlist.
  if (candidate.startsWith('-')) return { ok: false, error: 'That URL is not allowed' };
  let parsed;
  try { parsed = new URL(candidate); } catch { return { ok: false, error: "This doesn't look like a media URL" }; }
  // Re-serialize the query through searchParams so a legitimate multi-param
  // URL (`?p=3&t=5`) is CANONICAL: every value percent-encoded, only the
  // structural `&`/`=` separators left literal. The full query is preserved
  // (gate C2). Then re-check the whole URL against the injection guard, minus
  // the query separator `&`: after searchParams canonicalization `&` appears
  // ONLY as a legitimate separator, and the URL is a positional argv element
  // (never shelled), so a query `&` is safe -- every OTHER shell/control
  // metacharacter is still rejected.
  if (parsed.search !== '') parsed.search = parsed.searchParams.toString();
  candidate = parsed.toString();
  if (UNIVERSAL_FORBIDDEN_CHARS.test(candidate)) return { ok: false, error: 'That URL contains characters that are not allowed' };
...
// ---- v1.41.13: the one-off lane classifier (YouTube vs universal) ----------
//
// The ONE decision the download route makes about a pasted URL (design D1a).
// YouTube FIRST, byte-for-byte unchanged: `classifySingleVideo` (which is
// `validateChannelUrl` + the single-video shape check). If that accepts, it's
// the legacy lane. Only if it does NOT -- AND the URL is not a YouTube-host
// URL at all (a YouTube channel/playlist/handle stays a hard reject; we never
// silently reroute a YouTube URL into the universal lane, which would launder
// it past the /clip/-style refusals) -- do we try the universal lane via
// `isPlausibleMediaUrl`. Returns:
//   { ok:true, lane:'youtube',   videoId, watchUrl }
//   { ok:true, lane:'universal', sourceUrl }
//   { ok:false, error }   -- neutral, never leaks an internal field name
function isYouTubeHostUrl(raw) {
  // Cheap host-only probe on the normalized candidate: is this even a YouTube
  // host? Used only to keep a FAILED YouTube URL (e.g. a /clip/ shape) out of
  // the universal lane. Never throws.
  const norm = isPlausibleMediaUrl(raw);
  if (!norm.ok) return false;
  try { return ALLOWED_HOSTS.has(new URL(norm.url).hostname.toLowerCase()); } catch { return false; }
}

function classifyOneOffUrl(raw) {
  const yt = classifySingleVideo(raw);
  if (yt.ok) return { ok: true, lane: 'youtube', videoId: yt.videoId, watchUrl: yt.watchUrl };
  // A YouTube-host URL that failed single-video classification is a
  // channel/playlist/handle/unsupported-YT-shape -> hard reject, today's
  // behavior (never the universal lane).
  if (isYouTubeHostUrl(raw)) {
    return { ok: false, error: yt.error || 'That YouTube URL is not a single video' };
  }
  const media = isPlausibleMediaUrl(raw);
  if (media.ok) return { ok: true, lane: 'universal', sourceUrl: media.url };
...
module.exports = {
  ...
  classifySingleVideo,
  isChannelRootUrl,
  // v1.41.13 (universal one-offs, P1): the source-aware bracket parser and the
  // loose intake gate + its SSRF host guard. Pure; exported for direct unit
  // coverage (the delete/SEAM-2/consume wiring lands in P2/P3).
  extractMediaRef,
  isPlausibleMediaUrl,
