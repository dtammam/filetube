...
function isPrivateOrLocalHost(rawHost) {
  ...
  if (host.startsWith('[') && host.endsWith(']')) {
    ...
    // Loopback, unspecified, link-local (fe80::/10), ULA (fc00::/7), and
    // IPv4-mapped (::ffff:a.b.c.d) all classified.
    ...
    if (mapped) {
      const octets = normalizeIpv4(mapped[1]);
...
function isPlausibleMediaUrl(raw) {
  ...
  if (FORBIDDEN_CHARS.test(candidate)) return { ok: false, error: 'That URL contains characters that are not allowed' };
  let parsed;
