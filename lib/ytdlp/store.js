...
/**
 * v1.25 QoL bugfix: serve-time (read-only, never persisted) fallback avatar
 * lookup for a SINGLE media item -- matches the item's OWN captured
 * `channelUrl`/`channelId` against every subscription's identity fields and
 * returns the FIRST matching subscription's already-captured avatar,
 * re-validated here again via `sanitizeChannelAvatarUrl` (defense-in-depth,
 * the same re-validate-at-every-boundary posture this file uses everywhere
 * else -- a hand-edited/corrupted subscription record can never leak an
 * unsafe value through this join). Pure, synchronous, never throws.
 *
 ...
 * read that changes nothing on disk, so it also covers an item whose
 * `channelUrl`/`channelId` were captured some other way (e.g. a MeTube-
 * imported video the scan never routed through the yt-dlp download tree at
 * all) -- as long as it matches a subscription's identity, its avatar is
 * served without ever needing a fresh network probe.
 * @param {object} db a loaded database object (`{ ytdlp: { subscriptions } }`)
 * @param {{channelUrl?: string, channelId?: string}} item a `db.metadata` entry
 * @returns {string|null}
 */
function resolveItemChannelAvatarUrl(db, item) {
  ...
  const match = subs.find((sub) => sub && (
    (channelUrl && sub.channelUrl === channelUrl)
    || (channelId && sub.channelId === channelId)
  ));
  if (!match) return null;
  return sanitizeChannelAvatarUrl(match.channelAvatarUrl);
}
