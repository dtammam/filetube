...
// `ytdlpDb` deps (Wave 5 of the relational-migration arc: the namespace is a
// feature store - `ytdlpDb.holder()` is the read snapshot, `ytdlpDb.mutate`
// inside an `updateDatabase` tick is the write; see the FEATURE definition at
// the bottom) - the same v1.9.0 serialized-writer primitive server.js
// uses for everything else) -- this module never touches the filesystem or
// the database directly, and never imports server.js. Because this module is
...
// v1.338: the ONE strict link check (v1.337's watch Share) guards the saved page link too.
...
// disabled path byte-identical -- an old or disabled instance simply never
...
// Read-ONLY view of the ytdlp namespace: the same defensive defaults as
// ensureYtdlp for the fields the READ paths touch, but it NEVER writes to `db`.
// The avatar READ paths (resolveItemChannelAvatarUrl, getChannelAvatar) run
// against the SHARED getCachedDatabase() object; ensureYtdlp's normalizing
// writes would MUTATE that shared cache in place (the v1.42 aliasing hazard) -
// silent corruption in production, and a hard throw under the test runner's
// cache-guard Proxy. Either way a read must not mutate. Writers keep calling
// ensureYtdlp (they hand it a mutable loadDatabase() result).
function readYtdlpNamespace(db) {
  const ns = db && typeof db === 'object' && db.ytdlp && typeof db.ytdlp === 'object' && !Array.isArray(db.ytdlp)
    ? db.ytdlp : null;
  return {
    subscriptions: ns && Array.isArray(ns.subscriptions) ? ns.subscriptions : [],
    channelAvatars: ns && ns.channelAvatars && typeof ns.channelAvatars === 'object' && !Array.isArray(ns.channelAvatars)
      ? ns.channelAvatars : {},
  };
}
...
function ensureYtdlp(db, nowMs = Date.now()) {
  if (!db.ytdlp || typeof db.ytdlp !== 'object') {
    ...
    db.ytdlp = { allowMembersOnly: false, subscriptions: [], downloadMeta: {}, pins: [], channelAvatars: {} };
  } else {
    ...
    // v1.25.x QoL bugfix (channel avatar REGISTRY): the channelId-keyed
    // avatar map -- SAME non-destructive backfill posture as `downloadMeta`
    // immediately above (a present object, even `{}`, is left completely
    // untouched; only a missing/non-object/array value is replaced with a
    // fresh, empty map -- never a shared reference). Distinct from
    // `downloadMeta` (a short-lived, per-videoId bridge consumed at first
    // scan) and from a subscription's own `channelAvatarUrl` (a per-
    // subscription cache): this is the CANONICAL, long-lived, channelId ->
    // avatar map that `resolveItemChannelAvatarUrl` (below) now joins against
    // FIRST, so an item resolves its channel's avatar uniformly regardless of
    // subscription status/recency/URL form (see `registerChannelAvatar`'s own
    // doc comment for the full rationale).
    if (!db.ytdlp.channelAvatars || typeof db.ytdlp.channelAvatars !== 'object' || Array.isArray(db.ytdlp.channelAvatars)) {
      db.ytdlp.channelAvatars = {};
    }
  }
  ...
  for (let index = 0; index < db.ytdlp.subscriptions.length; index++) {
    ...
    // v1.314: `pushBell` backfill mirrors `paused`/`skipShorts` EXACTLY - a
    // boolean, default-FALSE toggle, so every subscription written before the
    // bell existed reads "no push" (Dean: opt IN, existing channels included).
    if (sub && typeof sub.pushBell !== 'boolean') sub.pushBell = false;
    // v1.69 (D15): `libraryPlace` backfill mirrors `paused` - a strict
    // two-value field; anything else (absent, junk) migrates to 'default'
    // (today's behavior) in memory, persisting on the next write.
    if (sub && sub.libraryPlace !== 'default' && sub.libraryPlace !== 'podcasts') sub.libraryPlace = 'default';
    ...
    // present-but-non-integer value (defensive: a hostile/corrupt row) is
...
/**
 * Validate an optional per-subscription `minDurationSeconds` FLOOR (v1.285).
 * Mirrors `validateMaxDurationSeconds` EXACTLY: `undefined` (field absent) is
 * valid and means "no minimum"; any other value must be a non-negative integer
 * no greater than `MAX_SUB_MAX_DURATION_SECONDS` (reusing the same 0..24h bound
 * -- a floor can be anywhere the ceiling can). `0` = no minimum (distinct,
 * mirroring the ceiling's `0` = unbounded). Unlike the ceiling, the floor has NO
 * global fallback: unset/`0` simply contributes no clause. The `min <= max`
 * cross-check lives in `validateSubscriptionInput`/`validateSubscriptionPatch`
 * (it needs BOTH fields), not here.
 */
function validateMinDurationSeconds(value) {
  if (value === undefined) return { ok: true, value: undefined };
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 0 || value > MAX_SUB_MAX_DURATION_SECONDS) {
    return { ok: false, error: `minDurationSeconds must be an integer between 0 and ${MAX_SUB_MAX_DURATION_SECONDS}` };
  }
  return { ok: true, value };
}

/**
 * v1.285: reject an EMPTY duration window. The build-arg clauses are
 * `duration >= min & duration < max`, so the window `[min, max)` is non-empty
 * only when `min < max`. `0` on either side means "no bound on that side" (min
 * `0` = no floor, max `0` = unbounded), so it never triggers this. Called with
 * the two effective PER-SUB values (a patch resolves patch-value ?? stored-value
 * BEFORE calling, so setting just one side is still checked against the other).
 * The global ceiling is deliberately NOT consulted (it resolves at build time
 * and can change independently).
 */
function validateDurationWindow(minSeconds, maxSeconds) {
  if (typeof minSeconds === 'number' && minSeconds > 0
    && typeof maxSeconds === 'number' && maxSeconds > 0
    && minSeconds >= maxSeconds) {
    return { ok: false, error: `minDurationSeconds (${minSeconds}) must be less than maxDurationSeconds (${maxSeconds})` };
  }
  return { ok: true };
}
...
/**
 * v1.314 (Dean): the per-subscription WEB PUSH opt-in bell. `true` means a
 * download from this channel sends a web push notification; `false` (the
 * default, including every subscription that predates the field) means the
 * download only lands in the in-app feed. Same boolean posture as `paused` /
 * `skipShorts`: absent = unchanged, non-boolean = reject.
 */
function validatePushBell(value) {
  if (value === undefined) return { ok: true, value: undefined };
  if (typeof value !== 'boolean') {
    return { ok: false, error: 'pushBell must be a boolean' };
  }
  return { ok: true, value };
}
...
function validateSubscriptionInput(body) {
  ...
  // v1.285: the per-channel duration FLOOR, same optional posture as the ceiling.
  const minDurationSecondsResult = validateMinDurationSeconds(input.minDurationSeconds);
  if (!minDurationSecondsResult.ok) {
    return { ok: false, error: minDurationSecondsResult.error };
  }
  // v1.285: the window must be non-empty. Only bind the two PER-SUB values (the
  // global ceiling can change independently and is resolved at build-arg time);
  // `0` on either side means "no bound on that side", so it never triggers this.
  const minDurationCheck = validateDurationWindow(minDurationSecondsResult.value, maxDurationSecondsResult.value);
  if (!minDurationCheck.ok) {
    return { ok: false, error: minDurationCheck.error };
  }
  ...
  return {
    ...
    value: {
      ...
      minDurationSeconds: minDurationSecondsResult.value,
...
function validateSubscriptionPatch(body) {
  ...
  // v1.285: the per-channel duration FLOOR, same optional-subset-of-a-patch posture.
  const minDurationSecondsResult = validateMinDurationSeconds(input.minDurationSeconds);
  if (!minDurationSecondsResult.ok) {
    return { ok: false, error: minDurationSecondsResult.error };
  }
  if (minDurationSecondsResult.value !== undefined) value.minDurationSeconds = minDurationSecondsResult.value;
  // Cross-guard the empty window when BOTH sides are in THIS patch (the edit
  // sheet always sends both, so this catches the normal path with a clean 400).
  // A one-sided patch (only one field present, the other from storage) can't be
  // resolved here without the stored record, so `updateSubscription` re-checks
  // the MERGED window defensively before it writes.
  if (minDurationSecondsResult.value !== undefined && maxDurationSecondsResult.value !== undefined) {
    const windowCheck = validateDurationWindow(minDurationSecondsResult.value, maxDurationSecondsResult.value);
  ...
  // v1.314: the push bell, same optional-subset-of-a-patch posture. Applied in
  // `updateSubscription` below - a field validated here but not applied there
  // is the "validates and never applies" lesson recorded at the bottom of that
  // function.
  const pushBellResult = validatePushBell(input.pushBell);
  if (!pushBellResult.ok) {
    return { ok: false, error: pushBellResult.error };
  }
  if (pushBellResult.value !== undefined) value.pushBell = pushBellResult.value;
  ...
  // v1.69 (D15): the file-under-Podcasts toggle. Same optional-subset
  // posture as every field above; a strict two-value allowlist.
  if (input.libraryPlace !== undefined) {
    if (input.libraryPlace !== 'default' && input.libraryPlace !== 'podcasts') {
      return { ok: false, error: "libraryPlace must be 'default' or 'podcasts'" };
    }
    value.libraryPlace = input.libraryPlace;
  }
...
/**
 * v1.142 (Dean's one-off avatar gap): derive the CANONICAL channel URL from a
 * bare `UC…` channelId. A channel with a KNOWN id but no captured channelUrl
 * was unprobeable at every avatar-populate point (ensureChannelAvatar's
 * channelUrl guard, the one-off probe fold's, the refresh batch's skip
 * branch) - yet the id alone fully determines the canonical channel
 * endpoint, so those skips were throwing away a perfectly probeable target.
 * Pure; returns `null` for anything not matching CHANNEL_ID_PATTERN (never
 * builds a URL from unvalidated input - same boundary posture as every
 * other identity helper in this file).
 */
function deriveChannelUrlFromId(channelId) {
  if (typeof channelId !== 'string' || !CHANNEL_ID_PATTERN.test(channelId)) return null;
...
// v1.33 T3: bound for a captured video TITLE (the real title, emoji intact,
// off the `title` key the widened CHANNEL_META_PRINT_TEMPLATE now selects).
// YouTube's own title limit is 100 characters; 300 is a comfortable margin
// for multi-byte/combining sequences and any future drift while still
// rejecting pathological garbage outright -- the same posture as
// MAX_CAPTURED_CHANNEL_NAME_LENGTH above.
const MAX_CAPTURED_TITLE_LENGTH = 300;

/**
 * v1.33 T3: sanitize a captured video title -- creator-controlled text, the
 * same trust level as `channelName`. Strips control characters (emoji and
 * every other printable non-ASCII character deliberately SURVIVE -- they are
 * the entire point of this field), trims, caps the length. Returns the
 * sanitized string, or `null` when the input is absent/not-a-string/empty
 * after stripping. Pure, synchronous, never throws. NOTE: the stored value
 * is data, not markup -- every renderer in this codebase already uses
 * textContent/createTextNode, never innerHTML, so a title containing
 * HTML-looking text stays inert.
 * @param {*} raw
 * @returns {string | null}
 */
function sanitizeCapturedTitle(raw) {
  if (typeof raw !== 'string') return null;
  // eslint-disable-next-line no-control-regex
  const stripped = raw.replace(/[\x00-\x1f\x7f]/g, '').trim();
  if (stripped === '') return null;
  if (stripped.length <= MAX_CAPTURED_TITLE_LENGTH) return stripped;
  // CODE-POINT-aware truncation (gate follow-up): a plain `.slice` counts
  // UTF-16 code units and can split a surrogate pair exactly at the cap,
  // leaving a lone surrogate that UTF-8 encoding degrades to U+FFFD. Emoji
  // are the whole point of this field, so truncate on code points instead.
  return Array.from(stripped).slice(0, MAX_CAPTURED_TITLE_LENGTH).join('');
}
...
// ---- v1.48 item 2: the video's view count, captured the day it was pulled ---
//
...
//
// Before this, view counts were entirely fabricated: `getMockViews`
// (public/js/common.js) hashed the media id and folded in the file size, and
// yt-dlp captured `view_count` nowhere at all. So this is a clean new capture,
// not a change to existing data -- an item with no captured count keeps
// rendering the mock one.
//
// A captured count is explicitly a SNAPSHOT of the day it was taken and drifts
// from reality from then on. That is what Dean asked for, and the UI labels it
// as such ("when downloaded"); a re-heat genuinely re-snapshots it.
//
// Bounds, in order: yt-dlp reports `view_count` as a JSON number, but a numeric
// STRING is accepted too because the `%(...)j` print template has rendered
// fields as strings before. Rejects non-integers (a float view count is
// nonsense and would render as "1.5 views"), negatives, and NaN/Infinity --
// which `JSON.parse` can produce from a malformed dump and which would
// otherwise reach `toLocaleString` and print "Infinity views". The upper bound
// is deliberately generous (1e15, ~5 orders of magnitude above the most-viewed
// video ever) -- it exists only to stop an absurd value from being stored, not
// to second-guess a real one.
const MAX_PLAUSIBLE_VIEW_COUNT = 1e15;
function parseCapturedViewCount(raw) {
  let n;
  if (typeof raw === 'number') n = raw;
  else if (typeof raw === 'string' && raw.trim() !== '') n = Number(raw);
  else return null;
  if (!Number.isFinite(n)) return null;      // NaN / +-Infinity / unparseable string
  if (!Number.isInteger(n)) return null;     // a fractional view count is never real
  if (n < 0 || n > MAX_PLAUSIBLE_VIEW_COUNT) return null;
  return n;
}

// v1.54 (Dean's real subscriber counts): the follower-count validator --
// parseCapturedViewCount's exact posture (number-or-numeric-string, integer,
// non-negative, generous ceiling). 1e12 is ~250x the most-subscribed channel
// ever; the bound stops absurdity, never second-guesses a real count. Note
// the value is APPROXIMATE by construction (yt-dlp parses "24K" -> 24000).
const MAX_PLAUSIBLE_FOLLOWER_COUNT = 1e12;
function parseCapturedFollowerCount(raw) {
  let n;
  if (typeof raw === 'number') n = raw;
  else if (typeof raw === 'string' && raw.trim() !== '') n = Number(raw);
  else return null;
  ...
  if (n < 0 || n > MAX_PLAUSIBLE_FOLLOWER_COUNT) return null;
  return n;
}
...
/**
 * v1.25 QoL bugfix (channel-avatar capture): given the RAW `thumbnails[]`
 * array from a CHANNEL endpoint's `--dump-single-json` (see
 * `run.probeChannelAvatar`, lib/ytdlp/run.js -- `--dump-single-json
 * --playlist-items 0` against a `/channel/<id>` or `@handle` URL), pick the
 * single entry that is the channel's AVATAR, never its banner. Pure,
 * synchronous, never throws. Returns the RAW (unvalidated) url string, or
 * `null` when nothing usable is present -- the caller is required to run the
 * result through `sanitizeChannelAvatarUrl` before it is ever stored/used,
 * exactly like every other untrusted-capture value in this file.
 *
 * WHY NOT the per-video `channel_thumbnail` field this replaces: verified
 * against a live yt-dlp (2026.07.04) `--dump-json` for an actual video, that
 * field simply does not exist on a per-video info dict -- only `thumbnail`/
 * `thumbnails` (the VIDEO's own thumbnails) are present. The channel's own
 * avatar/banner images only show up on the CHANNEL endpoint's `thumbnails[]`,
 * which is what this function is fed.
 *
 * SELECTION HEURISTIC (verified against two real channels, including an
 * `@handle` url): a channel's `thumbnails[]` mixes several WIDE banner crops
 * (e.g. `1060x175` up to `2560x424`, `id`s `"0"`-`"5"`/`"banner_uncropped"`)
 * with the actual avatar -- a SQUARE, sized entry (e.g. `900x900`, `id:"7"`)
 * plus an `id:"avatar_uncropped"` full-resolution entry with no known
 * dimensions. This function:
 *   1. Filters to entries that are SQUARE (`width === height`, both present
 *      and truthy) -- a banner crop is always wide, never square, so this
 *      alone excludes every banner entry. Among the survivors, the LARGEST
 *      (by `width`) is preferred -- there can be more than one sized square
 *      variant.
 *   2. Falls back to the `id: 'avatar_uncropped'` entry only when no sized
 *      square survives step 1 (its own dimensions are unknown, so it can
 *      never be picked over a confirmed square any other way).
 *   3. Returns `null` when neither survives -- "no avatar found," never a
 *      banner mistaken for one.
 * @param {*} thumbnails the raw `thumbnails` array from a channel endpoint's
 *   `--dump-single-json` output (untrusted)
 * @returns {string|null}
 */
function selectChannelAvatarUrl(thumbnails) {
  if (!Array.isArray(thumbnails)) return null;
  const squares = thumbnails.filter((t) => (
    t && typeof t.url === 'string' && t.url !== ''
    && typeof t.width === 'number' && t.width > 0
    && typeof t.height === 'number' && t.height > 0
    && t.width === t.height
  ));
  if (squares.length) {
    squares.sort((a, b) => b.width - a.width); // largest sized square wins
    return squares[0].url;
  }
  const uncropped = thumbnails.find((t) => t && typeof t.url === 'string' && t.url !== '' && t.id === 'avatar_uncropped');
  return uncropped ? uncropped.url : null;
}

// v1.338 (Dean: "There was an icon from this source"; plan 2026-09-26-first-class-any-site D7, his
// pick: style A, the brand disc): a download from another site has no uploader photo (Reddit and
// Facebook give yt-dlp none), so its avatar is the SOURCE site's badge - a bundled file per site
// (public/assets/sites/, Simple Icons marks, CC0). Keyed by the yt-dlp extractor key's PREFIX (a key is
// the extractor class name minus "IE": RedditIE -> 'Reddit', TikTokVMIE -> 'TikTokVM'; verified against
// yt-dlp master yt_dlp/extractor/_extractors.py), case-insensitive. A fixed path from this table - never
// stored, never user data - so it skips the https-only sanitizer that guards CAPTURED avatar URLs.
const SITE_BADGES = [
  ['reddit', 'reddit'], ['facebook', 'facebook'], ['instagram', 'instagram'], ['tiktok', 'tiktok'],
  ['twitter', 'x'], ['vimeo', 'vimeo'], ['twitch', 'twitch'], ['dailymotion', 'dailymotion'],
  ['soundcloud', 'soundcloud'], ['bandcamp', 'bandcamp'], ['bilibili', 'bilibili'], ['rumble', 'rumble'],
];
function siteBadgeAvatarUrl(item) {
  const source = item && typeof item.sourceExtractor === 'string' ? item.sourceExtractor.toLowerCase() : '';
  if (source === '' || source === 'youtube') return null;
  const hit = SITE_BADGES.find(([prefix]) => source.startsWith(prefix));
  return hit ? `/assets/sites/${hit[1]}.svg` : null;
}

/**
 * v1.25.x QoL bugfix (THE channelId-keyed avatar REGISTRY fix): serve-time
 * (read-only, never persisted) avatar resolution for a SINGLE media item.
 * Pure, synchronous, never throws.
 *
 * PRECEDENCE (checked in order, first survivor wins):
 *   1. `item.channelAvatarUrl` -- already baked onto this item at capture
 *      time (re-validated here, defense-in-depth).
 *   2. `getChannelAvatar(db, item.channelId)` -- THE FIX: the new uniform
 *      path. `item.channelId` is the stable `UC…` id every FTCHMETA-captured
 *      download already carries; this joins against the CANONICAL registry
 *      (`db.ytdlp.channelAvatars`, populated independently of subscription
 *      status/recency/URL form -- see `registerChannelAvatar`'s own doc
 *      comment above), so an item captured as `/channel/UC…` resolves an
 *      avatar EVEN WHEN the matching subscription (if any) was added as
 *      `@handle` -- a different `channelUrl` string the OLD step-4 sub-join
 *      below would have missed entirely (the confirmed root cause this
 *      registry ships to resolve).
 *   3. A registry entry whose OWN captured `channelUrl` matches this item's
 *      `channelUrl` OR `channelHandleUrl` -- covers an item that has a URL
 *      but no `channelId` at all (e.g. captured before T11's FTCHMETA
 *      `channel_id` widening, or a MeTube import).
 *   4. The EXISTING subscription join (kept verbatim, back-compat): matches
 *      the item's `channelUrl`/`channelId` against every subscription's own
 *      identity fields and returns the FIRST matching subscription's own
 *      `channelAvatarUrl`. Covers a channel that was never independently
 *      registered (e.g. an old database migrated before this registry
 *      existed) but whose subscription record still carries its own cached
 *      avatar.
 *   5. `null` -- no avatar resolvable from any source; the caller/client
 *      falls back to the generated/first-letter avatar.
 *
 * v1.338 D7: an item with NO channel identity at all (no channelUrl,
 * channelHandleUrl or channelId) and no baked avatar exits before steps 2-5
 * with its source site's badge (`siteBadgeAvatarUrl`, a fixed bundled path,
 * or null for YouTube / an uncurated site), unless the caller passes
 * `{ siteBadge: false }` (Music: an artist circle with a badge would replace
 * the album-art mosaic).
 *
 * Every CAPTURED candidate URL is re-validated via `sanitizeChannelAvatarUrl` again
 * immediately before being returned (defense-in-depth, the same
 * re-validate-at-every-boundary posture this file uses everywhere else -- a
 * hand-edited/corrupted row can never leak an unsafe value through this
 * join, regardless of which precedence step produced it). The site badge is
 * not captured data (a fixed path from `SITE_BADGES`), so it skips that check.
 *
 * Distinct from `index.js`'s `matchChannelDirToSubscription`/
 * `backfillChannelIdentityFromFolder`, which match by FOLDER PATH and
 * persist the result onto `db.metadata` at SCAN time: this is a per-request
 * read that changes nothing on disk.
 * @param {object} db a loaded database object (`{ ytdlp: { subscriptions, channelAvatars } }`)
 * @param {{channelAvatarUrl?: string, channelUrl?: string, channelHandleUrl?: string, channelId?: string}} item a `db.metadata` entry
 * @param {{siteBadge?: boolean}} [opts] `siteBadge: false` returns null instead of the site badge
 * @returns {string|null}
 */
function resolveItemChannelAvatarUrl(db, item, opts) {
  // Defense-in-depth: the only real caller (server.js's `GET
  // /api/videos/:id`) always hands this a `loadDatabase()` result, which
  // never returns null/undefined -- but the doc comment above promises
  // "never throws" unconditionally, so this guard makes that literally true
  // rather than merely true-in-practice (mirrors `ensureYtdlp`'s own
  // "unreachable today, still guarded" postures elsewhere in this file).
  if (!db || typeof db !== 'object') return null;
  if (!item || typeof item !== 'object') return null;

  // 1. already-baked item-level avatar.
  const direct = sanitizeChannelAvatarUrl(item.channelAvatarUrl);
  if (direct) return direct;

  const channelUrl = typeof item.channelUrl === 'string' && item.channelUrl !== '' ? item.channelUrl : null;
  const channelHandleUrl = typeof item.channelHandleUrl === 'string' && item.channelHandleUrl !== '' ? item.channelHandleUrl : null;
  const channelId = typeof item.channelId === 'string' && item.channelId !== '' ? item.channelId : null;
  // v1.338 D7: no channel identity at all - a download from another site shows its site's badge.
  if (!channelUrl && !channelHandleUrl && !channelId) return opts && opts.siteBadge === false ? null : siteBadgeAvatarUrl(item);

  // READ-ONLY: this resolves against the SHARED getCachedDatabase() from
  // /api/channels + the modern grid, so it must not mutate. Before #3a it called
  // ensureYtdlp, which - the moment an item with a channelId and no baked avatar
  // reached here - mutated the shared cache in place (v1.42 aliasing; a 500 under
  // the test cache-guard). readYtdlpNamespace reads without writing.
  const ns = readYtdlpNamespace(db);

  // 2. THE FIX: the channelId-keyed registry, independent of subscription
  // status/recency/URL form.
  if (channelId) {
    const registryHit = getChannelAvatar(db, channelId);
    if (registryHit) return registryHit;
  }

  // 3. registry entry whose OWN captured channelUrl matches this item's
  // channelUrl/channelHandleUrl -- covers an item with a URL but no channelId.
  if (channelUrl || channelHandleUrl) {
    for (const entry of Object.values(ns.channelAvatars)) {
      if (!entry || typeof entry.channelUrl !== 'string') continue;
      if (entry.channelUrl !== channelUrl && entry.channelUrl !== channelHandleUrl) continue;
      const safe = sanitizeChannelAvatarUrl(entry.avatarUrl);
      if (safe) return safe;
    }
  }

  // 4. EXISTING sub-join (kept verbatim, back-compat).
  const match = ns.subscriptions.find((sub) => sub && (
    (channelUrl && sub.channelUrl === channelUrl)
    || (channelId && sub.channelId === channelId)
  ));
  if (match) {
    const safe = sanitizeChannelAvatarUrl(match.channelAvatarUrl);
    if (safe) return safe;
  }
 ...
 */
// v1.41.13 (universal one-offs, design D2): the composite downloadMeta key for
// a NON-YouTube capture is `<extractor_lowercased> <rawId>` -- globally unique
// across extractors (a bare numeric id collides between Vimeo and Dailymotion)
// and consistent with yt-dlp's own `make_archive_id` format. YouTube keeps its
// bare-videoId key (byte-for-byte unchanged). Derived identically at record
// and consume time from a `{source, id}` pair.
function compositeMetaKey(source, id) {
  return `${String(source).toLowerCase()} ${id}`;
}

// The minimal gate for a RAW (non-YouTube) extractor id (design D2, DELTA-1):
// NOT the bracket charset -- real extractor ids legitimately contain slashes,
// spaces, and `=` (`austrian/page=1`, archive.org disc paths). The raw id never
// becomes a path or an argv element (the sourceUrl is the download target; the
// on-disk bracket is yt-dlp's own sanitized rendering), so only length + a
// control-char strip (newline = archive-line-forgery guard) are needed.
function sanitizeRawExtractorId(value) {
...
// The extractor_key charset (yt-dlp class name minus "IE": [A-Za-z0-9_]). A
// plugin key carries a `+plugin` suffix (design S1) -> treated as no usable
// key here (the capture still records under whatever id it has, but with no
// bracket-recoverable source; acceptable, metadata-only).
const EXTRACTOR_KEY_PATTERN = /^[A-Za-z0-9_]{1,64}$/;

function sanitizeCapturedChannelMeta(raw) {
  const input = raw && typeof raw === 'object' ? raw : {};

  // v1.41.13: a NON-YouTube capture (source present and not 'Youtube', and not
  // a bare YouTube video id) takes the universal branch -- no YouTube
  // channelUrl requirement (there is none), a minimal raw-id gate, and the
  // channel label sourced from `channel` (else `uploader`). The YouTube branch
  // below is byte-for-byte unchanged. A proxy-host YouTube capture (source ===
  // 'Youtube' but reached via yewtu.be etc. -- design D1a) falls through to the
  // YouTube branch, so it keeps its real YouTube identity + youtubeId.
  const source = typeof input.source === 'string' ? input.source : '';
  const isYouTubeSource = source === '' || source.toLowerCase() === 'youtube';
  if (!isYouTubeSource) {
    if (!EXTRACTOR_KEY_PATTERN.test(source)) return null; // unusable extractor key (plugin '+' suffix, etc.)
    const sourceId = sanitizeRawExtractorId(input.videoId);
    if (sourceId === null) return null; // no safe id -> drop (never a partial store)
    // Channel label: real channel name, else uploader. Control-stripped +
    // capped exactly like the YouTube channelName path below.
    let channelName;
    for (const candidate of [input.channelName, input.uploader]) {
      if (typeof candidate === 'string') {
        // eslint-disable-next-line no-control-regex
        const stripped = candidate.replace(/[\x00-\x1f\x7f]/g, '').trim();
        if (stripped !== '') {
    ...
    const sourceTitle = sanitizeCapturedTitle(input.title);
    // v1.48 item 2: the day-of view count (universal lane). Same bounded
    // standalone validator the YouTube branch below uses -- never a second one.
    const sourceViewCount = parseCapturedViewCount(input.sourceViewCount);
    // v1.54: the subscriber count (universal lane), same standalone gate.
    const sourceFollowerCount = parseCapturedFollowerCount(input.sourceFollowerCount);
    // v1.338 (plan first-class-any-site D1/D2): the page the download came from - http(s) only, no
    // userinfo, no hidden characters (the v1.337 check); anything else is simply not saved.
    const sourceUrl = sanitizeSourceShareUrl(input.webpageUrl);
    // Design D5 / review note-1: the downloadMeta key for a universal entry is
    // the RENDERED on-disk BASENAME yt-dlp itself reported (filepath), so the
    // scan looks it up by exact `path.basename(item.filePath)` -- never a
    // raw-vs-sanitized-id guess (an id with a `/` sanitizes on disk; the raw
    // composite would miss it). The filepath is untrusted yt-dlp-reported
    // input: used ONLY via path.basename, never resolved or touched as a path.
    // Falls back to the raw composite when no filepath was captured (older
    // captures / a stub without it).
    let renderedBasename = null;
    if (typeof input.filePath === 'string' && input.filePath !== '') {
      const base = path.basename(input.filePath);
      if (base !== '' && base !== '.' && base !== '..') renderedBasename = base;
    }
    return {
      key: renderedBasename || compositeMetaKey(source, sourceId),
      universal: true,
      ...
      ...(sourceFollowerCount !== null ? { sourceFollowerCount } : {}),
      ...(sourceUrl ? { sourceUrl } : {}),
    };
  }
  ...
  // v1.142 (Dean's one-off avatar gap): a capture whose URLs are absent or
  // fail validation but which carries a VALID `UC…` channelId is NOT
  // identity-less - the id alone determines the canonical channel URL, and
  // dropping the whole capture here silently threw away the identity (no
  // downloadMeta record, no scan bridge, no avatar - ever). Derive the
  // canonical URL (pattern-gated) before concluding "no identity".
  if (!channelUrl) {
    channelUrl = deriveChannelUrlFromId(typeof input.channelId === 'string' ? input.channelId : null);
  }
  if (!channelUrl) return null; // neither URL survives validation AND no derivable id -- no identity
  ...
  // v1.33 T3: the video's REAL title (emoji intact) -- control-stripped,
  // trimmed, length-capped. Absent when it does not survive sanitization.
  const sourceTitle = sanitizeCapturedTitle(input.title);
  // v1.48 item 2: the day-of view count (YouTube lane).
  const sourceViewCount = parseCapturedViewCount(input.sourceViewCount);
  // v1.54: the subscriber count (YouTube lane).
  const sourceFollowerCount = parseCapturedFollowerCount(input.sourceFollowerCount);
...
function recordDownloadChannelMeta(deps, entry) {
  ...
  return deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => {
    const ns = ensureYtdlp(db);
    // v1.41.13: a universal (non-YouTube) capture is keyed by its composite
    // `<extractor> <id>` and carries sourceExtractor/sourceId + channelName
    // (no YouTube channelUrl/channelId/avatar). The YouTube entry shape is
    // unchanged.
    const key = sanitized.universal ? sanitized.key : sanitized.videoId;
    ns.downloadMeta[key] = sanitized.universal ? {
      universal: true,
      sourceExtractor: sanitized.sourceExtractor,
      ...
      ...(sanitized.sourceTitle ? { sourceTitle: sanitized.sourceTitle } : {}),
      // v1.48 item 2: `!== undefined`, never a truthiness test -- a genuine
      // 0-view capture is real data and a `? :` on the value would silently
      // drop it and fall the UI back to a fabricated mock count.
      ...(sanitized.sourceViewCount !== undefined ? { sourceViewCount: sanitized.sourceViewCount } : {}),
      ...(sanitized.sourceFollowerCount !== undefined ? { sourceFollowerCount: sanitized.sourceFollowerCount } : {}),
      ...
      ...(sanitized.releaseDate !== undefined ? { releaseDate: sanitized.releaseDate } : {}),
      ...(sanitized.sourceTitle ? { sourceTitle: sanitized.sourceTitle } : {}),
      // v1.48 item 2: see the universal arm above for why this is `!== undefined`.
      ...(sanitized.sourceViewCount !== undefined ? { sourceViewCount: sanitized.sourceViewCount } : {}),
      ...(sanitized.sourceFollowerCount !== undefined ? { sourceFollowerCount: sanitized.sourceFollowerCount } : {}),
...
function consumeDownloadChannelMeta(db, videoId) {
  ...
  // v1.33 T3: re-validated through the SAME sanitizer `recordDownloadChannelMeta`
  // already ran at capture time (untrusted-until-proven, both boundaries).
  const reCheckedTitle = sanitizeCapturedTitle(entry.sourceTitle);
  if (reCheckedTitle !== null) result.sourceTitle = reCheckedTitle;
  ...
  // v1.48 item 2: re-validated at the read boundary through the SAME standalone
  // validator capture used. Unlike releaseDate (stored transformed, as epoch ms
  // from a YYYYMMDD string), a viewCount is stored in exactly the form it was
  // validated in -- a non-negative integer -- so re-running the ORIGINAL
  // validator is correct here rather than a forked re-check.
  const reCheckedViews = parseCapturedViewCount(entry.sourceViewCount);
  // v1.54: the subscriber count re-validates at the read boundary through
  // its OWN original validator, unit-paired with the entry's capturedAt
  // exactly like the view count below.
  const reCheckedFollowers = parseCapturedFollowerCount(entry.sourceFollowerCount);
  if (reCheckedFollowers !== null) {
    result.sourceFollowerCount = reCheckedFollowers;
    if (typeof entry.capturedAt === 'number' && Number.isFinite(entry.capturedAt) && entry.capturedAt > 0) {
      result.sourceFollowerCountCapturedAt = entry.capturedAt;
    }
  }
  if (reCheckedViews !== null) {
    result.sourceViewCount = reCheckedViews;
    // The count and the moment it was taken travel as a UNIT -- the UI labels
    // the number "when downloaded", so a count that arrived without its own
    // date would have to borrow one and would silently misdate itself. Uses the
    // entry's OWN `capturedAt` (stamped when the download captured it) rather
    // than "now": the scan that consumes this bridge usually runs seconds after
    // the download, but a deferred/manual scan can run far later, and stamping
    // consume-time there would date a months-old count as today.
    if (typeof entry.capturedAt === 'number' && Number.isFinite(entry.capturedAt) && entry.capturedAt > 0) {
      result.sourceViewCountCapturedAt = entry.capturedAt;
    }
  }
...
/**
 * v1.41.13 (universal one-offs): the scan-time bridge read for a NON-YouTube
 * item, keyed by the composite `<extractor> <id>` (compositeMetaKey). Mirrors
 * consumeDownloadChannelMeta's contract exactly -- operates directly on `db`,
 * DELETES the key after reading (consumed), and re-validates every field with
 * the same untrusted-until-proven posture. Returns the pseudo-channel identity
 * a non-YouTube item needs: sourceExtractor/sourceId (authoritative, raw) +
 * channelName (the D7 pseudo-channel label) + optional sourceTitle/releaseDate.
 * NO channelUrl/avatar (YouTube-only). Null when the entry is absent, is not a
 * universal entry, or fails re-validation.
 * @param {object} db
 * @param {string} compositeKey  the `<extractor_lowercased> <rawId>` key
 * @returns {{sourceExtractor: string, sourceId: string, channelName?: string, sourceTitle?: string, releaseDate?: number} | null}
 */
function consumeUniversalDownloadMeta(db, compositeKey) {
  if (typeof compositeKey !== 'string' || compositeKey === '') return null;
  const ns = ensureYtdlp(db);
  const entry = ns.downloadMeta[compositeKey];
  if (!entry || typeof entry !== 'object' || entry.universal !== true) return null;
  delete ns.downloadMeta[compositeKey]; // consumed -- bounded growth regardless of what follows

  if (!EXTRACTOR_KEY_PATTERN.test(entry.sourceExtractor || '')) return null;
  const sourceId = sanitizeRawExtractorId(entry.sourceId);
  if (sourceId === null) return null;

  const result = { sourceExtractor: entry.sourceExtractor, sourceId };
  if (typeof entry.channelName === 'string' && entry.channelName !== '') {
    result.channelName = entry.channelName;
  }
  const reCheckedTitle = sanitizeCapturedTitle(entry.sourceTitle);
  if (reCheckedTitle !== null) result.sourceTitle = reCheckedTitle;
  // v1.338 (plan first-class-any-site D1): the saved page link, re-checked at the read boundary like
  // every sibling (the bridge row is stored data; a backup or a hand edit can plant anything).
  const reCheckedUrl = sanitizeSourceShareUrl(entry.sourceUrl);
  if (reCheckedUrl !== null) result.sourceUrl = reCheckedUrl;
  if (
    typeof entry.releaseDate === 'number' &&
    Number.isFinite(entry.releaseDate) &&
    entry.releaseDate >= MIN_PLAUSIBLE_RELEASE_DATE_MS &&
  ...
  // v1.48 item 2: same read-boundary re-validation as the YouTube consumer above.
  const reCheckedViews = parseCapturedViewCount(entry.sourceViewCount);
  // v1.54: the subscriber count re-validates at the read boundary through
  // its OWN original validator, unit-paired with the entry's capturedAt
  // exactly like the view count below.
...
function listSubscriptions(deps) {
  const subs = ensureYtdlp(deps.ytdlpDb.holder(['subscriptions'])).subscriptions; // Wave 5: the snapshot, from its table
 ...
 */
function addSubscription(deps, { channelUrl, format, quality, name, maxVideos, maxDurationSeconds, minDurationSeconds, paused, skipShorts, filetype, cutoffDate } = {}, nowMs = Date.now()) {
  ...
  // v1.285: the duration FLOOR, same fail-safe-to-unset posture. If a direct
  // caller (bypassing validateSubscriptionInput's cross-guard) would leave an
  // EMPTY window (min >= max, both > 0), drop the FLOOR - failing safe toward
  // downloading MORE, never toward a silently-empty subscription.
  const minDurationSecondsResult = validateMinDurationSeconds(minDurationSeconds);
  let resolvedMinDurationSeconds = minDurationSecondsResult.ok ? minDurationSecondsResult.value : undefined;
  if (!validateDurationWindow(resolvedMinDurationSeconds, resolvedMaxDurationSeconds).ok) {
    resolvedMinDurationSeconds = undefined;
  }
  ...
  return deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => {
    ...
    record = {
      ...
      minDurationSeconds: resolvedMinDurationSeconds,
      paused: resolvedPaused,
      skipShorts: resolvedSkipShorts,
      pushBell: false, // v1.314: web push is opt-IN per channel; the add body never sets it
      ...
      lastCheckedAt: null,
      lastStatus: null,
      // v1.69 (D15): which library place lists this sub's downloads.
      // 'default' = today's behavior; 'podcasts' surfaces it as a show in
      // the Podcasts place. Edit-only (the settings sheet PATCH) - adds
      // always start 'default'.
      libraryPlace: 'default',
    };
 ...
 * gate FIX 1) and the v1.36 F2 backoff pair (`checkFailures`/`backoffUntil`,
 * see the inline comment below) are validated at this boundary -- see below.
 *
 ...
 * contract.
 */
function setSubscriptionStatus(deps, id, { lastCheckedAt, lastStatus, cutoffDate, checkFailures, backoffUntil } = {}) {
  let updated = false;
  return deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => {
    const ns = ensureYtdlp(db);
    ...
    // v1.36 F2: per-channel check-failure backoff fields, folded into this
    // SAME atomic write (identical rationale to cutoffDate's FIX 1 above --
    // a cycle's status and its backoff decision must never be two separate
    // round trips). Both are validated at THIS persistence boundary,
    // defense-in-depth like cutoffDate:
    //   `checkFailures` -- only a non-negative integer is ever stored.
    //   `backoffUntil`  -- only `null` (explicit clear, the success path) or
    //                      a string that parses to a real timestamp; a
    //                      malformed value is ignored, never persisted (the
    //                      read side, isInCheckBackoff, ALSO fails open on a
    //                      malformed stored value -- two independent layers).
    // `undefined` (both fields) leaves the record completely untouched,
    // exactly like every other omitted field in this function's contract.
    if (Number.isInteger(checkFailures) && checkFailures >= 0) {
      sub.checkFailures = checkFailures;
    }
    if (backoffUntil === null) {
      sub.backoffUntil = null;
    } else if (typeof backoffUntil === 'string' && Number.isFinite(Date.parse(backoffUntil))) {
      sub.backoffUntil = backoffUntil;
    }
    updated = true;
  })).then(() => updated);
}
...
/**
 * v1.25.x QoL bugfix (finally giving subs a stable id): best-effort backfill
 * of a subscription's OWN `channelId` from a freshly probed/captured value.
 * Matches by `channelUrl` -- the SAME identity `recordSubscriptionChannelAvatar`
 * immediately above already matches subscriptions by (a caller only ever has
 * a captured/probed entry's `channelUrl` to key off of, never a subscription
 * id).
 *
 * Unlike `recordSubscriptionChannelAvatar` (which deliberately OVERWRITES --
 * an avatar image can legitimately change), `channelId` is treated as a
 * write-once IDENTITY field, matching this file's `channelUrl`/`name`
 * posture (AC17): a subscription that already carries a `channelId` is left
 * completely untouched (never reassigned, even to the "same" value -- this
 * function is a no-op the instant a `channelId` is already present). This is
 * the fix for the confirmed root cause this registry ships to resolve:
 * `addSubscription` never wrote `channelId` at all, so `resolveItemChannelAvatarUrl`'s
 * old sub-join (`sub.channelId === item.channelId`) was structurally DEAD --
 * every subscription's `channelId` was permanently `undefined`. This mutator
 * is what finally lets a subscription accumulate a real one (from the
 * subscribe-time avatar probe, or the refresh-avatars batch's compatibility
 * write -- see `lib/ytdlp/index.js`'s `ensureChannelAvatar`/
 * `runRefreshAvatarsBatch`).
 *
 * Never throws; a no-match, an invalid `channelId`, or an already-set
 * `channelId` is a silent no-op (resolves `false`) -- mirrors this file's
 * other best-effort/tolerant-of-a-miss mutators.
 * @param {object} deps `{ updateDatabase }`
 * @param {*} channelUrl the ALREADY-validated/normalized channelUrl to match
 *   subscriptions against (never a raw, unvalidated string)
 * @param {*} channelId a raw, untrusted channel id (re-validated here via
 *   `CHANNEL_ID_PATTERN`)
 * @returns {Promise<boolean>} whether a subscription's record was changed
 */
function recordSubscriptionChannelId(deps, channelUrl, channelId) {
  if (typeof channelId !== 'string' || !CHANNEL_ID_PATTERN.test(channelId)) return Promise.resolve(false);
  if (typeof channelUrl !== 'string' || channelUrl.trim() === '') return Promise.resolve(false);
  let changed = false;
  return deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => {
    const ns = ensureYtdlp(db);
    const sub = ns.subscriptions.find((s) => s.channelUrl === channelUrl);
    if (!sub) return false; // no matching subscription -- nothing to backfill, not an error
    if (typeof sub.channelId === 'string' && sub.channelId !== '') return false; // write-once identity field (AC17 posture) -- never reassigned
...
//
// THE FIX for the confirmed root cause: the OLD serve-time join
// (`resolveItemChannelAvatarUrl`'s old, and still-kept-as-a-fallback, "sub
// join") keys on an EXACT `item.channelUrl === sub.channelUrl` string match
// (plus a `sub.channelId` branch that was structurally DEAD -- `addSubscription`
// never wrote it, see `recordSubscriptionChannelId`'s own doc comment above).
// So an item captured as `/channel/UC…` never matched a subscription added as
// `@handle` (or vice versa), and an item from a channel the user was never
// (or no longer) subscribed to never resolved an avatar at all, even if that
// exact channel's avatar had already been captured for some OTHER item.
//
// This registry is CANONICAL and keyed by the one truly stable identity a
// YouTube channel has -- its `UC…` id (`CHANNEL_ID_PATTERN`, defined above) --
// completely independent of subscription status, recency, or which URL FORM
// (`/channel/<id>` vs `@handle`) any particular item or subscription happened
// to capture. `item.channelAvatarUrl`/`sub.channelAvatarUrl` remain caches/
// fallbacks (see `resolveItemChannelAvatarUrl`'s new precedence order below)
// so no already-working surface regresses; this registry is simply now
// checked FIRST (after an item's own already-baked value), so any item whose
// `channelId` matches a REGISTERED channel resolves an avatar uniformly.

// FIFO cap for `db.ytdlp.channelAvatars` -- mirrors `MAX_DOWNLOAD_META`'s own
// posture/magnitude exactly (this project has no realistic path to tens of
// thousands of distinct channels; this is a backstop against unbounded growth
// over a long-running server's lifetime, not a realistic operating ceiling).
const MAX_CHANNEL_AVATARS = 5000;

// Default "freshness" window `ensureChannelAvatar` (lib/ytdlp/index.js) uses
// to decide whether a channel's registry entry is recent enough to skip a
// fresh probe (populate-once-per-channel, never storm the network re-probing
// a channel whose avatar rarely changes). 90 days is generous -- a channel's
// avatar image changes rarely -- while still keeping the registry from ever
// going permanently stale. The EXPLICIT "Refresh channel avatars" button
// always forces a fresh probe regardless of this window (see
// `runRefreshAvatarsBatch`'s own doc comment, lib/ytdlp/index.js).
const AVATAR_TTL_MS = 90 * 24 * 60 * 60 * 1000;

/**
 * v1.25.x QoL bugfix: upsert ONE channel's avatar into the canonical,
 * channelId-keyed registry (`db.ytdlp.channelAvatars`). Pure boundary
 * validation, same re-validate-at-every-boundary posture as
 * `recordSubscriptionChannelAvatar`: `channelId` must match
 * `CHANNEL_ID_PATTERN` and `avatarUrl` must survive `sanitizeChannelAvatarUrl`
 * (https-only, bounded, well-formed) or this is a silent no-op (resolves
 * `false`, never rejects/throws) -- an invalid identity or avatar is simply
 * never written, exactly like every other boundary-validated mutator in this
 * file. `channelUrl`/`name` are optional, best-effort denormalized display/
 * matching fields (re-validated the SAME way `sanitizeCapturedChannelMeta`
 * validates them elsewhere in this file); either is silently omitted when
 * invalid, never allowed to fail the whole upsert.
 *
 * Deliberately OVERWRITES an existing entry for the same `channelId` (a
 * channel's avatar image, and its captured `channelUrl`/`name`, can
 * legitimately change over time) -- mirrors `recordSubscriptionChannelAvatar`'s
 * own overwrite posture, unlike the write-once identity fields elsewhere in
 * this file.
 *
 * Enforces the `MAX_CHANNEL_AVATARS` FIFO cap exactly like
 * `recordDownloadChannelMeta` enforces `MAX_DOWNLOAD_META`: once the map
 * exceeds it, the OLDEST entries (by `fetchedAt`) are evicted first, so the
 * map can never grow unbounded over a long-running server's lifetime.
 * @param {object} deps `{ updateDatabase }`
 * @param {{channelId?: *, avatarUrl?: *, channelUrl?: *, name?: *}} entry
 * @param {number} [nowMs] injectable "current time" for deterministic tests;
 *   defaults to the real `Date.now()`.
 * @returns {Promise<boolean>} whether the entry was recorded
 */
function registerChannelAvatar(deps, entry, nowMs = Date.now()) {
  const input = entry && typeof entry === 'object' ? entry : {};
  const { channelId } = input;
  if (typeof channelId !== 'string' || !CHANNEL_ID_PATTERN.test(channelId)) return Promise.resolve(false);
  const safeAvatarUrl = sanitizeChannelAvatarUrl(input.avatarUrl);
  if (!safeAvatarUrl) return Promise.resolve(false);

  let safeChannelUrl;
  if (typeof input.channelUrl === 'string' && input.channelUrl.trim() !== '') {
    const check = validateChannelUrl(input.channelUrl);
    if (check.ok) safeChannelUrl = check.url;
  }
  let safeName;
  if (typeof input.name === 'string') {
    // eslint-disable-next-line no-control-regex
    const stripped = input.name.replace(/[\x00-\x1f\x7f]/g, '').trim();
    if (stripped !== '') {
      safeName = stripped.length > MAX_CAPTURED_CHANNEL_NAME_LENGTH
        ? stripped.slice(0, MAX_CAPTURED_CHANNEL_NAME_LENGTH)
        : stripped;
    }
  }
  const fetchedAt = typeof nowMs === 'number' && Number.isFinite(nowMs) ? nowMs : Date.now();

  return deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => {
    const ns = ensureYtdlp(db);
    ns.channelAvatars[channelId] = {
      avatarUrl: safeAvatarUrl,
      fetchedAt,
      ...(safeChannelUrl ? { channelUrl: safeChannelUrl } : {}),
      ...(safeName ? { name: safeName } : {}),
    };
    const keys = Object.keys(ns.channelAvatars);
    if (keys.length > MAX_CHANNEL_AVATARS) {
      const oldestFirst = keys.sort((a, b) => (ns.channelAvatars[a].fetchedAt || 0) - (ns.channelAvatars[b].fetchedAt || 0));
      const overflow = oldestFirst.length - MAX_CHANNEL_AVATARS;
      for (let i = 0; i < overflow; i++) delete ns.channelAvatars[oldestFirst[i]];
    }
  })).then(() => true);
}

/**
 * v1.25.x QoL bugfix: read-only registry lookup by `channelId`. Re-validates
 * the stored `avatarUrl` through `sanitizeChannelAvatarUrl` again before
 * returning it (defense-in-depth, the SAME re-validate-at-every-boundary
 * posture this file uses everywhere else -- a hand-edited/corrupted row
 * can never leak an unsafe value through this read). Pure, synchronous,
 * never throws.
 * @param {object} db a loaded database object
 * @param {*} channelId
 * @returns {string|null} the avatar URL, or `null` when absent/invalid
 */
function getChannelAvatar(db, channelId) {
  if (!db || typeof db !== 'object') return null;
  if (typeof channelId !== 'string' || !CHANNEL_ID_PATTERN.test(channelId)) return null;
  // READ-ONLY (see readYtdlpNamespace): safe on the frozen getCachedDatabase().
  const entry = readYtdlpNamespace(db).channelAvatars[channelId];
  if (!entry || typeof entry !== 'object') return null;
  return sanitizeChannelAvatarUrl(entry.avatarUrl);
}

/**
 * v1.25.x QoL bugfix: the populate-once dedup gate `ensureChannelAvatar`
 * (lib/ytdlp/index.js) checks before spending a network probe on a channel.
 * Pure, synchronous, never throws.
 * @param {object} db a loaded database object
 * @param {*} channelId
 * @param {number} [maxAgeMs] freshness window in ms. `0`, absent, or any
 *   other non-positive/non-finite value means "have ANY entry at all" -- a
 *   channel is never re-probed once it has ever been successfully recorded,
 *   matching `ensureChannelAvatar`'s "populate once per channel" contract.
 * @returns {boolean}
 */
function hasFreshChannelAvatar(db, channelId, maxAgeMs) {
  if (!db || typeof db !== 'object') return false;
  if (typeof channelId !== 'string' || !CHANNEL_ID_PATTERN.test(channelId)) return false;
  const entry = ensureYtdlp(db).channelAvatars[channelId];
  if (!entry || typeof entry !== 'object' || !sanitizeChannelAvatarUrl(entry.avatarUrl)) return false;
  if (typeof maxAgeMs !== 'number' || !Number.isFinite(maxAgeMs) || maxAgeMs <= 0) return true;
...
function updateSubscription(deps, id, patch = {}) {
  ...
  return deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => {
    ...
    // v1.285 (data-safety): snapshot the prior duration bounds so an empty-window
    // patch can be reverted to the exact prior state below (never a bogus default).
    const priorMaxDuration = sub.maxDurationSeconds;
    const priorMinDuration = sub.minDurationSeconds;
    ...
    if (body.minDurationSeconds !== undefined) {
      // v1.285: the duration FLOOR, mirroring maxDurationSeconds above.
      const result = validateMinDurationSeconds(body.minDurationSeconds);
      if (result.ok) sub.minDurationSeconds = result.value;
    }
    // v1.285 (data-safety, defense-in-depth): after applying either duration
    // bound, if the MERGED window is empty (min >= max, both > 0) the channel
    // would silently download NOTHING. The route's validateSubscriptionPatch
    // already 400s the both-present case; this catches a one-sided direct-API
    // patch (only one field sent, the other from storage) that would empty it.
    // Matching this function's silent-ignore-invalid posture, REVERT both bounds
    // to their exact prior values rather than persist an empty window.
    if ((body.minDurationSeconds !== undefined || body.maxDurationSeconds !== undefined)
      && !validateDurationWindow(sub.minDurationSeconds, sub.maxDurationSeconds).ok) {
      sub.maxDurationSeconds = priorMaxDuration;
      sub.minDurationSeconds = priorMinDuration;
    }
    ...
    if (body.pushBell !== undefined) {
      // v1.314: re-validated immediately before mutate, same posture as
      // paused/skipShorts above.
      const result = validatePushBell(body.pushBell);
      if (result.ok) sub.pushBell = result.value;
    }
    ...
    if (body.libraryPlace !== undefined) {
      // v1.69 (D15): re-validated immediately before mutate, same
      // defense-in-depth posture as every field above. (And a lesson
      // re-learned at birth: the PATCH validator accepted this field before
      // this block APPLIED it - the validate-but-never-apply gap the
      // integration test caught.)
      if (body.libraryPlace === 'default' || body.libraryPlace === 'podcasts') {
        sub.libraryPlace = body.libraryPlace;
      }
    }
    record = sub;
  })).then(() => record);
...
function getAllowMembersOnly(deps) {
  return ensureYtdlp(deps.ytdlpDb.holder(['allowMembersOnly'])).allowMembersOnly; // Wave 5
}
...
// LEGACY (v1.43 chunk 4b): the four deps-backed doc-table pin accessors
// below (listPins/addPin/removePin/reorderPins) are NO LONGER called by any
// route -- channel pins are per-user (user_channel_pins rows; the routes in
// index.js run the pure reducers against `deps.userChannelPins`). They
// remain ONLY as utilities over the frozen pre-auth `db.ytdlp.pins` record
// (unit-covered). Do NOT wire a route back onto them: writing the frozen
// record post-adoption is the divergence class design finding #6 forbids.
//
...
function listPins(deps) {
  const pins = ensureYtdlp(deps.ytdlpDb.holder(['pins'])).pins; // Wave 5: the snapshot, from its table
...
// Wave 5 of the relational-migration arc: the namespace as five tables behind
// a feature store (lib/db/featureStore.js). `subscriptions` and `pins` are
// ORDERED record lists (id, position, json - the array order is kept verbatim;
// the routes sort by `order` on the way out as before), `downloadMeta` one row
// per capture (meta_key: a video id, a '<extractor> <id>' composite or a
// rendered basename), `channelAvatars` one row per channel id, and
// `allowMembersOnly` a value backed by the internal ytdlp_settings table (no
// `settings` key on the namespace). ensureYtdlp / readYtdlpNamespace keep
// running on the holder the store hands out; every writer above is
// `deps.updateDatabase(() => deps.ytdlpDb.mutate((db) => ...))`.
const { defineFeatureStore } = require('../db/featureStore');
const FEATURE = defineFeatureStore({
  name: 'ytdlp',
  parts: {
    allowMembersOnly: { kind: 'value', via: 'settings', defaultValue: false },
    subscriptions: { kind: 'records' },
    downloadMeta: { kind: 'map', keyColumn: 'meta_key', table: 'download_meta' },
    pins: { kind: 'records' },
    channelAvatars: { kind: 'map', keyColumn: 'channel_id', table: 'channel_avatars' },
    settings: { kind: 'kv', internal: true },
  },
});

module.exports = {
  siteBadgeAvatarUrl, // v1.338 D7: a download from another site's badge (pure)
  FEATURE,
  createYtdlpStore: (adapter, opts) => FEATURE.createStore(adapter, opts),
  ensureYtdlp,
  deriveDisplayName,
  normalizeChannelUrl,
  ...
  // v1.285: per-channel duration FLOOR + the empty-window cross-guard.
  validateMinDurationSeconds,
  ...
  // v1.41.13 (universal one-offs): the non-YouTube bridge read + the composite
  // key derivation, exported for the scan-side wiring + direct test coverage.
  consumeUniversalDownloadMeta,
  compositeMetaKey,
  CHANNEL_ID_PATTERN,
  deriveChannelUrlFromId,
  ...
  // v1.48 item 2: captured day-of view-count validator (bounded non-negative
  // integer), exported for direct unit testing like its releaseDate sibling.
  parseCapturedViewCount,
  parseCapturedFollowerCount,
  MAX_PLAUSIBLE_VIEW_COUNT,
  MAX_PLAUSIBLE_FOLLOWER_COUNT,
  // v1.33 T3: captured video-title sanitizer (emoji-preserving display titles).
  sanitizeCapturedTitle,
  MAX_CAPTURED_TITLE_LENGTH,
  sanitizeChannelAvatarUrl,
  recordSubscriptionChannelAvatar,
  MAX_CHANNEL_AVATAR_URL_LENGTH,
  // v1.25.x QoL bugfix: the channelId-keyed avatar REGISTRY -- the fix for
  // the confirmed root cause (see `registerChannelAvatar`'s own doc comment
  // above for the full rationale). `recordSubscriptionChannelId` is the
  // companion write-once mutator that finally gives a subscription record a
  // stable `channelId` (previously permanently dead -- `addSubscription`
  // never wrote it).
  recordSubscriptionChannelId,
  registerChannelAvatar,
  getChannelAvatar,
  hasFreshChannelAvatar,
  MAX_CHANNEL_AVATARS,
  AVATAR_TTL_MS,
  // v1.25 QoL bugfix: real channel-avatar capture (the per-video
  // `channel_thumbnail` field this replaces never existed on a real yt-dlp
  // per-video dict -- see `run.probeChannelAvatar`'s own doc comment) + the
  // read-only serve-time item join `GET /api/videos/:id` (server.js) uses.
  selectChannelAvatarUrl,
  resolveItemChannelAvatarUrl,
