...
function sanitizeCapturedChannelMeta(raw) {
  ...
  if (!isYouTubeSource) {
    ...
    const viewCount = parseCapturedViewCount(input.viewCount);
...
function recordDownloadChannelMeta(deps, entry) {
  ...
  return deps.updateDatabase((db) => {
    ...
    ns.downloadMeta[key] = sanitized.universal ? {
      ...
      ...(sanitized.viewCount !== undefined ? { viewCount: sanitized.viewCount } : {}),
      capturedAt: Date.now(),
    } : {
...
function consumeDownloadChannelMeta(db, videoId) {
  ...
  const reCheckedViews = parseCapturedViewCount(entry.viewCount);
  if (reCheckedViews !== null) {
    result.viewCount = reCheckedViews;
    ...
    if (typeof entry.capturedAt === 'number' && Number.isFinite(entry.capturedAt) && entry.capturedAt > 0) {
      result.viewCountCapturedAt = entry.capturedAt;
    }
  }
