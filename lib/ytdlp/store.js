 ...
 */
function addSubscription(deps, { channelUrl, format, quality, name, maxVideos, maxDurationSeconds, paused, skipShorts, filetype } = {}) {
...
function reduceAddPin(pins, { id, channelDir, label, pinnedAt }) {
  ...
  const record = { id, channelDir, label, pinnedAt };
...
// Reads the current pin list. Read-only: backfills IN MEMORY (via
// ensureYtdlp on the freshly-loaded db) so a caller always sees a
// well-formed array, but never writes -- mirrors `listSubscriptions`.
function listPins(deps) {
  const db = deps.loadDatabase();
  return ensureYtdlp(db).pins;
}
