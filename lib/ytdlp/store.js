...
function ensureYtdlp(db) {
  ...
  // shared/frozen array or object. `maxVideos` is deliberately left
  // untouched (stays `undefined` when absent) -- it falls back to the
  // global default at build-arg time (FR-C), not backfilled to any number.
 ...
 */
function addSubscription(deps, { channelUrl, format, quality, name, maxVideos, paused } = {}) {
