...
// against getCachedDatabase() - a hot, FROZEN reader - where ensureYtdlp's
// normalizing writes throw (a read must not mutate). Writers keep calling
...
function resolveItemChannelAvatarUrl(db, item) {
  ...
  // READ-ONLY: this resolves against getCachedDatabase() (a frozen hot reader)
  // from /api/channels + the modern grid, so it must not mutate (ensureYtdlp's
  // normalizing writes would throw a 500 the moment an item with a channelId
  // and no baked avatar reached here - v1.85 #3a root cause).
