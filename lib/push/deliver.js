...
function createPushDelivery(deps) {
  ...
  function payloadForRow(row) {
    // v1.73: the WHOLE row rides in (kind carried) - resolveMeta dispatches
    // per kind and echoes it back; the deep link follows the kind.
    ...
    setImmediate(async () => {
      try {
        do {
          ...
          if (c.truncated && c.sent > 0) again = true;
