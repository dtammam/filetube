...
FileTube is a personal media application that lets you consume your local video, music, and book files in a web browser using a nostalgic, classic YouTube interface. It runs on your own server or local network, so your data stays with you.
...
- **Music library** - A first-class music section with its own scan folders and album art: browse by Albums / Artists / Songs / Liked, search and sort, shuffle, and play in context. An art-forward, phone-first now-playing experience (a collapsing album header, a playing-row highlight, a "Playing from" line, and a browse-while-playing mini-player). Smart resume restarts short songs from the top but resumes long mixes mid-track; ALAC transcodes on demand for non-Apple browsers.
- **Books library + reader** - Read your EPUB and PDF books in the browser: a paginated reader with a table of contents, adjustable text size, paper/sepia/night themes, and per-account reading positions. Pair it with "Listen from Here" (below) to have chapters read aloud on the lock screen.
...
| `DATA_DIR` | Host folder path (e.g. `./data`) | Where the database (`filetube.db`), thumbnails, album art, and caches are saved |
...
### Books & music libraries

Books (EPUB/PDF) and music are first-class libraries with their **own** scan-folder
boxes in Settings, separate from your video/audio folders. Mount their folders in
`docker-compose.yml` the same way, then add the container paths under **Settings →
Book Folders** and **Settings → Music Folders**. The three sets of folders must be
**disjoint** (a folder can't be claimed by more than one library); FileTube refuses
an overlapping folder at save time with a clear message. Books appear under **Library
→ Books** and music under **Library → Music**; music metadata comes from embedded
tags first, falling back to `Artist/Album/NN Title` path conventions.
