...
| `FILETUBE_YTDLP_LIST_TIMEOUT_MINUTES` | `5` | Base budget for a channel's metadata LIST pass. The effective budget additionally scales with `maxVideos` × `SLEEP_REQUESTS` (the pacing that legitimately slows a listing down), capped at 60 minutes. Integer from `1` to `60`. |
